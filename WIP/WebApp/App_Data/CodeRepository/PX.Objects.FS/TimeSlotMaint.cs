...
namespace PX.Objects.FS
{
    public class TimeSlotMaint : PXGraph<TimeSlotMaint, FSTimeSlot>
    {
        ...
        public virtual List<Scheduler.Slot> CompressAndIntersectSlots(List<Scheduler.Slot> slotList)
        {
            ...
            AppointmentEntry.SlotIsContained isContained = AppointmentEntry.SlotIsContained.NotContained;
            ...
            foreach (Scheduler.Slot slotRow in resultSlotsAvailability)
            {
                ...
                {
                    isContained = SlotIsContainedInSlot(slotBegin, slotEnd, slotUnavailabilityRow.DateTimeBegin, slotUnavailabilityRow.DateTimeEnd);
                    if (isContained == AppointmentEntry.SlotIsContained.Contained)
                    {
                    ...
                    if (isContained == AppointmentEntry.SlotIsContained.PartiallyContained && slotUnavailabilityRow.DateTimeBegin < slotBegin)
                    {
                        slotBegin = slotUnavailabilityRow.DateTimeEnd;
                    }

                    if (isContained == AppointmentEntry.SlotIsContained.PartiallyContained && slotUnavailabilityRow.DateTimeBegin < slotEnd)
                    {
                        if (slotBegin < slotUnavailabilityRow.DateTimeBegin)
                        {
                    ...
                    if (isContained == AppointmentEntry.SlotIsContained.ExceedsContainment)
                    {
                        timeCanceled = true;
                        break;
                    }
        ...
        public virtual AppointmentEntry.SlotIsContained SlotIsContainedInSlot(DateTime? slotBegin, DateTime? slotEnd, DateTime? beginTime, DateTime? endTime)
        {
