	   ...
	   public virtual void PostAllocatedTrans(PMAllocationDetail step, PMProject project, List<PMTranWithTrace> allocated)
		...
		public virtual List<PMTranWithTrace> AllocateBudget(PMTask task, PMProject project, PMAllocationDetail step, int origAccountGroupID, int inventoryID, int costCodeID, string UOM, decimal amount, decimal quantity)
		{
			...
			*/
			...
			if (step.UpdateGL == true)
			{
				...
				new object[] { tran.SubID, step.SubID, project.DefaultSalesSubID, task.DefaultSalesSubID, project.DefaultExpenseSubID, task.DefaultExpenseSubID },
				new Type[] { typeof(PMTran.subID), typeof(PMAllocationDetail.subID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(PMProject.defaultExpenseSubID), typeof(PMTask.defaultExpenseSubID) });
				...
				if (tran.OffsetAccountID != null)
				{
					...
					new object[] { tran.OffsetSubID, step.OffsetSubID, project.DefaultSalesSubID, task.DefaultSalesSubID, project.DefaultExpenseSubID, task.DefaultExpenseSubID },
					new Type[] { typeof(PMTran.subID), typeof(PMAllocationDetail.subID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(PMProject.defaultExpenseSubID), typeof(PMTask.defaultExpenseSubID) });
		...
		public virtual object Evaluate(PMObjectType objectName, string fieldName, string attribute, PMTran row)
		{
			switch (objectName)
			{
				...
				case PMObjectType.PMBudget:
					...
					PMBudget budget = selectBudget.Select(row.OrigAccountGroupID, row.ProjectID, row.TaskID, row.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID), row.CostCodeID.GetValueOrDefault(CostCodeAttribute.DefaultCostCode.GetValueOrDefault()));
					if (budget != null)
					{
		...
		public virtual IList<PMTran> Transform(PMAllocationDetail step, PMTask task, PMTran original, bool calculateFormulas, bool isWipStep)
		{
			...
			if (doubleEntry)
			{
				...
				PMTask debitTask = PMTask.PK.Find(this, debitTaskID);
				...
				if (step.UpdateGL == true)
				{
					...
					{
						VerifyMask(step.SubMask, step, debitProject, debitTask, debitTran.SubID);

						object value = PMSubAccountMaskAttribute.MakeSub<PMAllocationDetail.subMask>(this, step.SubMask,
						new object[] { original.SubID, step.SubID, debitProject.DefaultSalesSubID, task.DefaultSalesSubID, debitProject.DefaultExpenseSubID, task.DefaultExpenseSubID },
						new Type[] { typeof(PMTran.subID), typeof(PMAllocationDetail.subID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(PMProject.defaultExpenseSubID), typeof(PMTask.defaultExpenseSubID) });
				...
				if (creditAccountGroup != null)
				{
					...
					PMTask creditTask = PMTask.PK.Find(this, creditTaskID);
					...
					if (step.UpdateGL == true)
					{
						...
						if (original.SubID != null)
						{
							VerifyMask(step.OffsetSubMask, step, creditProject, creditTask, creditTran.OffsetSubID);

							object value = PMSubAccountMaskAttribute.MakeSub<PMAllocationDetail.offsetSubMask>(this, step.OffsetSubMask,
							new object[] { original.SubID, step.SubID, creditProject.DefaultSalesSubID, creditTask.DefaultSalesSubID, creditProject.DefaultExpenseSubID, creditTask.DefaultExpenseSubID },
							new Type[] { typeof(PMTran.subID), typeof(PMAllocationDetail.subID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(PMProject.defaultExpenseSubID), typeof(PMTask.defaultExpenseSubID) });
			...
			else
			{
				...
				if (step.UpdateGL == true)
				{
					...
					{
						VerifyMask(step.SubMask, step, debitProject, debitTask, tran.SubID);

						object value = PMSubAccountMaskAttribute.MakeSub<PMAllocationDetail.subMask>(this, step.SubMask,
						new object[] { tran.SubID, step.SubID, debitProject.DefaultSalesSubID, task.DefaultSalesSubID, debitProject.DefaultExpenseSubID, task.DefaultExpenseSubID },
						new Type[] { typeof(PMTran.subID), typeof(PMAllocationDetail.subID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(PMProject.defaultExpenseSubID), typeof(PMTask.defaultExpenseSubID) });
						Transactions.Cache.RaiseFieldUpdating<PMTran.subID>(tran, ref value);
						tran.SubID = (int?)value;
					}
					...
					if (tran.OffsetAccountID != null)
					{
						VerifyMask(step.OffsetSubMask, step, debitProject, debitTask, tran.OffsetSubID);

						object offsetValue = PMSubAccountMaskAttribute.MakeSub<PMAllocationDetail.offsetSubMask>(this, step.OffsetSubMask,
						new object[] { tran.OffsetSubID, step.OffsetSubID, debitProject.DefaultSalesSubID, task.DefaultSalesSubID, debitProject.DefaultExpenseSubID, task.DefaultExpenseSubID },
						new Type[] { typeof(PMTran.offsetSubID), typeof(PMAllocationDetail.offsetSubID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(PMProject.defaultExpenseSubID), typeof(PMTask.defaultExpenseSubID) });
						Transactions.Cache.RaiseFieldUpdating<PMTran.offsetSubID>(tran, ref offsetValue);
						tran.OffsetSubID = (int?)offsetValue;
					}
				}
		...
		protected virtual void VerifyMask(string mask, PMAllocationDetail step, PMProject project, PMTask task, int? tranSubID)
        {
			if ( mask == null)
				throw new PXException(Messages.StepSubMaskSpecified, step.AllocationID, step.StepID);

			if (step.SubMask.Contains(PMAcctSubDefault.MaskSource) && tranSubID == null)
			{
			...
			if (step.SubMask.Contains(PMAcctSubDefault.ProjectSales) && project.DefaultSalesSubID == null)
			{
				throw new PXException(Messages.ProjectSalesSubNotSpecified);
			}

			if (step.SubMask.Contains(PMAcctSubDefault.TaskSales) && task.DefaultSalesSubID == null)
			{
				throw new PXException(Messages.TaskSalesSubNotSpecified, task.TaskCD);
			}

			if (step.SubMask.Contains(PMAcctSubDefault.ProjectCost) && project.DefaultExpenseSubID == null)
			{
				throw new PXException(Messages.ProjectCostSubNotSpecified);
			}

			if (step.SubMask.Contains(PMAcctSubDefault.TaskCost) && task.DefaultExpenseSubID == null)
			{
				throw new PXException(Messages.TaskCostSubNotSpecified, task.TaskCD);
			}
		}
