...
namespace PX.Objects.IN
{
		...
		where TWMSHeader : WMSHeader, IBqlTable, new()
	{
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Reset")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Scan", Visible = false)]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Pick")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Pack")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Ship")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "PO Receive")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Put Away")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Item Lookup")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Storage Lookup")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "IN Issue")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "IN Receive")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "IN Transfer")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "PI Count")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "OK")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Remove")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Set Qty")]
		...
		protected bool EnsureNoUIErrors<TTable>(TTable row, Action rollbackAction = null)
			where TTable : class, IBqlTable, new()
		{
			var uiErrors = GetUIErrors(row);
			if (uiErrors.Any())
			{
				rollbackAction?.Invoke();
				ClearHeaderInfo();

				ReportError(uiErrors.First().Value);
		...
		protected IReadOnlyDictionary<string, string> GetUIErrors<TTable>(TTable row)
			where TTable : class, IBqlTable, new()
		{
			var cache = Base.Caches<TTable>();
			var uiErrors =
				PXUIFieldAttribute.GetErrors(cache, row, PXErrorLevel.Error).Concat(
				PXUIFieldAttribute.GetErrors(cache, row, PXErrorLevel.RowError)).ToDictionary();
			return uiErrors;
		}
		...
		public abstract class Msg
		{
			...
			public const string CartInvalidSite = "The warehouse of the {0} cart differs from the warehouse of the selected document.";
			...
			public const string LocationNotSet = "The location is not selected.";

			public const string ToLocationNotSelected = "The destination location is not selected.";
			...
			public const string LotSerialPrompt = "Scan the lot/serial number.";
			public const string LotSerialReady = "The {0} lot/serial number is selected.";
			public const string LotSerialNotSet = "The lot/serial number is not selected.";

			public const string LotSerialExpireDatePrompt = "Scan the lot/serial expiration date.";
			public const string LotSerialExpireDateReady = "The expiration date is set to {0:d}.";
			public const string LotSerialExpireDateBadFormat = "The date format does not fit the locale settings.";
			public const string LotSerialExpireDateNotSet = "The expiration date is not selected.";
			...
			public const string GS1WasNotHandled = "The values in the GS1 barcode are not valid for the current step.";
