...
namespace PX.Objects.SO
{
	...
	public partial class SOOrderShipment : PX.Data.IBqlTable
	{
		...
		{
			public static SOOrderShipment Find(PXGraph graph, string orderType, string orderNbr, Guid? shippingRefNoteID)
				=> FindBy(graph, orderType, orderNbr, shippingRefNoteID);
		}
		...
		public static implicit operator SOOrderShipment(PXResult<POReceipt, SOOrder, POReceiptLine> item)
		{
			...
			var receipt = (POReceipt)item;
			ret.ShippingRefNoteID = receipt.NoteID;
			ret.Operation = (receipt.ReceiptType == PO.POReceiptType.POReturn) ? SOOperation.Receipt : SOOperation.Issue;
