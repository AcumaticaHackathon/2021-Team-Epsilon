...
#pragma warning restore PX1043 // Changes cannot be saved to the database from the event handler
        ...
        protected virtual void _(Events.RowSelected<AMBomItem> e)
        {
            ...
            var eccAllowEdit = ECRAllowEdit(e.Row.BOMID);
            EnableBomItemFields(e.Cache, e.Row);
            var isApi = IsImport || IsContractBasedAPI;
            var hasRequiredFields = e.Row.InventoryID != null && e.Row.SiteID != null;
            EnableOperCache(isApi || (e.Row.Hold.GetValueOrDefault() && eccAllowEdit && hasRequiredFields));
            EnableOperChildCache(isApi || (e.Row.Hold.GetValueOrDefault() && eccAllowEdit && hasRequiredFields));
        ...
        protected virtual void EnableBomItemFields(PXCache cache, AMBomItem item)
        {
            ...
            PXUIFieldAttribute.SetEnabled<AMBomItem.descr>(cache, null, item.Hold.GetValueOrDefault() || IsImport || IsContractBasedAPI);
            PXUIFieldAttribute.SetEnabled<AMBomItem.effStartDate>(cache, null, item.Hold.GetValueOrDefault() || IsImport || IsContractBasedAPI);
            PXUIFieldAttribute.SetEnabled<AMBomItem.effEndDate>(cache, null, item.Hold.GetValueOrDefault() || IsImport || IsContractBasedAPI);
        }
