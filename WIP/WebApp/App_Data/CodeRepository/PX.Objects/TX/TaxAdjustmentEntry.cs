...
namespace PX.Objects.TX
{
	public class TaxAdjustmentEntry : PXGraph<TaxAdjustmentEntry, TaxAdjustment>
	{
		...
		public PXAutoAction<TaxAdjustment> initializeState;
		...
		protected virtual void TaxAdjustment_VendorID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			else if (!PXAccess.FeatureInstalled<FeaturesSet.accountLocations>())
			{
				sender.SetDefaultExt<TaxAdjustment.vendorLocationID>(e.Row);
			}
		}
		...
		{
			var tax = Tax.PK.Find(this, (e.Row as TaxTran)?.TaxID);

			if (tax != null && Document.Current != null)
			{
				switch (Document.Current.DocType)
				{
					case TaxAdjustmentType.AdjustOutput:
						e.NewValue = SalesTax_Select.GetValueExt<Tax.salesTaxAcctID>(tax);
						break;
					case TaxAdjustmentType.AdjustInput:
						e.NewValue = SalesTax_Select.GetValueExt<Tax.purchTaxAcctID>(tax);
						break;
				}
			}
		}
		...
		protected virtual void TaxTran_SubID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (tax != null && Document.Current != null)
			{
				...
				{
					case TaxAdjustmentType.AdjustOutput:
						e.NewValue = SalesTax_Select.GetValueExt<Tax.salesTaxSubID>(tax);
						break;
					case TaxAdjustmentType.AdjustInput:
						e.NewValue = SalesTax_Select.GetValueExt<Tax.purchTaxSubID>(tax);
						break;
				}
			}
		}
		...
		protected virtual void TaxTran_TaxBucketID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			TaxRev taxRev = SalesTaxRev_Select.Select(tran.TaxID, tran.TaxType, tran.TranDate);

			if (taxRev != null)
			{
				e.NewValue = taxRev.TaxBucketID;
				sender.SetValue<TaxTran.taxRate>(e.Row, taxRev.TaxRate);
			}
		}
