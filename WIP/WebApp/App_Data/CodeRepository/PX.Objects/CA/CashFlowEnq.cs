...
namespace PX.Objects.CA
{
	...
	public class CashFlowEnq : PXGraph<CashFlowEnq>
	{
		...
		public partial class CashFlowFilter : PX.Data.IBqlTable
		{
			...
			[OrganizationTree(null, null, onlyActive: false)]
			[PXUIRequired(typeof(FeatureInstalled<FeaturesSet.multipleBaseCurrencies>))]
			[PXUIVisible(typeof(FeatureInstalled<FeaturesSet.branch>))]
			...
			#endregion
			#region OrganizationBaseCuryID
			public abstract class organizationBaseCuryID : PX.Data.BQL.BqlString.Field<organizationBaseCuryID> { }
			...
			public String OrganizationBaseCuryID
			{
				get
				{
					return this._OrganizationBaseCuryID;
				}
				set
				{
					this._OrganizationBaseCuryID = value;
				}
			}
			#endregion
		...
		protected virtual void RetrieveARInvoices(Dictionary<FlowKey, CashFlowForecast> flow, CashFlowFilter filter)
		{
			...
			{
				arInvSelect.WhereAnd<Where<ARInvoice.branchID, InsideBranchesOf<Current<CashFlowFilter.orgBAccountID>>>>();
			}
		...
		protected virtual void RetrieveAPInvoices(Dictionary<FlowKey, CashFlowForecast> flow, CashFlowFilter filter)
		{
			...
			if (filter.OrgBAccountID.HasValue)
			{
				...
				apInvSelect.WhereAnd<Where<APInvoice.branchID, InsideBranchesOf<Current<CashFlowFilter.orgBAccountID>>>>();
			}
		...
		protected virtual void RetrieveAPPayments(Dictionary<FlowKey, CashFlowForecast> flow, CashFlowFilter filter)
		{
			...
			if (filter.OrgBAccountID.HasValue)
			{
				apPaymentSelect.WhereAnd<Where<APPayment.branchID, InsideBranchesOf<Current<CashFlowFilter.orgBAccountID>>>>();
			}
		...
		protected virtual void RetrieveARPayments(Dictionary<FlowKey, CashFlowForecast> flow, CashFlowFilter filter)
		{
			...
			if (filter.OrgBAccountID.HasValue)
			{
				arPaymentSelect.WhereAnd<Where<ARPayment.branchID, InsideBranchesOf<Current<CashFlowFilter.orgBAccountID>>>>();
			}
		...
		protected virtual void RetriveCashForecasts(Dictionary<FlowKey, CashFlowForecast> flow, CashFlowFilter filter)
		{
			...
			if (filter.OrgBAccountID.HasValue)
			{
				cashForecastSelect.WhereAnd<Where<CashAccount.branchID, InsideBranchesOf<Current<CashFlowFilter.orgBAccountID>>>>();
			}
			if (filter.CashAccountID.HasValue)
			{
		...
		public void CalcCashAccountBalance(DateTime startDate, CashFlowFilter filter, Dictionary<int, CashFlowForecast> result)
		{
			...
			if (filter.OrgBAccountID.HasValue)
			{
				acctBalanceSelect.WhereAnd<Where<CashAccount.branchID, InsideBranchesOf<Current<CashFlowFilter.orgBAccountID>>>>();
			}
