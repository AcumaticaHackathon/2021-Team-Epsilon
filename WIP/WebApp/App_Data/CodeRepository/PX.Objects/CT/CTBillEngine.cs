	...
	public class CTBillEngine : PXGraph<CTBillEngine>, IContractInformation
	{
		...
		object IContractInformation.Evaluate(CTObjectType objectName, string fieldName, string attribute, CTFormulaDescriptionContainer row)
		{
			...
			switch (objectName)
			{
				...
				case CTObjectType.PMTran:
					...
					{
						PMTran pmTran = PMTran.PK.Find(this, row.pmTranIDs[0]);
						if (pmTran != null)
						{
		...
		private void ClearFutureHistory(Contract contract)
		{
			PXDatabase.Delete<ContractRenewalHistory>(
				new PXDataFieldRestrict(nameof(ContractRenewalHistory.ContractID), PXDbType.Int, 4, contract.ContractID, PXComp.EQ),
				new PXDataFieldRestrict(nameof(ContractRenewalHistory.RevID), PXDbType.Int, 4, contract.RevID, PXComp.GT));
		}

		private void ClearFutureDetails(Contract contract)
		{
			PXDatabase.Delete<ContractDetail>(
				new PXDataFieldRestrict(nameof(ContractDetail.ContractID), PXDbType.Int, 4, contract.ContractID, PXComp.EQ),
				new PXDataFieldRestrict(nameof(ContractDetail.RevID), PXDbType.Int, 4, contract.RevID, PXComp.GT));
		}
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (contract.ScheduleStartsOn == Contract.scheduleStartsOn.SetupDate)
				{
					...
					if (schedule.Type != BillingType.OnDemand)
						schedule.NextDate = GetNextBillingDateConsiderExpiration(schedule.Type, contract.CustomerID, date, date, contract.ExpireDate);
				}
				contract.EffectiveFrom = contract.StartDate;
				BillingSchedule.Update(schedule);
				Contract.Events
					.Select(ev=>ev.SetupContract)
					.FireOn(this, contract);
		...
		public virtual void Activate(int? contractID, DateTime? date, bool setActivationDate=true)
		{
			...
			if (contract.OriginalContractID != null)
			{
				...
				Contract.Events
					.Select(ev=>ev.ExpireContract)
					.FireOn(this, origContract);
			}
			#endregion
				...
				if (contract.ScheduleStartsOn == Contract.scheduleStartsOn.SetupDate)
				{
					if (schedule.Type != BillingType.OnDemand)
					{
						while (schedule.NextDate < date)
						{
							...
							schedule.NextDate = GetNextBillingDateConsiderExpiration(schedule.Type, contract.CustomerID, schedule.LastDate, schedule.StartBilling, contract.ExpireDate);
						}
					}
					else
					{
		...
		public virtual void SetupAndActivate(int? contractID, DateTime? date)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (schedule.Type != BillingType.OnDemand)
					schedule.NextDate = GetNextBillingDateConsiderExpiration(schedule.Type, contract.CustomerID, date.Value, date, contract.ExpireDate);
				contract.EffectiveFrom = contract.StartDate;
				schedule.StartBilling = date;
		...
		{
			ARInvoiceEntry invoiceEntry = CreateInvoiceGraph();
			...
			foreach (InvoiceData invData in invoices)
			{
				...
				copy.DocDesc = GetInvoiceDescription(action, contract, customer, copy);
				copy.DocDate = invData.InvoiceDate;
		...
		private static ARInvoiceEntry CreateInvoiceGraph()
		{
			...
			invoiceEntry.FieldDefaulting.AddHandler<ARInvoice.retainageApply>((PXCache sender, PXFieldDefaultingEventArgs e) => { e.NewValue = false; e.Cancel = true; });
			invoiceEntry.FieldVerifying.AddHandler<ARInvoice.retainageApply>((PXCache sender, PXFieldVerifyingEventArgs e) => { e.NewValue = false; e.Cancel = true; });
		...
		public virtual void Bill(int? contractID, DateTime? date = null)
		{
			...
			else
			{
				...
				schedule.NextDate = GetNextBillingDateConsiderExpiration(schedule.Type, contract.CustomerID, schedule.NextDate, schedule.StartBilling, contract.ExpireDate);
			}
			BillingSchedule.Update(schedule);
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				if (data.Count > 0)
				{
					...
					copy.CustomerID = customer.BAccountID;
					copy.CustomerLocationID = location.LocationID;
					copy.DocDesc = GetInvoiceDescription(ContractAction.Bill, contract, customer, copy);
		...
		public virtual void Renew(int? contractID, DateTime renewalDate)
		{
			...
			contract.ServiceActivate = true;
			Contract newContract = PXCache<Contract>.CreateCopy(contract);
			newContract.StartDate = newStartDate;
			contract.ExpireDate = PXFormulaAttribute.Evaluate<Contract.expireDate>(this.Contracts.Cache, newContract) as DateTime?;
			Contract.Events
				.Select(ev=>ev.ActivateContract)
				.FireOn(this, contract);
			if (contract.ExpireDate == null)
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				if (data.Count > 0)
				{
					...
					PXCache invCache = invoiceEntry.Document.Cache;
					ARInvoice copy = (ARInvoice)invCache.CreateCopy(invoiceEntry.Document.Current);
					copy.DocType = ARDocType.Invoice;
					copy.DocDate = renewalDate;
					copy.BranchID = null;
					copy.CustomerID = customer.BAccountID;
					copy.CustomerLocationID = location.LocationID;
					copy.ProjectID = contract.ContractID;
					copy.CuryID = contract.CuryID;
					invoiceEntry.Document.Update(copy);
		...
		public virtual void UndoBilling(int? contractID)
		{
			...
			ContractRenewalHistory previousHistory = new PXSelect<
				ContractRenewalHistory,
				Where<
					ContractRenewalHistory.contractID, Equal<Required<Contract.contractID>>,
			...
			ContractBillingSchedule schedule = BillingSchedule.Select(contractID);
			ContractRenewalHistory undoingHistory = CurrentRenewalHistory.SelectSingle(contract.ContractID, contract.RevID);
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (undoingHistory.Action == ContractAction.Terminate)
				{
					contract.TerminationDate = null;
				}

				if (undoingHistory.Action == ContractAction.Renew && undoingHistory.ChildContractID != null)
				{
					...
					graph.Contracts.Current = PXSelect<
						...
						.Select(graph, undoingHistory.ChildContractID);
				...
				foreach(ContractDetail undoingDetail in SelectFrom<ContractDetail>
					.Where<ContractDetail.contractID.IsEqual<@P.AsInt>
						.And<ContractDetail.revID.IsEqual<@P.AsInt>>>
					.View
					.Select(this, contract.ContractID, undoingHistory.RevID))
				{
					ContractDetail previousDetail = SelectFrom<ContractDetailExt>
						.Where<ContractDetail.contractID.IsEqual<@P.AsInt>
							.And<ContractDetail.lineNbr.IsEqual<@P.AsInt>>
							.And<ContractDetail.revID.IsEqual<@P.AsInt>>>
						.View
						.ReadOnly
						.SelectSingleBound(this, null, contract.ContractID, undoingDetail.LineNbr, previousHistory.RevID);

					previousDetail.Used = undoingDetail.UsedTotal - previousDetail.UsedTotal + previousDetail.Used;
					previousDetail.UsedTotal = undoingDetail.UsedTotal;
					ContractDetails.Update(previousDetail);
				}
		...
		protected virtual DateTime? GetNextBillingDateConsiderExpiration(string scheduleType, int? customerID, DateTime? date, DateTime? startDate, DateTime? expireDate)
		{
			DateTime? nextBillingDate = GetNextBillingDate(scheduleType, customerID, date, startDate);
			if (expireDate != null && expireDate < nextBillingDate)
			{
				nextBillingDate = expireDate;
			}
		...
		protected virtual void ProcessSingleRecord(Contract contract, TranNotePair tran, PXResult<ContractDetail, ContractItem, InventoryItem> res, List<UsageData> addedData)
		{
			...
			if (billing != null)
			{
				...
				if (!availableQty.TryGetValue(tran.Tran.InventoryID.Value, out available)) 
				{
					available = 0m;
				}
		...
		public virtual decimal? CalculatePrice(Contract contract, UsageData tranData)
		{
			...
			decimal? price = ARSalesPriceMaint.CalculateSalesPrice(ContractDetails.Cache, location.CPriceClassID, contract.CustomerID, tranData.InventoryID, ci, tranData.UOM, tranData.PreciseQty, effectiveDate, null, taxCalcMode: taxCalcMode);
			ciCache.Delete(ci);
			return price;
		}
		...
		protected virtual void AutoReleaseInvoice(Contract contract)
		{
			if (doclist.Count > 0)
			{
				...
				if (template.AutomaticReleaseAR == true)
				{
					...
					{
						throw new PXException(PM.Messages.AutoReleaseAROnBillingFailed, ex);
					}
				}
			}
		}
		...
		protected virtual void RaiseErrorIfUnreleasedUsageExist(Contract contract)
		{
			...
			PXSelectBase<CRPMTimeActivity> selectActivities = new PXSelectJoin<
				...
				Where<
					...
					And<CRPMTimeActivity.released, Equal<False>,
