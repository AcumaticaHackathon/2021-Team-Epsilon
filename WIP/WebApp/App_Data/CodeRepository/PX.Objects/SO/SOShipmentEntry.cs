...
namespace PX.Objects.SO
{
	public partial class SOShipmentEntry : PXGraph<SOShipmentEntry, SOShipment>, IGraphWithInitialization
	{
		...
		public PXMenuItem<SOShipment> confirmShipmentAction;
		...
		[PXMenuItem(nameof(action), ShipmentActions.ConfirmShipment)]
		protected virtual IEnumerable ConfirmShipmentAction(PXAdapter adapter)
		{
			var list = adapter.Get<SOShipment>().ToList();

			List<object> _persisted = Caches[typeof(SOOrder)].Updated.Cast<SOOrder>().ToList<object>();
			Save.Press();
			...
			PXLongOperation.StartOperation(this, delegate ()
			{
				...
				foreach (SOShipment shipment in list)
				{
					...
					catch (Exception ex) when (adapter.MassProcess)
					{
						PXProcessing<SOShipment>.SetError(ex);
					}
					finally
					{
						IsShipmentReadyForConfirmation = false;
					}
				}
			});
		...
		public PXMenuItem<SOShipment> createInvoice;
		...
		[PXMenuItem(nameof(action), ShipmentActions.CreateInvoice)]
		...
		[PXMenuItem(nameof(action), ShipmentActions.PostInvoiceToIN)]
		protected virtual IEnumerable updateIN(PXAdapter adapter, List<SOShipment> shipmentList = null)
		{
			...
			if (!UnattendedMode && NeedWarningShipNotInvoicedUpdateIN(sosetup.Current, shipments))
			{
			...
			PXLongOperation.StartOperation(this, delegate ()
			{
				...
				if (shipmentEntry.sosetup.Current.AutoReleaseIN == true && createdINDocs.Count > 0 && createdINDocs[0].Hold == false)
					INDocumentRelease.ReleaseDoc(createdINDocs, false, adapterSlice.QuickProcessFlow);
		...
		public static bool NeedWarningShipNotInvoicedUpdateIN(SOSetup setup, IEnumerable<SOShipment> shipments)
		{
			...
			return 
				setup.UseShippedNotInvoiced != true &&
				setup.UseShipDateForInvoiceDate != true &&
				shipments.Any(shipment => shipment.Status == SOShipmentStatus.Confirmed && !IsNotBillable(shipment));
		}

		public PXMenuItem<SOShipment> applyAssignmentRules;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = ShipmentActions.Messages.ApplyAssignmentRules, Visible = false)]
		[PXMenuItem(nameof(action), ShipmentActions.ApplyAssignmentRules)]
		...
		public PXMenuItem<SOShipment> correctShipmentAction;
		...
		[PXMenuItem(nameof(action), ShipmentActions.CorrectShipment)]
		...
		public PXMenuItem<SOShipment> createDropshipInvoice;
		...
		[PXMenuItem(nameof(action), ShipmentActions.CreateDropshipInvoice)]
		...
		public PXMenuItem<SOShipment> printLabels;
		...
		[PXMenuItem(nameof(action), ShipmentActions.PrintLabels)]
		...
		public PXMenuItem<SOShipment> getReturnLabelsAction;
		...
		[PXMenuItem(nameof(action), ShipmentActions.GetReturnLabels)]
		...
		public PXMenuItem<SOShipment> cancelReturn;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = ShipmentActions.Messages.CancelReturn, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXMenuItem(nameof(action), ShipmentActions.CancelReturn)]
		protected virtual IEnumerable CancelReturn(PXAdapter adapter)
		{
			var list = adapter.Get<SOShipment>().ToList();
		...
		public PXMenuItem<SOShipment> printPickListAction;
		...
		[PXMenuItem(nameof(action), ShipmentActions.PrintPickList)]
		...
		protected virtual void SOShipment_RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			if (e.Row == null)
				return;
			...
			{
				row.PackageCount = PXSelect<SOPackageDetailEx, Where<SOPackageDetailEx.shipmentNbr, Equal<Required<SOShipment.shipmentNbr>>>>.Select(sender.Graph, row.ShipmentNbr).Count();
			}
		}
		...
		{
			UpdateShipmentCntr(sender, e.Row, 1);
		}
		...
		protected virtual void SOOrderShipment_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			{
				UpdateShipmentCntr(sender, e.OldRow, -1);
				UpdateShipmentCntr(sender, e.Row, 1);
			}
		}
		...
		public virtual void CreateShipment(SOOrder order, int? SiteID, DateTime? ShipDate, bool? useOptimalShipDate, string operation, DocumentList<SOShipment> list, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			...
			if (order.HasLegacyCCTran == true)
			{
				throw new PXException(Messages.CantProcessSOBecauseItHasLegacyCCTran, order.OrderType, order.OrderNbr);
			}
			...
			try
			{
				...
				foreach (ShipmentSchedule ss in schedulesList)
				{
					...
					{
						CreateShipmentFromSchedules(ss.Result, ss.ShipLine, ordertype, operation, list);
					}
			...
			if (list != null)
			{
				if (OrderList.Cache.Inserted.Count() > 0 || OrderList.SelectWindowed(0, 1) != null)
				{
					...
					if (list.Find(Document.Current) is null)
		...
		{
			ConfirmShipment(docgraph, shiporder);
		}
		...
		public virtual void ConfirmShipment(SOOrderEntry docgraph, SOShipment shiporder)
        {
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.wMSAdvancedPicking>() && Document.Current.CurrentWorksheetNbr != null)
			{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				SOShipment.Events
					...
					.FireOn(docgraph, Document.Current);
				docgraph.Save.Press();
		...
		public virtual bool TryCompleteWorksheet(SOPickingWorksheet worksheet)
		{
			bool needToCompleteWorksheet =
				...
				View.Select(this, worksheet.WorksheetNbr)
				.RowCast<SOShipment>()
				.AsEnumerable()
			...
			{
				worksheet.Status = SOPickingWorksheet.status.Completed;
				this.Caches<SOPickingWorksheet>().Update(worksheet);
				this.EnsureCachePersistence<SOPickingWorksheet>();
				return true;
			}
			return false;
		}
		...
		protected virtual void ConfirmSingleLine(SOOrderEntry docgraph, SOLine line, SOShipLine shipline, string lineShippingRule, ref bool backorderExists)
		{
			...
			else
			{
				...
				else if (shipline.ShipmentNbr != null || lineShippingRule != SOShipComplete.ShipComplete)
				{
					...
					{
						docgraph.Document.Current.MarkCompleted();
					}
				...
				try
				{
						...
						And<SOOrderShipment.invoiceNbr, IsNull>>>>.Select(docgraph, poreceipt.ShipmentNbr))
					{
						...
						if (shipmentInvoices != null)
						{
							...
							var docType = ie.GetInvoiceDocType(orderType, shipment.Operation);

							var subList = new InvoiceList(docgraph);
							subList.AddRange(shipmentInvoices.GetInvoices(docType));
							int oldCount = subList.Count;

							ie.InvoiceOrder((DateTime)invoiceDate, record, details, null, subList);

							if (subList.Count > oldCount)
								created.Add((ARInvoice)subList[oldCount], (SOInvoice)subList[oldCount], (CurrencyInfo)subList[oldCount][typeof(CurrencyInfo)]);
						}
						else
							ie.InvoiceOrder((DateTime)invoiceDate, record, details, null, created);
		...
		public virtual IEnumerable<PXResult<SOShipLine, SOLine>> CollectDropshipDetails(SOOrderShipment shipment)
		{
			foreach (PXResult<POReceiptLine, SOLineSplit, SOLine> line in PXSelectJoin<POReceiptLine,
				...
				Where<POReceiptLine.lineType, In3<POLineType.goodsForDropShip, POLineType.nonStockForDropShip>,
					...
					And<SOLine.orderNbr, Equal<Current<SOOrderShipment.orderNbr>>>>>>>
				.SelectMultiBound(this, new object[] { shipment })
		...
		public virtual void PostReceipt(INIssueEntry docgraph, PXResult<SOOrderShipment, SOOrder> sh, ARInvoice invoice, DocumentList<INRegister> list)
		{
			...
			selectDropshipReceiptsCmd = BqlCommand.AppendJoin<
				...
				LeftJoin<INTran, On<INTran.sOShipmentNbr, Equal<POReceiptLine.receiptNbr>, And<INTran.sOShipmentType, Equal<SOShipmentType.dropShip>, And<INTran.sOShipmentLineNbr, Equal<POReceiptLine.lineNbr>, And<INTran.sOOrderType, Equal<SOLine.orderType>, And<INTran.sOOrderNbr, Equal<SOLine.orderNbr>, And<INTran.sOOrderLineNbr, Equal<SOLine.lineNbr>>>>>>>>>>(selectDropshipReceiptsCmd);
				...
				if (reports.LaserLabels.Count > 0 && reports.ReportRedirect != null)
				{
					if (PXAccess.FeatureInstalled<FeaturesSet.deviceHub>())
						SMPrintJobMaint.CreatePrintJobGroup(
							...
							ShipmentActions.Messages.PrintLabels);
				}
			}
