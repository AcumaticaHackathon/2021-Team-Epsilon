...
namespace PX.Objects.FA
{
	...
	public class AssetProcess : PXGraph<AssetProcess>
	{
		...
		//public static bool? PreviousUnderConstructionValue 
		//{ 
		//	get; 
		//	set; 
		//}
		...
		public static void SuspendAsset(FixedAsset asset)
		{
			...
			FixedAsset.Events
				.Select(ev=>ev.SuspendAsset)
				.FireOn(docgraph, asset);
		...
		public static void UnsuspendAsset(FixedAsset asset, string CurrentPeriod)
		{
			...
			int? postingBookID = books
				.Where(book => book.Value.UpdateGL == true)
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (FABookBalance bookBalance in PXSelect<FABookBalance, Where<FABookBalance.assetID, Equal<Current<FixedAsset.assetID>>>>.SelectMultiBound(docgraph, new object[] { asset })
					...
					.Where(item => item.Status == FixedAssetStatus.Suspended))
					{
						...
						string CurrentBookPeriod = bookBalance.UpdateGL == true 
							? CurrentPeriod 
							: docgraph.GetService<IFABookPeriodRepository>().GetFABookPeriodIDOfDate(
									docgraph.GetService<IFABookPeriodUtils>().GetFABookPeriodStartDate(
										CurrentPeriod, 
										postingBookID ?? bookBalance.BookID, 
										bookBalance.AssetID), 
									bookBalance.BookID, 
									bookBalance.AssetID);
		...
		public static void SuspendAssetToPeriod(IEnumerable<FABookBalance> balances, DateTime? Date, string PeriodID)
		{
			...
			foreach (FABookBalance bookbal in balances)
			{
				...
				int Periods = docgraph.GetService<IFABookPeriodUtils>().PeriodMinusPeriod(BookPeriodID, bookbal.CurrDeprPeriod ?? BookPeriodID, bookbal.BookID, bookbal.AssetID) ?? 0;
				SuspendBalanceForPeriods(docgraph, bookbal, Periods);
			}
		...
		public static void CheckIfAssetIsUnderConstruction(PXGraph graph, Int32? assetID)
		{
			...
			if (asset.UnderConstruction == true)
			{
		...
		public static void DisposeAsset(TransactionEntry docgraph, FixedAsset asset, IEnumerable<FABookBalance> books, FASetup setup, string reason)
		{
			...
			foreach (FABookBalance bal in books)
			{
				...
				string period;
				if(bookbal.Depreciate == true && asset.UnderConstruction != true)
				{
				...
				if (string.IsNullOrEmpty(book.Key.LastDeprPeriod))
				{
					...
					if ((book.Value.Op == PXDBOperation.Insert && asset.SplittedFrom == null) || book.Value.Op == PXDBOperation.Update)
					{
						FABookHist hist = new FABookHist
						{
							...
							FinPeriodID = GetDeprFromPeriod(docgraph, book.Key, asset, details) // This is a workaround.
						};
						...
						bookBal.CurrDeprPeriod = GetDeprFromPeriod(docgraph, book.Key, asset, details);
						...
						if (!docgraph.UpdateGL)
						{
							...
							if (!string.IsNullOrEmpty(book.Key.LastDeprPeriod))
							{
								...
								if (string.CompareOrdinal(book.Key.LastDeprPeriod, book.Key.DeprToPeriod) < 0)
								{
									...
									bookBal.InitPeriod = GetDeprFromPeriod(docgraph, book.Key, asset, details);
									bookBal.LastPeriod = asset.UnderConstruction == true ? bookBal.InitPeriod : book.Key.DeprToPeriod;
									docgraph.bookbalances.Update(bookBal);
								}
								...
								if (string.CompareOrdinal(book.Key.LastDeprPeriod, book.Key.DeprToPeriod) == 0)
								{
									...
									{
										asset = docgraph.Asset.Update(docgraph.Asset.Locate(asset) ?? asset);
										FixedAsset.Events
		...
		protected static string GetDeprFromPeriod(PXGraph docgraph, FABookBalance bookbal, FixedAsset asset, FADetails details)
		{
			if (asset.UnderConstruction == true && bookbal.DeprFromPeriod == null && details.DepreciateFromDate == null)
			{
				return docgraph.GetService<IFABookPeriodRepository>().GetFABookPeriodIDOfDate(details.ReceiptDate, bookbal.BookID, bookbal.AssetID);
			}

			return bookbal.DeprFromPeriod;
		}
		...
		public static string GetFixedAssetStatus(PXGraph graph, FADetails details)
		{
			...
			return IsFullyDepreciatedAsset(graph, details.AssetID) ? FixedAssetStatus.FullyDepreciated : asset.Status;
		}

		public static void AdjustFixedAssetStatus(PXGraph graph, int? assetID)
		{
			FixedAsset asset = (FixedAsset)graph.Caches[typeof(FixedAsset)].Current;
			if (asset == null || asset.AssetID != assetID)
			{
				asset = SelectFrom<FixedAsset>
					.Where<FixedAsset.assetID.IsEqual<@P.AsInt>>
					.View
			...
			switch (asset.Status)
			{
				case FixedAssetStatus.Active 
					when IsFullyDepreciatedAsset(graph, assetID):
					FixedAsset.Events.Select(ev=>ev.FullyDepreciateAsset)
						.FireOn(graph, asset);
					break;
				case FixedAssetStatus.FullyDepreciated 
					when !IsFullyDepreciatedAsset(graph, assetID):
					FixedAsset.Events.Select(ev=>ev.ActivateAsset)
						.FireOn(graph, asset);
					break;
			}
		}
		...
		public virtual void ProcessAssetTran(JournalEntry je, FARegister doc, DocumentList<Batch> created)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
																																	...
																																	And<FATran.released, Equal<False>>>>.Select(this, doc.RefNbr))
				{
					...
					if (asset.Status == FixedAssetStatus.Hold)
					{
						throw new PXException(Messages.TranPostedOnHold);
					}

					if (asset.Status == FixedAssetStatus.Disposed && fatran.Origin == FARegister.origin.Reversal)
					{
						throw new PXException(Messages.CantReverseDisposedAsset, asset.AssetCD);
					}

					if (asset.Status != FixedAssetStatus.Disposed && fatran.Origin == FARegister.origin.DisposalReversal)
					{
						throw new PXException(Messages.CantReverseDisposal, asset.AssetCD, new FixedAssetStatus.ListAttribute().ValueLabelDic[asset.Status]);
					}
					
					if (posthist.Suspended == true 
						&& fatran.TranType != FATran.tranType.PurchasingPlus 
						&& fatran.TranType != FATran.tranType.PurchasingMinus
						&& fatran.TranType != FATran.tranType.ReconciliationPlus
						&& fatran.TranType != FATran.tranType.ReconciliationMinus
					...
					if (fatran.TranType == "P+" || fatran.TranType == "P-")
					{
						...
						foreach (PXResult<FABookPeriod, OrganizationFinPeriod> p in PXSelectJoin<
							...
							Where<FABookPeriod.bookID, Equal<Required<FATran.bookID>>,
								...
								And<FABookPeriod.organizationID, Equal<Required<FABookPeriod.organizationID>>>>>>>>
							.Select(this, fatran.BookID, GetDeprFromPeriod(je, bookbal, asset, det), fatran.FinPeriodID, PXAccess.GetParentOrganizationID(fatran.BranchID)))
						{
					...
					switch (fatran.TranType)
					{
						...
						case "TP":
							if (!fatran.IsOriginReversal)
								fatran.TranAmt = bookbal.YtdAcquired ?? posthist.YtdAcquired;
							asset = (FixedAsset)fixedasset.Cache.Locate(asset) ?? asset;
					...
					switch (fatran.TranType)
					{
						...
						case FATran.tranType.ReconciliationMinus:
							if (IsFullyDepreciatedAsset(this, asset.AssetID)
								&& string.CompareOrdinal(fatran.FinPeriodID, bookbal.DeprToPeriod) > 0)
							{
								hist.Closed = true;
							}
							break;
					...
					switch (doc.Origin)
					{
						case FARegister.origin.Purchasing:
							...
							bookbal.InitPeriod = GetDeprFromPeriod(je, bookbal, asset, det);
							bookbal.LastPeriod = asset.UnderConstruction == true ? bookbal.InitPeriod : bookbal.DeprToPeriod;
						...
						case FARegister.origin.Disposal:
							...
							{
								asset = this.fixedasset.Update(this.fixedasset.Locate(asset) ?? asset);
								FixedAsset.Events
									.Select(ev=>ev.DisposeAsset)
									.FireOn(this, asset);
							}
							break;
						...
						case FARegister.origin.DisposalReversal:
							...
							if(lhist != null)
							{
								bookbal.DeprToPeriod = bookbal.Depreciate == true && asset.UnderConstruction != true
							...
							bookbal.Status = bookbal.Depreciate == true && asset.UnderConstruction != true && isFullyDepreciated ? FixedAssetStatus.FullyDepreciated : FixedAssetStatus.Active;
							...
							string status = bookbal.Status == FixedAssetStatus.FullyDepreciated && IsFullyDepreciatedAsset(this, bookbal.AssetID)
						...
						case FARegister.origin.Reconcilliation:
							...
							// 
							if ((asset.Depreciable != true || asset.UnderConstruction == true )&& 
							...
							break;
						case FARegister.origin.Transfer:
							bookbal = (FABookBalance)bookbalance.Cache.Locate(bookbal) ?? bookbal;
							if (string.IsNullOrEmpty(bookbal.CurrDeprPeriod))
							{
								// the depreciation for assets under construction will start in the date of the transfer transaction 
								bookbal.CurrDeprPeriod = bookbal.DeprFromPeriod ?? fatran.FinPeriodID;
								bookbalance.Update(bookbal);
							}
							break;
					}
		...
		public static void TransferAsset(PXGraph graph, FixedAsset asset, FALocationHistory location, ref FARegister register)
		{
			...
			PXCache detailsCache = graph.Caches[typeof(FADetails)];

			if (asset.UnderConstruction == true)
			{
				FATran fatran = SelectFrom<FATran>
					.Where<FATran.tranType.IsIn<FATran.tranType.calculatedPlus, FATran.tranType.calculatedMinus, 
												FATran.tranType.depreciationPlus, FATran.tranType.depreciationMinus, 
												FATran.tranType.adjustingDeprPlus, FATran.tranType.adjustingDeprMinus>
						.And<FATran.assetID.IsEqual<@P.AsInt>>>
					.View.ReadOnly.Select(graph, asset.AssetID);

				if (fatran != null)
				{
					throw new PXException(Messages.AssetHasDepreciationTransactionsAndCannotBeTransferred);
				}
			}

			FAClass prevClass = graph.GetAssetClass(asset.OldClassID);
			var previousUnderConstructionValue = prevClass?.UnderConstruction == true;

			if (asset.UnderConstruction != true && previousUnderConstructionValue == true)
			{
				if ((detailsCache.Current as FADetails)?.DepreciateFromDate == null)
				{
					throw new PXException(PX.Data.ErrorMessages.FieldIsEmpty, PXUIFieldAttribute.GetDisplayName<FADetails.depreciateFromDate>(detailsCache));
				}
			}
			...
			PXResultset<FABookBalance> balset;  // This result set works wrong:
												// For the very first time, the query works incorrectly and returns all books and ignores the condition FABook.UpdateGL = true.
												// For every next time, the query works right and returns only posing book.
			if (fasetup.UpdateGL == true &&
				(RequireTransferAsset(oldAccounts, newAccounts) || RequireTransferDepreciation(asset, oldAccounts, newAccounts, previousUnderConstructionValue)) && 
				(balset = SelectFrom<FABookBalance>.Where<FABookBalance.assetID.IsEqual<@P.AsInt>>.View.Select(graph, asset.AssetID)).Count > 0)
			{
				if (register == null)
				{
				...
				if (asset.IsAcquired == true && !string.IsNullOrEmpty(location.PeriodID) && oldAccounts.BranchID != null)
				{
					foreach (FABookBalance bal in balset)
					{
						...
						{
							InsertTransferAssetTransaction(transactCache, ref register, bal, asset, location, oldAccounts, newAccounts, previousUnderConstructionValue);
						}
												
						if (RequireTransferDepreciation(asset, oldAccounts, newAccounts, previousUnderConstructionValue))
						{
		...
		{
			FixedAsset origAsset = graph.Caches[typeof(FixedAsset)].GetOriginal(asset) as FixedAsset ?? asset;
		...
		protected static FAAccounts GetNewAccounts(PXGraph graph, PXCache assetCache, PXCache locationsCache, FASetup fasetup, FixedAsset asset, FALocationHistory location)
		{
			...
			FAClass prevClass = graph.GetAssetClass(asset.ClassID);
			asset.UnderConstruction = prevClass.UnderConstruction;
		...
		protected static bool RequireTransferDepreciation(FixedAsset asset, FAAccounts oldAccounts, FAAccounts newAccounts, bool? previousUnderConstructionValue)
		{
			return
				asset.Depreciable == true &&
				previousUnderConstructionValue != true &&
				asset.UnderConstruction != true &&
				oldAccounts.AccumulatedDepreciationAccountID != null &&
				oldAccounts.AccumulatedDepreciationSubID != null &&
				(
					newAccounts.BranchID != oldAccounts.BranchID ||
		...
		protected static void InsertTransferAssetTransaction(PXCache transactCache, ref FARegister register, FABookBalance bal, FixedAsset asset, FALocationHistory location, FAAccounts oldAccounts, FAAccounts newAccounts, bool? previousUnderConstructionValue)
		{
			string finPeriodID;

			if(asset.UnderConstruction != true && previousUnderConstructionValue == true || asset.UnderConstruction == true)
			{
				finPeriodID = bal.CurrDeprPeriod ?? bal.LastPeriod;
			}
			else
			{
	...
	public static class FAHelper
	{
		...
		{
			FAClass prev = SelectFrom<FAClass>
				.Where<FAClass.assetID.IsEqual<@P.AsInt>>
				.View.Select(graph, classID);
