...
namespace PX.Objects.FS
{
    public class BillingCycleMaint : PXGraph<BillingCycleMaint, FSBillingCycle>
    {
        ...
        protected virtual void _(Events.RowPersisted<FSBillingCycle> e)
        {
            ...
            {
                PreUpdateBillingInfoDocs(this, null, fsBillingCycleRow.BillingCycleID);
            }
            ...
            if (e.TranStatus == PXTranStatus.Completed)
            {
                if (fsBillingCycleRow.UpdateCutOffDate == true)
                {
                    ...
                    UpdateBillingInfoInDocsLO(this, null, fsBillingCycleRow.BillingCycleID);
                }
            }
        }
        ...
        public virtual void UpdateBillingInfoInDocs(int? currentCustomerID, int? currentBillingCycleID)
        {
            ...
            foreach (PXResult<FSServiceOrder, FSSetup, FSCustomerBillingSetup, FSBillingCycle> result in resultSet)
            {
                ...
                FSBillingCycle fsBillingCycleRow = (FSBillingCycle)result;

                UpdateBillingInfoInDoc(tempGraph,
                                       fsServiceOrderRow,
                                       fsCustomerBillingSetupRow.CBID,
                                       fsCustomerBillingSetupRow.BillingCycleID,
                                       fsBillingCycleRow.BillingBy,
                                       fsBillingCycleRow.InvoiceOnlyCompletedServiceOrder,
        ...
        protected virtual void UpdateBillingInfoInDoc(
            PXGraph tempGraph,
            FSServiceOrder fsServiceOrderRow,
            int? newCBID,
            int? newBillingCycleID,
            string newBillingBy,
            bool? newBillOnlyCompletedClosed,
            bool updateCutOffDate,
            bool forceUpdate,
            Dictionary<BillingCycleIDAndDate, DateTime?> cutOffDateCache)
        {
            ...
            if (fsServiceOrderRow.PostedBy == null)
            {
                using (PXTransactionScope ts = new PXTransactionScope())
                {
                    ...
                    if (newBillingCycleID != null)
                    {
                        ...
                        {
                            newCutOffDate = GetCutOffDate(tempGraph, fsServiceOrderRow.CBID, fsServiceOrderRow.OrderDate, fsServiceOrderRow.SrvOrdType);
                            cutOffDateCache.Add(billingCycleIDAndDate, newCutOffDate);
                        }
                    }

                    if (fsServiceOrderRow.CBID != newCBID 
                            || fsServiceOrderRow.CutOffDate != newCutOffDate
                            || fsServiceOrderRow.BillingBy != newBillingBy
                            || fsServiceOrderRow.BillOnlyCompletedClosed != newBillOnlyCompletedClosed)
                    {
                        var billingCycle = 
                            (FSBillingCycle)PXSelect<FSBillingCycle, 
                                                Where<FSBillingCycle.billingCycleID, Equal<Required<FSBillingCycle.billingCycleID>>>>
                                           .Select(tempGraph, newBillingCycleID);

                        if (billingCycle != null && billingCycle.BillingBy == ID.Billing_By.APPOINTMENT)
                        {
                            PXUpdate<
                                Set<FSServiceOrder.cBID, Required<FSServiceOrder.cBID>,
                                Set<FSServiceOrder.cutOffDate, Required<FSServiceOrder.cutOffDate>,
                                Set<FSServiceOrder.allowInvoice, False>>>,
                            FSServiceOrder,
                            Where<
                        ...
                        PXUpdate<
                            Set<FSServiceOrder.cBID, Required<FSServiceOrder.cBID>,
                            Set<FSServiceOrder.cutOffDate, Required<FSServiceOrder.cutOffDate>,
                            Set<FSServiceOrder.billingBy, Required<FSServiceOrder.billingBy>,
                            Set<FSServiceOrder.billOnlyCompletedClosed, Required<FSServiceOrder.billOnlyCompletedClosed>>>>>,
                        FSServiceOrder,
                        Where<
                    ...
                    foreach (FSAppointment fsAppointmentRow in appointmentSet)
                    {
                        ...
                        if (newBillingCycleID != null)
                        {
                            ...
                            if (cutOffDateCache.TryGetValue(billingCycleIDAndDate, out newCutOffDate) == false)
                            {
                                newCutOffDate = GetCutOffDate(tempGraph, fsServiceOrderRow.CBID, fsAppointmentRow.ActualDateTimeEnd, fsAppointmentRow.SrvOrdType);
                                cutOffDateCache.Add(billingCycleIDAndDate, newCutOffDate);
                            }
                        }
