    ...
    public class PaymentReclassifyProcess : PXGraph<PaymentReclassifyProcess>
    {
        ...
        public static CATran AddAPTransaction(ICADocSource parameters, CurrencyInfo aCuryInfo, IList<ICADocAdjust> aAdjustments, bool aOnHold)
        {
            if (parameters.OrigModule == GL.BatchModule.AP)
            {
                ...
                doc.BranchID = te.Accessinfo.BranchID;
                ...
                if (aAdjustments != null)
                {
                    ...
                    {
						if (it.PaymentsByLinesAllowed == true)
						{
							foreach (APTran tran in
										...
										.SelectMultiBound(te, new object[] { it }))
							{
            ...
            if (parameters.OrigModule == GL.BatchModule.AR)
            {
                ...
                if (aAdjustments != null)
                {
					foreach (ARAdjust it in aAdjustments)
                    {
						...
						adjust.AdjdDocType = it.AdjdDocType;
						adjust.AdjdRefNbr = it.AdjdRefNbr;
						adjust.AdjdLineNbr = it.AdjdLineNbr;
						try
						{
							adjust = te.Adjustments.Insert(adjust);
						}
						...
						{
							adjust.CuryAdjgAmt = it.CuryAdjgAmt ?? adjust.CuryAdjgAmt ?? 0m;
							curyAppliedAmt += (adjust.CuryAdjgAmt ?? Decimal.Zero);
						}
						adjust.WriteOffReasonCode = it.WriteOffReasonCode ?? adjust.WriteOffReasonCode;
						adjust.WOBal = it.WOBal ?? adjust.WOBal;
						adjust.AdjWOAmt = it.AdjWOAmt ?? adjust.AdjWOAmt;
						adjust.CuryAdjdWOAmt = it.CuryAdjdWOAmt ?? adjust.CuryAdjdWOAmt;
						adjust.CuryAdjgWOAmt = it.CuryAdjgWOAmt ?? adjust.CuryAdjgWOAmt;
						adjust.CuryWOBal = it.CuryWOBal ?? adjust.CuryWOBal;
						adjust.CuryAdjgDiscAmt = it.CuryAdjgDiscAmt ?? adjust.CuryAdjgDiscAmt ?? 0m;
						adjust.CuryAdjgWOAmt = it.CuryAdjgWOAmt ?? adjust.CuryAdjgWOAmt ?? 0m;
