...
{
    public class ServiceOrderUpdateException : PXOuterException
    {
        public ServiceOrderUpdateException(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
        }

        public ServiceOrderUpdateException(Dictionary<string, string> innerExceptions, Type graphType, object row, string message)
            : base(innerExceptions, graphType, row, message)
        {
        }

        public ServiceOrderUpdateException(Dictionary<string, string> innerExceptions, Type graphType, object row, string format, params object[] args)
            : base(innerExceptions, graphType, row, format, args)
        {
        }
    }

    public class ServiceOrderUpdateException2 : PXException
    {
        public ServiceOrderUpdateException2(string format, params object[] args)
            : base(format, args)
        {
        }

        public ServiceOrderUpdateException2(string message, Exception innerException)
    ...
    public class SharedClasses
    {
        ...
        public class InvoiceItem
        {
                                 ...
                                 FSSrvOrdType fsSrvOrdTypeRow)
            {
                ...
                if (fsSODetBase != null)
                {
                    ...
                    {
                        acctID = ServiceOrderEntry.Get_TranAcctID_DefaultValueInt(graph,
            ...
            private void AddAppointmentLineToList(PXGraph graph, FSAppointmentDet fsAppointmentDetRow, FSAppointmentDet fsAppointmentInventoryItem, AppointmentInfo appointmentInfo, bool addToArApList, bool addToSoList)
            {
                ...
                else if (fsAppointmentInventoryItem != null)
                {
                    ...
                    appointmentLineRow.Qty              = fsAppointmentInventoryItem.ActualQty;
                    ...
                    appointmentLineRow.IsFree           = false;
                }
            ...
            private void AddSOLineToList(PXGraph graph, FSSODet fsSODetRow, ServiceOrderInfo serviceOrderInfo, bool addToArApList, bool addToSoList)
            {
                ...
                if (fsSODetRow != null)
                {
                    ...
                    soLineRow.IsFree           = fsSODetRow.IsFree;
                }
        ...
        public class SOARLineEquipmentComponent
        {
            ...
            public SOARLineEquipmentComponent(IDocLine docLine, SOLine sOLineRow, FSxSOLine fSxSOLineRow)
            {
                ...
                this.fsARTranRow = null;
            }

            public SOARLineEquipmentComponent(IDocLine docLine, ARTran arTranRow, FSARTran fsARTranRow)
            {
                this.componentID = docLine.ComponentID;
        ...
        public class SOPrepaymentBySO
        {
            ...
            public List<FSARTran> fsARTranList;

            public SOPrepaymentBySO(SOLine soLineRow, FSxSOLine fsxSOLineRow, decimal soTaxLine)
            {
                SrvOrdType = fsxSOLineRow.SrvOrdType;
                ServiceOrderRefNbr = fsxSOLineRow.ServiceOrderRefNbr;
                unpaidAmount = 0;
            ...
            public SOPrepaymentBySO(ARTran arTranRow, FSARTran fsARTran) : this(arTranRow, fsARTran, 0m)
            {
            }

            public SOPrepaymentBySO(ARTran arTranRow, FSARTran fsARTran, decimal arTaxLine)
            {
                SrvOrdType = fsARTran.SrvOrdType;
                ServiceOrderRefNbr = fsARTran.ServiceOrderRefNbr;
                unpaidAmount = 0;
                unpaidAmount += (arTranRow.CuryTranAmt ?? 0m) + arTaxLine;
                originalAmount = unpaidAmount;
                fsARTranList = new List<FSARTran>();
                fsARTranList.Add(fsARTran);
            }

            public PXResultset<ARPayment> GetPrepaymentBySO(PXGraph graph)
            {
                if (string.IsNullOrEmpty(ServiceOrderRefNbr) == true)
                {
                    return null;
                }
                ...
                return PXSelectReadonly2<ARPayment,
                       ...
                       Where<
                        ...
                        .Select(graph, SrvOrdType, ServiceOrderRefNbr);
            }
        }
        ...
        public class SOPrepaymentHelper
        {
            ...
            public void Add(SOLine soLineRow, FSxSOLine fsxSOLineRow, decimal soTaxLine)
            {
                ...
                SOPrepaymentBySO row = SOPrepaymentList.Find(x => x.SrvOrdType.Equals(fsxSOLineRow.SrvOrdType)
                                                                  && x.ServiceOrderRefNbr.Equals(fsxSOLineRow.ServiceOrderRefNbr));
            ...
            [Obsolete(Common.Messages.MethodIsObsoleteRemoveInLaterAcumaticaVersions)]
            public void Add(ARTran arTranRow, FSARTran fsARTranRow)
            {
                Add(arTranRow, fsARTranRow, 0m);
            }

            public void Add(ARTran arTranRow, FSARTran fsARTranRow, decimal arTaxLine)
            {
                if (fsARTranRow.ServiceOrderRefNbr == null)
                    return;

                SOPrepaymentBySO row = SOPrepaymentList.Find(x => x.SrvOrdType.Equals(fsARTranRow.SrvOrdType)
                                                                  && x.ServiceOrderRefNbr.Equals(fsARTranRow.ServiceOrderRefNbr));

                if (row != null)
                {
                    row.unpaidAmount += (arTranRow.CuryTranAmt ?? 0m) + arTaxLine;
                    row.fsARTranList.Add(fsARTranRow);
                }
                else
                {
                    SOPrepaymentBySO newRow = new SOPrepaymentBySO(arTranRow, fsARTranRow, arTaxLine);
                    SOPrepaymentList.Add(newRow);
                }
            }
        }
