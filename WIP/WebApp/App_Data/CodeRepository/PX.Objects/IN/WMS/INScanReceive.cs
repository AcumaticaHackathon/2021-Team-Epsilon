...
namespace PX.Objects.IN
{
	...
	public class INScanReceive : WMSBase
	{
		...
		protected override void _(Events.RowSelected<Header> e)
		{
			...
			if (Receipt == null && !string.IsNullOrEmpty(HeaderView.Current?.RefNbr))
		...
		protected override bool DocumentIsEditable => base.DocumentIsEditable && INRegister.PK.Find(Base, Receipt)?.Released != true;
		...
		protected virtual void ProcessConfirm()
		{
			...
			else
			{
				...
				var uiErrors = GetUIErrors(existTransaction);
				if (uiErrors.Any())
				{
			...
			if (Base.receipt.Cache.GetStatus(Base.receipt.Current) != PXEntryStatus.Inserted)
				SaveChanges();
