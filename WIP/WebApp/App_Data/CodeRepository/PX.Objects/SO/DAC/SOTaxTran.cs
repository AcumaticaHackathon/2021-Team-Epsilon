...
namespace PX.Objects.SO
{
    ...
    public partial class SOTaxTran : TaxDetail, PX.Data.IBqlTable
    {
		...
		public class UK
		{
			...
			{
				public static SOTaxTran Find(PXGraph graph, string recordID) => FindBy(graph, recordID);
			}
			public class ByJurisNameAndType : PrimaryKeyOf<SOTaxTran>.By<orderType, orderNbr, lineNbr, taxID, jurisName, jurisType>
			{
				public static SOTaxTran Find(PXGraph graph, string orderType, string orderNbr, int? lineNbr, string taxID, string jurisName, string jurisType)
					=> FindBy(graph, orderType, orderNbr, lineNbr, taxID, jurisName, jurisType);
			}
		}
