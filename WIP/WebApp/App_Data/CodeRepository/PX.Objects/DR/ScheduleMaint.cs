	...
	public class ScheduleMaint : PXGraph<ScheduleMaint>
	{
		...
		protected virtual void DRSchedule_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				{
					Document.Cache.RaiseExceptionHandling<DRScheduleDetail.documentType>(Document.Current, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(DRScheduleDetail.documentType)}]"));
				}

				if (string.IsNullOrEmpty(row.FinPeriodID))
				{
					Document.Cache.RaiseExceptionHandling<DRScheduleDetail.finPeriodID>(Document.Current, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(DRScheduleDetail.finPeriodID)}]"));
				}
			}
		}
