...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	public class CreatePaymentExt : CreatePaymentExtBase<SOOrderEntry, SOOrder, SOAdjust>
	{
		...
		[PXButton(ImageKey = PX.Web.UI.Sprite.Main.AddNew, Tooltip = Messages.CreatePayment, VisibleOnDataSource = false, PopupCommand = nameof(syncPaymentTransaction))]        
		...
		[PXButton(ImageKey = PX.Web.UI.Sprite.Main.AddNew, Tooltip = Messages.CreatePrepayment, VisibleOnDataSource = false, PopupCommand = nameof(syncPaymentTransaction))]
		...
		protected virtual void _(Events.FieldVerifying<SOOrder, SOOrder.cancelled> eventArgs)
		{
			if ((bool?)eventArgs.NewValue == true)
			{
				...
				if (adj != null)
					throw new PXException(Messages.OrderWithAppliedPaymentsCantBeCancelled, eventArgs.Row.OrderNbr, adj.AdjgRefNbr);
			}
		}
