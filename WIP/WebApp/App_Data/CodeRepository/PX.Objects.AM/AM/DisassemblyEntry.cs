...
namespace PX.Objects.AM
{
    ...
    /// </summary>
    public class DisassemblyEntry : PXGraph<DisassemblyEntry, AMDisassembleBatch>, ICaptionable<object>
    {
        ...
        {
            var row = item as AMDisassembleBatch;
            if (row == null || Document.Cache.GetStatus(row) == PXEntryStatus.Inserted)
            {
            ...
            var trantype = row.TranType == AMTranType.Disassembly ? AMTranType.ListDesc.Disassembly : AMTranType.ListDesc.Correction;
            return $"{trantype} {row.BatNbr}{desc}";
        }
    }
}