...
namespace PX.Objects.TX
{
	...
	public partial class TaxPeriodFilter : IBqlTable
	{
		...
		[OrganizationBAccountID(typeof(organizationID), typeof(branchID))]
...
>>>>>>>>>>>>))]
	...
	public class ReportTaxReview : PXGraph<ReportTax>
	{
		...
		{
			DateTime finStartDate = FinPeriodRepository.PeriodStartDate(filter.TaxPeriodID, filter.OrganizationID);
			DateTime finEndDate = FinPeriodRepository.PeriodEndDate(filter.TaxPeriodID, filter.OrganizationID);
			if ((finStartDate == null && finEndDate == null) || (filter.StartDate != finStartDate) || (filter.EndDate != finEndDate))
			{
				return false;
			}
			else
		...
		protected virtual void TaxPeriodFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			string existingError = PXUIFieldAttribute.GetErrorOnly<TaxPeriodFilter.taxPeriodID>(sender, filter);
			if (string.IsNullOrEmpty(existingError))
			{
				...
				Vendor vendor = Vendor.SelectWindowed(0, 1);
				if (vendor != null && vendor.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod && filter.StartDate != filter.EndDate && !IsSynchronizedWithFinPeriods(filter))
				{
					sender.RaiseExceptionHandling<TaxPeriodFilter.taxPeriodID>(filter, filter.TaxPeriodID, new PXSetPropertyException(Messages.ReleaseTaxRepPeriodNotSync, PXErrorLevel.Error, FinPeriodIDFormattingAttribute.FormatForError(filter.TaxPeriodID), PXAccess.GetOrganizationCD(filter.OrganizationID), Messages.SynchronizeWithGL, filter.TaxPeriodID.Substring(0, 4)));
				}
			}

			existingError = PXUIFieldAttribute.GetErrorOnly<TaxPeriodFilter.vendorID>(sender, filter);
			if (string.IsNullOrEmpty(existingError))
			{
				if (ReportTaxProcess.CheckForUnprocessedSVAT(this, filter.OrganizationID, filter.BranchID, Vendor.Select(), taxPeriod?.EndDate))
				{
					sender.RaiseExceptionHandling<TaxPeriodFilter.vendorID>(e.Row, filter.VendorID, new PXSetPropertyException(Messages.TaxReportHasUnprocessedSVAT, PXErrorLevel.Warning));
				}
				else
				{
		...
		{
			TaxPeriodFilter tp = Period_Header.Current;
			Vendor currentVendor = Vendor.SelectWindowed(0, 1);
			if (currentVendor != null && currentVendor.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod && tp.StartDate != tp.EndDate && !IsSynchronizedWithFinPeriods(tp))
			{
					throw new PXException(Messages.ReleaseTaxRepPeriodNotSync, FinPeriodIDFormattingAttribute.FormatForError(tp.TaxPeriodID), PXAccess.GetOrganizationCD(tp.OrganizationID), Messages.SynchronizeWithGL, tp.TaxPeriodID.Substring(0, 4));
			}
			else
			{
	...
	public class ReportTax : PXGraph<ReportTax>
	{
		...
		[InjectDependency]
		public IFinPeriodRepository FinPeriodRepository
		{
			get; set;
		}
		...
		protected virtual void TaxPeriodFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (preparedPeriod != null && preparedPeriod.TaxPeriodID != taxper.TaxPeriodID)
			{
				sender.RaiseExceptionHandling<TaxPeriodFilter.taxPeriodID>(e.Row, taxPeriodIDForMessage,
					new PXSetPropertyException<TaxPeriodFilter.taxPeriodID>(Messages.CannotPrepareReportExistPrepared, FinPeriodIDFormattingAttribute.FormatForError(preparedPeriod.TaxPeriodID), PXErrorLevel.Error));
			...
			string existingError = PXUIFieldAttribute.GetErrorOnly<TaxPeriodFilter.taxPeriodID>(sender, filter);
			if (string.IsNullOrEmpty(existingError) && vendor.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod && filter.StartDate != filter.EndDate && !IsSynchronizedWithFinPeriods(filter))
			{
				sender.RaiseExceptionHandling<TaxPeriodFilter.taxPeriodID>(filter, filter.TaxPeriodID, new PXSetPropertyException(
					Messages.PrepareTaxRepPeriodNotSync,
					PXErrorLevel.Error,
					FinPeriodIDFormattingAttribute.FormatForError(filter.TaxPeriodID),
					PXAccess.GetOrganizationCD(filter.OrganizationID),
					Messages.SynchronizeWithGL,
					filter.TaxPeriodID.Substring(0, 4)));
			}
		}
		...
		public virtual IEnumerable FileTax(PXAdapter adapter)
		{
			...
			Vendor currentVendor = Vendor.SelectWindowed(0, 1, p.VendorID);
			if (currentVendor != null && currentVendor.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod && p.StartDate != p.EndDate && !IsSynchronizedWithFinPeriods(p))
			{
				throw new PXException(Messages.PrepareTaxRepPeriodNotSync, FinPeriodIDFormattingAttribute.FormatForError(p.TaxPeriodID), PXAccess.GetOrganizationCD(p.OrganizationID), Messages.SynchronizeWithGL, p.TaxPeriodID.Substring(0, 4));
			}
			else
			{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (tranlist.Count > 0)
				{
					foreach (KeyValuePair<int?, KeyValuePair<APInvoice, List<APTran>>> pair in tranlist)
					{
						...
						new_apdoc.FinPeriodID = docgraph.vendor.Current.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod
								? taxper.TaxPeriodID
								: FinPeriodRepository.FindFinPeriodByDate(docDate, p.OrganizationID)?.FinPeriodID;
						FinPeriodUtils.VerifyAndSetFirstOpenedFinPeriod<APInvoice.finPeriodID, APInvoice.branchID>(docgraph.Document.Cache, new_apdoc, docgraph.finperiod, typeof(OrganizationFinPeriod.aPClosed));
						FinPeriod firstOpenedFinPeriod = FinPeriodRepository.GetByID(new_apdoc.FinPeriodID, p.OrganizationID);
						new_apdoc.DocDate = ((DateTime)firstOpenedFinPeriod.EndDate).AddDays(-1); ;
						...
						PXResultset<TaxTran> taxTransWOTaxRevs = PXSelectJoin<TaxTran,
							LeftJoin<TaxRev,
								...
								Where<TaxRev.revisionID, IsNull,
									And<TaxTran.released, Equal<True>,
									And<TaxTran.voided, Equal<False>,
									And<TaxTran.taxPeriodID, Equal<Required<TaxPeriod.taxPeriodID>>,
									And<TaxTran.taxType, NotEqual<TaxType.pendingPurchase>,
									And<TaxTran.taxType, NotEqual<TaxType.pendingSales>>>>>>>>.Select(this, taxPeriod.TaxPeriodID);
