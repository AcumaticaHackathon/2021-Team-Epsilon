...
namespace PX.Objects.CR.Extensions
{
		...
		where TDefLocationID : class, IBqlField
	{
		...
		protected virtual void SubstituteBaseLocationsView()
		{
			...
			*/
			if (BaseLocationsViewName != null
				&& Base.IsContractBasedAPI
				&& Base.Views.ContainsKey(BaseLocationsViewName))
			{
				Base.Views[BaseLocationsViewName] =
		...
		[PXFormula(typeof(Switch<
				Case<Where<CurrentValue<BAccount.defAddressID>, IsNull>, Null,
				Case<Where<CRLocation.defAddressID, Equal<CurrentValue<BAccount.defAddressID>>>, False>>,
			True>))]
		[PXMergeAttributes(Method = MergeMethod.Append)]
		protected virtual void _(Events.CacheAttached<CRLocation.overrideAddress> e) { }

		[PXFormula(typeof(Switch<
				Case<Where<CurrentValue<BAccount.defContactID>, IsNull>, Null,
				Case<Where<CRLocation.defContactID, Equal<CurrentValue<BAccount.defContactID>>>, False>>,
			True>))]
		[PXMergeAttributes(Method = MergeMethod.Append)]
		...
		[PXFormula(typeof(Switch<
				Case<Where<CurrentValue<BAccount.defAddressID>, IsNull>, Null,
				Case<Where<CRLocation.defAddressID, Equal<CurrentValue<BAccount.defAddressID>>>, True>>,
			False>))]
		[PXMergeAttributes(Method = MergeMethod.Append)]
		...
		[PXFormula(typeof(Switch<
				Case<Where<CurrentValue<BAccount.defContactID>, IsNull>, Null,
				Case<Where<CRLocation.defContactID, Equal<CurrentValue<BAccount.defContactID>>>, True>>,
			False>))]
		[PXMergeAttributes(Method = MergeMethod.Append)]
		...
		public virtual void SetLocationAsDefault(CRLocation row)
		{
			...
			foreach (CRLocation other in Base1.Locations.Select()) 
			{
				...
				if (!object.Equals(other.LocationID, other.VPaymentInfoLocationID))
				{
					...
					if (object.Equals(other, row) && current != null)
					{
						...
						other.VPaymentByType = current.VPaymentByType;
		...
		public virtual void DefaultFrom<TSourceField>(PXFieldDefaultingEventArgs e, PXCache source, object sourceObject, bool allowNull = true, object nullValue = null)
		...
		public virtual void DefaultFrom<TSourceField>(PXFieldDefaultingEventArgs e, PXCache source, bool allowNull = true, object nullValue = null)
			where TSourceField : IBqlField
		{
			DefaultFrom<TSourceField>(e, source, source.Current, allowNull, nullValue);
		}
