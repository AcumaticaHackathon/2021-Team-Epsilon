    ...
    public class SM_SOOrderEntry : FSPostingBase<SOOrderEntry>, IInvoiceContractGraph
    {
        ...
        protected virtual void _(Events.RowPersisted<SOOrder> e)
        {
            ...
            if (e.TranStatus == PXTranStatus.Open)
            {
                if (e.Operation == PXDBOperation.Delete)
                {
                    var requiredAllocationList = FSAllocationProcess.GetRequiredAllocationList(Base, soOrderRow);

                    FSAllocationProcess.ReallocateServiceOrderSplits(requiredAllocationList);

                    CleanPostingInfoLinkedToDoc(soOrderRow);
                    CleanContractPostingInfoLinkedToDoc(soOrderRow);
                }
        ...
        {
            return MessageHelper.GetErrorInfo<SOLine>(Base.Document.Cache, Base.Document.Current, Base.Transactions);
        }
        ...
        public override void CreateInvoice(PXGraph graphProcess, List<DocLineExt> docLines, short invtMult, DateTime? invoiceDate, string invoiceFinPeriodID, OnDocumentHeaderInsertedDelegate onDocumentHeaderInserted, OnTransactionInsertedDelegate onTransactionInserted, PXQuickProcess.ActionFlow quickProcessFlow)
        {
            ...
            foreach (DocLineExt line in docLines)
            {
                ...
                sOLineRow.SalesPersonID = fsAppointmentRow == null ? fsServiceOrderRow.SalesPersonID : fsAppointmentRow.SalesPersonID;
        ...
        protected virtual void Configure(WorkflowContext<SOOrderEntry, SOOrder> context)
        {
            ...
            var createServiceOrder = context.ActionDefinitions
                .CreateExisting<SM_SOOrderEntry>(g => g.CreateServiceOrder, a => a
                .InFolder(FolderType.ActionsFolder)
                .IsHiddenWhen(!conditions.IsFSActivatedForOrderType));

            var openAppointmentBoard = context.ActionDefinitions
                .CreateExisting<SM_SOOrderEntry>(g => g.OpenAppointmentBoard, a => a
                .InFolder(FolderType.ActionsFolder)
                .IsHiddenWhen(!conditions.IsFSActivatedForOrderType)
                .PlaceAfter(createServiceOrder));

            var viewServiceOrder = context.ActionDefinitions
                .CreateExisting<SM_SOOrderEntry>(g => g.ViewServiceOrder, a => a
            ...
            context.UpdateScreenConfigurationFor(screen =>
            {
                return screen
                    .WithActions(actions =>
                    {
                        actions.Add(createServiceOrder);
                        actions.Add(openAppointmentBoard);
                        actions.Add(viewServiceOrder);
                    });
            });
        }
