	...
	public abstract class DiscountEngine : PXGraph<DiscountEngine>
	{
			...
			bool ignoreCurrency = false)
		{
			...
			if (!ignoreCurrency && dLine?.MappedLine != null
				...
				CuryConvBase(cache, dLine.MappedLine, curyDiscountableAmount, out curyDiscountableAmount);
		...
		protected virtual DiscountDetailLine CreateDiscountDetails(PXCache cache, DiscountLineFields dLine, PXResult<DiscountSequence, DiscountSequenceDetail> discountResult, DateTime date)
		{
			...
			{
				CuryConvCury(cache, dLine.MappedLine, (decimal)bestDiscountDetail.Discount, out decimal discountAmount, CommonSetupDecPl.PrcCst);
			...
			else
			{
				if (dLine != null)
				{
					CuryConvCury(cache, dLine.MappedLine, bestDiscountDetail.Amount ?? 0m, out decimal curyAmount, CommonSetupDecPl.PrcCst);
					newDiscountDetail.AmountFrom = curyAmount;
					if (bestDiscountDetail.AmountTo != null)
					{
						CuryConvCury(cache, dLine.MappedLine, bestDiscountDetail.AmountTo ?? 0m, out decimal curyAmountTo, CommonSetupDecPl.PrcCst);
		...
		public static void RemoveFromCachedInventoryPriceClasses(int? inventoryID)
		{
			...
			var cachedInventoryPriceClasses = PXDatabase.GetSlot<ConcurrentDictionary<int, string>>(InventoryPriceClassesSlotName, typeof(DummyTable));
		...
		public static void RemoveFromCachedCustomerPriceClasses(int? bAccountID)
		{
			...
			var cachedCustomerPriceClasses = PXDatabase.GetSlot<ConcurrentDictionary<Tuple<int, int>, string>>(CustomerPriceClassIDSlotName, typeof(DummyTable));
		...
		internal static ApplyQuantityDiscountByBaseUOMOption ApplyQuantityDiscountByBaseUOM(PXGraph graph)
		{
				...
				PXAccess.FeatureInstalled<FeaturesSet.vendorDiscounts>() &&
				CurrentConfiguration.ActualAPSetup.ApplyQuantityDiscountBy ==
					ApplyQuantityDiscountType.BaseUOM,

				PXAccess.FeatureInstalled<FeaturesSet.customerDiscounts>() &&
				CurrentConfiguration.ActualARSetup.ApplyQuantityDiscountBy ==
					ApplyQuantityDiscountType.BaseUOM);
		}
		...
		public enum DiscountCalculationOptions
		{
			...
			[Description("Set this flag to disable all document discount recalculation except for External Document Discounts")]
			CalculateExternalDocumentDiscountsOnly = 256
		}
