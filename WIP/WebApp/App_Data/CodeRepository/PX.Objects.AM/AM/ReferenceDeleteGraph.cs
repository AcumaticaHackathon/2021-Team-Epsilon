...
namespace PX.Objects.AM
{
    ...
    public class ReferenceDeleteGraph : PXGraph<ReferenceDeleteGraph>
    {
        ...
        /// <summary>
        /// Indicates if given <see cref="AMMTran"/> record contains related transactions that have been released
        /// </summary>
        ...
        {
            return HasReleasedReferenceDocs(graph, ammTran, false);
        }
        ...
        /// <returns>True if released records found</returns>
        public static bool HasReleasedReferenceDocs(PXGraph graph, AMMTran ammTran, bool includeSubReferences)
        {
            ...
            if (includeSubReferences)
            {
                foreach (AMMTran subTran in AMLineReferences(graph, ammTran, false))
                {
                    var inCheckSub = INLineReferences(graph, subTran, true);
                    if (inCheckSub != null && inCheckSub.Count > 0)
                    {
                        return true;
                    }

                    var glCheckSub = GLLineReferences(graph, subTran, true);
                    if (glCheckSub != null && glCheckSub.Count > 0)
                    {
                        return true;
                    }
                }
        ...
        /// <summary>
        /// Indicates if given <see cref="AMBatch"/> record contains related transactions that have been released
        /// </summary>
        /// <param name="graph">Calling graph</param>
        ...
        {
            return HasReleasedReferenceDocs(graph, amBatch, false);
        }
        ...
        /// <param name="includeSubReferences">When true we will check child AM references and its related IN and GL transactions</param>
        /// <returns>True if released records found</returns>
        public static bool HasReleasedReferenceDocs(PXGraph graph, AMBatch amBatch, bool includeSubReferences)
        {
            ...
            if(includeSubReferences)
            {
                foreach(AMBatch subBatch in AMBatchReferences(graph, amBatch, false))
                {
                    var inCheckSub = INBatchReferences(graph, subBatch, true);
                    if (inCheckSub != null && inCheckSub.Count > 0)
                    {
                        return true;
                    }

                    var glCheckSub = GLBatchReferences(graph, subBatch, true);
                    if (glCheckSub != null && glCheckSub.Count > 0)
                    {
                        return true;
                    }
