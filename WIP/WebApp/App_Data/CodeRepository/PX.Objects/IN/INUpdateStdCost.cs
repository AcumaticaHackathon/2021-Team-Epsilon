		...
		public virtual void UpdateStdCost(INUpdateStdCostRecord itemsite)
		{
			using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					{
						PXResultset<INItemSite> itemSites = PXSelect<INItemSite,
						Where<INItemSite.inventoryID, Equal<Required<INItemSite.inventoryID>>,
							And<INItemSite.valMethod, Equal<INValMethod.standard>,
							And<INItemSite.stdCostOverride, Equal<False>>>>>.Select(this, itemsite.InventoryID);
						...
						if (itemSites.Any(x => ((INItemSite)x).Active == false))
						{
							...
							foreach (INItemSite site in itemSites.Where(x => ((INItemSite)x).Active == true))
							{
								PXDatabase.Update<INItemSite>(
									...
									new PXDataFieldRestrict("SiteID", PXDbType.Int, site.SiteID),
