	...
	public class CustomerMaint : PXGraph<CustomerMaint, Customer>
	{
		...
		public virtual IEnumerable NewSalesOrder(PXAdapter adapter)
		{
			...
			if (customer != null && customer.BAccountID > 0L)
			{
				...
				newDoc.ContactID = customer.PrimaryContactID;
		...
		public PXAction<Customer> Activate;
		...
		protected virtual IEnumerable activate(PXAdapter adapter) => adapter.Get<Customer>();

		public PXAction<Customer> PutOnHold;
		...
		protected virtual IEnumerable putOnHold(PXAdapter adapter) => adapter.Get<Customer>();

		public PXAction<Customer> PutOnCreditHold;
		...
		protected virtual IEnumerable putOnCreditHold(PXAdapter adapter) => adapter.Get<Customer>();

		public PXAction<Customer> RemoveCreditHold;
		...
		protected virtual IEnumerable removeCreditHold(PXAdapter adapter) => adapter.Get<Customer>();

		public PXAction<Customer> Deactivate;
		...
		protected virtual IEnumerable deactivate(PXAdapter adapter) => adapter.Get<Customer>();

		public PXAction<Customer> SetAsOneTime;
		...
		protected virtual IEnumerable setAsOneTime(PXAdapter adapter) => adapter.Get<Customer>();

		public PXAction<Customer> SetAsRegular;
		...
		protected virtual IEnumerable setAsRegular(PXAdapter adapter) => adapter.Get<Customer>();
		...
		protected virtual void Customer_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetRequired<Customer.cOrgBAccountID>(cache, PXAccess.FeatureInstalled<FeaturesSet.multipleBaseCurrencies>());
		...
		protected virtual void Customer_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			Customer oldRow = e.OldRow as Customer;
			...
			if (row.ParentBAccountID == null)
			{
				...
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers [Graph should be created for WF action invokation]
				// Acuminator disable once PX1071 PXActionExecutionInEventHandlers [Action should be invoked as statuses can only be changed via WF actions]
				UpdateChildAccountsStatus(cache, row, GetChildAccounts(sharedCreditPolicy: true).Where(func), newValue);
			}
			else if (!this.UnattendedMode && row.SharedCreditChild == true)
			{
				Customer parent = GetCustomerParent(row);

				if (parent != null)
				{
					string newValue = (string)row.Status;
					...
					if (!isValidStatus)
					{
						cache.RaiseExceptionHandling<Customer.status>(row, oldRow.Status, new PXSetPropertyException(Messages.SharedChildCreditHoldChange));
					}
				}
			}
		}
		...
		protected virtual void UpdateChildAccountsStatus(PXCache cache, Customer parent, IEnumerable<Customer> enumr, string targetStatus)
		{
			if (PXAccess.FeatureInstalled<FeaturesSet.parentChildAccount>() &&
				parent != null &&
				parent.ParentBAccountID == null)
			{
				foreach (Customer child in enumr)
				{
					if (targetStatus != (cache.GetValue<Customer.status>(child) as string))
					{
						CustomerMaint_Workflow.SetStatusTo(child, targetStatus);
					}
				}
			}
		}
		...
		public class PaymentDetailsExt : PXGraphExtension<CustomerMaint>
		{
			...
			protected virtual void _(Events.FieldUpdated<CustomerPaymentMethodDetail, CustomerPaymentMethodDetail.value> e)
			{
				...
				if (def != null)
				{
					...
					{
						string id = CustomerPaymentMethodMaint.IDObfuscator.AdjustedMaskID(row.Value, def.DisplayMask);
			...
			.WithCombinedTypeValidation
		{
			...
			public override bool DoValidateAddresses()
			{
				bool res = base.DoValidateAddresses();

				Address billAddress = Base.BillAddress.SelectSingle();
				if (billAddress != null && billAddress.IsValidated == false)
				{
					if (PXAddressValidator.Validate<Address>(Base, billAddress, true))
					{
						return res;
					}
				}
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class LocationDetailsExt : LocationDetailsExt<CustomerMaint, DefContactAddressExt, Customer, Customer.bAccountID> { }
