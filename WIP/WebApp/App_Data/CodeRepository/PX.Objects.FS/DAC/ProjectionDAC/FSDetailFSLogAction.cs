...
namespace PX.Objects.FS
{
    ...
    public class FSDetailFSLogAction : IBqlTable
    {
        ...
        {
            public static FSDetailFSLogAction Find(PXGraph graph, string srvOrdType, string refNbr, int? lineNbr) => FindBy(graph, srvOrdType, refNbr, lineNbr);
        }

        public class UK : PrimaryKeyOf<FSDetailFSLogAction>.By<appDetID>
        {
            public static FSDetailFSLogAction Find(PXGraph graph, int? appDetID) => FindBy(graph, appDetID);
        }
