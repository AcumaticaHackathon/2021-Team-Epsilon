                ...
                if (schdTime > 0m)
                {
                    if (wcSchdDetail != null)
                    {
                        ...
                        else
                        {
                            ...
                            wcSchdDetail.RunTimeBase = schdTime;
                            wcSchdDetail.RunTime = schdEfficiencyTime;
                            wcSchdDetail.SchdTime = wcSchdDetail.RunTime;

                            if (UseShiftCrewSizeEfficiency(workCenter) && Features.AdvancedPlanningEnabled())
                            {
                                wcSchdDetail.RunTimeBase = CalcTotalEfficiencyTimeReverse(wcSchdDetail.RunTimeBase, amShift?.CrewSize, amShift?.ShftEff);
                            }
