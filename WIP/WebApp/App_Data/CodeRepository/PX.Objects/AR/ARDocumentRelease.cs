	...
	public class ARDocumentRelease : PXGraph<ARDocumentRelease>
	{
		...
		public PXProcessingJoin<BalancedARDocument,
			LeftJoin<ARInvoice, On<ARInvoice.docType, Equal<BalancedARDocument.docType>,
				...
				And<ARRegister.hold, Equal<boolFalse>, And<ARRegister.voided, Equal<boolFalse>, And<ARRegister.scheduled, Equal<boolFalse>,
				And<Where<BalancedARDocument.released, Equal<boolFalse>, And<BalancedARDocument.origModule, In3<GL.BatchModule.moduleAR, GL.BatchModule.moduleEP>,
					Or<BalancedARDocument.released, Equal<boolTrue>, And<BalancedARDocument.openDoc, Equal<boolTrue>, And<ARAdjust.adjdRefNbr, IsNotNull>>>>>>>>>>>
			ARDocumentList;
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				try
				{
					...
					if (onsuccess != null || onreleasecomplete != null)
					{
						...
						PXTimeStampScope.DuplicatePersisted(rg.ARDocument.Cache, doc, typeof(ARPayment));
					}
		...
		private void SetClosedPeriodsFromLatestApplication(ARRegister doc)
		{
			...
			foreach (string docType in docTypes)
			{
				foreach (ARAdjust adj in PXSelect<ARAdjust,
					...
					.Select(this, docType, doc.RefNbr, docType, doc.RefNbr))
				{
			...
			if (invoicesNotCSL.Count() > 0)
			{
				foreach (SOInvoice soinvoice in invoicesNotCSL)
				{
					...
					if (soinvoice.IsCCCaptured == true)
					{
						...
						RaiseInvoiceEvent(invoice, ARInvoice.Events.Select(ev => ev.OpenDocument));
				...
				else if (ardoc.DocType != ARDocType.SmallCreditWO)
				{
					...
					if (ardoc.IsChildRetainageDocument())
					{
						...
						if (origRetainageDoc != null)
						{
							...
							decimal sign = origRetainageDoc.SignAmount * ardoc.SignAmount ?? 0m;

							origRetainageDoc.CuryRetainageUnreleasedAmt -= (ardoc.CuryOrigDocAmt + ardoc.CuryRoundDiff) * sign;
...
;						}
				...
				if (_IsIntegrityCheck == false)
				{
							...
							And<ARAdjust.voided, Equal<False>>>>>>.Select(this, doc.DocType, doc.RefNbr))
					{
						...
						if (adj.CuryAdjdAmt > 0m)
						{
							...
							if (string.Compare(payment.AdjTranPeriodID, adj.AdjgTranPeriodID) >= 0)
							{
			...
			decimal docInclTaxDiscrepancy)
		{
			if (docInclTaxDiscrepancy != 0)
			{
				if (Math.Abs(docInclTaxDiscrepancy) > CurrencyCollection.GetCurrency(currencyInfo.BaseCuryID).RoundingLimit &&
				...
				)
			{
					throw new PXException(AP.Messages.RoundingAmountTooBig,
						...
						IN.PXDBQuantityAttribute.Round(CurrencyCollection.GetCurrency(currencyInfo.BaseCuryID).RoundingLimit));
				}
		...
		protected virtual void VerifyRoundingAllowed(ARInvoice document, Batch batch, string baseCuryID, decimal taxesDiff)
		{
			...
			if (roundDiff > CurrencyCollection.GetCurrency(baseCuryID).RoundingLimit)
			{
				throw new PXException(AP.Messages.RoundingAmountTooBig, baseCuryID, roundDiff,
					IN.PXDBQuantityAttribute.Round(CurrencyCollection.GetCurrency(baseCuryID).RoundingLimit));
			}
		}
		...
		public virtual void VerifyAdjustedDocumentAndClose(ARRegister adjddoc)
		{
			...
			if (ardoc.IsOriginalRetainageDocument()
				? IsFullyProcessedOriginalRetainageDocument(ardoc)
			...
			Tuple<ARAdjust, CurrencyInfo> lastAdjustment)
		{
			...
			if (hasZeroBalance || isVoidingDoc)
			{
			...
			if (paymentRegister.IsChildRetainageDocument() &&
		...
		protected void DeactivateOneTimeCustomerIfAllDocsIsClosed(Customer customer)
		{
			...
			CustomerMaint_Workflow.SetStatusTo(customer, CustomerStatus.Inactive);
		}
			...
			Currency paycury)
		{
				...
				foreach (PXResult<ARAdjust, CurrencyInfo, Currency, ARRegister, ARInvoice, ARPayment, ARTran> adjres in group)
			{
				...
				ARTran line = adjres;
					...
					{
						PXCache<ARRegister>.RestoreCopy(adjgdoc, (ARRegister)adjres);
				...
				if (adjustedInvoice.RefNbr != null)
				{
					if(notValidatingCrossCustomerFromAdjusting)
					{
							...
							if (_IsIntegrityCheck == false && payment.Released == true && payment.AdjDate < adjustedInvoice.DocDate)
							{
				...
				Batch oldBatch = (Batch)je.BatchModule.Cache.CreateCopy(je.BatchModule.Current);
					ProcessAdjustmentAdjusting(je, adj, payment, adjustedInvoice, paymentCustomer, new_info);
		...
		private void ProcessAdjustmentSalesPersonCommission(ARAdjust adj, ARPayment payment, ARRegister adjustedInvoice, Currency cury, ARTran tran)
		{
			if (payment.DocType != ARDocType.CreditMemo) //Credit memos are treates as negative invoice
			{
				...
				{
					ARSalesPerTran invoiceSPT = PXSelect<ARSalesPerTran,
					...
					ARSalesPerTran paymentSPT = PXSelect<
						ARSalesPerTran,
						Where<ARSalesPerTran.docType, Equal<Required<ARSalesPerTran.docType>>,
							And<ARSalesPerTran.refNbr, Equal<Required<ARSalesPerTran.refNbr>>,
							And<ARSalesPerTran.salespersonID, Equal<Required<ARSalesPerTran.salespersonID>>,
		...
		private void ProcessAdjustmentAdjusting(JournalEntry je, ARAdjust adj, ARPayment payment, ARRegister adjustedDocument, Customer paymentCustomer, CurrencyInfo new_info)
		{
			...
			tran.TranClass = adj.IsOrigSmallCreditWOApp() ? GLTran.tranClass.Normal : GLTran.tranClass.Payment;
		...
		private void ProcessAdjustmentAdjusted(JournalEntry je, ARAdjust adj, ARRegister adjustedDocument, ARPayment payment, CurrencyInfo vouch_info, CurrencyInfo new_info)
		{
			...
			tran.TranClass = adj.IsOrigSmallCreditWOApp() ? GLTran.tranClass.Payment : ARDocType.DocClass(adj.AdjdDocType);
			tran.RefNbr = adj.IsOrigSmallCreditWOApp() ? adj.AdjdRefNbr : adj.AdjgRefNbr;
			tran.TranDesc = adj.IsOrigSmallCreditWOApp() ? adjustedDocument.DocDesc : payment.DocDesc;
			tran.TranDate = adj.AdjgDocDate;
			FinPeriodIDAttribute.SetPeriodsByMaster<GLTran.finPeriodID>(
		...
		protected virtual void PerformBasicReleaseChecks(PXGraph selectGraph, ARRegister document)
		{
			...
			if (arsetup.IntegratedCCProcessing == true && documentAsPayment?.SyncLock == true)
			{
			...
			{
				var soInvoice = SOInvoice.PK.Find(this, document.DocType, document.RefNbr);
				if (soInvoice?.HasLegacyCCTran == true)
				{
					throw new PXException(SO.Messages.CantProcessSOInvoiceBecauseItHasLegacyCCTran, soInvoice.RefNbr);
				}
					...
					if (doc.Released != true &&
						...
						if (_IsIntegrityCheck == false &&
							...
							foreach (ARInvoice updInvoice in updInvoices)
								PXTimeStampScope.DuplicatePersisted(ARDocument.Cache, updInvoice, typeof(ARInvoice));
						...
						foreach (PXResult<ARAdjust> adjres in ARAdjust_AdjgDocType_RefNbr_CustomerID.Select(doc.DocType, doc.RefNbr, _IsIntegrityCheck, doc.AdjCntr))
							...
							if (!appsByPeriod.TryGetValue(adj.AdjgTranPeriodID, out apps))
								{
						...
						foreach (KeyValuePair<string, List<PXResult<ARAdjust>>> pair in appsByPeriod)
								{
							FinPeriod postPeriod = FinPeriodRepository.GetFinPeriodByMasterPeriodID(PXAccess.GetParentOrganizationID(doc.BranchID), pair.Key).GetValueOrRaiseError();

							JournalEntry.SegregateBatch(je, BatchModule.AR, doc.BranchID, doc.CuryID, datesByPeriod[pair.Key], postPeriod.FinPeriodID,
		...
		protected virtual PXResultset<ARRegister> GetDocumentsForIntegrityCheckProc(string minPeriod)
		{
			...
			PXResultset<ARRegister> custdocs = new PXResultset<ARRegister>();
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.retainage>())
			{
				...
				var origdocs = custdocs.RowCast<ARRegister>().ToHashSet(ARDocument.Cache.GetComparer());
				foreach (var retdoc in retainage)
				{
					if (!origdocs.Contains((ARRegister)retdoc))
					{
						custdocs.Add(retdoc);
					}
				}
		...
		{
			var allDocsKeys = documents.RowCast<ARRegister>().ToHashSet(ARDocument.Cache.GetComparer());
		...
		protected virtual void RaiseInvoiceEvent(ARRegister doc, PX.Data.WorkflowAPI.SelectedEntityEvent<ARInvoice> invEvent)
		{
			if (doc is ARInvoice)
			{
				invEvent.FireOn<ARRegister>(this, (ARInvoice)doc);
				ARDocument.Cache.Update(doc);
		...
		protected virtual void RaisePaymentEvent(ARRegister doc, PX.Data.WorkflowAPI.SelectedEntityEvent<ARPayment> pntEvent)
		{
			if (doc is ARPayment)
			{
				pntEvent.FireOn<ARRegister>(this, (ARPayment)doc);
				ARDocument.Cache.Update(doc);
				ARDocument.Cache.RestoreCopy(doc, ARDocument.Cache.Locate(doc));
			}
		}
		...
		protected virtual void RaiseReleaseEvent(ARRegister doc)
		{
			if (ARDocument.Cache.ObjectsEqual(doc, ARInvoice_DocType_RefNbr.Current))
			{
				...
				ARInvoice.Events
					...
					.FireOn<ARRegister>(this, invoice);
			...
			else if (ARDocument.Cache.ObjectsEqual(doc, ARPayment_DocType_RefNbr.Current))
			{
				...
				ARPayment.Events
					.Select(e => e.ReleaseDocument)
					.FireOn<ARRegister>(this, payment);
