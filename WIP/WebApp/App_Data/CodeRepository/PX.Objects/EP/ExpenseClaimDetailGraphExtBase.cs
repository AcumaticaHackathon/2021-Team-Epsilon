		...
		where TGraph : PXGraph
	{
		...
		public virtual void RefNbrUpdated(PXCache receiptsCache, EPExpenseClaim claim, EPExpenseClaimDetails receipt, string oldRefNbr)
		{
			...
			if (claim != null)
			{
				...
				EPExpenseClaim oldClaim = (EPExpenseClaim)Claim.Cache.CreateCopy(claim);
				var oldReceipt = CalculateReceiptAndClaimAmounts(receipt);

                var sumCalculationIsNeeded = IsSumCalculationNeeded(claim, oldClaim, receipt, oldReceipt, oldRefNbr);

                if (sumCalculationIsNeeded && (!receipt.IsPaidWithCard || receipt.IsPaidWithCard && receipt.CorpCardID != null))
				{
                ...
                {
                    RecalculateAmountsInOldClaim(receipt, oldRefNbr);
                }
			}
			else
			{
        ...
        private static bool IsSumCalculationNeeded(EPExpenseClaim newClaim, EPExpenseClaim oldClaim, EPExpenseClaimDetails newReceipt, 
			EPExpenseClaimDetails oldReceipt, string oldRefNbr)
		{
            var receiptAmountHasChanged = false;
            var claimAmountHasChanged = false;
			if (oldReceipt.ClaimCuryTranAmt != newReceipt.ClaimCuryTranAmt ||
                oldReceipt.ClaimCuryTranAmtWithTaxes != newReceipt.ClaimCuryTranAmtWithTaxes ||
                oldReceipt.ClaimCuryTaxTotal != newReceipt.ClaimCuryTaxTotal ||
                oldReceipt.ClaimCuryTaxRoundDiff != newReceipt.ClaimCuryTaxRoundDiff ||
                oldReceipt.ClaimCuryVatExemptTotal != newReceipt.ClaimCuryVatExemptTotal ||
                oldReceipt.ClaimCuryVatTaxableTotal != newReceipt.ClaimCuryVatTaxableTotal)
            {
                receiptAmountHasChanged = true;
            }
            if (oldClaim.CuryDocBal != newClaim.CuryDocBal ||
                oldClaim.CuryTaxTotal != newClaim.CuryTaxTotal ||
                oldClaim.CuryTaxRoundDiff != newClaim.CuryTaxRoundDiff ||
                oldClaim.CuryVatExemptTotal != newClaim.CuryVatExemptTotal ||
                oldClaim.CuryVatTaxableTotal != newClaim.CuryVatTaxableTotal)
            {
                claimAmountHasChanged = true;
			}
			if (receiptAmountHasChanged && claimAmountHasChanged) return false; //Sum calculation has done
			if (receiptAmountHasChanged && !claimAmountHasChanged) return true; //Sum calculation has not done
			else if (oldRefNbr == null || newClaim.RefNbr != oldRefNbr) return true; //New claim or adding receipt to an existing claim
            return false;
        }
        ...
        private void RecalculateAmountsInOldClaim(EPExpenseClaimDetails receipt, string oldRefNbr)
        {
            ...
            if (oldClaim != null)
            {
                SumClaimValues(oldClaim, null, receipt);
            }
        }
		...
		public virtual void SumClaimValues(EPExpenseClaim claim, EPExpenseClaimDetails receipt, EPExpenseClaimDetails oldReceipt)
		{
			...
			var t = (decimal?)sumCalculator.Calculate<EPExpenseClaimDetails.claimCuryTranAmtWithTaxes>(Receipts.Cache, receipt, oldReceipt);
			newClaim.CuryDocBal += t;
		...
		{
			if (oldRow.CuryTranAmt != newRow.CuryTranAmt ||
				oldRow.CuryTranAmtWithTaxes != newRow.CuryTranAmtWithTaxes ||
				oldRow.TranAmtWithTaxes != newRow.TranAmtWithTaxes ||
				oldRow.CuryTaxTotal != newRow.CuryTaxTotal ||
				oldRow.CuryTaxRoundDiff != newRow.CuryTaxRoundDiff ||
				oldRow.CuryVatTaxableTotal != newRow.CuryVatTaxableTotal ||
				oldRow.CuryVatExemptTotal != newRow.CuryVatExemptTotal)
			{
				RecalcAmountInClaimCury(newRow);
			}
		}
		...
		public static object MakeSubAccountByMaskForReceipt<SubMaskField, CompanySubField, EmployeeSubField, ItemSubField, ProjectSubField, TaskSubField>(PXCache receiptCache, EPExpenseClaimDetails receipt, string subMask)
			...
			where TaskSubField : IBqlField
		{
			...
			Contract contract = PXSelect<Contract, Where<Contract.contractID, Equal<Required<Contract.contractID>>>>.Select(receiptCache.Graph,
																															receipt.ContractID);
			PMTask task = PMTask.PK.Find(receiptCache.Graph, receipt.TaskID);
			...
			int? project_SubID = (int?)receiptCache.Graph.Caches[typeof(Contract)].GetValue<ProjectSubField>(contract);
			int? task_SubID = (int?)receiptCache.Graph.Caches[typeof(PMTask)].GetValue<TaskSubField>(task);
			...
			value = SubAccountMaskAttribute.MakeSub<SubMaskField>(receiptCache.Graph, subMask,
				...
				new Type[] { typeof(EmployeeSubField), typeof(ItemSubField), typeof(CompanySubField), typeof(ProjectSubField), typeof(TaskSubField), typeof(Location.cSalesSubID) });
			...
			if (row.SalesAccountID != null)
			{
				...
				if (row.Billable == true)
				{
					value = MakeSubAccountByMaskForReceipt<EPSetup.salesSubMask,
																				...
																				PMTask.defaultSalesSubID>(sender, row, salesSubMask);
				}
		...
		public static void ExpenseSubID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e, string expenseSubMask)
		{
			...
			if (row.ExpenseAccountID != null)
			{
				object value = MakeSubAccountByMaskForReceipt<EPSetup.expenseSubMask,
																					...
																					PMTask.defaultExpenseSubID>(sender, row, expenseSubMask);
