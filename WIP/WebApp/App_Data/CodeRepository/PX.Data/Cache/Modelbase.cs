...
namespace PX.Data
{
	...
	public abstract partial class PXCache
	{
		...
		protected internal virtual object GetValueInt(object data, string fieldName, bool forceState, bool externalCall) { return null; }
		/// <exclude/>
		...
		internal virtual bool _InvokeMeetVerifier(object data, SQLTree.Query query, object[] pars)
		{
			return true;
		}
        ...
        /// <returns>If the values of any of the fields you specify differ, the data records are considered different and the method returns false.</returns>
        public bool ObjectsEqualBy<TFields>(object a, object b)
            where TFields : ITypeArrayOf<IBqlField>
        {
            return TypeArray.CheckAndExtract<IBqlField>(typeof(TFields)).All(f => object.Equals(GetValue(a, f.Name), GetValue(b, f.Name)));
        }
		...
		internal virtual Common.Serialization.IPXWeakCollection GetExtensionCollection()
		{
			return null;
		}
...
#pragma warning restore 618
		...
		{
			public Type Field;
			public fieldContainer[] Children;
		}

		protected static (List<fieldContainer> list, fieldContainer tree) getAllNestedTypes(Type root, bool isExposedView)
		{
			List<fieldContainer> children = new List<fieldContainer>();
			List<fieldContainer> ret = new List<fieldContainer>();
			var tree = new fieldContainer { Field = root };
			ret.Add(tree);
			foreach (Type child in root.GetNestedTypes())
			{
				if (isExposedView)
				{
					(List<fieldContainer> subret, fieldContainer subtree) = getAllNestedTypes(child, isExposedView);
					children.Add(subtree);
					ret.AddRange(subret);
				}
				else
				{
					fieldContainer fc = new fieldContainer { Field = child, Children = new fieldContainer[0] };
					ret.Add(fc);
					children.Add(fc);
				}
			}
		...
		public FieldTree _Tree;
		public class FieldTree
		{
			public string Field;
			public FieldTree[] Children;
			public FieldTree(fieldContainer root)
			{
				Field = root.Field.Name;
				Children = SortFieldChildren(root.Children).Select(_ => new FieldTree(_)).ToArray();
			}
			public FieldTree(string name)
			{
				Field = name;
				Children = new FieldTree[0];
			}
		}

		private static IEnumerable<fieldContainer> SortFieldChildren(IEnumerable<fieldContainer> children)
		{
			var result = new List<fieldContainer>();

			List<(fieldContainer field, Type placeBefore, Type placeAfter)> fields = children
				.Select(child =>
				{
					(var placeBefore, var placeAfter) = UI.PXUIControlFieldAttribute.GetPlacementFor(child.Field);
					return (child, placeBefore, placeAfter);
				})
				.ToList();
			var processed = new HashSet<Type>();
			List<fieldContainer> sorted = new List<fieldContainer>();

			foreach (var field in fields)
			{
				if (processed.Contains(field.field.Field))
					continue;

				if (field.placeBefore is null && field.placeAfter is null)
				{
					processed.Add(field.field.Field);
					sorted.Add(field.field);
				}
			}

			bool doPlaceFields = true;
			while (doPlaceFields)
			{
				doPlaceFields = false;
				foreach (var field in fields)
				{
					...
					if (!(field.placeBefore is null))
					{
						var index = sorted.FindIndex(v => v.Field == field.placeBefore);
						if (index >= 0)
						{
							processed.Add(field.field.Field);
							sorted.Insert(index, field.field);
							doPlaceFields = true;
						}
					}
					else if (!(field.placeAfter is null))
					{
						var index = sorted.FindIndex(v => v.Field == field.placeAfter);
						if (index >= 0)
						{
							processed.Add(field.field.Field);
							sorted.Insert(index + 1, field.field);
							doPlaceFields = true;
						}
					}
				}
		...
		internal protected static DACFieldDescriptor[] _GetProperties(Type type, List<Type> extensions, out Dictionary<string, List<PropertyInfo>> nameToFieldPropertyInfo, bool isExposedView, out int origLength, out fieldContainer tree)
		{
			DACFieldDescriptor[] orig = type.GetProperties(typeof(UI.PXExposedView).IsAssignableFrom(type) ? (BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic) : (BindingFlags.Public | BindingFlags.Instance))
				.OrderBy(f => f, new PXGraph.FieldInfoComparer()).Select(_ => new DACFieldDescriptor { Property = _ }).ToArray();
			DACFieldDescriptor[] info = new DACFieldDescriptor[orig.Length];
			int ii = 0;
			while (ii < info.Length)
			{
				int oi = orig.Length - ii - 1;
				while (oi > 0 && orig[orig.Length - ii - 1].Property.DeclaringType == orig[oi - 1].Property.DeclaringType)
				{
					oi--;
				}
			...
			Dictionary<string, int> nametoposition = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
			info.EnumWithIndex().ForEach(_ => nametoposition[_.Value.Property.Name] = _.Index);

			nameToFieldPropertyInfo = null;

			if (typeof(UI.PXExposedView).IsAssignableFrom(type))
			{
				List<DACFieldDescriptor> bqlonly = new List<DACFieldDescriptor>();
				var (fieldsList, mainTree) = getAllNestedTypes(type, typeof(UI.PXExposedView).IsAssignableFrom(type));
				tree = mainTree;
				foreach (Type field in fieldsList.Select(_ => _.Field))
				{
					DACFieldDescriptor descr = null;
					if (typeof(UI.PXExposedAction).IsAssignableFrom(field) || typeof(UI.PXExposedView).IsAssignableFrom(field))
					{
						descr = new DACFieldDescriptor { BqlField = field, TargetBqlField = null, IsSameView = true, ViewName = field.Name };
					}
					else if (typeof(IBqlField).IsAssignableFrom(field) && field.BaseType == typeof(object))
					{
						descr = new DACFieldDescriptor { BqlField = field, TargetBqlField = null, IsSameView = true, ViewName = field.DeclaringType.Name };
					}
					else if (typeof(IBqlField).IsAssignableFrom(field) && field.BaseType.DeclaringType.DeclaringType == type.DeclaringType)
					{
						descr = new DACFieldDescriptor { BqlField = field, TargetBqlField = field.BaseType.BaseType, IsSameView = false, ViewName = field.BaseType.DeclaringType.Name };
					}
					else if (typeof(IBqlField).IsAssignableFrom(field))
					{
						descr = new DACFieldDescriptor { BqlField = field, TargetBqlField = field.BaseType, IsSameView = true, ViewName = field.Name };
					}

					if (descr != null)
					{
						if (nametoposition.TryGetValue(field.Name, out var i))
						{
							descr.Property = info[i].Property;
							info[i] = descr;
						}
						else
						{
				...
				if (bqlonly.Count > 0)
				{
					Array.Resize(ref info, info.Length + bqlonly.Count);
					Array.Copy(bqlonly.ToArray(), 0, info, info.Length - bqlonly.Count, bqlonly.Count);
				}
			}
			else
			{
			...
			origLength = info.Length;

			if (extensions.Count > 0)
			{
				HashSet<string> baseproperties = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
				List<DACFieldDescriptor> addproperties = new List<DACFieldDescriptor>();
				foreach (DACFieldDescriptor basepi in info)
				{
					if (basepi.Property != null)
					{
						baseproperties.Add(basepi.Property.Name);
					}
				}
				nameToFieldPropertyInfo = new Dictionary<string, List<PropertyInfo>>(StringComparer.OrdinalIgnoreCase);
				var nameToBqlDesc = new Dictionary<string, List<DACFieldDescriptor>>(StringComparer.OrdinalIgnoreCase);
				List<DACFieldDescriptor> bqlDescriptions = new List<DACFieldDescriptor>();
				foreach (Type ext in extensions)
				{
					...
					if (typeof(UI.PXExposedView).IsAssignableFrom(type))
					{
						var (extFieldsList, extTree) = getAllNestedTypes(ext, typeof(UI.PXExposedView).IsAssignableFrom(ext));
						tree = MergeFieldsTree(tree, extTree);
						foreach (Type field in extFieldsList.Select(_ => _.Field))
						{
							DACFieldDescriptor descr = null;
							if (typeof(UI.PXExposedAction).IsAssignableFrom(field) || typeof(UI.PXExposedView).IsAssignableFrom(field))
							{
							...
							if (descr != null)
							{
								if (!nameToBqlDesc.TryGetValue(descr.Name, out var found))
								{
									found = new List<DACFieldDescriptor>();
									nameToBqlDesc[descr.Name] = found;
								}
								found.Insert(0, descr);

								bqlDescriptions.Add(descr);
							}
						}
					}
				}
                               ...
                               extensions.Contains(m.Extension)))
                {
                    foreach (FieldInfo fi in map.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public))
                    {
                        ...
                        if (fi.FieldType == typeof(Type) &&
                            ...
                            nameToFieldPropertyInfo.TryGetValue(fi.Name, out List<PropertyInfo> propertyInfoList))
                        {
                            var extensionProperty = map.Extension.GetProperty(fi.Name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase);
                            if (extensionProperty == null)
                                throw new PXException(MsgNotLocalizable.NoMappingInExtension, fi.Name, map.Extension.FullName);

                            nameToFieldPropertyInfo.TryGetValue(fvname, out List<PropertyInfo> existingPropertyInfoList);
                            
                            if (!propertyInfoList.Contains(extensionProperty))
                            {
                                //check if there is a duplicate mapping. enforce mapping to same field.
                                if (existingPropertyInfoList != null &&
                                    existingPropertyInfoList.Contains(extensionProperty))
                                    continue;
                                else
                                    throw new PXException(MsgNotLocalizable.NoMappingInExtension, fi.Name, map.Extension.FullName);
                            }

                            propertyInfoList.Remove(extensionProperty);
                            if (propertyInfoList.Count == 0)
                                nameToFieldPropertyInfo.Remove(fi.Name);
                            if (existingPropertyInfoList != null)
                                existingPropertyInfoList.Add(extensionProperty);
                            else
                                nameToFieldPropertyInfo[fvname] = new List<PropertyInfo> { extensionProperty };
                        }
                    }
                }

                {
					foreach (List<PropertyInfo> extpi in nameToFieldPropertyInfo.Values)
					{
						...
						{
							addproperties.Add(new DACFieldDescriptor { Property = extpi[0] });
						}
					}
				}
				...
				info.EnumWithIndex().ForEach(x => nametoposition[x.Value.Name] = x.Index);
				var addBqlProperties = new List<DACFieldDescriptor>();
				var addedBqlProperties = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
				foreach (var descr in bqlDescriptions)
				{
					if (addedBqlProperties.Contains(descr.Name))
						continue;

					addedBqlProperties.Add(descr.Name);

					if (nametoposition.TryGetValue(descr.Name, out var i))
					{
						if (info[i].BqlField == null)
						{
							info[i].BqlField = descr.BqlField;
							info[i].TargetBqlField = descr.TargetBqlField;
							info[i].IsSameView = descr.IsSameView;
							info[i].ViewName = descr.ViewName;
						}
					}
					else
					{
						addBqlProperties.Add(descr);
					}
				}

				if (addBqlProperties.Count > 0)
				{
					Array.Resize(ref info, info.Length + addBqlProperties.Count);
					Array.Copy(addBqlProperties.ToArray(), 0, info, info.Length - addBqlProperties.Count, addBqlProperties.Count);
				}
			}
			return info;
		}

		private static fieldContainer MergeFieldsTree(fieldContainer tree, fieldContainer extTree)
		{
			if (tree == null)
				return extTree;
			if (extTree == null)
			...
			var children = new List<fieldContainer>(tree.Children);
			children.AddRange(extTree.Children);
    ...
    public static class PXCacheEx
	{
		...
		public static IDictionary<string, (object LeftValue, object RightValue)> GetDifference(this PXCache cache, IBqlTable left, IBqlTable right, bool allowDifferentTypes = false)
		{
			var difference = new Dictionary<string, (object LeftValue, object RightValue)>();
			...
			{
				difference[typeof(Type).FullName] = (left.GetType().FullName, right.GetType().FullName);
				return difference;
			}

			foreach (string field in cache.Fields)
			{
				if (field == nameof(PXMappedCacheExtension.Base))
					continue; // it contains the record itself

				object leftValue = cache.GetValue(left, field);
				object rightValue = cache.GetValue(right, field);

				if (leftValue == null && rightValue == null || object.Equals(leftValue, rightValue))
					continue;

				difference[field] = (leftValue, rightValue);
			}
		...
		where TEntity : class, IBqlTable, new()
	{
		...
		public void Set<TValue>(Expression<Func<TEntity, TValue>> fieldSelector, TValue value) => Set(((MemberExpression)fieldSelector.Body).Member.Name, value);
		private void Set(string fieldName, object value)
		{
			if (_fireFieldEvents)
				_cache.SetValueExt(_entity, fieldName, value);
			else
				_cache.SetValue(_entity, fieldName, value);
		}
		public ValueSetter<TEntity> WithEventFiring => new ValueSetter<TEntity>(_cache, _entity, true);
		public Ext<TExt> With<TExt>() where TExt : PXCacheExtension<TEntity> => new Ext<TExt>(this);

		[ImmutableObject(true)]
		public readonly struct Ext<TExt>
			where TExt : PXCacheExtension<TEntity>
		{
			private readonly ValueSetter<TEntity> _parent;
			internal Ext(ValueSetter<TEntity> parent) => _parent = parent;
			public void Set<TValue>(Expression<Func<TExt, TValue>> fieldSelector, TValue value) => _parent.Set(((MemberExpression)fieldSelector.Body).Member.Name, value);
			public void Set<TValue>(Expression<Func<TEntity, TValue>> fieldSelector, TValue value) => _parent.Set(((MemberExpression)fieldSelector.Body).Member.Name, value);
			public Ext<TExt> WithEventFiring => new Ext<TExt>(_parent.WithEventFiring);
		}
	}
}
