    ...
    public class JointChecksCreator
    {
        ...
        protected virtual IEnumerable<int?> GetBillLineNumbersForVendorCheck()
        {
            return IsPaymentsByLinesAllowed()
                ? InvoiceEntry.Transactions.SelectMain().Where(tran => tran.CuryTranBal > 0).Select(tran => tran.LineNbr)
                : ((int?) decimal.Zero).AsSingleEnumerable();
        }

        protected void OpenPaymentCycleWorkflow()
        {
	        InvoiceEntry.Document.Current.IsPaymentCycleWorkflow = true;
        ...
        {
            GeneratePayment(InvoiceEntry.Document.Current);
            if (InvoiceEntry.Document.Current.IsRetainageDocument == true &&
                InvoiceEntry.Document.Current.PaymentsByLinesAllowed == true)
            {
                InsertAdjustments(TransformLineNumbersFromOriginalToCurrentDocument(jointCheckGenerationModel.BillLineNumbers));
            }
            else
            {
        ...
        private List<int?> TransformLineNumbersFromOriginalToCurrentDocument(IEnumerable<int?> billLineNumbers)
        {
            Dictionary<int, int> lineNumbers = new Dictionary<int, int>();
            foreach (APTran tran in InvoiceEntry.Transactions.Select())
            {
                if (tran.OrigLineNbr != null)
                    lineNumbers[tran.OrigLineNbr.Value] = tran.LineNbr.Value;
            }

            List<int?> result = new List<int?>();
            foreach (int lineNbr in billLineNumbers)
            {
                if (lineNumbers.ContainsKey(lineNbr))
                {
                    result.Add(lineNumbers[lineNbr]);
                }
                else
                {
                    result.Add(lineNbr);
