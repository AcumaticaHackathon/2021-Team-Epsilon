...
public partial class User_PageTitle : TitlePanel, ITitleModuleController
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!string.IsNullOrEmpty(screenTitle))
		{
			...
			{
				href = PXSessionStateStore.GetSessionUrl(HttpContext.Current, url);
				hascaption = false;
			}
            else
            {
				href = url;
				hascaption = ds.HasKeys();
			}
			...
			if (PXSiteMap.CurrentNode != null && PXList.Provider.HasList(PXSiteMap.CurrentNode.ScreenID))
			{
				...
				if (listNode != null)
				{
					href = ResolveUrl(listNode.Url);
				}
			}
			bool isFavoritable = !PXList.Provider.HasList(PXSiteMap.CurrentNode.ScreenID);
			string captionValue = $"{{\"title\":\"{Uri.EscapeDataString(screenTitle)}\", \"caption\":\"\", \"hasCaption\": \"{hascaption}\", \"href\":\"{href}\", \"isFavoritable\":\"{isFavoritable}\"";
			if (nodeID != null)
			{
				captionValue = $"{captionValue}, \"nodeID\":\"{nodeID.Value}\"";
				if (isFavoritable == true)
				{
					captionValue = $"{captionValue}, \"isFavorite\":\"{this.IsInFavorites(nodeID.Value)}\", \"Tooltip\": {{\"AddToFavorites\": \"{InfoMessages.AddToFavorites}\", \"RemoveFromFavorites\": \"{InfoMessages.RemoveFromFavorites}\"}}";
				}
			}
			captionValue = $"{captionValue}}}";
			pnlCaption.Value = captionValue;
		}
	...
	//---------------------------------------------------------------------------
	/// <summary>
	/// The Profiler panel load event handler.
	/// </summary>
	...
	private void FillTitleCaption()
    {
		...
		if (ds.PrimaryView != null)
		{
			...
			bool isFavoritable = !PXList.Provider.HasList(PXSiteMap.CurrentNode.ScreenID);
			if (pnlCaption.HasCaption())
			{
				var form = (this.Page.Master as BaseMasterPage)?.GetDocumentsForm() as PXBoundPanel;
				cap = Uri.EscapeDataString(ds.GetPrimaryKeysCaption(form));
				string listScreenID = PXList.Provider.GetListID(PXSiteMap.CurrentNode.ScreenID);
				PXSiteMapNode listNode = PXSiteMap.Provider.FindSiteMapNodeByScreenID(listScreenID);
				if (listNode != null)
				{
			...
			string captionValue = $"{{\"title\":\"{Uri.EscapeDataString(screenTitle)}\", \"caption\":\"{cap}\", \"hasCaption\": \"{cap != ""}\", \"href\":\"{href}\", \"isFavoritable\":\"{isFavoritable}\"";
			if (nodeID != null)
			{
				captionValue = $"{captionValue}, \"nodeID\":\"{nodeID.Value}\"";
				if (isFavoritable == true)
	...
	private void InitHelpMenu()
	{
        ...
        btn.MenuItems.Add(new PXMenuItem("Trace...") { NavigateUrl = "~/Frames/Trace.aspx?preserveSession=true", Target = "_blank", Value = "trace" });
    ...
    private List<PXMenuItem> CreateFirstItemsListReversed(out PXSiteMapNode siteMapNode)
    {
        ...
        if (!String.IsNullOrEmpty(this.screenID))
        {
            ...
            if (WebServicesAvailable && (siteMapNode != null && !String.IsNullOrEmpty(siteMapNode.GraphType)
                                                             && System.Web.Compilation.PXBuildManager.GetType(
                                                                 siteMapNode.GraphType, false) != null) ||
