...
namespace PX.Objects.FS
{
    ...
    {
        public FSSelectorContactAttribute(Type currentCustomer)
            : base(
                  BqlTemplate.OfCommand<
                    Search2<Contact.contactID,
                       ...
                       And<
                           ...
                           And<
                               Where<
                                   BAccount.bAccountID, Equal<Current<BqlPlaceholder.A>>,
                               Or<
                                   Current<BqlPlaceholder.A>, IsNull>>>>>>>>.
                  Replace<BqlPlaceholder.A>(currentCustomer).
                  ToType())
        {
            ...
            : base(BqlTemplate.OfCommand<
                   ...
                   Where2<
                       ...
                       And2<
                           ...
                           And<WhereType>>>,
                   Aggregate<GroupBy<InventoryItem.inventoryID, GroupBy<InventoryItem.noteID>>>>>
                    ...
                    Search2<BAccountStaffMember.bAccountID,
                       ...
                       Where2<
                           ...
                           And<
                               ...
                               Or<
                                   Vendor.vStatus, Equal<VendorStatus.oneTime>>>>>,
                       Or<
                           Where<
                               FSxEPEmployee.sDEnabled, Equal<True>>>>,
                       OrderBy<
                ...
                typeof(
                    Search<EPEmployee.bAccountID,
                       Where<
                           FSxEPEmployee.sDEnabled, Equal<True>,
                       And<
                           EPEmployee.vStatus, NotEqual<VendorStatus.inactive>,
                       And<
        ...
        public FSSelector_Driver_RouteDocumentRouteIDAttribute()
            : base(
                ...
                new Type[]
                {
                    ...
                    typeof(EPEmployeeFSRouteEmployee.vStatus),
                    ...
                    And<
                        Where2<
                            Where<Current<FSAppointment.appointmentID>, Greater<Zero>,
                                Or<FSServiceOrder.openDoc, Equal<True>>>,
        ...
        public FSSelectorContractSrvOrdTypeAttribute()
            : base(
                typeof(Search<FSSrvOrdType.srvOrdType,
                       ...
                       And<
                           FSSrvOrdType.behavior, NotEqual<FSSrvOrdType.behavior.Values.routeAppointment>,
                       And<
                           FSSrvOrdType.behavior, NotEqual<FSSrvOrdType.behavior.Values.internalAppointment>>>>>>))
        {
            DescriptionField = typeof(FSSrvOrdType.descr);
        }
    }
            ...
            : base(
                typeof(Search<FSSrvOrdType.srvOrdType,
                       Where<
                           FSSrvOrdType.active, Equal<True>,
                       And<
                           FSSrvOrdType.behavior, Equal<FSSrvOrdType.behavior.Values.routeAppointment>>>>))
        {
            DescriptionField = typeof(FSSrvOrdType.descr);
        }
    }
    ...
    public class FSSelectorExtensionMaintenanceEquipmentAttribute : PXSelectorAttribute
    {
        public FSSelectorExtensionMaintenanceEquipmentAttribute(Type customerID)
            : base(BqlTemplate.OfCommand<
                    ...
                    Or<
                        Where<FSEquipment.status, Equal<FSEquipment.status.Active>,
                        And<
                            Where2<
                                Where<FSEquipment.ownerType, Equal<FSEquipment.ownerType.Customer>,
                                    And<FSEquipment.ownerID, Equal<Current<BqlPlaceholder.A>>>>,
                                Or<Where<FSEquipment.ownerType, Equal<FSEquipment.ownerType.OwnCompany>,
                                    And<FSEquipment.customerID, Equal<Current<BqlPlaceholder.A>>>>>>>>>>>>
                    .Replace<BqlPlaceholder.A>(customerID)
                    .ToType(),
        SelectorBase_Equipment.selectorColumns)
        {
            SubstituteKey = typeof(FSEquipment.refNbr);
    ...
    public class FSSelectorComponentIDSalesOrderAttribute : PXCustomSelectorAttribute
    {
        ...
        protected virtual IEnumerable GetRecords()
        {
            ...
            if (currentFSxSOLineRow.NewEquipmentLineNbr != null)
            {
                foreach (object item in cache.Cached)
                {
                    SOLine currentSOLine_temp = item as SOLine;

                    if (currentSOLine_temp.LineNbr == currentFSxSOLineRow.NewEquipmentLineNbr)
                    {
    ...
    public class FSSelectorNewTargetEquipmentSOInvoiceAttribute : PXCustomSelectorAttribute
    {
        ...
        protected virtual IEnumerable GetRecords()
        {
            var rows = PXSelectJoin<ARTran,
                       InnerJoin<FS.FSARTran,
                       On<
                           FS.FSARTran.FK.ARTranLine>,
                       InnerJoin <InventoryItem,
                       On<
                       ...
                       Where<
                           FSxEquipmentModel.equipmentItemClass, Equal<ListField_EquipmentItemClass.ModelEquipment>,
                       And<
                           FS.FSARTran.equipmentAction, Equal<ListField_EquipmentActionBase.SellingTargetEquipment>,
                       And<
                           Where<
            ...
            if (rows?.Count > 0)
            {
                foreach (PXResult<ARTran, FS.FSARTran, InventoryItem> row in rows)
                {
            ...
            : base(
                typeof(Search2<FSEquipmentComponent.lineNbr,
                       ...
                       Where2<
                           Where<
                               FSEquipmentComponent.status, Equal<FSEquipmentComponent.status.Active>,
                                And<FSEquipmentComponent.SMequipmentID, Equal<Current<FSARTran.sMEquipmentID>>>>,
                           And<
                                Where2<
                                    Where<FSEquipmentComponent.itemClassID, Equal<InventoryItem.itemClassID>,
                                        Or<Current<FSARTran.equipmentAction>, Equal<FSARTran.equipmentAction.None>>>,
                                And<
                                    Where<
                                        Current<FSARTran.componentID>, IsNull,
                                        Or<FSEquipmentComponent.componentID, Equal<Current<FSARTran.componentID>>>>>>>>>),
                new Type[]
                {
                    typeof(FSEquipmentComponent.lineRef),
                    typeof(FSEquipmentComponent.componentID),
            ...
            : base(
                ...
                typeof(Search2<Vendor.bAccountID,
                       ...
                       Where<
                           FSxVendor.sDEnabled, Equal<True>,
                       And<
                           Vendor.vStatus, NotEqual<VendorStatus.inactive>>>>),
                typeof(Vendor.acctCD),
                new Type[]
                {
                    typeof(Vendor.acctCD),
    ...
    public class FSINLotSerialNbrAttribute : PXCustomSelectorAttribute
    {
        ...
        {
            AppointmentEntry apptGraph = (AppointmentEntry)sender.Graph;
            if (apptGraph?.SkipLotSerialFieldVerifying == true)
            {
                return;
            }
    ...
    {
        #region Custom PostBatch
        [Serializable]
        [PXVirtual]
        public class PostBatch : PX.Data.IBqlTable
        {
            #region BatchID
            ...
            #endregion
            #region IsContractBatch
            public abstract class isContractBatch : PX.Data.BQL.BqlBool.Field<isContractBatch> { }
        ...
        public FSPostBatchAttribute()
            : base(typeof(PostBatch.batchID), typeof(PostBatch.batchNbr))
        {
            this.SubstituteKey = typeof(PostBatch.batchNbr);
            this.ValidateValue = false;
        }

        protected virtual IEnumerable GetRecords()
        {
            PXCache cache = this._Graph.Caches[typeof(FSBillHistory)];

            object current = null;

            foreach (object item in PXView.Currents)
            {
                if (item != null && (item.GetType() == typeof(FSBillHistory)))
                {
                    current = item;
                    break;
                }
            ...
            FSBillHistory currentFSBillHistory = (FSBillHistory)current;

            var fsPostBatchList = PXSelect<FSPostBatch>.Select(_Graph);

            if (string.IsNullOrEmpty(currentFSBillHistory.ServiceContractRefNbr))
            {

                if (fsPostBatchList.Count > 0)
                {
                    foreach (PXResult<FSPostBatch> row in fsPostBatchList)
                    {
                        FSPostBatch fsPostBatchRow = (FSPostBatch)row;
                        yield return new PostBatch { BatchID = fsPostBatchRow.BatchID, BatchNbr = fsPostBatchRow.BatchNbr, IsContractBatch = false };
                    }
                }
            }
            else 
            { 
                var fsContractPostBatchList = PXSelect<FSContractPostBatch>.Select(_Graph);

                if (fsContractPostBatchList.Count > 0)
                {
                    foreach (PXResult<FSContractPostBatch> row in fsContractPostBatchList)
                    {
                        FSContractPostBatch fsContractPostBatchRow = (FSContractPostBatch)row;
                        yield return new PostBatch { BatchID = fsContractPostBatchRow.ContractPostBatchID, BatchNbr = fsContractPostBatchRow.ContractPostBatchNbr, IsContractBatch = true };
                    }
                }
            }
        }
