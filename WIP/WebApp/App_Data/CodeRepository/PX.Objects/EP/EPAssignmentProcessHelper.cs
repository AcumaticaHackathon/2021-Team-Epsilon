...
namespace PX.Objects.EP
{
		...
		where Table : class, IBqlTable
	{
				...
				if (IsPassed(item, route))
				{
					if (route.WorkgroupID != null || route.OwnerID != null || route.OwnerSource != null)
					{
						...
						if (route.OwnerSource != null)
						{
							...
							string code = PXTemplateContentParser.Instance.Process(route.OwnerSource, graph, typeof(Table), null);
							...
							if (int.TryParse(code, out int contactID))
							{
								// Like 3205
								...
								contact =
									PXSelect<
										...
										.SelectWindowed(this, 0, 1, contactID);
						...
						if(route.UseWorkgroupByOwner == true && WorkgroupID != null && OwnerID != null)
						{
							...
							PXSelectJoin<EPCompanyTreeMember,
								...
								InnerJoin<EPEmployee, On<EPEmployee.defContactID, Equal<EPCompanyTreeMember.contactID>>>>,
		...
		{			
			...
			List<object> result = 
				select.View.Select(null, new object[]{employee.DefContactID}, null, null, null, new PXFilterRow[] { filter }, ref startRow, 1, ref totalRows);
