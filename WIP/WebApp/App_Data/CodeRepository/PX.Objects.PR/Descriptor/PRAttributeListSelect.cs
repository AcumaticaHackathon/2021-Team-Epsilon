...
namespace PX.Objects.PR
{
		...
		where TTypeMeta : SettingDefinitionListAttribute
	{
		...
		protected IEnumerable viewHandler()
		{
			...
			Dictionary<TaxSettingAdditionalInformationKey, PRTaxSettingAdditionalInformation> additionalInformation = SelectFrom<PRTaxSettingAdditionalInformation>.View.Select(_Graph).FirstTableItems
			...
			foreach (IPRSetting definition in definitionList)
			{
				...
				else
				{
					...
					TEntity newRecord = this.Insert(CreateDacRecord(Cache, dynamicDefinition, additionalInformation));
		...
		private TEntity CreateDacRecord(PXCache cache, IPRSetting definition, Dictionary<TaxSettingAdditionalInformationKey, PRTaxSettingAdditionalInformation> infoDefinitionDict)
		{
		...
		private TEntity CreateDacRecord(PXCache cache, PRSettingDefinition<TDynamicType, TTypeMeta> definition, Dictionary<TaxSettingAdditionalInformationKey, PRTaxSettingAdditionalInformation> infoDefinitionDict)
		{
			TEntity attr = CreateDacRecord(cache, (IPRSetting)definition, infoDefinitionDict);
		...
		protected virtual void ValueFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			if (SettingDefinitions.TryGetValue(row.SettingName, out settingDefinition))
			{
				...
				if (required && e.ReturnValue == null)
				{
					...
					|| sender.Fields.Contains(nameof(PRCompanyTaxAttribute.UsedForSymmetry)) && !Equals(sender.GetValueOriginal(row, nameof(PRCompanyTaxAttribute.UsedForSymmetry)), sender.GetValue(row, nameof(PRCompanyTaxAttribute.UsedForSymmetry))))
				{
		...
		{
			SettingDefinitions.TryGetValue(record.SettingName, out PRSettingDefinition<TDynamicType, TTypeMeta> settingDefinition);
		...
		protected virtual bool AssignAdditionalInformation(PXCache cache, IPRSetting record, Dictionary<TaxSettingAdditionalInformationKey, PRTaxSettingAdditionalInformation> infoDefinitionDict)
		{
			...
			if (cache.Fields.Contains(nameof(PRCompanyTaxAttribute.UsedForSymmetry))
				&& !Equals(cache.GetValue(record, nameof(PRCompanyTaxAttribute.UsedForSymmetry)), infoDefinition.UsedForSymmetry))
			{
				cache.SetValue(record, nameof(PRCompanyTaxAttribute.UsedForSymmetry), infoDefinition.UsedForSymmetry);
				updated = true;
			}
