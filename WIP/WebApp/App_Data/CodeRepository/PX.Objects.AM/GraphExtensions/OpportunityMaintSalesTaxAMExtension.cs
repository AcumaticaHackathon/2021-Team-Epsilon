...
{
    public class OpportunityMaintSalesTaxAMExtension : PXGraphExtension<OpportunityMaint.SalesTax, OpportunityMaint>
    {
            ...
            Action<object, decimal, decimal, decimal> del)
        {
            ...
            var oRow = (CROpportunity)Base1.Documents.Cache.GetMain(Base1.Documents.Current);
            if (oRow == null)
            {
                return;
            }

            var locatedRow = Base.Opportunity.Cache.LocateElse(oRow);
            if (locatedRow == null)
            {
                return;
            }

            var oppExt = PXCache<CROpportunity>.GetExtension<CROpportunityExt>(locatedRow);
            if (oppExt == null)
            {
                return;
            }

            var curyEstimateTotal = oppExt.AMCuryEstimateTotal ?? 0m;
            ...
            var docTotalWithEstimate = curyDocTotal + curyEstimateTotal;

            ParentSetValue<CROpportunity.curyProductsAmount>(locatedRow, docTotalWithEstimate);
        }

        [Obsolete(InternalMessages.MethodIsObsoleteAndWillBeRemoved2021R2)]
        private decimal GetEstimateCuryExtPrice()
        {
            var total = 0m;
            foreach (AMEstimateReference detrow in GetDetails())
            {
                total += detrow?.CuryExtPrice ?? 0m;
            }
        ...
        protected virtual void SetProductsAmount(CROpportunity row)
        {
            var oppExt = PXCache<CROpportunity>.GetExtension<CROpportunityExt>(row);
            var curyEstimateTotal = oppExt?.AMCuryEstimateTotal ?? 0m;

            row.CuryProductsAmount = row.CuryAmount - row.CuryDiscTot + curyEstimateTotal;
        }
        ...
        protected virtual PXResultset<AMEstimateReference> GetDetails()
        {
            return PXSelectJoin<AMEstimateReference, InnerJoin<AMEstimateItem, On<AMEstimateReference.estimateID,
                ...
                Where<AMEstimateReference.opportunityQuoteID, Equal<Current<CROpportunity.quoteNoteID>>>>.Select(Base);
        }

        // -------------------------------------------------------------------------------------------------------
        // PASTE FULL COPY FROM TaxBaseGraph BELOW

        protected Type DocumentTypeCuryDocBal = typeof(CROpportunity.curyProductsAmount);
        ...
        // Copied from OpportunityMaint.SalesTax
        ...
        protected virtual void _CalcDocTotals(object row, decimal CuryTaxTotal, decimal CuryInclTaxTotal, decimal CuryWhTaxTotal)
        {
                ...
                Equals(CuryTaxTotal, doc_CuryTaxTotal) == false)
            {
                ...
                ParentSetValue<CROpportunityExt.aMCuryEstimateTotal>(estimateTotal);
        ...
        protected virtual void CurrencyInfo_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e, PXRowUpdated del)
        {
            ...
            if (taxCalc == TaxCalc.Calc || taxCalc == TaxCalc.ManualLineCalc)
            {
                ...
                {
                    var graphExt = Base.GetExtension<OpportunityMaintAMExtension>();
        ...
        protected virtual void CROpportunity_TaxZoneID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e, PXFieldUpdated del)
        {
            del?.Invoke(sender, e);
        ...
        /// <param name="e">Parameters of the event.</param>
        // need to change from Document_* or you will get a failed to subscribe event error during runtime/page load
        protected virtual void CROpportunity_DocumentDate_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e, PXFieldUpdated del)
        {
            del?.Invoke(sender, e);
        ...
        /// <param name="e">Parameters of the event.</param>
        // need to change from Document_ or you will get a failed to subscribe event error during runtime/page load
        protected virtual void CROpportunity_TaxCalcMode_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e, PXFieldUpdated del)
        {
            del?.Invoke(sender, e);
        ...
        public override void Initialize()
        {
            ...
            AllowEstimates = Base.GetExtension<OpportunityMaintAMExtension>()?.AllowEstimates == true;
        }
