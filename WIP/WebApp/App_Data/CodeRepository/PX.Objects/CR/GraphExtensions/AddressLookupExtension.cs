...
namespace PX.Objects.CR.Extensions
{
		...
		where TAddress : class, IBqlTable, IAddressLocation, new()
	{
		...
		public override void Initialize()
		{
			...
			AddAction(AddressLookupSelectActionName, CR.Messages.AddressLookupButton, AddressLookupSelectAction, true, false);
		}

		protected void AddAction(string name, string displayName, PXButtonDelegate handler, bool visible, bool visibleOnDataSource = true)
		{
			...
			PXButtonAttribute buttAttr = new PXButtonAttribute() { VisibleOnDataSource = visibleOnDataSource};
		...
		protected virtual IEnumerable AddressLookup(PXAdapter adapter)
		{
			...
			if (Base.Views.TryGetValue(AddressView, out var addressView))
            {
				// SelectControlsIDs="form" RepaintControls="None" RepaintControlsIDs="ds,formA"
				// here we only fire refresh for form view and primary view, but I'm not sure that we cover RepaintControlsIDs="ds" and SelectControlsIDs="form"
				addressView.RequestRefresh();
				Base.Views[Base.PrimaryView].RequestRefresh();
            }
