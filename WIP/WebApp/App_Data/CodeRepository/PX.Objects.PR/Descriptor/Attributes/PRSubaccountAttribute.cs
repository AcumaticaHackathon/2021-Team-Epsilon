...
namespace PX.Objects.PR
{
	...
	public abstract class PRSubaccountAttribute : SubAccountAttribute
	{
		...
		protected virtual void RowInsertedUpdated(PXCache cache, object row, object oldRow)
		{
			...
			if (DefaultingDependencies.Any(pair => HasImpactOnSubDefault(cache, pair.Key.Name, pair.Value, row, oldRow)) && !cache.Graph.IsImportFromExcel)
			{
	...
	public class BenLiabilitySubAccountAttribute : PRSubaccountAttribute
	{
		...
		{
			return base.IsFieldRequired(cache, row) && cache.GetValue(row, _IsPayableBenefitField.Name) as bool? != true;
		}
	}
