...
namespace PX.Objects.AM
{
    internal class AMRuleFormulaEngine
    {
        ...
        protected AMRuleFormulaEngine(PXGraph graph, bool queryCache)
        {
            Container = new RuleCacheContainer(graph, queryCache);
        }
        ...
        public static RuleReturn Run(PXGraph graph, bool queryCache)
        {
            return Run(new AMRuleFormulaEngine(graph, queryCache));
        }
    ...
    internal class RuleCacheContainer
    {
        ...
        public RuleCacheContainer(PXGraph graph, bool queryCache)
        {
        ...
        private void SetFeaturesDictionary(PXGraph graph)
        {
            ...
            foreach (var configFeature in GetConfigurationResults<AMConfigurationFeature, AMConfigurationFeature.configurationID, AMConfigurationFeature.revision>(graph))
            {
            ...
            foreach (var resultFeature in GetConfigResults<AMConfigResultsFeature, AMConfigResultsFeature.configResultsID>(graph))
            {
        ...
        private void SetRulesDictionary(PXGraph graph)
        {
            ...
            foreach(var configRule in GetConfigurationResults<AMConfigurationRule, AMConfigurationRule.configurationID, AMConfigurationRule.revision>(graph))
            {
            ...
            foreach(var resultRule in GetConfigResults<AMConfigResultsRule, AMConfigResultsRule.configResultsID>(graph))
            {
        ...
        private void SetAttributesDictionary(PXGraph graph)
        {
            ...
            foreach (var configAttribute in GetConfigurationResults<AMConfigurationAttribute, AMConfigurationAttribute.configurationID, AMConfigurationAttribute.revision>(graph))
            {
            ...
            foreach (var resultAttribute in GetConfigResults<AMConfigResultsAttribute, AMConfigResultsAttribute.configResultsID>(graph))
            {
        ...
        private void SetOptionsDictionary(PXGraph graph)
        {
            ...
            foreach (var configOption in GetConfigurationResults<AMConfigurationOption, AMConfigurationOption.configurationID, AMConfigurationOption.revision>(graph))
            {
            ...
            foreach (var resultOption in GetConfigResults<AMConfigResultsOption, AMConfigResultsOption.configResultsID>(graph))
            {
        ...
        /// </summary>
        private IEnumerable<TRow> GetConfigurationResults<TRow, TConfigurationId, TRevision>(PXGraph graph)
            where TRow : class, IBqlTable, IProductConfiguration, new()
            where TConfigurationId : IBqlField
            where TRevision : IBqlField
        {
            if (_QueryCache)
            {
                var currentConfig = (AMConfigurationResults)graph.Caches[typeof(AMConfigurationResults)]?.Current;
                if (currentConfig != null)
                {
                    return graph.Caches[typeof(TRow)].Cached.RowCast<TRow>()
                        .Where(r => r.ConfigurationID == currentConfig.ConfigurationID && r.Revision == currentConfig.Revision);
                }
            }

            return PXSelect<TRow,
                    Where<TConfigurationId, Equal<Current<AMConfigurationResults.configurationID>>,
                        And<TRevision, Equal<Current<AMConfigurationResults.revision>>>>>
                .Select(graph).ToFirstTable();
        }

        /// <summary>
        /// Query results for given TRow
        /// </summary>
        private IEnumerable<TRow> GetConfigResults<TRow, TConfigResultsId>(PXGraph graph)
            where TRow : class, IBqlTable, IProductConfigResult, new()
            where TConfigResultsId : IBqlField
        {
            if (_QueryCache)
            {
                var currentConfig = (AMConfigurationResults)graph.Caches[typeof(AMConfigurationResults)]?.Current;
                ...
                {
                    return graph.Caches[typeof(TRow)].Cached.RowCast<TRow>()
                        .Where(r => r.ConfigResultsID == currentConfig.ConfigResultsID);
                }
            }

            return PXSelect<TRow,
                    Where<TConfigResultsId,
                        Equal<Current<AMConfigurationResults.configResultsID>>>>.Select(graph).ToFirstTable();
        }
