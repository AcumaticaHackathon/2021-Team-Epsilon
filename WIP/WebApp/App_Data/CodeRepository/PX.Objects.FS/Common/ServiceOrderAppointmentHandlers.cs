    ...
    public static class ServiceOrderAppointmentHandlers
    {
                ...
                where DAC : class, IBqlTable, IFSSODetBase, new()
        {
            ...
            if (isInventoryLine == true
                    ...
                    && (fsAppointmentDetRow == null || (row.Status == ID.Status_AppointmentDet.NOT_STARTED
                                                        || row.Status == ID.Status_AppointmentDet.RequestForPO
                                                        || (fsAppointmentDetRow != null 
                                                            && fsAppointmentDetRow.CanChangeMarkForPO == true )))
                    && (fsSODetRow == null || fsSODetRow.EnablePO == false || fsSODetRow.POSource == ListField_FSPOSource.PurchaseToServiceOrder))
            {
                isEnabled = true;
            }
        ...
        public static void X_ManualPrice_FieldUpdated<DAC, CuryUnitPrice, CuryBillableExtPrice>(PXCache cache, PXFieldUpdatedEventArgs e)
