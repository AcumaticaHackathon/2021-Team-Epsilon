	...
	public class ARExpiringCardsProcess: PXGraph<ARExpiringCardsProcess>
	{
        ...
        public PXFilteredProcessingJoin<CustomerPaymentMethod, ARExpiringCardFilter,
                                InnerJoin<Customer, On<Customer.bAccountID, Equal<CustomerPaymentMethod.bAccountID>,
                                      And<Customer.status, NotEqual<CustomerStatus.inactive>>>,
		...
		public PXSelectJoin<
			...
			InnerJoin<Customer,
				...
				And<Customer.status, NotEqual<CustomerStatus.inactive>>>,
			LeftJoin<CR.Contact,
				On<CR.Contact.bAccountID, Equal<CustomerPaymentMethod.bAccountID>,
				And<
				Where2<
					Where<CustomerPaymentMethod.billContactID, IsNull,
						And<CR.Contact.contactID, Equal<Customer.defBillContactID>>>,
					Or<Where<CustomerPaymentMethod.billContactID, IsNotNull,
			...
			Where<CustomerPaymentMethod.expirationDate, GreaterEqual<Required<CustomerPaymentMethod.expirationDate>>,
				And<CustomerPaymentMethod.expirationDate, LessEqual<Required<CustomerPaymentMethod.expirationDate>>>>> CardsView;
		...
		public virtual IEnumerable cards()
		{
			...
			if (filter != null)
			{
				...
				{
					CardsView.WhereAnd<Where<Customer.customerClassID, Equal<Required<Customer.customerClassID>>>>();
				}

				if (filter.DefaultOnly == true)
				{
					CardsView.WhereAnd<Where<CustomerPaymentMethod.pMInstanceID, Equal<Customer.defPMInstanceID>>>();
				}
				if ((bool)filter.ActiveOnly)
				{
					CardsView.WhereAnd<Where<CustomerPaymentMethod.isActive, Equal<BQLConstants.BitOn>>>();
				}
				foreach (PXResult<CustomerPaymentMethod, Customer, Contact> it in CardsView.Select(filter.BeginDate, filter.EndDate, filter.CustomerClassID, filter.ActiveOnly))
				{
	...
	public class ARExpiredCardsProcess : PXGraph<ARExpiringCardsProcess>
	{
		...
		public PXSelectJoin<
			...
			InnerJoin<Customer,
				...
				And<Customer.status, NotEqual<CustomerStatus.inactive>>>,
			LeftJoin<CR.Contact, 
				On<CR.Contact.bAccountID, Equal<Customer.bAccountID>,
				And<CR.Contact.contactID, Equal<Customer.defBillContactID>>>>>,
			Where<CustomerPaymentMethod.isActive, Equal<True>,
				...
				And<CustomerPaymentMethod.expirationDate, GreaterEqual<Required<CustomerPaymentMethod.expirationDate>>>>>>
			CardsView;
        ...
        public virtual IEnumerable cards()
		{
			...
			if (filter != null)
			{
				...
				foreach (PXResult<CustomerPaymentMethod, Customer, Contact> it in CardsView.Select(filter.BeginDate, filter.EndDate, filter.CustomerClassID))
				{
					CustomerPaymentMethod cpm = (CustomerPaymentMethod)it;
