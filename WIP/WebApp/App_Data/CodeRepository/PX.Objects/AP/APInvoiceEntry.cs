...
{ 
	[Serializable]
	public partial class APInvoiceEntry : APDataEntryGraph<APInvoiceEntry, APInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		#region Extensions
		...
		[PXUIField(DisplayName = Messages.VoidPrepayment, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select, Visible = true)]
		[PXLookupButton]
		[APMigrationModeDependentActionRestriction(
			restrictInMigrationMode: false,
			restrictForRegularDocumentInMigrationMode: true,
		...
		public virtual IEnumerable RecalculateDiscountsAction(PXAdapter adapter)
		{
			if (adapter.MassProcess)
			{
				...
				{
					_discountEngine.RecalculatePricesAndDiscounts(Transactions.Cache, Transactions, Transactions.Current, DiscountDetails, Document.Current.VendorLocationID, Document.Current.DocDate, recalcdiscountsfilter.Current, GetDefaultAPDiscountCalculationOptions(Document.Current));
					this.Save.Press();
				});
			}
		...
		public PXSetup<Vendor>.Where<Vendor.bAccountID.IsEqual<APInvoice.vendorID.AsOptional>> vendor;
		...
		public PXSetup<EPEmployee, Where<EPEmployee.defContactID, Equal<Current<APInvoice.employeeID>>>> employee;
		...
		public virtual void ReverseInvoiceProc(APRegister doc)
		{
			...
			foreach (PXResult<APInvoice, CurrencyInfo, Terms, Vendor> res in APInvoice_CurrencyInfo_Terms_Vendor.Select(this, (object)doc.DocType, doc.RefNbr))
			{
				...
				invoice.RefNbr = null;
				if (APSetup.Current?.MigrationMode == false) 
				{
		...
		protected virtual void APInvoice_CuryDocBal_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if ((status == PXEntryStatus.Inserted || status == PXEntryStatus.Updated)
				...
				&& apdoc.Released != true && apdoc.Prebooked != true)
			{
				bool recalculateApplications = false;
						...
						.Where(adj => adj != null && adj.CuryAdjdAmt > 0))
				{
					...
					Adjustments.Cache.MarkUpdated(adj);
					recalculateApplications = true;
				}
				if (recalculateApplications) 
				{
        ...
        public virtual APInvoiceState GetDocumentState(PXCache cache, APInvoice document)
        {
            ...
            result.IsRetainageDebAdj = result.IsDocumentDebitAdjustment && 
                                       (document.IsOriginalRetainageDocument() || document.IsChildRetainageDocument());
        ...
        protected virtual void APInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			switch(document.DocType)
			{
				...
				case APDocType.CreditAdj:
					this.payInvoice.SetCaption(AP.Messages.Pay);
					break;
				case APDocType.Prepayment:
					this.payInvoice.SetCaption(AP.Messages.PayApply);
					break;
				default:
					this.payInvoice.SetCaption(AP.Messages.Apply);
					break;
			}
			this.voidDocument.SetCaption(AP.Messages.Void);
			this.createSchedule.SetCaption(document.Status == APDocStatus.Scheduled
			...
			recalculateDiscountsAction.SetEnabled(true);
			...
			if (invoiceState.IsDocumentReleasedOrPrebooked || invoiceState.IsDocumentVoided)
			{
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.dueDate>(cache, document,
					!invoiceState.IsDocumentDebitAdjustment && document.OpenDoc == true && document.PendingPPD != true);
				PXUIFieldAttribute.SetEnabled<APInvoice.paySel>(cache, document,
					!invoiceState.IsDocumentDebitAdjustment && document.OpenDoc == true);
				PXUIFieldAttribute.SetEnabled<APInvoice.payLocationID>(cache, document, document.OpenDoc == true);
				PXUIFieldAttribute.SetEnabled<APInvoice.payAccountID>(cache, document, document.OpenDoc == true);
				PXUIFieldAttribute.SetEnabled<APInvoice.payTypeID>(cache, document, document.OpenDoc == true);
				...
				PXUIFieldAttribute.SetEnabled<APInvoice.discDate>(cache, document,
					!invoiceState.IsDocumentDebitAdjustment && !invoiceState.IsDocumentPrepayment && document.OpenDoc == true && document.PendingPPD != true);

				cache.AllowDelete = false;
				cache.AllowUpdate = document.OpenDoc == true || Enable1099 || invoiceState.IsDocumentPrebookedNotCompleted;
			...
			else
			{
				...
				{
					if (vendorRecord.VStatus == VendorStatus.Inactive)
					{
			...
			PXUIFieldAttribute.SetEnabled<APInvoice.curyInitDocBal>(cache, document, isUnreleasedMigratedDocument);
			...
			if (document.Hold != true && document.Released != true && document.Prebooked != true && document.RoundDiff != 0)
			{
				...
				{
					if (Math.Abs(document.RoundDiff.Value) > Math.Abs(CurrencyCollection.GetCurrency(currencyinfo.Current.BaseCuryID).RoundingLimit.Value))
					{
						string baseCuryID = currencyinfo.Current?.BaseCuryID ?? Accessinfo.BaseCuryID;
						cache.RaiseExceptionHandling<APInvoice.curyRoundDiff>(document, document.CuryRoundDiff,
							new PXSetPropertyException(Messages.RoundingAmountTooBig, baseCuryID, PXDBQuantityAttribute.Round(document.RoundDiff),
								PXDBQuantityAttribute.Round(CurrencyCollection.GetCurrency(currencyinfo.Current.BaseCuryID).RoundingLimit)));
					}
				}
				else
				{
			...
			PXUIFieldAttribute.SetVisible<APInvoice.intercompanyInvoiceNoteID>(
				cache,
				e.Row, 
				vendor.Current?.IsBranch == true
					&& (document.DocType == APDocType.Invoice 
						|| document.DocType == APDocType.DebitAdj
						|| document.DocType == APDocType.CreditAdj));
			PXUIFieldAttribute.SetEnabled<APInvoice.intercompanyInvoiceNoteID>(cache, document, document.Status != APDocStatus.Rejected);

			ARTran defferalCodeARTran = null;
			ARInvoice relatedInvoice = null;
			if (cache.GetStatus(document) == PXEntryStatus.Inserted)
			{
				defferalCodeARTran = SelectFrom<ARTran>
					.LeftJoin<ARInvoice>
						.On<ARTran.tranType.IsEqual<ARInvoice.docType>
							.And<ARTran.refNbr.IsEqual<ARInvoice.refNbr>>>
					.Where<ARInvoice.noteID.IsEqual<@P.AsGuid>
						.And<ARTran.deferredCode.IsNotNull>>
					.View
					.SelectSingleBound(this, null, document.IntercompanyInvoiceNoteID);

				relatedInvoice = SelectFrom<ARInvoice>
					.Where<ARInvoice.noteID.IsEqual<@P.AsGuid>>
					.View
					.SelectSingleBound(this, null, document.IntercompanyInvoiceNoteID);
			}

			if (defferalCodeARTran != null)
			{
				cache.RaiseExceptionHandling<APInvoice.refNbr>(
					document,
					document.RefNbr,
					new PXSetPropertyException(Messages.CannotCopyDeferralCodeFromARDocument, PXErrorLevel.Warning));
			}
			if(relatedInvoice != null)
			{
				if(relatedInvoice.CuryOrigDocAmt != document.CuryOrigDocAmt)
				{
					cache.RaiseExceptionHandling(
						APSetup.Current.RequireControlTotal == true
							? nameof(APInvoice.CuryOrigDocAmt)
							: nameof(APInvoice.CuryDocBal),
						document,
						APSetup.Current.RequireControlTotal == true
							?document.CuryOrigDocAmt
							:document.CuryDocBal,
						new PXSetPropertyException(Messages.IntercompanyRelatedDocumentTotalIsDiffer, PXErrorLevel.Warning));
				}
				if (relatedInvoice.CuryTaxTotal != document.CuryTaxTotal)
				{
					cache.RaiseExceptionHandling<APInvoice.curyTaxTotal>(
						document,
						document.CuryTaxTotal,
						new PXSetPropertyException(Messages.IntercompanyRelatedTaxTotalIsDiffer, PXErrorLevel.Warning));
				}
			}
		}
		...
		protected virtual void APInvoice_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (doc.Released != true)
			{
				...
				{
					_discountEngine.AutoRecalculatePricesAndDiscounts(Transactions.Cache, Transactions, null, DiscountDetails, doc.VendorLocationID, doc.DocDate, GetDefaultAPDiscountCalculationOptions(doc));
				}
				...
				if (sender.GetStatus(doc) != PXEntryStatus.Deleted && !sender.ObjectsEqual<APInvoice.curyDiscTot>(e.OldRow, e.Row))
				{
					...
					if (!_discountEngine.IsInternalDiscountEngineCall && e.ExternalCall)
					{
						_discountEngine.SetTotalDocDiscount(Transactions.Cache, Transactions, DiscountDetails,
							Document.Current.CuryDiscTot, GetDefaultAPDiscountCalculationOptions(doc));
		...
		protected virtual void APTran_AccountID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (vendor.Current == null || IsCopyPasteContext || IsReverseContext) return;
			...
			else if (apTran.InventoryID == null 
				&& vendor.Current.Type == BAccountType.EmployeeType)
			{
		...
		protected virtual void APTran_SubID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			EPEmployee employeeByContactID = PXSelect<
				EPEmployee,
				Where<
					EPEmployee.defContactID, Equal<Required<Contact.contactID>>>>
				.Select(this, Document.Current?.EmployeeID ?? PXAccess.GetContactID());
			...
			{
				PMTask task = PMTask.PK.Find(this, documentLine.TaskID);
				if (task != null)
					projectTaskSubaccountID = task.DefaultExpenseSubID;
			}
			...
			int? employeeByUserSubaccountID = (int?)Caches[typeof(EPEmployee)].GetValue<EPEmployee.expenseSubID>(employeeByContactID);
			int? companySubaccountID = (int?)Caches[typeof(CRLocation)].GetValue<CRLocation.cMPExpenseSubID>(companyLocation);
			int? projectSubaccountID = project.DefaultExpenseSubID;
			...
			sender.RaiseFieldUpdating<APTran.subID>(documentLine, ref subaccountValue);
				sender.RaiseFieldVerifying<APTran.subID>(documentLine, ref subaccountValue);
		...
		protected virtual void APTran_TaxCategoryID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			APTran row = (APTran)e.Row;
			if (row == null) return;
			if ((string)e.OldValue == row.TaxCategoryID) return;
		...
		protected virtual void APTran_DiscountID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			{
				_discountEngine.UpdateManualLineDiscount(sender, Transactions, row, DiscountDetails, Document.Current.BranchID, Document.Current.VendorLocationID, Document.Current.DocDate, GetDefaultAPDiscountCalculationOptions(Document.Current));
			}
		}
		...
		protected virtual void APTran_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
				...
				doc.Released != true)
			{
				...
				PXUIFieldAttribute.SetEnabled<APTran.dRTermStartDate>(Transactions.Cache, null, false);
				PXUIFieldAttribute.SetEnabled<APTran.dRTermEndDate>(Transactions.Cache, null, false);
			}
			#endregion
		}
		...
		protected virtual void APTran_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
            ...
            if (Document.Current != null && Document.Cache.GetStatus(Document.Current) != PXEntryStatus.Deleted && Document.Cache.GetStatus(Document.Current) != PXEntryStatus.InsertedDeleted)
			{
				...
				{
					_discountEngine.RecalculateGroupAndDocumentDiscounts(sender, Transactions, null, DiscountDetails, Document.Current.BranchID, Document.Current.VendorLocationID, Document.Current.DocDate, GetDefaultAPDiscountCalculationOptions(Document.Current));
				}
				RecalculateTotalDiscount();
			}
		}
		...
		protected virtual void CurrencyInfo_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (info != null)
			{
				bool curyenabled = info.AllowUpdate(this.Transactions.Cache) && (Document.Current?.Released != true && Document.Current?.Prebooked != true);
		...
		protected virtual void CurrencyInfo_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (currencyInfo?.CuryRate == null || Document.Current == null) return;
			...
			if (!_discountEngine.IsInternalDiscountEngineCall && discountDetail != null)
			{
				if (discountDetail.DiscountID != null)
				{
					_discountEngine.InsertManualDocGroupDiscount(Transactions.Cache, Transactions,
						...
						GetDefaultAPDiscountCalculationOptions(Document.Current));
					RecalculateTotalDiscount();
				}

				if (_discountEngine.SetExternalManualDocDiscount(Transactions.Cache, Transactions, DiscountDetails, discountDetail, null, GetDefaultAPDiscountCalculationOptions(Document.Current)))
					RecalculateTotalDiscount();
			}
		}
		...
		protected virtual void APInvoiceDiscountDetail_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (!_discountEngine.IsInternalDiscountEngineCall && discountDetail != null)
			{
				...
				{
					_discountEngine.UpdateDocumentDiscount(Transactions.Cache, Transactions, DiscountDetails, Document.Current.BranchID, Document.Current.VendorLocationID, Document.Current.DocDate, discountDetail.Type != DiscountType.Document, GetDefaultAPDiscountCalculationOptions(Document.Current));
				RecalculateTotalDiscount();
			}
		...
		public virtual List<POReceiptLineS> GetInvoicePOReceiptLines(POReceipt receipt, APInvoice doc)
		{
			return PXSelectReadonly2<POReceiptLineS,
				...
				.Select(this, receipt.ReceiptNbr, doc.RefNbr, doc.DocType).RowCast<POReceiptLineS>().ToList();
		}
			...
			bool errorIfUnreleasedAPExists = true)
		{
			...
			if (list != null)
			{
				...
				else
				{
					...
					if (PXAccess.FeatureInstalled<FeaturesSet.vendorRelations>())
					{
						Document.Cache.SetValueExt<APInvoice.vendorID>(newdoc, payToVendorPO);
						newdoc.VendorLocationID = receipt.VendorID == payToVendorPO ? receipt.VendorLocationID : null;
						if (newdoc.VendorLocationID == null)
							Document.Cache.SetDefaultExt<APInvoice.vendorLocationID>(newdoc);
					}
					else
					{
						Document.Cache.SetValueExt<APInvoice.vendorID>(newdoc, receipt.VendorID);
						newdoc.VendorLocationID = receipt.VendorLocationID;
					}
					Document.Cache.SetValueExt<APInvoice.suppliedByVendorID>(newdoc, receipt.VendorID);
					PXFieldState state1 = (PXFieldState)Document.Cache.GetStateExt<APInvoice.suppliedByVendorID>(newdoc);
					if (state1 != null && !string.IsNullOrEmpty(state1.Error))
						throw new PXException(state1.Error);

					Document.Cache.SetValueExt<APInvoice.suppliedByVendorLocationID>(newdoc, receipt.VendorLocationID);
					PXFieldState state2 = (PXFieldState)Document.Cache.GetStateExt<APInvoice.suppliedByVendorLocationID>(newdoc);
					if (state2 != null && !string.IsNullOrEmpty(state2.Error))
						throw new PXException(state2.Error);

					//clear underfilled cache, otherwise it will be misused into AttachPrepayment method
					Adjustments.Cache.Clear();
					...
					try
					{
						newdoc = PXCache<APInvoice>.CreateCopy(newdoc);
					}
					finally
					{
					...
					if (usePOParameters && posetup.Current != null && 
						(receipt.ReceiptType == POReceiptType.POReceipt || receipt.ReceiptType == POReceiptType.POReturn))
					{
						CurrencyInfo bill_CuryInfo = currencyinfo.Select();
						CurrencyInfo receipt_CuryInfo = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<APInvoice.curyInfoID>>>>.Select(this, receipt.CuryInfoID);
						bill_CuryInfo.CuryID = receipt_CuryInfo.CuryID;
						bill_CuryInfo.CuryEffDate = receipt_CuryInfo.CuryEffDate;
						bill_CuryInfo.CuryRateTypeID = receipt_CuryInfo.CuryRateTypeID;
						bill_CuryInfo.CuryRate = receipt_CuryInfo.CuryRate;
						bill_CuryInfo.RecipRate = receipt_CuryInfo.RecipRate;
						bill_CuryInfo.CuryMultDiv = receipt_CuryInfo.CuryMultDiv;
						this.currencyinfo.Update(bill_CuryInfo);
					}
			...
			List<POReceiptLineS> orderLines = GetInvoicePOReceiptLines(receipt, newdoc);

			HashSet<string> ordersWithDiscounts = new HashSet<string>();
			foreach (POReceiptLineS orderline in orderLines)
			{
							...
							And<APAdjust.adjdRefNbr, IsNull>>>>>>>>>.Select(this))
					{
							...
							.Select(this, payment.DocType, payment.RefNbr))
						{
								...
								&& lazyTrans.Value.Any(t => t.POOrderType == orderPrepay.OrderType && t.PONbr == orderPrepay.OrderNbr))
							{
								...
								{
									existing = (APAdjust)Adjustments.Cache.CreateCopy(existing);
								}
		...
		{
			List<POLineS> lines = GetPOOrderLines(order, Document.Current, addBilled);
			ProcessPOOrderLines(
				lines,
				duplicates,
				keepOrderTaxes);
		...
		public List<POLineS> GetPOOrderLines(POOrder order, APInvoice doc, bool addBilled)
		{
			...
			bool isSubcontractDebitAdj = order.OrderType == POOrderType.RegularSubcontract && doc.DocType == APDocType.DebitAdj;
			if (!isSubcontractDebitAdj)
			{
				cmd.WhereAnd<Where<POLineS.closed, Equal<False>>>();
			}
			if (!addBilled)
			{
			...
			return cmd.Select(order.OrderType, order.OrderNbr).RowCast<POLineS>().ToList();
		}
		...
		public virtual void ProcessPOOrderLines(IEnumerable<IAPTranSource> orderlines, HashSet<APTran> duplicates = null, bool keepOrderTaxes = false)
		{
			...
			foreach (var orderline in orderlines)
			{
				...
				try
				{
					bool isSubcontractDebitAdj = orderline.OrderType == POOrderType.RegularSubcontract && Document.Current.DocType == APDocType.DebitAdj;
					if ((orderline.RetainagePct ?? 0m) != 0m && !isSubcontractDebitAdj)
					{
						EnableRetainage();
					}
		...
		public virtual APTran AddPOReceiptLine(IAPTranSource aLine, HashSet<APTran> checkForDuplicates)
		{
			...
			decimal unitCost = aLine.UnitCost ?? 0m;
			decimal curyUnitCost = aLine.CuryUnitCost ?? 0m;
			...
			DRDeferredCode deferralCode = PXSelect<DRDeferredCode, Where<DRDeferredCode.deferredCodeID, Equal<Required<DRDeferredCode.deferredCodeID>>>>.Select(this, item?.DeferredCode);
			if (deferralCode != null)
			{
				newtran.RequiresTerms = deferralCode.MultiDeliverableArrangement == false && DeferredMethodType.RequiresTerms(deferralCode.Method);
			}
			...
			else
			{
				...
				PXCurrencyAttribute.PXCurrencyHelper.CuryConvCury(this.Document.Cache, this.Document.Current, unitCost, out curyUnitCost, costPrecision);
			...
			{
				curyUnitCost = newtran.CuryUnitCost ?? 0m;
			...
			if (updTran == null)
			{
				...
				newtran = InsertTranOnAddPOReceiptLine(aLine, newtran);
				bool isSubcontractDebitAdj = aLine.OrderType == POOrderType.RegularSubcontract && Document.Current.DocType == APDocType.DebitAdj;
				if (!keepAmounts && retainPct != 0m && !isSubcontractDebitAdj)
				{
			...
			else
			{
				...
				updTran = UpdateTranOnAddPOReceiptLine(aLine, updTran);
				this.Transactions.Cache.RaiseExceptionHandling<APTran.qty>(updTran, updTran.Qty,
		...
		public virtual void CheckOrderTaskRule(PXCache sender, APTran row, int? newTaskID)
		{
			if (row.POOrderType != null && row.PONbr != null && row.POLineNbr != null && !POLineType.IsStock(row.LineType))
			{
				...
				{
					PMTask task = PMTask.PK.Find(this, row.TaskID);
					string taskCd = task != null ? taskCd = task.TaskCD : null;

					if (posetup.Current.OrderRequestApproval == true)
					{
						string message = poLine.OrderType == POOrderType.RegularSubcontract ? PO.Messages.TaskDiffersRSError : PO.Messages.TaskDiffersError;
						sender.RaiseExceptionHandling<APTran.taskID>(row, taskCd,
							new PXSetPropertyException(message, PXErrorLevel.Error));
					}
					else
					{
						string message = poLine.OrderType == POOrderType.RegularSubcontract ? PO.Messages.TaskDiffersRSWarning : PO.Messages.TaskDiffersWarning;
						sender.RaiseExceptionHandling<APTran.taskID>(row, taskCd,
		...
		public virtual DiscountEngine.DiscountCalculationOptions GetDefaultAPDiscountCalculationOptions(APInvoice doc)
		{
			return DiscountEngine.DefaultAPDiscountCalculationParameters |
		...
		protected virtual void RecalculateDiscounts(PXCache sender, APTran line)
		{
				...
				if (line.Qty != null)
			{
					...
					DiscountEngine.DiscountCalculationOptions discountCalculationOptions = GetDefaultAPDiscountCalculationOptions(Document.Current);
