...
namespace PX.Objects.EP
{
	public class EPSetupMaint : PXGraph<EPSetupMaint>
	{
		...
		{
			if (UsePRPostingOption(graph, setup, employeeID, out PRSetup payrollPreferences))
			{
				return payrollPreferences.TimePostingOption;
			}
			else
			{
		...
		public static int? GetOffBalancePostingAccount(PXGraph graph, EPSetup setup, int? employeeID)
		{
			if (UsePRPostingOption(graph, setup, employeeID, out PRSetup payrollPreferences))
			{
				return payrollPreferences.OffBalanceAccountGroupID;
			}
			else
			{
				return setup.OffBalanceAccountGroupID;
			}
		}

		private static bool UsePRPostingOption(PXGraph graph, EPSetup setup, int? employeeID, out PRSetup payrollPreferences)
		{
			...
			PREmployee prEmployee = PXSelect<PREmployee>.Search<PREmployee.bAccountID>(graph, employeeID).FirstOrDefault();
			if (prEmployee?.ActiveInPayroll != true)
			{
				return false;
			}

			payrollPreferences = new SelectFrom<PRSetup>.View(graph).SelectSingle();
	...
	protected virtual void EPSetup_OvertimeMultiplier_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		...
		private DateTime? GetLasttUsingWeek()
		{
            ...
            {
				startDate = PXWeekSelector2Attribute.GetWeekStartDate(this, lastTimeCard.WeekID.Value);
			}
