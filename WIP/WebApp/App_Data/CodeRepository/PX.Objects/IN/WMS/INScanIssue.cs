...
namespace PX.Objects.IN
{
	...
	public class INScanIssue : WMSBase
	{
		...
		protected override void _(Events.RowSelected<Header> e)
		{
			...
			if (Issue == null && !string.IsNullOrEmpty(HeaderView.Current?.RefNbr))
		...
		protected override bool DocumentIsEditable => base.DocumentIsEditable && INRegister.PK.Find(Base, Issue)?.Released != true;
		...
		protected virtual void ProcessConfirm()
		{
			...
			if (!EnsureSplitsLotSerial(userLotSerial, header, rollbackAction) || !EnsureSplitsLocation(header.LocationID, header, rollbackAction))
				return;

			if (Base.issue.Cache.GetStatus(Base.issue.Current) != PXEntryStatus.Inserted)
				SaveChanges();
