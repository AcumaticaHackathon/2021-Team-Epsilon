...
{
	[PXInternalUseOnly]
	public class CRGramProcessor : CRGrammProcess.Processor
	{
		...
		{
			var currentGrams = DoCreateGramms(document).Cast<CRGrams>().Where(gram => gram.CreateOnEntry != CreateOnEntryAttribute.Allow).ToList();
			if (currentGrams.Count == 0)
				return null;

			var duplicateRecords = duplicates.Cast<PXResult<CRDuplicateRecord, Contact, DuplicateContact, BAccountR>>().ToList();
