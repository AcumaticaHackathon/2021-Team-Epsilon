				...
				try
				{
					...
					foreach (APInvoiceExt docLine in group)
					{
						...
						foreach (APTran detail in details)
						{
							...
							tranNew.ProjectID = PXAccess.FeatureInstalled<FeaturesSet.projectModule>()
					...
					if (!isAutoRelease && Base.apsetup.Current.HoldEntry == false && Base.IsApprovalRequired(invoice, Base.Document.Cache))
					{
		...
		protected virtual void Configure(WorkflowContext<APInvoiceEntry, APInvoice> context)
		{
			...
			var conditions = new
			{
				IsNotRetenageApplied
					= Bql<Not<APRegister.docType.IsEqual<APDocType.invoice>
						.And<APRegister.released.IsEqual<True>>
						.And<APRegister.retainageApply.IsEqual<True>>
						.And<APRegister.curyRetainageUnreleasedAmt.IsGreater<decimal0>>>>(),
				IsNotInvoice
					= Bql<docType.IsNotEqual<APDocType.invoice>>(),
				IsRetainageMigrationMode =
				...
				return screen
					.WithActions(actions =>
					{
						actions.Add<APInvoiceEntryRetainage>(g => g.releaseRetainage,
							c => c.InFolder(FolderType.ActionsFolder, g => g.payInvoice)
								.PlaceAfter(g => g.payInvoice)
								.IsHiddenWhen(conditions.IsNotInvoice || conditions.IsRetainageMigrationMode)
