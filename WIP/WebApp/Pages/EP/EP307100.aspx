...
<asp:Content ID="cont2" ContentPlaceHolderID="phF" runat="Server">
	<px:PXPanel ID="PXPanel1" runat="server" CssClass="FormContent" RenderStyle="Simple">
		<px:PXLayoutRule runat="server" StartColumn="True" />
		<px:PXFormView ID="headerForm" runat="server" DataSourceID="ds" DataMember="Document" Style="z-index: 100" Width="100%" SkinID="Transparent">
			<Template>
			...
			<AutoSize Enabled="True" Container="Parent" MinHeight="75" />
		</px:PXFormView>
		<px:PXLayoutRule runat="server" StartColumn="True" />
		<px:PXFormView ID="headerFilter" runat="server" DataSourceID="ds" DataMember="Filter" Style="z-index: 100" Width="100%" SkinID="Transparent">
			<Template>
				<px:PXLayoutRule runat="server" StartColumn="true" />
				<px:PXSegmentMask DataField="ProjectID" runat="server" ID="edProjectID" CommitChanges="true" />
				<px:PXSelector DataField="ProjectTaskID" runat="server" ID="edProjectTaskID" CommitChanges="true" AutoRefresh="true" />
				<px:PXDropDown DataField="Day" runat="server" ID="edDay" CommitChanges="true" />

				<px:PXLayoutRule runat="server" StartColumn="true" />
				<px:PXLayoutRule runat="server" GroupCaption="Regular" />
				<px:PXTimeSpan DataField="RegularTime" runat="server" ID="edRegularTime" SummaryMode="true" InputMask="hh:mm" Width="100" LabelWidth="60" />
				<px:PXTimeSpan DataField="BillableTime" runat="server" ID="edBillableTime" SummaryMode="true" InputMask="hh:mm" Width="100" LabelWidth="60" />
				...
				<px:PXTimeSpan DataField="Overtime" runat="server" ID="edOvertime" SummaryMode="true" InputMask="hh:mm" Width="100" />
				<px:PXTimeSpan DataField="BillableOvertime" runat="server" ID="edBillableOvertime" SummaryMode="true" InputMask="hh:mm" Width="100" />
				<px:PXLayoutRule runat="server" EndGroup="true" />

				<px:PXLayoutRule runat="server" StartColumn="true" GroupCaption="Total" SuppressLabel="true" />
				<px:PXTimeSpan DataField="TotalTime" runat="server" ID="edTotalTime" SummaryMode="true" InputMask="hh:mm" Width="100" />
				<px:PXTimeSpan DataField="TotalBillableTime" runat="server" ID="edTotalBillableTime" SummaryMode="true" InputMask="hh:mm" Width="100" />
				<px:PXLayoutRule runat="server" EndGroup="true" />
			</Template>
		</px:PXFormView>
	</px:PXPanel>
...
<asp:Content ID="cont3" ContentPlaceHolderID="phG" runat="Server">
	<px:PXTab ID="tab" runat="server" Width="100%" Height="150px" DataSourceID="ds">
		<Items>
			<px:PXTabItem Text="Time Activities">
				<Template>
					<px:PXGrid ID="gridTimeActivities" runat="server" DataSourceID="ds" Style="z-index: 100" Width="100%" Height="150px" SkinID="Details" AutoAdjustColumns="true" SyncPosition="true">
						<Levels>
							<px:PXGridLevel DataMember="TimeActivities">
								<RowTemplate>
									...
									<px:PXSelector DataField="Date" ID="edDate" runat="server" />
									<px:PXDateTimeEdit DataField="Date_Time" ID="edDateTime" runat="server" TimeMode="True" />
								</RowTemplate>
								<Columns>
									<px:PXGridColumn DataField="Hold" Type="CheckBox" />
									<px:PXGridColumn DataField="OwnerID" Width="200" CommitChanges="true" />
									<px:PXGridColumn DataField="OwnerID_EPEmployee_AcctName" />
									<px:PXGridColumn DataField="ApprovalStatus" />
									<px:PXGridColumn DataField="Date" CommitChanges="true" Width="175" />
									<px:PXGridColumn DataField="Date_Time" CommitChanges="true" />
			...
			<px:PXTabItem Text="Crew Members">
						...
						<Levels>
							<px:PXGridLevel DataMember="WorkgroupTimeSummary">
								<Columns>
									...
									<px:PXGridColumn DataField="ContactID_EPEmployee_AcctName" />
				...
				<AutoCallBack Command="Refresh" Target="membersForm" ActiveBehavior="true">
					<Behavior RepaintControlsIDs="membersForm" />
				</AutoCallBack>
			</px:PXTabItem>
		</Items>
		<AutoSize Container="Window" Enabled="True" MinHeight="150" />
					...
					<Levels>
						<px:PXGridLevel DataMember="BulkEntryTimeActivities">
							<RowTemplate>
								...
								<px:PXSelector DataField="Date" ID="edBulkDate" runat="server" />
								<px:PXDateTimeEdit DataField="Date_Time" ID="edBulkDateTime" runat="server" TimeMode="True" />
							</RowTemplate>
							<Columns>
	...
	<script>
		...
		function RefreshFilter(callbackContext) {
			var isColumnFiltering = callbackContext.control.dataMember === 'TimeActivities';
			if (callbackContext.info.name == 'Refresh' && callbackContext.controlID.indexOf('gridTimeActivities') >= 0 && isColumnFiltering) {
