...
namespace PX.Data
{
		...
		IPXFieldUpdatingSubscriber
	{
		...
		private string prepValue(string value, string symbol)
		{
			...
			for (int i = 0; i < _Definition.Dimensions[_Dimension].Length; i++)
			{
				...
				else
				{
					if (symbol == "")
					{
						if (customNumberingSymbol != null)
						{
							symbol = customNumberingSymbol;
						}
						else
						{
		...
		public virtual void RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			NewNumber = GetNextNumber(sender, e.Row);
		...
		protected string GetNextNumber(PXCache sender, Object row)
		{
			if (IS_SEPARATE_SCOPE)
			{
				using (new PXConnectionScope())
				{
					...
					{
						String NewNumber = GetNextNumberInt(sender, row);
						ts.Complete();
						return NewNumber;
					}
				}
				...
				return GetNextNumberInt(sender, row);
		}
		protected string GetNextNumberInt(PXCache sender, Object row)
		{
			...
			if (NewNumber.CompareTo(WarnNbr) >= 0)
			{
				...
				PXUIFieldAttribute.SetWarning(sender, row, _FieldName, ErrorMessages.WarningNumReached);
			}
		...
		[PXInternalUseOnly]
		public static void SetCustomNumbering<Field>(PXCache cache, string numberingSeq, int? segmentID, string newSymbol)
			where Field : IBqlField
		{
			SetCustomNumbering(cache, typeof(Field).Name, numberingSeq, segmentID, newSymbol);
		}
		[PXInternalUseOnly]
		public static void SetCustomNumbering(PXCache cache, string name, string numberingSeq, int? segmentID, string newSymbol)
		{
			...
			foreach (PXDimensionAttribute attr in cache.GetAttributes(name).OfType<PXDimensionAttribute>())
			{
				...
				attr.customNumberingSymbol = newSymbol;
			}
		}
		#endregion
	}
