...
namespace PX.Objects.PM
{
	...
	public class ProformaEntry : PXGraph<ProformaEntry, PMProforma>, IGraphWithInitialization
	{
		...
		public readonly TotalsKey PayByLineOffKey = new TotalsKey(0, CostCodeAttribute.DefaultCostCode.GetValueOrDefault(), PMInventorySelectorAttribute.EmptyInventoryID, 0);
		...
		public PXSelect<PMProforma, Where<PMProforma.corrected, NotEqual<True>>> Document;
		...
		public ProformaEntry()
		{
			...
			uploadFromBudget.SetVisible(Setup.Current.MigrationMode == true);
			this.CopyPaste.SetVisible(false);
			this.Insert.SetVisible(Setup.Current.MigrationMode == true);
		...
		[PXUIField(DisplayName = "Upload from Budget")]
		[PXProcessButton]
		public IEnumerable UploadFromBudget(PXAdapter adapter)
		{
			if (Document.Current != null && Document.Current.ProjectID != null)
			{
				...
				{
					string key = string.Format("{0}.{1}.{2}.{3}", line.TaskID, line.AccountGroupID, line.CostCodeID, line.InventoryID);
				...
				foreach (PXResult<PMRevenueBudget, PMAccountGroup> res in select.Select(Document.Current.ProjectID))
				{
					...
					PMTask task = PMTask.PK.Find(this, line.TaskID);
					PMBillingRule rule = PMBillingRule.PK.Find(this, task?.BillingID);
					...
					string key = string.Format("{0}.{1}.{2}.{3}", proformaLine.TaskID, proformaLine.AccountGroupID, proformaLine.CostCodeID, proformaLine.InventoryID);
		...
		protected virtual IEnumerable RemoveHold(PXAdapter adapter) 
		{
			if (Document.Current != null)
			{
				...
				ValidateInclusiveTaxesOnUnhold(Document.Current);
		...
		protected virtual void _(Events.FieldUpdated<PMProformaProgressLine, PMProformaProgressLine.completedPct> e)
		{
			...
			if (budget != null)
			{
				...
				decimal invoicedAmount = budget.CuryActualAmount.GetValueOrDefault() + budget.CuryInvoicedAmount.GetValueOrDefault() + pendingInvoiceAmount - e.Row.CuryLineTotal.GetValueOrDefault() - GetLastInvoicedBeforeCorrection(e.Row);
		...
		protected virtual void _(Events.FieldSelecting<PMProformaProgressLine, PMProformaProgressLine.completedPct> e)
		{
			if (e.Row != null)
			{
				...
				if (budget != null)
				{
					decimal invoicedAmount = GetLineTotalInProjectCurrency(e.Row) + ((UseBaseCurrency) ? GetPreviouslyInvoicedAmount(e.Row).LineTotal : GetPreviouslyInvoicedAmount(e.Row).CuryLineTotal);
					decimal result = 0;
		...
		protected virtual void _(Events.FieldDefaulting<PMProformaTransactLine, PMProformaTransactLine.taxCategoryID> e)
		{
			if (Project.Current != null)
			{
				...
				if (e.NewValue == null)
				{
					PMTask task = PMTask.PK.Find(this, e.Row.TaskID);
		...
		protected virtual void _(Events.FieldVerifying<PMProformaTransactLine, PMProformaTransactLine.taskID> e)
		{
			...
			{
				PMTask task = PMTask.PK.Find(this, (int?)e.NewValue);
				if (task != null)
				{
		...
		protected virtual void _(Events.FieldVerifying<PMProforma, PMProforma.locationID> e)
		{
			if (e.Row != null)
			{
				Location location = CR.Location.PK.Find(this, e.Row.CustomerID, (int?)e.NewValue);
				if (location?.CTaxCalcMode == TaxCalculationMode.Gross)
				{
					PXSetPropertyException ex = new PXSetPropertyException(Messages.TheGrossTaxCalcModeForCustomerIsNotSupportedInProFormaInvoicesChangeTaxCalModeOrLocation, location.LocationCD, Customer.Current?.AcctCD, location.LocationCD);
		...
		protected virtual void _(Events.RowSelected<PMProforma> e)
		{
			...
			if (Project.Current != null)
			{
				...
				PXUIFieldAttribute.SetVisible<PMProformaProgressLine.inventoryID>(ProgressiveLines.Cache, null, Setup.Current.MigrationMode == true || !CostCodeAttribute.UseCostCode() && Project.Current.BudgetLevel != BudgetLevels.Task);
			...
			//Migration Mode:
			PXUIFieldAttribute.SetEnabled<PMProforma.projectID>(Document.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProforma.customerID>(Document.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProforma.aRInvoiceDocType>(Document.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProforma.aRInvoiceRefNbr>(Document.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProformaTransactLine.taskID>(TransactionLines.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProformaTransactLine.inventoryID>(TransactionLines.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProformaProgressLine.accountGroupID>(ProgressiveLines.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProformaProgressLine.taskID>(ProgressiveLines.Cache, null, Setup.Current.MigrationMode == true);
			PXUIFieldAttribute.SetEnabled<PMProformaProgressLine.inventoryID>(ProgressiveLines.Cache, null, Setup.Current.MigrationMode == true);
			ProgressiveLines.Cache.AllowInsert = Setup.Current.MigrationMode == true;
			...
			if (e.Row != null)
			{
				...
				uploadFromBudget.SetEnabled(e.Row.Hold == true);
				...
				PXUIFieldAttribute.SetDisplayName<PMRevenueBudget.curyRevisedAmount>(Caches[typeof(PMRevenueBudget)], UseBaseCurrency ? Messages.RevisedBudgetedAmountInProjectCurrency : Messages.RevisedBudgetedAmount);
				PXUIFieldAttribute.SetDisplayName<PMRevenueBudget.curyInvoicedAmount>(Caches[typeof(PMRevenueBudget)], UseBaseCurrency ? Messages.DraftInvoiceAmountInProjectCurrency : Messages.DraftInvoiceAmount);
				PXUIFieldAttribute.SetDisplayName<PMRevenueBudget.curyActualAmount>(Caches[typeof(PMRevenueBudget)], UseBaseCurrency ? Messages.ActualAmountInProjectCurrency : Messages.ActualAmount);
			}
		}
		...
		protected virtual void _(Events.RowSelected<PMProformaProgressLine> e)
		{
			if (e.Row != null)
			{
				ProformaTotals totals = GetPreviouslyInvoicedAmount(e.Row);
				// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Initializing non-db field]
				e.Row.CuryPreviouslyInvoiced = totals.CuryLineTotal;
				// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Initializing non-db field]
				e.Row.PreviouslyInvoiced = totals.LineTotal;
			}
		}
		...
		public virtual void ReleaseDocument(PMProforma doc)
		{
			...
			if (Setup.Current.MigrationMode == true && !string.IsNullOrEmpty(doc.ARInvoiceDocType) && !string.IsNullOrEmpty(doc.ARInvoiceRefNbr))
			{
				foreach (PMProformaProgressLine line in ProgressiveLines.Select())
				{
		...
		protected virtual ARInvoice ProcessRevision()
		{
			...
			var finPeriod = FinPeriodRepository.GetByID(invoiceEntry.Document.Current.FinPeriodID, PXAccess.GetParentOrganizationID(invoiceEntry.Document.Current.BranchID));
			var canPostRes = FinPeriodUtils.CanPostToPeriod(finPeriod);
		...
		private void ReverseCurrentDocument(ARInvoiceEntry invoiceEntry)
        {
			...
			{
				invoiceEntry.reverseInvoice.Press();
			}
            else
            {
				invoiceEntry.reverseInvoiceAndApplyToMemo.Press();
			}
		}
		...
		protected virtual bool IsReversingDocumentAlreadyExists(ARInvoice origDoc)
		{
			...
			return reversingDoc != null;
		}

		protected virtual string GetReversingDocType(string docType)
		{
			switch (docType)
			{
		...
		protected virtual void RollbackRevision(PMProforma deletedDoc)
		{
			if (deletedDoc != null)
			{
				...
				if (corrected != null)
				{
					...
					Document.Cache.MarkUpdated(corrected);
		...
		public Dictionary<TotalsKey, ProformaTotals> previouslyInvoicedAmounts;
		public string previouslyInvoicedAmountsKey = null;

		/// <summary>
		/// Returns Previously Invoiced Amount in Project Currency
		/// </summary>
		public virtual ProformaTotals GetPreviouslyInvoicedAmount(PMProformaProgressLine row)
		{			
			if (previouslyInvoicedAmounts == null || Document.Current.RefNbr != previouslyInvoicedAmountsKey)
			{
				previouslyInvoicedAmounts = GetProformaPreviousTotals();
				previouslyInvoicedAmountsKey = Document.Current.RefNbr;
			}
			TotalsKey key = new TotalsKey(row.TaskID.GetValueOrDefault(), row.CostCodeID.GetValueOrDefault(CostCodeAttribute.GetDefaultCostCode()), row.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID), row.AccountGroupID.GetValueOrDefault());

			ProformaTotals result = new ProformaTotals();
		...
		public virtual Dictionary<TotalsKey, ProformaTotals> GetProformaPreviousTotals()
		{
			Dictionary<TotalsKey, ProformaTotals> totalsPrevious = new Dictionary<TotalsKey, ProformaTotals>();
			...
			foreach (PMProformaLine line in totalsPreviousSelect.Select())
			{
				...
				totals.Key = new TotalsKey(line.TaskID.GetValueOrDefault(), line.CostCodeID.GetValueOrDefault(CostCodeAttribute.GetDefaultCostCode()), line.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID), line.AccountGroupID.GetValueOrDefault());
				...
				totalsPrevious.Add(totals.Key, totals);
			}

			return totalsPrevious;
		}
		...
		protected virtual void ReAllocateContractRetainage(PMProject project, decimal totalInvoiceUptoDate, decimal retainageToAllocate)
		{
			...
			foreach (PXResult<PMProformaProgressLine, PMRevenueBudget> res in ProgressiveLines.Select())
			{
				...
				decimal invoicedLineTotal = GetInvoicedAmount(revenue);
				...
				ProgressiveLines.Cache.SetValue<PMProformaLine.curyAllocatedRetainedAmount>(line, allocate);
			}

			Document.Cache.SetValue<PMProforma.curyAllocatedRetainedTotal>(Document.Current, retainageToAllocate);
		}
		...
		public virtual Dictionary<TotalsKey, decimal> GetBilledRetainageToDate(DateTime? cutoffDate)
		{
			Dictionary<TotalsKey, decimal> billedByTask = new Dictionary<TotalsKey, decimal>();
			...
			var selectBilledRetainageByLine = new PXSelectJoinGroupBy<ARTran,
				...
				Aggregate<GroupBy<ARTran.tranType, GroupBy<ARTran.taskID, GroupBy<ARTran.costCodeID, GroupBy<ARTran.inventoryID, Sum<ARTran.curyTranAmt>>>>>>>(this);

			foreach (ARTran tran in selectBilledRetainageByLine.Select(cutoffDate))
			{
				TotalsKey key = new TotalsKey(tran.TaskID.GetValueOrDefault(), tran.CostCodeID.GetValueOrDefault(CostCodeAttribute.DefaultCostCode.GetValueOrDefault()), tran.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID), 0);
		...
		public struct TotalsKey
		{
			...
			public TotalsKey(int taskID, int costCodeID, int inventoryID, int accountGroupID)
			{
				TaskID = taskID;
		...
		where TPrimaryView : class, IBqlTable, new()
	{
		...
		protected override object GetSourceRow(string sourceType, CRPMTimeActivity activity)
		{
			...
			if(sourceRow != null && sourceType == PMNotificationSource.Project)
			{
				...
				PMProject rec = PMProject.PK.Find(_Graph, projectID);
