...
namespace PX.Objects.PM
{
	public class ARInvoiceEntryExt : PXGraphExtension<ARInvoiceEntry>
	{
		...
		{
			if (Base.Document.Current.IsRetainageDocument != true && Base.Document.Current.ProformaExists != true && (row.TaskID != oldRow.TaskID || row.TranAmt != oldRow.TranAmt || row.AccountID != oldRow.AccountID))
			{
		...
		public virtual void AddToDraftRetained(ARTran line, int? revenueAccountGroup, int mult = 1)
		{
			if (line.TaskID == null)
				return;
			...
			if (project != null && project.NonProject != true)
			{
				if (revenueAccountGroup != null && line.ProjectID != null && line.TaskID != null)
				{
					...
					if (project.CuryID == project.BillingCuryID)
					{
						invoiced.CuryDraftRetainedAmount += mult * line.CuryRetainageAmt.GetValueOrDefault();
						invoiced.DraftRetainedAmount += mult * line.RetainageAmt.GetValueOrDefault();
					}
					else
					{
						invoiced.CuryDraftRetainedAmount += mult * line.RetainageAmt.GetValueOrDefault();
						invoiced.DraftRetainedAmount += mult * line.RetainageAmt.GetValueOrDefault();
					}
				}
			}
