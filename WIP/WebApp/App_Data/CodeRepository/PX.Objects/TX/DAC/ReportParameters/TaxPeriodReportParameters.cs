...
namespace PX.Objects.TX.DAC.ReportParameters
{
	public class TaxPeriodReportParameters: IBqlTable
	{
		...
		[PXUIRequired(typeof(FeatureInstalled<CS.FeaturesSet.multipleBaseCurrencies>))]
	...
	public class TaxTreeSelect : PXSelectOrganizationTree
	{
		...
		public override IEnumerable tree([PXString] string AcctCD)
		{
			...
			foreach (PXResult<BAccountR, Branch, Organization> row in
				SelectFrom<BAccountR>
					...
					.Where<Brackets<Branch.branchID.IsNull
						...
						.And<Organization.organizationID.IsNull
							.Or<Organization.organizationType.IsNotEqual<OrganizationTypes.group>
					...
					.Select(_Graph))
			{
				...
				item.CanSelect = !(organization.FileTaxesByBranches == true && item.BAccountID == organization.BAccountID);
				result.Add(item);
			}
			return result;
		}
