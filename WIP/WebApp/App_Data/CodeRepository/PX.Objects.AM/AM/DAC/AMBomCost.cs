...
{
	[Serializable]
    [PXCacheName("BOM Cost")]
    [System.Diagnostics.DebuggerDisplay("{DebuggerDisplay,nq}")]
    public class AMBomCost : IBqlTable, IBomRevision
	{
        internal string DebuggerDisplay => $"BOMID = {BOMID}, RevisionID = {RevisionID}, InventoryID = {InventoryID}, SiteID = {SiteID}";

        #region Keys

        public class PK : PrimaryKeyOf<AMBomCost>.By<bOMID, revisionID, userID>
        {
            public static AMBomCost Find(PXGraph graph, string bOMID, string revisionID, Guid userID)
                => FindBy(graph, bOMID, revisionID, userID);
            public static AMBomCost FindDirty(PXGraph graph, string bOMID, string revisionID, Guid userID)
                => PXSelect<AMBomCost,
                    Where<bOMID, Equal<Required<bOMID>>,
                        And<revisionID, Equal<Required<revisionID>>,
                        And<userID, Equal<Required<userID>>>>>>
                    .SelectWindowed(graph, 0, 1, bOMID, revisionID, userID);
        }
        
        public static class FK
        {
            public class BOM : AMBomItem.PK.ForeignKeyOf<AMBomCost>.By<bOMID, revisionID> { }
        }
        ...
        [PXFormula(typeof(Add<AMBomCost.matlManufacturedCost, AMBomCost.matlNonManufacturedCost>))]
        ...
        [PXUIField(DisplayName = "User ID", Enabled = false, Visible = false)]
        ...
        [PXDBPriceCost()]
        [PXDefault(TypeCode.Decimal, "0.0")]
        [PXUIField(DisplayName = "Current Standard Cost", Enabled = false)]
        ...
        [PXDBPriceCost()]
        [PXDefault(TypeCode.Decimal, "0.0")]
        [PXUIField(DisplayName = "Pending Standard Cost")]
