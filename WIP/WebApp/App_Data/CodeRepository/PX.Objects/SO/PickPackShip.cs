...
namespace PX.Objects.SO
{
	...
	public class PickPackShip : WMSBase
	{
		...
		public class Header : WMSHeader, ILSMaster
		{
			...
			[PXUIVisible(typeof(processingSucceeded.IsNotNull))]
		...
		#region Return
		public SelectFrom<SOShipLineSplit>.View Returned;
		protected virtual IEnumerable returned() => Picked.Select();
		#endregion
		...
		protected override void ProcessDocumentNumber(string barcode)
		{
			...
			{
				ReportError(Msg.ShipmentInvalid, shipment.ShipmentNbr, Base.Document.Cache.GetStateExt<SOShipment.status>(shipment));
				return;
			}
		...
		protected virtual void ConfirmShipment(bool confirmAll)
		{
			...
			if (SelectedPackage?.Confirmed == false)
				if (AutoConfirmPackage(Setup.Current.ConfirmEachPackageWeight == false) == false || HeaderView.Current.ScanState == ScanStates.BoxWeight)
				return;
		...
		protected static void ConfirmPickListHandler(SOPickingWorksheet worksheet, SOPicker pickList, int? sortingLocationID)
		{
			...
			throw new PXOperationCompletedException(Msg.PickListIsConfirmed);
		}
		...
		protected override void OnWaitEnd(PXLongRunStatus status, SOShipment primaryRow)
		{
			...
			if (picker?.Confirmed == true || shipmentConfirmed)
			{
				...
				{
					HeaderSetter.WithEventFiring.Set(x => x.RefNbr, null);
					HeaderSetter.WithEventFiring.Set(x => x.WorksheetNbr, null);
					HeaderSetter.WithEventFiring.Set(x => x.PickerNbr, null);
		...
		public new abstract class Msg : WMSBase.Msg
		{
			...
			public const string ShipmentInvalid = "The {0} shipment cannot be processed because it has the {1} status.";
			...
			public const string ShipmentIsConfirmed = "The shipment has been successfully confirmed.";
			...
			public const string PickListIsConfirmed = "The pick list has been successfully confirmed.";
