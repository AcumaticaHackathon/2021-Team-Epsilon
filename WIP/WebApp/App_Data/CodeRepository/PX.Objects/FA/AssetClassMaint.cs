...
namespace PX.Objects.FA
{
	public class AssetClassMaint : PXGraph<AssetClassMaint, FixedAsset>
	{
		...
		[PXSelector(typeof(Search<FixedAsset.assetCD, Where<FixedAsset.recordType, Equal<FARecordType.classType>>>),
			...
			typeof(FixedAsset.underConstruction),
		...
		[PXDBBool]
		[PXUIField(DisplayName = "Under Construction", Visibility = PXUIVisibility.SelectorVisible)]
		[PXDefault(false)]
		protected virtual void _(Events.CacheAttached<FixedAsset.underConstruction> e) { }
		#endregion
		...
		protected virtual void FixedAsset_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if(header.UnderConstruction == true && header.Depreciable == false)
			{
				sender.RaiseExceptionHandling<FixedAsset.underConstruction>(header, header.UnderConstruction, new PXSetPropertyException(Messages.UnderConstructionCheckboxCanBeSelectedOnlyForDepreciableAssetClasses));
			}
		}
		...
		protected virtual void _(Events.FieldVerifying<FixedAsset.underConstruction> e)
		{
			FixedAsset _class = (FixedAsset)e.Row;
			if(_class.UnderConstruction != ((bool?)e.NewValue))
			{
				FixedAsset asset = SelectFrom<FixedAsset>
					.Where<FixedAsset.classID.IsEqual<@P.AsInt>>
					.View.Select(this, _class.AssetID);

				if (asset != null)
				{
					throw new PXSetPropertyException(Messages.StateOfUnderConstructionCheckboxCannotBeChangedSomeAssetsAssociatedWithClass);
				}
			}
		}
		#endregion
		...
		protected void FABookSettings_AveragingConvention_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			if (e.NewValue != null && !new FAAveragingConvention.ListAttribute().GetAllowedValues(sender).Contains(e.NewValue))
			{
				throw new PXSetPropertyException(ErrorMessages.ElementDoesntExist, $"[{nameof(FABookSettings.averagingConvention)}]");
			}
		}
		...
		protected virtual void FABookSettings_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (set.Depreciate == true && header.UnderConstruction == false)
			{
