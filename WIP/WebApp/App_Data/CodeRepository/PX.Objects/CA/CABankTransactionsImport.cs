...
{
	public class CABankTransactionsImport : PXGraph<CABankTransactionsImport, CABankTranHeader>, PXImportAttribute.IPXPrepareItems, ICaptionable<object>
	{
		[Serializable]
		...
		[PXUIField(DisplayName = Messages.ProcessTransactions, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select, Visible = true)]
		[PXButton()]
		public virtual IEnumerable ProcessTransactions(PXAdapter adapter)
		{
			CABankTranHeader headerRow = Header.Current;
			if (headerRow?.CashAccountID != null)
			{
				var graph = PXGraph.CreateInstance<CABankTransactionsMaint>();
				graph.TranFilter.Current.CashAccountID = headerRow.CashAccountID;
		...
		{
			CABankTranHeader currentItem = item as CABankTranHeader;
			if (currentItem == null) return "";

			CashAccount cashAcc = PXSelectorAttribute.Select<CashAccount.cashAccountID>(Header.Cache, currentItem) as CashAccount;
			if (this.Header.Cache.GetStatus(currentItem) == PXEntryStatus.Inserted)
			{
				return (cashAcc == null) ? PXMessages.Localize(PX.Data.InfoMessages.NewRecord) : $"{cashAcc.CashAccountCD} {cashAcc.Descr}";
			}
			else
			{
				return (cashAcc == null) ? currentItem.RefNbr : $"{currentItem.RefNbr} - {cashAcc.CashAccountCD} {cashAcc.Descr}: {currentItem.DocDate.Value.ToShortDateString()}";
			}
		}
		...
		public override void Persist()
		{
			...
			Header.Cache.RaiseRowSelected(Header.Current);
		}
		...
		protected virtual void CABankTranHeader_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row == null) return;
			bool isNew = this.Header.Cache.GetStatus(row) == PXEntryStatus.Inserted;
			...
			processTransactions.SetEnabled(!isNew && row.CashAccountID.HasValue);
		...
		protected virtual void CABankTran_TranDate_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (row != null)
			{
				sender.SetDefaultExt<CABankTran.matchingPaymentDate>(row);
				sender.SetDefaultExt<CABankTran.matchingfinPeriodID>(row);
			}
		}

		protected virtual void CABankTran_MatchingPaymentDate_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			CABankTran row = e.Row as CABankTran;
			if (row != null)
			{
