    ...
    public class SchdDateDetail<TSchdDetail> where TSchdDetail : ISchdDetail<TSchdDetail>
    {
        ...
        public SchdDateDetail(int blockSize, AMDateInfo dateInfo, List<TSchdDetail> dateDetail)
        {
            ...
            if (dateDetail != null)
            {
                ...
                UsedDateDetail = dateDetail.OrderBy(d => d.OrderByDate.GetValueOrDefault()).Where(d => d.RunTime.GetValueOrDefault() > 0).ToList();
            }
        ...
        protected virtual TSchdDetail UpdateSchdDetail(TSchdDetail schDetail)
        {
            ...
            schDetail.RunTimeBase = AMDateInfo.GetDateMinutes(schDetail.StartTime.GetValueOrDefault(), schDetail.EndTime.GetValueOrDefault());
            schDetail.RunTime = schDetail.RunTimeBase;
            schDetail.SchdBlocks = ProductionScheduleEngine.MinutesToBlocks(schDetail.RunTime.GetValueOrDefault(), _BlockSize, true);
            return schDetail;
        }
        ...
        public virtual SchdDetailResults<TSchdDetail> GetAvailableBlocks(int requestedBlockSize, ReadDirection direction, DateTime startingTime)
        {
            ...
            foreach (var det in orderedList)
            {
                ...
                summary.RunTimeBase = summary.RunTimeBase.GetValueOrDefault() + newSchdDetail.RunTimeBase.GetValueOrDefault();
                summary.RunTime = summary.RunTime.GetValueOrDefault() + newSchdDetail.RunTime.GetValueOrDefault();
