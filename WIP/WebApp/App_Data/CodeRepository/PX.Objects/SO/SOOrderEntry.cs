	...
	public partial class SOOrderEntry : PXGraph<SOOrderEntry, SOOrder>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess, IGraphWithInitialization
	{
		...
		public virtual IEnumerable defPaymentMethodInstanceDetails()
		{
			if (DefPaymentMethodInstance.Current != null)
			{
				...
				return CCProcessingHelper.GetPMdetails(this, DefPaymentMethodInstance.Current);
			}
			else
			{
				return null;
		...
		public PXMenuItem<SOOrder> createShipment;
		...
		[PXMenuItem(nameof(action), OrderActions.Values.CreateShipment)]
					...
					for (int i = 0; i < list.Count; i++)
					{
						...
						else
						{
								...
								OrderBy<Asc<SOShipmentPlan.siteID>>>.SelectMultiBound(shipmentEntry, new object[] { order }))
							{
								...
								if (GroupHelper.IsAccessibleToUser(shipmentEntry.Caches[typeof(INSite)], inSite, Accessinfo.UserName, forceUnattended: true))
						...
						foreach (int? SiteID in sites)
						{
							...
							try
							{
								using (var ts = new PXTransactionScope())
								{
									...
									shipmentEntry.CreateShipment(order, SiteID, filter.ShipDate, adapterSlice.MassProcess, operation, created, adapterSlice.QuickProcessFlow);
									ts.Complete();
								}
		...
		public PXMenuItem<SOOrder> applyAssignmentRules;
		...
		[PXMenuItem(nameof(action), OrderActions.Values.ApplyAssignmentRules)]
		...
		public PXMenuItem<SOOrder> postInvoiceToIN;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = OrderActions.DisplayNames.PostInvoiceToIN, Visible = false, MapEnableRights = PXCacheRights.Select)]
		[PXMenuItem(nameof(action), OrderActions.Values.PostInvoiceToIN)]
		protected virtual IEnumerable PostInvoiceToIN(PXAdapter adapter)
		{
			Save.Press();
		...
		public PXMenuItem<SOOrder> createPurchaseOrder;
		...
		[PXMenuItem(nameof(action), OrderActions.Values.CreatePurchaseOrder)]
		...
		public PXMenuItem<SOOrder> createTransferOrder;
		...
		[PXMenuItem(nameof(action), OrderActions.Values.CreateTransferOrder)]
		...
		public PXMenuItem<SOOrder> reopenOrder;
		...
		[PXMenuItem(nameof(action), OrderActions.Values.ReopenOrder)]
		...
		protected virtual IEnumerable CompleteOrder(PXAdapter adapter) => adapter.Get<SOOrder>();
		...
		public virtual IEnumerable PrepareInvoice(PXAdapter adapter)
		{
			...
			PXLongOperation.StartOperation(this, delegate ()
			{
				...
				this.InvoiceOrder(adapter.Arguments, list, created, adapter.MassProcess, adapter.QuickProcessFlow);

				if (adapter.MassProcess) // order is updated and saved somewhere in InvoiceOrder method
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Copy Order", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		public virtual void CopyOrderProc(SOOrder sourceOrder, CopyParamFilter copyFilter)
		{
			...
			foreach (PXResult<SOOrder, CurrencyInfo> res in orderWithCurrency)
			{
				...
				targetOrder.HasLegacyCCTran = false;
		...
		[PXButton(VisibleOnDataSource = false/*ImageKey = PX.Web.UI.Sprite.Main.Process*/)]
		...
		public virtual IEnumerable RecalculateDiscountsAction(PXAdapter adapter)
		{
			...
			else if (adapter.ExternalCall == false || this.IsExport == true)
				{
		...
		public PXWorkflowEventHandler<SOOrder, SOShipment> OnShipmentConfirmed;
		public PXWorkflowEventHandler<SOOrder, SOShipment> OnShipmentCorrected;
		...
		protected virtual void SOOrder_Cancelled_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			ExternalTranHelper.UpdateCCPaymentState(order, state);
			if (order.IsCCAuthorized == true)
			{
				bool authIsValid = false;
				if (order.IsCCAuthorized == true && order.CCAuthExpirationDate.HasValue)
				{
					authIsValid = order.CCAuthExpirationDate.Value > PXTimeZoneInfo.Now;
				}
				if (authIsValid)
				{
		...
		public void SOOrder_PaymentProfileID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (order != null)
			{
				...
				if (paymentProfileID != null)
				{
					...
					{
						throw new PXSetPropertyException(ErrorMessages.ValueDoesntExistOrNoRights, $"[{nameof(SOOrder.PaymentProfileID)}]", paymentProfileID);
					}
		...
		protected virtual void SOOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			else
			{
				...
				Adjustments.Cache.AllowInsert = PaymentsAndApplicationsEnabled && doc.HasLegacyCCTran != true;
				Adjustments.Cache.AllowDelete = PaymentsAndApplicationsEnabled;
				Adjustments.Cache.AllowUpdate = PaymentsAndApplicationsEnabled && doc.HasLegacyCCTran != true;
			...
			PXUIFieldAttribute.SetVisible<SOOrder.externalTaxExemptionNumber>(cache, doc, !IsTransferOrder);
			...
			PXUIFieldAttribute.SetVisible<SOOrder.curyUnreleasedPaymentAmt>(cache, doc, !IsTransferOrder && !IsCashSale);
			PXUIFieldAttribute.SetVisible<SOOrder.curyCCAuthorizedAmt>(cache, doc, !IsTransferOrder && !IsCashSale);
			PXUIFieldAttribute.SetVisible<SOOrder.curyPaidAmt>(cache, doc, !IsTransferOrder && !IsCashSale);
			PXUIFieldAttribute.SetVisible<SOOrder.curyBilledPaymentTotal>(cache, doc, !IsTransferOrder && !IsCashSale);
			...
			Adjustments.View.AllowSelect = !IsTransferOrder && !IsCashSale;
			...
			PXUIFieldAttribute.SetVisible<SOOrder.extRefNbr>(cache, doc, !IsTransferOrder && IsInvoiceOrder && IsCashSale);
		...
		protected virtual void ApplyFreightTerms(SOOrder row, Carrier carrier, FreightCalculator freightCalculator)
		{
			...
			if (carrier?.IsExternal != true || freightCalculator.IsFlatRate(Document.Cache, row))
			{
				...
				freightCalculator.ApplyFreightTerms<SOOrder, SOOrder.curyFreightAmt>(Document.Cache, row, res.Count);
			}
		}
		...
		protected virtual void CustomerPaymentMethodDetail_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				if (procCenterId != null && CCPluginTypeHelper.CheckProcessingCenterPlugin(this, procCenterId) != CCPluginCheckResult.Ok)
				{
		...
		{
			if (order.Behavior.IsIn(SOBehavior.SO, SOBehavior.RM)
		...
		public class SOQuickProcess : PXGraphExtension<SOOrderEntry>
		{
			...
			[PXButton(CommitChanges = true, VisibleOnDataSource = false), PXUIField(DisplayName = "Quick Process")]
			...
			protected virtual void SOOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
			{
				quickProcess.SetVisible(Base.soordertype.Current.AllowQuickProcess == true);
			}
			...
			protected virtual Tuple<string, int> OrderAvailabilityStatus(int? SiteID, DateTime? ShipDate)
			{
				...
				decimal? availabilityFetch(int? InventoryID, int? SubItemID)
				{
