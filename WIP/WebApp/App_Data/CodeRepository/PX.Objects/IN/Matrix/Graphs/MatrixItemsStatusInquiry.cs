		...
		public class MatrixItemsStatusInquiryImpl : MatrixGridExt<MatrixItemsStatusInquiry, EntryHeader>
		{
			...
			protected override List<PXResult<CSAnswers, InventoryItem>> SelectInventoryWithAttributes()
			{
				...
				Type select = BqlTemplate.OfCommand<
					...
					Where<InventoryItem.templateItemID, Equal<Current<EntryHeader.templateItemID>>,
				...
				{
					return view.SelectMulti().Cast<PXResult<CSAnswers, InventoryItem>>().ToList();
				}
			}

			protected override InventoryMapValue CreateInventoryMapValue(int? inventoryID, PXResult<CSAnswers, InventoryItem> result)
			{
