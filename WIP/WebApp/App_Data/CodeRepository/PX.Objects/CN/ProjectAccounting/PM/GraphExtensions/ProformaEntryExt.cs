...
namespace PX.Objects.CN.ProjectAccounting.PM.GraphExtensions
{
	public class ProformaEntryExt : PXGraphExtension<ProformaEntry>
    {
		...
		[PXUIField(DisplayName = SharedMessages.CorrectProformaInvoice)]
		[PXProcessButton]
		...
		{
			PMProject project = ProjectProperties.Select();
			if (project != null && project.LastProformaNumber == e.Row.ProjectNbr && string.IsNullOrEmpty(project.LastProformaNumber) == false)
			{
	...
	public class ProformaEntryExt_Workflow : PXGraphExtension<ProformaEntry_Workflow, ProformaEntry>
	{
		...
		{
			WorkflowContext<ProformaEntry, PMProforma> context = config.GetScreenConfigurationContext<ProformaEntry, PMProforma>();
