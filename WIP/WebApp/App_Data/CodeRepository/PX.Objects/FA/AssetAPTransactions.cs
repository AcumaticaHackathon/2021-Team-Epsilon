...
namespace PX.Objects.FA
{
	...
	public class AssetGLTransactions : PXGraph<AssetGLTransactions>
	{
		...
		protected virtual void FATran_RowUpdating(PXCache sender, PXRowUpdatingEventArgs e)
		{
			...
			FixedAsset cls = GetFAClass(tran);
			...
			if (tran.NewAsset == true) // new asset (default) or new component
			{
				...
				try
				{
					InsertAsset(
						...
						cls.UnderConstruction == true ? null : tran.DeprFromDate ?? gltran.TranDate,
			...
			if (cls.UnderConstruction == true)
			{
				sender.SetDefaultExt<FATran.deprFromDate>(tran);
			}
		...
		protected virtual void FATran_AssetCD_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (tran.NewAsset == true && (nbr == null || nbr.UserNumbering == true) && string.IsNullOrEmpty(e.NewValue as string))
				throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(FATran.assetCD)}]");
		...
		private FixedAsset GetFAClass(FATran transaction)
		{
			...
			if (transaction.NewAsset == true)
			{
				cls = SelectFrom<FixedAsset>
					.Where<FixedAsset.assetID.IsEqual<@P.AsInt>>.View.Select(this, transaction.ClassID);
			}
			else
			{
				FixedAsset target = SelectFrom<FixedAsset>
					.Where<FixedAsset.assetID.IsEqual<@P.AsInt>>.View.Select(this, transaction.TargetAssetID);
				cls = SelectFrom<FixedAsset>
					.Where<FixedAsset.assetID.IsEqual<FixedAsset.classID.FromCurrent>>.View.SelectSingleBound(this, new object[] { target });
			}
		...
		protected virtual void _(Events.FieldVerifying<FATran.tranDate> e)
		{
			...
			string deprFromDatePeriodID = null;
			if (transaction.DeprFromDate != null)
			{
				deprFromDatePeriodID = FABookPeriodRepository.GetFABookPeriodIDOfDate(transaction.DeprFromDate, transaction.BookID, transaction.AssetID);
			}

			FABook book = PXSelect<FABook, Where<FABook.bookID, Equal<Required<FABook.bookID>>>>.Select(this, transaction.BookID);
			FixedAsset cls = GetFAClass(transaction);
			...
			out int? assetID)
		{
			...
			try
			{
					...
					})))
				{
					...
					{
						location.PeriodID = asset.UnderConstruction == true ? graph.GetService<IFABookPeriodRepository>().GetFABookPeriodIDOfDate(det.ReceiptDate, postingBookID, asset.AssetID, check: false) : bal.DeprFromPeriod;
					}
				}
			}
			catch (PXException ex)
		...
		public override void Persist()
		{
			...
			foreach (FAAccrualTran ext in GLTransactions.Select().RowCast<FAAccrualTran>().Where(ext => ext.Selected == true))
			{
				...
				{
					FADetails det = new FADetails { AssetID = tran.AssetID };
					det = Details.Locate(det);

					FixedAsset asset = PXSelect<FixedAsset, Where<FixedAsset.assetID, Equal<Current<FATran.assetID>>>>
					...
					if (tran.NewAsset == true)
					{
						...
						tran.TranPeriodID = asset.UnderConstruction == true ? this.GetService<IFABookPeriodRepository>().GetFABookPeriodIDOfDate(det.ReceiptDate, glbal.BookID, glbal.AssetID, check: false) : glbal.DeprFromPeriod;
					}
						...
						And<FABookBalance.bookID, NotEqual<Current<FATran.bookID>>>>>.SelectMultiBound(this, new object[]{tran}))
					{
						...
						if (tran.NewAsset == true)
						{
							newtrn.TranPeriodID = asset.UnderConstruction == true ? this.GetService<IFABookPeriodRepository>().GetFABookPeriodIDOfDate(det.ReceiptDate, bal.BookID, bal.AssetID, check: false) : bal.DeprFromPeriod;
						}
						else
						{
			...
			foreach (FATran tran in ptrans)
			{
				...
				{
					reconTran.TranPeriodID = reconTran.FinPeriodID;
				}
				else
				{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach(var loc in locations)
				{
					if (Register.Current == null)
					{
						SetCurrentRegister(Register, (int)loc.BranchID);
					}
				...
				foreach (FATran tran in ptrans)
				{
					...
					if ((bookbal = Balances.Locate(bookbal)) != null)
					{
						FABookHist hist = new FABookHist
						{
							...
							FinPeriodID = bookbal.DeprFromPeriod ?? tran.FinPeriodID,
						};
						...
						{
							bookbal.CurrDeprPeriod = bookbal.DeprFromPeriod ?? tran.FinPeriodID;
	...
	public partial class GLTranFilter : IBqlTable
	{
		...
		[Account(null, typeof(Search<Account.accountID,
				...
				And2<Where<Current<GLSetup.ytdNetIncAccountID>, IsNull,
				Or<Account.accountID, NotEqual<Current<GLSetup.ytdNetIncAccountID>>>>,
				And<Where<Account.curyID, IsNull, Or<Account.curyID, Equal<Current<Company.baseCuryID>>>>>>>>>),
