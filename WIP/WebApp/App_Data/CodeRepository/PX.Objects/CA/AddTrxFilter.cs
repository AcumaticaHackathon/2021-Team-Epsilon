...
namespace PX.Objects.CA
{
	...
	public partial class AddTrxFilter : IBqlTable
	{
		...
		[PXSelector(typeof(Search2<CAEntryType.entryTypeId,
			...
			LeftJoin<ARSetup,
				On<ARSetup.createdDateTime, IsNotNull>,
			LeftJoin<APSetup,
				On<APSetup.createdDateTime, IsNotNull>>>>,
			Where<CashAccountETDetail.accountID, Equal<Current<AddTrxFilter.cashAccountID>>,
				...
				And2<Where<APSetup.migrationMode, NotEqual<True>,
					Or<CAEntryType.module, NotEqual<BatchModule.moduleAP>>>,
				And<Where<ARSetup.migrationMode, NotEqual<True>,
		...
		[PXDBLong]
		[CurrencyInfo(ModuleCode = BatchModule.CA, CuryIDField = "CuryID", SuppressDefaultBaseCury = true)]
		...
		{
			return VerifyAndCreateTransaction(graph, addFilter, currencyinfo, false);
		}

		public static CATran VerifyAndCreateTransaction(PXGraph graph, PXFilter<AddTrxFilter> addFilter, PXSelectBase<CurrencyInfo> currencyinfo, bool releaseTransaction)
		{
			...
			else if (parameters.OrigModule == GL.BatchModule.CA)
			{
				...
				return CreateCATransaction(graph, parameters, currencyinfo, releaseTransaction);
			}
			else
			{
		...
		private static CATran CreateCATransaction(PXGraph graph, AddTrxFilter parameters, PXSelectBase<CurrencyInfo> currencyinfo, bool releaseTransaction)
		{
			...
			if (releaseTransaction == true && adj.Approved == true)
			{
				ReleaseCATransaction(adj);
			}
			return (CATran)PXSelect<CATran, Where<CATran.tranID, Equal<Required<CAAdj.tranID>>>>.Select(graph, adj.TranID);
		}
		...
		private static void ReleaseCATransaction(CAAdj adj)
		{
			...
			CATrxRelease.GroupRelease(registerList, false);
		}
		...
		public class AddFilterAttribute : PXEventSubscriberAttribute, IPXRowSelectedSubscriber
		{
			...
			public virtual void RowSelected(PXCache sender, PXRowSelectedEventArgs e)
			{
				...
				if (row != null)
				{
					...
					bool isCAModuleOrNull = row.OrigModule == null || row.OrigModule == GL.BatchModule.CA;
					PXUIFieldAttribute.SetVisible<AddTrxFilter.accountID>(sender, row, isCAModuleOrNull);
					PXUIFieldAttribute.SetVisible<AddTrxFilter.subID>(sender, row, isCAModuleOrNull);
					...
					PXUIFieldAttribute.SetVisible<AddTrxFilter.hold>(sender, row, isCAModuleOrNull);
					PXUIFieldAttribute.SetVisible<AddTrxFilter.status>(sender, row, isCAModuleOrNull);
					PXUIFieldAttribute.SetVisible<AddTrxFilter.taxZoneID>(sender, row, isCAModuleOrNull);
