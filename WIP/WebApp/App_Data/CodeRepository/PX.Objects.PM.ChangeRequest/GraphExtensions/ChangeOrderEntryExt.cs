		...
		internal virtual void AddChangeRequest(PMChangeRequest cr)
		{
			...
			foreach(PMChangeRequestLine line in select.Select(cr.RefNbr))
			{
				if (string.IsNullOrEmpty(cr.CostChangeOrderNbr))
				{
					...
					if (cost != null)
					{
						Base.IncreaseDraftBucket(cost, -1);
				...
				if (Base.ChangeOrderClass.Current.IsRevenueBudgetEnabled == true)
				{
					...
					if (revenue != null)
					{
						Base.IncreaseDraftBucket(revenue, -1);
			...
			if (Base.ChangeOrderClass.Current.IsRevenueBudgetEnabled == true)
			{
				...
				RaiseChangeRequestEvent(cr, PMChangeRequest.Events.Select(ev => ev.Close));
			}
			...
			Base.RemoveObsoleteLines();
		}

		protected virtual void RaiseChangeRequestEvent(PMChangeRequest changeRequest, PX.Data.WorkflowAPI.SelectedEntityEvent<PMChangeRequest> crEvent)
		{
			crEvent.FireOn<PMChangeRequest>(Base, changeRequest);
		}
		...
		protected virtual void _(Events.RowDeleted<PMChangeRequest> e)
		{
			...
			foreach (PMChangeRequestLine line in select.Select(e.Row.RefNbr))
			{
				if (e.Row.CostChangeOrderNbr == Base.Document.Current.RefNbr)
				{
					...
					if (cost != null)
					{
						Base.IncreaseDraftBucket(costFromLine, 1);
				...
				if (Base.ChangeOrderClass.Current.IsRevenueBudgetEnabled == true)
				{
					...
					if (revenue != null)
					{
						Base.IncreaseDraftBucket(revenueFromLine, 1);
			...
			if (Base.ChangeOrderClass.Current.IsRevenueBudgetEnabled == true)
			{
				...
				RaiseChangeRequestEvent(e.Row, PMChangeRequest.Events.Select(ev => ev.Open));
			}
