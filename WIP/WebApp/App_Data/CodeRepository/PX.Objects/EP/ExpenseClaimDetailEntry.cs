	...
	public class ExpenseClaimDetailEntry : PXGraph<ExpenseClaimDetailEntry, EPExpenseClaimDetails>
	{
		...
		public PXSelectJoin<EPExpenseClaimDetails,
						...
						Where<EPEmployee.defContactID, Equal<Current<AccessInfo.contactID>>,
							...
							Or<EPEmployee.defContactID, TM.IsSubordinateOfContact<Current<AccessInfo.contactID>>,
		...
		public PXAutoAction<EPExpenseClaimDetails> initializeState;
        ...
        protected virtual void EPExpenseClaimDetails_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				Submit.SetEnabled(cache.GetStatus(row) != PXEntryStatus.Inserted && row.Hold == true);
				...
				if (employeeRow != null)
				{
					if (!allowEdit && this.Accessinfo.UserID == employeeRow.UserID)
					{
						allowEdit = true;
					}
		...
		protected virtual void EPExpenseClaimDetails_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if (row != null && e.Operation != PXDBOperation.Delete)
			{
				if (row.Hold == false)
				{
					...
					CurrencyInfo info = CurrencyInfoAttribute.GetCurrencyInfo<EPExpenseClaimDetails.curyInfoID>(cache, row);
					if (CurrencyCollection.GetCurrency(info?.BaseCuryID).RoundingLimit < Math.Abs(row.TaxRoundDiff ?? 0m))
					{
						throw new PXException(AP.Messages.RoundingAmountTooBig, currencyinfo?.Current?.BaseCuryID, row.TaxRoundDiff,
							PXDBQuantityAttribute.Round(CurrencyCollection.GetCurrency(info.BaseCuryID).RoundingLimit));
					}
				}
            ...
            if (e.ExternalCall && !this.IsMobile)
			{
				if (row.TaxZoneID != oldRow.TaxZoneID && !string.IsNullOrEmpty(row.TaxZoneID))
				{
					...
					if (row.TaxZoneID != taxZoneID)
					{
						...
						ReceiptEntryExt.AmtFieldUpdated(oldRow, row);
		...
		protected virtual void _(Events.CacheAttached<EPExpenseClaim.Tstamp> e)
		{ }
