    ...
    public class RMReportReaderPM : PXGraphExtension<RMReportReaderGL, RMReportReader>
    {
            ...
            if (rmType == ARmReport.PM)
            {
                ...
                else if (ds.Expand == ExpandType.ProjectTask)
                {
                    var tasksByCD = tasks
                        ...
                        .GroupBy(t => t.TaskCD)
                        .ToArray();
                    if (tasksByCD.Length > 0)
                    {
                        var maxTasksInGroup = tasksByCD.Max(g => g.Count());
                        var resultTasks = new PMTask[tasksByCD.Length * maxTasksInGroup];
                        tasksByCD.ForEach((g, gi) =>{
                            ...
                            g.ForEach((t, ti) => resultTasks[ti*tasksByCD.Length + gi] = t);
                        });
        ...
        public object CalculateAndExpandValue(bool drilldown, RMDataSource ds, RMDataSourceGL dsGL, RMDataSourcePM dsPM, ARmDataSet dataSet, List<PMAccountGroup> accountGroups, List<PMProject> projects, List<PMTask> tasks, List<InventoryItem> items, List<PMCostCode> costCodes, List<object[]> splitret)
        {
            ...
            Parallel.For(0, accountGroups.Count, options, accountGroup =>
            {
                ...
                if (!_pmhistoryPeriodsNested.TryGetValue(currentAccountGroup.GroupID.Value, out var accountDict)) return;
                ...
                Parallel.For(0, tasks.Count, options, task =>
                {
                    ...
                    if (!accountDict.TryGetValue(currentTask.TaskCD, out var taskDict)) return;
                    ...
                    Parallel.For(0, costCodes.Count, options, costCode =>
                    {
                        ...
                        if (!taskDict.TryGetValue(currentCostCode.CostCodeID.Value, out var costDict)) return;
                        ...
                        Parallel.For(0, items.Count, options, item =>
                        {
                            ...
                            List<PMHistory> periods = null;
                            if (costDict.TryGetValue((currentProject.ContractID.Value, currentItem.InventoryID.Value), out var periodsForKey))
                                periods = GetPeriodsToCalculate(periodsForKey, ds, dsGL, drilldown);
                            if (periods == null)
                            {
                                if (IsBudgetValue(ds))
                                {
		...
		public List<PMHistory> GetPeriodsToCalculate(Dictionary<string, PMHistory> periodsForKey, RMDataSource ds, RMDataSourceGL dsGL, bool allowStartOfProject)
        {
