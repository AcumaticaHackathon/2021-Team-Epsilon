        ...
        where TRecordType : IConstant<string>, IBqlOperand, new()
    {
        ...
        public void GenerateAPPSOUpdateContracts(List<Schedule> scheduleRules, string recordType, DateTime? fromDate, DateTime? toDate, FSSchedule fsScheduleRow)
        {
            ...
            using (PXTransactionScope ts = new PXTransactionScope())
            {
                ...
                if (customerRow.Status != CustomerStatus.Active && customerRow.Status != CustomerStatus.OneTime)
                {
                    throw new PXException(PXMessages.LocalizeFormat(AR.Messages.CustomerIsInStatus, new CustomerStatus.ListAttribute().ValueLabelDic[customerRow.Status]));
                }
                                         ...
                                         bool isPrepaidContract)
        {
            ...
            fsAppointmentRow.GeneratedByContract = true;

            fsAppointmentRow = graphAppointmentEntry.AppointmentRecords.Current = graphAppointmentEntry.AppointmentRecords.Insert(fsAppointmentRow);
            fsAppointmentRow = (FSAppointment)graphAppointmentEntry.AppointmentRecords.Cache.CreateCopy(fsAppointmentRow);

            fsAppointmentRow.SORefNbr = fsServiceOrderRow.RefNbr;

            fsAppointmentRow = (FSAppointment)graphAppointmentEntry.AppointmentRecords.Cache.CreateCopy(graphAppointmentEntry.AppointmentRecords.Update(fsAppointmentRow));
            ...
            {
                fsAppointmentRow.BillServiceContractID = fsServiceOrderRow.ServiceContractID;
            }
            ...
            {
                fsAppointmentRow.ScheduledDateTimeBegin = timeSlotAppointment.DateTimeBegin;
                fsAppointmentRow.ScheduledDateTimeEnd = timeSlotAppointment.DateTimeEnd;
            }
            else
            {
                ...
                fsAppointmentRow.ScheduledDateTimeBegin = scheduledDateTimeBegin;
            }

            fsAppointmentRow.HandleManuallyScheduleTime = fsScheduleRow.OverrideDuration;

            if (fsScheduleRow.OverrideDuration == true)
            {
                DateTime? endDate = fsAppointmentRow.ScheduledDateTimeBegin.Value.AddMinutes(Convert.ToDouble(fsScheduleRow.ScheduleDuration));
                fsAppointmentRow.ScheduledDateTimeEnd = endDate;
            }
        ...
        protected void CreateServiceOrder(TimeSlot timeSlotServiceOrder, bool createAppointmentFlag = false, bool appointmentsBelongToRoute = false)
        {
            ...
            fsServiceOrderRow = graphServiceOrderEntry.ServiceOrderRecords.Current = graphServiceOrderEntry.ServiceOrderRecords.Insert(fsServiceOrderRow);
            fsServiceOrderRow = (FSServiceOrder)graphServiceOrderEntry.ServiceOrderRecords.Cache.CreateCopy(fsServiceOrderRow);
            ...
            fsServiceOrderRow = (FSServiceOrder)graphServiceOrderEntry.ServiceOrderRecords.Cache.CreateCopy(graphServiceOrderEntry.ServiceOrderRecords.Update(fsServiceOrderRow));
            ...
            fsServiceOrderRow.ContactID = fsServiceContractRow.CustomerContactID;
            ...
            // We need this Update to run the FieldUpdated of BillCustomerID and assign the BillingBy
            fsServiceOrderRow = (FSServiceOrder)graphServiceOrderEntry.ServiceOrderRecords.Cache.CreateCopy(graphServiceOrderEntry.ServiceOrderRecords.Update(fsServiceOrderRow));

            if (isPrepaidContract == true 
                    && fsServiceOrderRow.BillingBy == ID.Billing_By.SERVICE_ORDER)
            {
            ...
            foreach (FSScheduleDet fsScheduleDetRow in fsScheduleDetSet)
            {
                ...
                else
                {
                    ...
                    fsSODetRow.EstimatedDuration = fsScheduleDetRow.EstimatedDuration;

                    //TODO: AC-169439
                    //This if needs to be removed after doing this issue

                    if (fsScheduleDetRow.LineType == ID.LineType_ALL.INSTRUCTION
                            || fsScheduleDetRow.LineType == ID.LineType_ALL.COMMENT)
                    {
                        fsSODetRow.EstimatedQty = 0m;
                    }
                    else
                    {
            ...
            if (createAppointmentFlag)
            {
                ...
                bool? origOpenDoc = fsServiceOrderRow.OpenDoc;
                ...
                if (origOpenDoc != true)
                {
                    ...
                    PXUpdate<
                        Set<FSServiceOrder.openDoc, True>,
                    FSServiceOrder,
                    Where<
                        FSServiceOrder.sOID, Equal<Required<FSServiceOrder.sOID>>>>
                    .Update(this, fsServiceOrderRow.SOID);
                }

                CreateAppointment(fsServiceOrderRow, timeSlotServiceOrder, fsScheduleRow, true, appointmentsBelongToRoute, isPrepaidContract);

                if (origOpenDoc != true)
                {
                    PXUpdate<
                        Set<FSServiceOrder.openDoc, Required<FSServiceOrder.openDoc>>,
                    FSServiceOrder,
                    Where<
                        FSServiceOrder.sOID, Equal<Required<FSServiceOrder.sOID>>>>
                    .Update(this, origOpenDoc, fsServiceOrderRow.SOID);
                }
            }
        }
