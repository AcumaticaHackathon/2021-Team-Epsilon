...
namespace PX.Objects.AM
{
    ...
    public class MoveOperationQtyBuilder
    {
        ...
        protected Dictionary<string, decimal> OperationScrapQty;
        ...
        private static MoveOperationQtyBuilder Construct(PXGraph graph, PXResultset<AMMTran> allTranResultSet, bool postOperUpdate)
        {
            ...
            var allOperations = PXSelectJoin<AMProdOper,
                InnerJoin<AMWC, On<AMProdOper.wcID, Equal<AMWC.wcID>>,
                    InnerJoin<AMMTranProductionOrder,
                        On<AMProdOper.orderType, Equal<AMMTranProductionOrder.orderType>,
                            And<AMProdOper.prodOrdID, Equal<AMMTranProductionOrder.prodOrdID>>>>>,
                Where<AMMTranProductionOrder.docType, Equal<Required<AMMTranProductionOrder.docType>>,
                    And<AMMTranProductionOrder.batNbr, Equal<Required<AMMTranProductionOrder.batNbr>>>>
            ...
            List<PXResult<AMProdOper, AMWC>> allOperations, bool postOperUpdate)
        {
            ...
            try
            {
                ...
                OperationScrapQty = new Dictionary<string, decimal>();
        ...
        protected decimal GetOperationScrapQty(AMProdOper prodOper)
        {
            var key = prodOper.JoinKeys();
            if (OperationScrapQty.ContainsKey(key))
            {
                return OperationScrapQty[key];
            }
            OperationScrapQty[key] = 0m;
            return 0m;
        }

        protected decimal UpdateOperationScrapQty(AMProdOper prodOper, decimal qtyScrap)
        {
            var qtyScrapTotal = GetOperationScrapQty(prodOper) + qtyScrap;
            OperationScrapQty[prodOper.JoinKeys()] = qtyScrapTotal;
            return qtyScrapTotal;
        }
        ...
        private void ProcessAllTranOperations(bool postOperUpdate)
        {
            foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in AllTranRows)
            {
                ...
                if (ammTran?.LineNbr == null || tranProdOper?.OperationID == null || amProdItem == null)
                {
                    continue;
                }
                ...
                {
                    //skip updating the operation labor cost if this is after the oper update and the cost method is estimated
                    ProcessPositiveMove(ammTran, tranProdOper, (postOperUpdate && amProdItem.CostMethod == CostMethod.Estimated));
                    continue;
                }
        ...
        private void ProcessPositiveMove(AMMTran ammTran, AMProdOper tranProdOper, bool postOperupdate)
        {
            ...
            foreach (PXResult<AMProdOper, AMWC> operResult in operationResults)
            {
                ...
                var tranScrapQty = sameOper ? ammTran.BaseQtyScrapped.GetValueOrDefault() : 0m;
                var tranMoveQty = sameOper && !ammTran.IsScrap.GetValueOrDefault() ? ammTran.BaseQty.GetValueOrDefault() : 0m;
                ...
                if(tranScrapQty != 0)
                {
                    UpdateOperationScrapQty(operation, tranScrapQty);
                }
                ...
                UpdateOper(operation, ammTran, qtyComplete, tranScrapQty, postOperupdate);
            }
        ...
        private void ProcessNegativeMove(AMMTran ammTran, AMProdOper tranProdOper, bool postOperUpdate)
        {
            ...
            foreach (PXResult<AMProdOper, AMWC> operResult in operationResults)
            {
                ...
                var updatedOper = UpdateOper(operation, ammTran, qtyComplete, tranScrapQty, postOperUpdate);
                lastOperQtyComplete = updatedOper?.BaseQtyComplete ?? 0m;
            }
            UpdateMoveTransactionTotals(ammTran, opTotals);
        }

        private AMProdOper UpdateOper(AMProdOper operation, AMMTran ammTran, decimal qtyComplete, decimal tranScrapQty, bool postOperupdate)
        {
            ...
            var laborCost = 0m;
            var laborTime = 0;
            if (ammTran.OrderType == operation.OrderType && ammTran.ProdOrdID == operation.ProdOrdID && ammTran.OperationID == operation.OperationID && !postOperupdate)
            {
        ...
        private void PostProcessAllTranRows()
        {
            foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in AllTranRows)
            {
                ...
                foreach (PXResult<AMProdOper, AMWC> operResult in operationResults)
                {
                    ...
                    var totalScrapQty = GetOperationScrapQty(operation);
                    ...
                    if (totalScrapQty != opTotal.TransactionTotalScrapBaseQty)
                    {
                        // Total all qty from direct/indirect numbers
                        opTotal.TransactionTotalScrapBaseQty = totalScrapQty;
                        update = true;
                    }
