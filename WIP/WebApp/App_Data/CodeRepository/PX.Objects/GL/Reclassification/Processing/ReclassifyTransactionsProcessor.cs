	...
	public class ReclassifyTransactionsProcessor : ReclassifyTransactionsBase<ReclassifyTransactionsProcessor>
	{
			...
			Batch batchForEditing = null)
		{
			...
			if (batchForEditing == null)
			{
				...
				Branch newBranch = SelectFrom<Branch>
					.Where<Branch.branchID.IsEqual<@P.AsInt>>
					.View
					.SelectSingleBound(this, null, representativeTranForReclass.NewBranchID);

				batch.BatchType = BatchTypeCode.Reclassification;
			    batch.BranchID = representativeTranForReclass.NewBranchID;
				FinPeriodIDAttribute.SetPeriodsByMaster<Batch.finPeriodID>(
					JournalEntryInstance.BatchModule.Cache, 
					batch,
			        representativeTranForReclass.TranPeriodID);
				batch.LedgerID = newBranch.LedgerID;
			...
			JournalEntryInstance.BatchModule.Cache.SetDefaultExt<Batch.hold>(batch);
