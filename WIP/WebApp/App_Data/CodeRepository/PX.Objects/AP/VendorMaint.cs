...
namespace PX.Objects.AP
{
	...
	public class VendorMaint : PXGraph<VendorMaint, VendorR>
	{
		...
		public VendorMaint()
		{
			...
			PXUIFieldAttribute.SetDisplayName<VendorR.acctName>(BAccount.Cache, CR.Messages.VendorAccountName);
		}
		...
		public virtual IEnumerable NewBillAdjustment(PXAdapter adapter)
		{
			...
			if (vendor != null && vendor.BAccountID > 0L)
			{
				...
				invEntry.Document.Cache.SetDefaultExt<APInvoice.finPeriodID>(newDoc);
				invEntry.Document.Cache.SetDefaultExt<APInvoice.tranPeriodID>(newDoc);
		...
		public virtual IEnumerable NewManualCheck(PXAdapter adapter)
		{
			...
			if (vendor != null && vendor.BAccountID > 0L)
			{
				...
				newDoc.BranchID = null;
				payEntry.Document.Cache.SetValueExt<APPayment.vendorID>(newDoc, vendor.BAccountID);
				payEntry.Document.Cache.SetDefaultExt<APPayment.adjFinPeriodID>(newDoc);
				payEntry.Document.Cache.SetDefaultExt<APPayment.adjTranPeriodID>(newDoc);
				payEntry.Document.Cache.SetDefaultExt<APPayment.finPeriodID>(newDoc);
				payEntry.Document.Cache.SetDefaultExt<APPayment.tranPeriodID>(newDoc);
		...
		public PXAction<VendorR> Activate;
		...
		protected virtual IEnumerable activate(PXAdapter adapter) => adapter.Get<VendorR>();

		public PXAction<VendorR> PutOnHold;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Hold", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable putOnHold(PXAdapter adapter) => adapter.Get<VendorR>();

		public PXAction<VendorR> HoldPayments;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Hold Payments", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable holdPayments(PXAdapter adapter) => adapter.Get<VendorR>();

		public PXAction<VendorR> Deactivate;
		...
		protected virtual IEnumerable deactivate(PXAdapter adapter) => adapter.Get<VendorR>();

		public PXAction<VendorR> SetAsOneTime;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Set as One-Time Vendor", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable setAsOneTime(PXAdapter adapter) => adapter.Get<VendorR>();

		public PXAction<VendorR> SetAsRegular;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Set as Regular Vendor", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable setAsRegular(PXAdapter adapter) => adapter.Get<VendorR>();
		...
		protected virtual void Vendor_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			DefLocationExt defLocationExt = GetExtension<DefLocationExt>();
			if (row.Vendor1099 == true && defLocationExt.DefLocation.Current != null)
			{
				if (row.TinType == null && defLocationExt.DefLocation.Current.TaxRegistrationID != null)
				{
					cache.DisplayFieldWarning<Vendor.tinType>(row, row.TinType, Messages.TinTypeCannotBeEmpty);
				}
				else if (row.TinType != null && defLocationExt.DefLocation.Current.TaxRegistrationID == null)
				{
					cache.DisplayFieldWarning<Vendor.tinType>(row, row.TinType, Messages.TinTypeMustBeEmpty);
				}
				else
				{
					cache.ClearFieldErrors<Vendor.tinType>(row);
				}
			}
		}
		...
		protected virtual void Vendor_VendorClassID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (VendorClass.Current != null && doCopyClassSettings)
			{
				...
				if (defLocationExt.DefLocation.Current != null)
					defLocationExt.DefLocation.Cache.SetDefaultExt<CRLocation.vTaxZoneID>(defLocationExt.DefLocation.Current);
		...
		public virtual void CopyAccounts(PXCache sender, Vendor row)
		{
			...
			sender.SetDefaultExt<Vendor.vOrgBAccountID>(row);
			...
			.WithCombinedTypeValidation
		{
			...
			{
				if (Base.CurrentVendor.Cache.GetStatus(Base.CurrentVendor.Current) == PXEntryStatus.Inserted)
				{
			...
			.WithCombinedTypeValidation
		{
			...
			public PXSelect<LocationBranchSettings,
				Where<LocationBranchSettings.bAccountID, Equal<Current<CRLocation.bAccountID>>,
					And<LocationBranchSettings.locationID, Equal<Current<CRLocation.locationID>>,
					And<LocationBranchSettings.branchID, Equal<Current<AccessInfo.branchID>>>>>>
				DefLocationBranchSettings;
			...
			[CashAccount(typeof(Search2<
				...
				Where2<Match<Current<AccessInfo.userName>>,
					...
					And<Where2<Not<FeatureInstalled<FeaturesSet.multipleBaseCurrencies>>, Or<CashAccount.baseCuryID, Equal<Current<Vendor.baseCuryID>>>>>>>>>>),
			...
			[PXFormula(typeof(Switch<
					Case<Where<CurrentValue<AP.VendorR.defAddressID>, IsNull>, Null,
					Case<Where<CRLocation.vRemitAddressID, Equal<CurrentValue<AP.VendorR.defAddressID>>>, False>>,
				True>))]
			...
			[PXFormula(typeof(Switch<
					Case<Where<CurrentValue<AP.VendorR.defAddressID>, IsNull>, Null,
					Case<Where<CRLocation.vRemitContactID, Equal<CurrentValue<AP.VendorR.defContactID>>>, False>>,
				True>))]
			[PXMergeAttributes(Method = MergeMethod.Append)] // for optimized export
			...
			[PXDBDefault(typeof(CRLocation.bAccountID))]
			protected virtual void _(Events.CacheAttached<LocationBranchSettings.bAccountID> e) { }

			[PXMergeAttributes(Method = MergeMethod.Merge)]
			[PXDBDefault(typeof(CRLocation.locationID))]
			protected virtual void _(Events.CacheAttached<LocationBranchSettings.locationID> e) { }
			...
			protected override void _(Events.RowSelected<CRLocation> e)
			{
				...
				if (e.Row != null)
				{
					...
					PXUIFieldAttribute.SetVisible<CRLocation.vSiteID>(e.Cache, e.Row, Base.BAccountAccessor.Current?.IsBranch != true);
					PXUIFieldAttribute.SetVisible<LocationBranchSettings.vSiteID>(DefLocationBranchSettings.Cache, null, Base.BAccountAccessor.Current?.IsBranch == true);
