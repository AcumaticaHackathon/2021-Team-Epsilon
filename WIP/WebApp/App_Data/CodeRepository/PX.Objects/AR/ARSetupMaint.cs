...
namespace PX.Objects.AR
{
    public class ARSetupMaint : PXGraph<ARSetupMaint>
    {
        ...
        public PXSelect<ARDunningSetup,
            Where<ARDunningSetup.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>> DunningSetup;   //MMK Dunnuing Letter parameters Setup 
        ...
        protected virtual void ARDunningSetup_DueDays_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if (row != null)
            {
                ...
                else
                {
                    int NextValue = 0;
                    int PrevValue = 0;
                    foreach (PXResult<ARDunningSetup> ii in PXSelect<ARDunningSetup, Where<ARDunningSetup.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>.Select(this))
                    {
                        ARDunningSetup v = ii;
        ...
        protected virtual void ARDunningSetup_DueDays_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
        {
            ...
            if (row?.DunningLetterLevel != null)
            {
                ...
                {
                    int PrevValue = 0;
                    foreach (PXResult<ARDunningSetup> ii in PXSelect<ARDunningSetup,
                        Where<ARDunningSetup.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>.Select(this))
                    {
                        ARDunningSetup v = ii;
                        if (v.DunningLetterLevel.Value == llevel - 1)
                        {
        ...
        {
            ARDunningSetup row = e.Row as ARDunningSetup;
            var items = PXSelect<ARDunningSetup, Where<ARDunningSetup.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>>>.Select(this).RowCast<ARDunningSetup>().ToList();
        ...
        protected virtual void ARDunningSetup_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            {
                int llevel = row.DunningLetterLevel.Value;
                ARDunningSetup nextDL = PXSelect<ARDunningSetup, Where< ARDunningSetup.baseCuryID, Equal<Current<AccessInfo.baseCuryID>>, And<ARDunningSetup.dunningLetterLevel, Greater<Required<ARDunningSetup.dunningLetterLevel>>>>>.Select(this, row.DunningLetterLevel);
                bool clear = true;
