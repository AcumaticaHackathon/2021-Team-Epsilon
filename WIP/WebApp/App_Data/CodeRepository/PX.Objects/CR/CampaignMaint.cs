	...
	public class CampaignMaint : PXGraph<CampaignMaint, CRCampaign>, PXImportAttribute.IPXPrepareItems
	{
		...
		protected virtual void CRCampaign_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				{
					if (cache.RaiseExceptionHandling<CRCampaign.startDate>(e.Row, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(CRCampaign.startDate)}]")))
					{
				...
				{
					if (cache.RaiseExceptionHandling<CRCampaign.projectTaskID>(e.Row, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(CRCampaign.projectTaskID)}]")))
					{
		...
		public virtual bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
		{
			...
			{
				if (values.Contains("ContactID") && values["ContactID"] != null)
				{
