...
namespace PX.Objects.AM.GraphExtensions
{
    ...
    public class POCreateAMExtension : PXGraphExtension<POCreate>
    {
        ...
        {
            foreach (PXResult<POFixedDemand, Vendor, POVendorInventory> rec in fixedDemands)
            {
                var demand = (POFixedDemand)rec;
                var vendor = (Vendor)rec;
                var price = (POVendorInventory)rec;
                Base.EnumerateAndPrepareFixedDemandRow(demand, vendor, price);
    ...
    public class FSPOCreateAMExtension : PXGraphExtension<POCreateAMExtension, SM_POCreate, POCreate>
    {
        ...
        protected virtual IEnumerable fixedDemand()
        {
            ...
            PXView select = new PXView(Base, false, query);

            Int32 totalrow = 0;
            Int32 startrow = PXView.StartRow;
            List<object> result = select.Select(PXView.Currents, parameters, PXView.Searches,
                PXView.SortColumns, PXView.Descendings, PXView.Filters, ref startrow, PXView.MaximumRows, ref totalrow);
            PXView.StartRow = 0;
            ...
            foreach (PXResult<POFixedDemand, Vendor, POVendorInventory, CRLocation, SOOrder, SOLine, DropShipLink, AMProdMatlSplitPlan, FSServiceOrder, FSSODetFSSODetSplit> rec in result)
            {
                var demand = (POFixedDemand)rec;
                var vendor = (Vendor)rec;
                var price = (POVendorInventory)rec;
