...
namespace PX.Objects.EP
{
	...
	public class EPCustomerBilling : PXGraph<EPCustomerBilling>
	{
		...
		public PXSelectJoinGroupBy<
			...
			InnerJoin<Customer,
				On<EPExpenseClaimDetails.customerID, Equal<Customer.bAccountID>>,
			LeftJoin<Contract,
				On<EPExpenseClaimDetails.contractID, Equal<Contract.contractID>,
			...
			Aggregate<
				GroupBy<EPExpenseClaimDetails.customerID,
				GroupBy<EPExpenseClaimDetails.customerLocationID>>>> CustomersView;
		...
		protected virtual IEnumerable customers()
		{
			...
			{
				CustomersView.WhereAnd<Where<Customer.customerClassID, Equal<Current<BillingFilter.customerClassID>>>>();
			}
			if (filter.CustomerID != null)
			{
				CustomersView.WhereAnd<Where<Customer.bAccountID, Equal<Current<BillingFilter.customerID>>>>();
			}

			foreach (PXResult<EPExpenseClaimDetails, Customer, Contract> res in CustomersView.Select())
			{
		...
		protected virtual void InsertARTran(ARInvoiceEntry arGraph, EPExpenseClaimDetails row, decimal signOperation, decimal tipQty=1m, bool isTipTransaction = false)
		{
			...
			if (isTipTransaction)
			{
				...
				else
				{
					...
					int? project_SubID = (int?)arGraph.Caches[typeof(Contract)].GetValue<Contract.defaultSalesSubID>(contract);
					int? task_SubID = (int?)arGraph.Caches[typeof(PMTask)].GetValue<PMTask.defaultSalesSubID>(task);
					...
					object value = SubAccountMaskAttribute.MakeSub<EPSetup.salesSubMask>(arGraph, epsetup.SalesSubMask,
						...
						new Type[] { typeof(EPEmployee.salesSubID), typeof(IN.InventoryItem.salesSubID), typeof(Location.cSalesSubID), typeof(Contract.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(Location.cSalesSubID) });
