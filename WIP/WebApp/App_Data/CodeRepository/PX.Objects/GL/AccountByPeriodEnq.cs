...
namespace PX.Objects.GL
{
	...
	public class AccountByPeriodEnq : PXGraph<AccountByPeriodEnq>
	{
		...
		protected virtual IEnumerable glTranEnq()
		{
			...
			int?[] branchIDs = _currentUserInformationProvider.GetAllBranches().Select(b => (int?) b.Id).Distinct().ToArray();
			...
			return records.Select(PXCache<GLTranR>.CreateCopy);
		}
		...
		protected virtual IEnumerable filter()
		{
			...
			if (filter.AccountID != null && filter.LedgerID != null && filter.StartPeriodID != null && filter.EndPeriodID != null)
			{
				...
				if (account != null)
				{
					if (filter.BranchID != null && filter.OrgBAccountID == null)
					{
						filter.OrgBAccountID = PXAccess.GetBranch(filter.BranchID)?.BAccountID;
					}
	...
	public class PMDocGraphCreator : IDocGraphCreator
	{
		...
		public virtual PXGraph Create(GLTran tran)
		{
			if (tran.PMTranID != null)
			{
				...
				PMTran pmTran = PMTran.PK.Find(destGraph, tran.PMTranID);
