...
namespace PX.Objects.AM
{
    ...
    public class ProductionCostCalculator
    {
        ...
        private ProductionOperationCostResults GetActualUnitCostByOperations(AMMTran ammTran, AMProdItem amProdItem, InventoryItem inventoryItem, MoveOperationQtyTotals moveOperationQtyTotals)
        {
            ...
            var transactionBaseQty = 0m;
            if (ammTran.LastOper.GetValueOrDefault())
            {
                var lastOper = moveOperationQtyTotals.GetLastOperation(amProdItem);
                transactionBaseQty = lastOper?.TransactionTotalMoveBaseQty ?? ammTran.BaseQty.GetValueOrDefault() + ammTran.BaseQtyScrapped.GetValueOrDefault();
                //The qty used for unit cost calculation should be the total quantity - qty scrapped, unless it is write-off at the last operation
                if (IncludeScrap && ammTran.ScrapAction == 0)
                {
                    transactionBaseQty -= ammTran.BaseQtyScrapped.GetValueOrDefault();
                }
            }
        ...
        protected virtual PlannedCosts GetActualCostByOperation(AMMTran ammTran, InventoryItem inventoryItem, AMProdOper amProdOper, decimal transactionBaseQty, decimal additionalWip)
        {
            ...
            var wipBalance = amProdOper.WIPBalance.GetValueOrDefault() + additionalWip - amProdOper.ScrapAmount.GetValueOrDefault();
        ...
        private ProductionOperationCostResults GetEstimatedUnitCostByOperation(AMMTran ammTran, InventoryItem inventoryItem, AMProdItem amProdItem, MoveOperationQtyTotals moveOperationQtyTotals)
        {
            ...
            //if scrap is included in completions, we want to subtract out the qty scrapped so that the good units absorb the scrap cost
            if (IncludeScrap && ammTran.ScrapAction == ScrapAction.NoAction)
            {
                // Only for no action scrap transaction where scrap cost is not moved out of wip
                unitCostQty -= lastOperTotal?.TransactionTotalScrapBaseQty ?? 0m;
            }
            ...
            decimal additionalWip, decimal scrapQty, out decimal operExpectedCost)
        {
            ...
            var remainingOperCost = operExpectedCost - amProdOper.WIPComp.GetValueOrDefault() + additionalWip - amProdOper.ScrapAmount.GetValueOrDefault();
            var operUnitcost = 0m;
            if (unitCostQty != 0)
            {
        ...
        {
            return GetPlannedLaborCost(graph, amProdOper, productionQty, remainingProductionQty, totalHours, amProdOper?.ActualLaborTime, amProdOper?.ActualLabor);
        }

        /// <summary>
        /// Get the planned labor unit and total cost for the given operation
        /// </summary>
        public static PlannedCosts GetPlannedLaborCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty, decimal? totalHours, int? laborTime, decimal? laborCost)
        {
            ...
            return GetPlannedLaborCost(wc, shiftMst, amProdOper, productionQty, remainingProductionQty, totalHours, UomHelper.CostDecimalPrecision, laborTime, laborCost);
        }
        ...
        {
            return GetPlannedLaborCost(wc, shiftMst, amProdOper, productionQty, remainingProductionQty, totalHours, decimalPrecision, amProdOper?.ActualLaborTime, amProdOper?.ActualLabor);
        }

        /// <summary>
        /// Get the planned labor unit and total cost for the given operation
        /// </summary>
        public static PlannedCosts GetPlannedLaborCost(AMWC wc, AMShiftMst shiftMst, AMProdOper amProdOper, decimal? productionQty, decimal? remainingProductionQty, decimal? totalHours, int decimalPrecision, int? laborTime, decimal? laborCost)
        {
            ...
            var actualLaborHours = laborTime.GetValueOrDefault().ToHours(9);
            ...
            var laborTotalCost = Math.Round(laborCost.GetValueOrDefault() + (planLaborRate * hoursRemaining), decimalPrecision);
            var laborUnitCost = productionQty == 0 ? laborTotalCost : (laborTotalCost / productionQty);
#if DEBUG
            if(productionQty.GetValueOrDefault() != 0)
            {
                AMDebug.TraceWriteMethodName($"[{amProdOper.OperationCD}] Total Plan Hours {totalHours}; Act Time {laborTime.GetValueOrDefault()} (Hours {actualLaborHours}); Remaining Hours {hoursRemaining}");
                AMDebug.TraceWriteMethodName($"[{amProdOper.OperationCD}] Total Labor Cost {laborTotalCost}; Act Labor Cost {laborCost.GetValueOrDefault()}; Remaining Labor Cost {planLaborRate * hoursRemaining}; Labor Unit Cost {laborUnitCost}; Labor Rate {planLaborRate}");
            }
#endif
        ...
        public ProductionOperationCostResults CalculateReturnUnitCost(AMMTran ammTran, AMProdItem amProdItem, MoveOperationQtyTotals moveOperationQtyTotals, decimal? previouslyReturnedBaseQty)
        {
            ...
            switch (inventoryItem.ValMethod)
            {
                case INValMethod.Standard:
                    ...
                    return operCostResults;
                //Removing this to use actual return cost instead of current lot cost
