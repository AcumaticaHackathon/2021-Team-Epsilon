	...
	public class ARCustomerCreditHoldProcess : PXGraph<ARCustomerCreditHoldProcess>
	{
		...
		public partial class CreditHoldParameters : IBqlTable
		{
			...
			public string IncludedCustomerStatus
			{
				get
				{
					if (Action?.EndsWith(nameof(CustomerMaint.PutOnCreditHold)) == true)
						return CustomerStatus.Active;
					else if (Action?.EndsWith(nameof(CustomerMaint.RemoveCreditHold)) == true)
						return CustomerStatus.CreditHold;
					return null;
				}
			}
		}
		...
		[Serializable]
		[PXBreakInheritance]
		public partial class DetailsResult : Customer
		{
			#region Selected
			...
			{
				foreach (string field in G.Caches[typeof(Customer)].Fields)
				{
					G.Caches[typeof(DetailsResult)].SetValue(this, field, G.Caches[typeof(Customer)].GetValue(cust, field));
				}
		...
		protected virtual PXResultset<Customer> GetCustomersToProcess(CreditHoldParameters header)
		{
		...
		protected virtual void CreditHoldParameters_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			Details.SetProcessTarget(null, null, header.Action, null);

			bool allowDatesFiltering = header.Action?.EndsWith(nameof(CustomerMaint.PutOnCreditHold)) == true;
