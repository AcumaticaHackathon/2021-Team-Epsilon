	...
	public class PercentTotalDBDecimalAttribute : PercentDBDecimalAttribute
	{
		...
		public override void RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
				...
				(val = sender.GetValue(e.Row, _FieldOrdinal)) != null && (decimal)val != 1m)
			{
				...
				if (_MapErrorTo == null)
				{
					if (sender.RaiseExceptionHandling(_FieldName, e.Row, val, new PXSetPropertyException(Messages.WrongValue, $"[{_FieldName}]")))
					{
				...
				else
				{
					...
					if (sender.RaiseExceptionHandling(name, e.Row, val, new PXSetPropertyException(Messages.WrongValue, name, $"[{_FieldName}]")))
					{
						throw new PXRowPersistingException(_FieldName, null, Messages.WrongValue, _FieldName);
					}
				}
