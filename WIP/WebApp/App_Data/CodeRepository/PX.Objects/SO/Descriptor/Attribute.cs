...
namespace PX.Objects.SO
{
	...
	where Field : IBqlField
	{
		...
		{
			if (row != null && !(row is SOLine) ||
				oldrow != null && !(oldrow is SOLine))
			{
				return null;
			}
			...
			var old = (SOLine)oldrow;
			var @new = (SOLine)row;

			if (row != null && @new.OpenLine == true && (@new.IsFree != true || @new.ManualDisc == true) 
				&& (oldrow == null || old.OpenLine != true || old.IsFree == true && old.ManualDisc == false))
			{
				return 1;
			}

			if (oldrow != null && old.OpenLine == true && (old.IsFree != true || old.ManualDisc == true) 
				&& (row == null || @new.OpenLine != true || @new.IsFree == true && @new.ManualDisc == false))
			{
				return -1;
			}
		...
		{
			short count = (short)records.OfType<SOLine>().Count(r => r.OpenLine == true && (r.IsFree != true || r.ManualDisc == true));
			return count;
		}
	...
	public class SOLineSplitPlanIDAttribute : INItemPlanIDAttribute
	{
		...
		public override void Parent_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (CustomerUpdated || DatesUpdated || RequestOnUpdated || CreditHoldApprovedUpdated
				|| !sender.ObjectsEqual<SOOrder.hold, SOOrder.cancelled, SOOrder.completed, SOOrder.backOrdered, SOOrder.shipComplete, SOOrder.prepaymentReqSatisfied>(e.Row, e.OldRow))
			{
