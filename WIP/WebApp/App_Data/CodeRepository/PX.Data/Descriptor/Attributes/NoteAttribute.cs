...
namespace PX.Data
{
    ...
    public class PXNoteAttribute : PXDBGuidAttribute, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXRowDeletedSubscriber, IPXReportRequiredField
    {
		...
		public static Guid? GetNoteIDReadonly(PXCache cache, object data, string name)
        {
            ...
            foreach (PXEventSubscriberAttribute attr in cache.GetAttributes(data, name))
            {
                if (attr is PXNoteAttribute note)
                {
                    return (Guid?)cache.GetValue(data, note._FieldOrdinal);
                }
            }
            return null;
        }
        ...
        private static object CastRow(PXCache cache, object data)
        {
            ...
            if (data is PXResult res)
                data = DynamicalyChangeType(res[itemType], itemType);
		...
		public static void ForcePassThrow<Field>(PXCache cache)
            where Field : IBqlField
        {
            ForcePassThrow(cache, typeof(Field).Name);
        }
        ...
        public static void ForceRetain<Field>(PXCache cache)
            where Field : IBqlField
        {
            ForceRetain(cache, typeof(Field).Name);
        }
            ...
            foreach (string field in sender.Fields)
            {
                ...
                if (description != null && description.IsRestriction && description.DataType != PXDbType.Timestamp
                    && (sender.BqlSelect == null
                        || description.Expr is Column column
                        && (column.Table() as SimpleTable).Name == _BqlTable.Name))
                {
        ...
        [PXInternalUseOnly]
        public static bool TryGetNoteEntityType(PXCache cache, Guid? noteID, out Type entityType)
        {
            if (cache != null)
            {
                var noteAttribute = cache
                    .GetAttributesReadonly("NoteID")
                    .OfType<PXNoteAttribute>()
                    .FirstOrDefault();
                if (noteAttribute != null)
                {
                    string entityTypeName = noteAttribute.GetEntityType(cache, noteID);
                    if (string.IsNullOrEmpty(entityTypeName) == false)
                    {
                        entityType = System.Web.Compilation.PXBuildManager.GetType(entityTypeName, false);
        ...
        [PXInternalUseOnly]
        public static bool TryGetNoteGraphType(PXCache cache, out Type graphType)
        {
            if (cache != null)
            {
                var noteAttribute = cache
                ...
                if (noteAttribute != null)
                {
                    string graphTypeName = noteAttribute.GetGraphType(cache.Graph);
                    if (string.IsNullOrEmpty(graphTypeName) == false)
                    {
                        graphType = System.Web.Compilation.PXBuildManager.GetType(graphTypeName, false);
        ...
        {
            if (TryGetNoteGraphType(cache, out Type graphType))
                return graphType.FullName;
            else
                return CustomizedTypeManager.GetTypeNotCustomized(cache.Graph).FullName;
        }
        protected virtual string GetGraphType(PXGraph graph)
        {
            Type graphType = CustomizedTypeManager.GetTypeNotCustomized(graph);
            return graphType.FullName;
        }
        ...
        {
            return PXContext.GetScreenID()?.Replace(".", "") ??
                (PXSiteMap.Provider as PXDatabaseSiteMapProvider)?
                .FindSiteMapNodeByGraphType(cache.Graph.GetType().FullName)?
                .ScreenID;
        }
                                                  ...
                                                  Action<PXCache, Guid, string> updateNoteRecord, Action<Note, string> assignText, string noteTextField, Action<bool> textRequired)
        {
            if (e.Row != null)
            {
                ...
                setNoteTextExists(sender, e.Row, noteText ?? String.Empty);
                if (_PassThrough || !sender.AllowUpdate && RowIsUnchanged(sender, e.Row))
                {
                ...
                else
                {
                    ...
                    {
                        if (GetView(sender.Graph).SelectSingle(id) is Note note)
                        {
                            assignText(note, noteText ?? String.Empty);
                            note.EntityType = GetEntityType(sender, note.NoteID);
                            note.GraphType = GetGraphType(sender.Graph);
                    ...
                    if (!referencedNoteExists && !string.IsNullOrEmpty(noteText))
                    {
                        Note note = new Note();
                        bool noteIdWasAssigned = id != null;
                        if (noteIdWasAssigned)
                            note.NoteID = id;
                        assignText(note, noteText);
                        note.EntityType = GetEntityType(sender, note.NoteID);
                        note.GraphType = GetGraphType(sender.Graph);
                    ...
                    if (RowIsUnchanged(sender, e.Row))
                    {
        ...
        {
            if (_ForceRetain || !(sender.GetValue(e.Row, _FieldOrdinal) is Guid id)) return;
            
            sender.Graph.Caches[typeof(Note)].Delete(new Note { NoteID = id });
        ...
        {
            if (sender.GetValue(e.Row, _FieldOrdinal) is Guid id)
            {
                ...
                using (PXTransactionScope ts = new PXTransactionScope())
                {
                    ...
                    foreach (NoteDoc doc in dcache.Inserted)
                    {
                        if (doc.NoteID == id)
                        {
                            ...
                            {
                                if (screenID == null)
                                    screenID = GetCurrentScreenID(sender);
                                if (screenID != null)
                                    UploadFileMaintenance.SetAccessSource(doc.FileID.Value, null, screenID);
                            }
                        }
                    }
                    ts.Complete();
                }
                if (!_ForceRetain && e.Operation.Command() == PXDBOperation.Delete)
                {
                    //Attached files do not removed on deletion of import or export scenarios (they removed on deletion of data provider)
                    if (!(e.Row is SYMapping))
                    {
                        var graph = new PXGraph();
                        foreach (UploadFile file in PXSelectReadonly2<UploadFile,
                            InnerJoin<NoteDoc, On<NoteDoc.fileID, Equal<UploadFile.fileID>>>,
                            Where<NoteDoc.noteID, Equal<Required<NoteDoc.noteID>>>>
                            .Select(sender.Graph, id).FirstTableItems)
                        {
                            //New graph is used in select because we need to get deleted
                            //records of the NoteDoc cache, which were not persisted yet
                            var fileNotes = PXSelect<NoteDoc,
                                Where<NoteDoc.fileID, Equal<Required<NoteDoc.fileID>>>>
                                .Select(graph, file.FileID).FirstTableItems.ToArray();

                            if (fileNotes.All(fileNote => fileNote.NoteID == id))
                            {
                                //Deletion of NoteDoc with current NoteID do not performed because it will be deleted further by PersistDeleted method of the cache
                                PXDatabase.Delete<NoteDoc>(new PXDataFieldRestrict<NoteDoc.fileID>(PXDbType.UniqueIdentifier, file.FileID), new PXDataFieldRestrict<NoteDoc.noteID>(PXDbType.UniqueIdentifier, 16, id, PXComp.NE));
                                PXDatabase.Delete<UploadFileRevision>(new PXDataFieldRestrict<UploadFileRevision.fileID>(PXDbType.UniqueIdentifier, file.FileID));
                                PXDatabase.Delete<UploadFile>(new PXDataFieldRestrict<UploadFile.fileID>(PXDbType.UniqueIdentifier, file.FileID));
                            }
                        }
                    }
            ...
            if (id != null)
            {
                ...
                if (id != null)
                {
                    ...
                    if (docs.Count > 0)
                    {
                        ...
                        {
                            if (doc.FileID.HasValue && GetFileByID(sender.Graph).SelectSingle(doc.FileID.Value) is UploadFile file)
                            {
                                if (!string.IsNullOrEmpty(file.Name) && (imageExts.Count == 0 || imageExts.Contains(file.Extansion)))
                                {
		...
		public virtual void noteFilesFieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
        {
            ...
            else if (e.NewValue is Guid[] docs)
            {
                Guid? id = (Guid?)sender.GetValue(e.Row, _FieldOrdinal);
                    ...
                    using (PXTransactionScope tran = new PXTransactionScope())
                    {
                        ...
                        foreach (Guid did in docs)
                        {
                            ...
                            if (!String.IsNullOrEmpty(screenID))
                                UploadFileMaintenance.SetAccessSource(did, null, screenID.Replace(".", ""));
                        }
		...
		protected virtual void InsertNote(PXCache sender, Guid id, string text = "")
			=> PXNoteAttribute.InsertNoteRecord(GetGraphType(sender.Graph), GetEntityType(sender, id), id, text);
        ...
        public static void InsertNoteRecord(PXCache sender, string entityType, Guid id, string text = "")
            => InsertNoteRecord(GetGraphType(sender), entityType, id, text);

        /// <exclude/>
        public static void InsertNoteRecord(string graphType, string entityType, Guid id, string text = "")
        {
            try
            {
                PXDatabase.Insert(typeof(Note),
                    ...
                    new PXDataFieldAssign<Note.graphType>(PXDbType.VarChar, graphType),
		...
		protected virtual void UpdateNote(PXCache sender, Guid id, string text = null, string popupText = null)
			=> PXNoteAttribute.UpdateNoteRecord(sender.Graph, GetGraphType(sender.Graph), GetEntityType(sender, id), id, text, popupText);
        ...
        public static void UpdateNoteRecord(PXCache sender, string entityType, Guid id, String text = null, String popupText = null)
            => UpdateNoteRecord(sender.Graph, GetGraphType(sender), entityType, id, text, popupText);

        /// <exclude/>
        public static void UpdateNoteRecord(PXGraph graph, string graphType, string entityType, Guid id, String text = null, String popupText = null)
        {
            Note note = new Note { NoteID = id };
            if (graph.Caches[typeof(Note)].GetStatus(note) == PXEntryStatus.Inserted)
                return;
        ...
        public static void AttachFile(PXCache sender, object data, FileInfo fileInfo)
        {
            ...
            if (!String.IsNullOrEmpty(screenID) && (node = PXSiteMap.Provider.FindSiteMapNodeByScreenID(screenID.Replace(".", ""))) != null && !String.IsNullOrEmpty(node.Title))
            {
                ...
                {
                    PXDatabase.Update<UploadFile>(new PXDataFieldAssign<UploadFile.name>(PXDbType.NVarChar, 255, prefix + "\\" + fileInfo.Name), new PXDataFieldRestrict<UploadFile.fileID>(PXDbType.UniqueIdentifier, 16, fileInfo.UID.Value));
            ...
            var imagesView = new PXView(sender.Graph, true, new Select<UploadFile>(),
                new PXSelectDelegate(
                    () =>
                    {
                        ...
                        if (sender.GetStateExt(sender.Current, _NoteImagesField) is PXNoteState state && state.Value is Guid[] fileIDs) //TODO: need review 'sender.Current'
                        {
    ...
    public static class PopupNoteManager
    {
        ...
        public static string Message
        {
            ...
            set
            {
                var currentMessage = PXContext.GetSlot<string>(_slotMessageKey);

                if (!string.IsNullOrEmpty(currentMessage) && !currentMessage.Equals(value, StringComparison.Ordinal))
                {
