...
namespace PX.Commerce.BigCommerce
{
	...
	public class BCSalesPriceProcessor : BCProcessorBulkBase<BCSalesPriceProcessor, BCSalesPriceEnityBucket, MappedBaseSalesPrice>, IProcessor
	{
		...
		#region Import
		public override List<BCSalesPriceEnityBucket> FetchBucketsImport(List<BCSyncStatus> ids)
		{
		...
		public override List<BCSalesPriceEnityBucket> FetchBucketsExport(List<BCSyncStatus> ids)
		{
			...
			foreach (var item in inventories)
			{
				...
				if (!updatedAny && !forceSync && current?.PendingSync != true) continue;
			...
			foreach (var entity in CurrentEntities)
			{
				...
				if (inventories.Any(x => x.Key.InventoryID == inventory.InventoryCD.Trim())) continue;
			...
			productBatchBulkRestDataProvider.UpdateAll(bulkPrices.Select(x => x.Price.Extern).ToList(), delegate (ItemProcessCallback<BulkPricingWithSalesPrice> callback)
				  {
					  ...
					  if (callback.IsSuccess)
					  {
						  ...
						  {
							  DeleteStatus(obj.Price, BCSyncOperationAttribute.ExternDelete);
						  }
						  else
						  {
