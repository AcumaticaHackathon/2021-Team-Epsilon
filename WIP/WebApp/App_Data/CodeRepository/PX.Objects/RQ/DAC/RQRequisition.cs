...
namespace PX.Objects.RQ
{
	...
	[PXEMailSource]
	[PXGroupMask(typeof(
		LeftJoinSingleTable<Customer, On<
			Customer.bAccountID.IsEqual<customerID>.
			And<Match<Customer, AccessInfo.userName.FromCurrent>>>,
		LeftJoinSingleTable<Vendor, On<
			Vendor.bAccountID.IsEqual<vendorID>.
			And<Match<Vendor, AccessInfo.userName.FromCurrent>>>>>),
		WhereRestriction = typeof(
			Brackets<Customer.bAccountID.IsNotNull.Or<customerID.IsNull>>.
			And<Vendor.bAccountID.IsNotNull.Or<vendorID.IsNull>>))]
	public partial class RQRequisition : PX.Data.IBqlTable, PX.Data.EP.IAssign
	{
		...
		#endregion
		#region Events
		public class Events : PXEntityEvent<RQRequisition>.Container<Events>
		{
		...
		[PXDBBool()]
		[PXUIField(DisplayName = "Cancel", Visibility = PXUIVisibility.Visible, Enabled = false)]
		[PXDefault(false)]
		...
		[PXDBBool()]
		[PXDefault(false, PersistingCheck = PXPersistingCheck.Nothing)]
		[PXUIField(DisplayName = "Quoted", Visibility = PXUIVisibility.Visible, Enabled = false)]
		...
		[PX.TM.Owner(typeof(RQRequisition.workgroupID), Visibility = PXUIVisibility.SelectorVisible)]
		...
		[PXDefault((object)null, typeof(Coalesce<
			Search<LocationBranchSettings.vSiteID, Where<Current<RQRequisition.shipDestType>, Equal<POShippingDestination.site>,
				And<LocationBranchSettings.bAccountID, Equal<Current<RQRequisition.vendorID>>,
				And<LocationBranchSettings.locationID, Equal<Current<RQRequisition.vendorLocationID>>,
				And<LocationBranchSettings.branchID, Equal<Current<RQRequisition.branchID>>>>>>>,
			Search<Location.vSiteID, Where<Current<RQRequisition.shipDestType>, Equal<POShippingDestination.site>,
	...
	public static class RQRequisitionExt
	{
		...
		{
			if (requisition != null && requisition.BiddingComplete != true)
			{
				graph.LiteUpdate(requisition, _ =>
				{
					_.Set(o => o.BiddingComplete, true);
				});

				RQRequisition.Events.Select(e => e.BiddingCompleted).FireOn(graph, requisition);
			}
		}
	}
}