...
namespace PX.Objects.FA
{
	public class DepreciationCalculation : PXGraph<DepreciationCalculation, FADepreciationMethod>
	{
		...
		public virtual void CalculateDepreciation(FABookBalance assetBalance, string maxPeriodID)
		{
			...
			if (assetBalance.Depreciate == true && cls.UnderConstruction != true && (string.IsNullOrEmpty(assetBalance.DeprFromPeriod) || string.IsNullOrEmpty(assetBalance.DeprToPeriod) || string.CompareOrdinal(assetBalance.DeprFromPeriod, assetBalance.DeprToPeriod) > 0))
			{
			...
			{
				OrganizationFinPeriod lastClosedPeriod = SelectFrom<OrganizationFinPeriod>
					.InnerJoin<FABookBalance>
						.On<FABookBalance.assetID.IsEqual<@P.AsInt>
							.And<FABookBalance.bookID.IsEqual<@P.AsInt>>>
					.InnerJoin<FixedAsset>
						.On<FixedAsset.assetID.IsEqual<FABookBalance.assetID>>
					.InnerJoin<Branch>
						.On<Branch.branchID.IsEqual<FixedAsset.branchID>>
					.Where<Branch.organizationID.IsEqual<OrganizationFinPeriod.organizationID>
						.And<OrganizationFinPeriod.finPeriodID.IsGreaterEqual<FABookBalance.deprFromPeriod>>
						.And<OrganizationFinPeriod.finPeriodID.IsLessEqual<FABookBalance.deprToPeriod>>
						.And<OrganizationFinPeriod.fAClosed.IsEqual<True>>
						.And<OrganizationFinPeriod.startDate.IsNotEqual<OrganizationFinPeriod.endDate>>>
					.OrderBy<Desc<OrganizationFinPeriod.finPeriodID>>
					.View
					.SelectSingleBound(this, null, assetBalance.AssetID, assetBalance.BookID);

				if (lastClosedPeriod != null)
				{
					AssetProcess.SetLastDeprPeriod(BookBalance, assetBalance, lastClosedPeriod.FinPeriodID);
					AssetProcess.AdjustFixedAssetStatus(this, assetBalance.AssetID);
 
					foreach (FABookHist prevHistoryItem in inserted.Where(item => string.CompareOrdinal(item.FinPeriodID, assetBalance.CurrDeprPeriod) <= 0))
					{
						if(!histdict.ContainsKey(prevHistoryItem.FinPeriodID))
						{
							histdict[prevHistoryItem.FinPeriodID] = prevHistoryItem;
						}
					}
				}
			}
			...
			if (yearlyAccountancy)
			{
				...
				for (int i = fromPeriod; i <= toPeriod; i++)
				{
					...
					if (switchToSL)
					{
						...
						decimal SLRate = Params.DepreciationBasis == slAccumulatedDepreciation
							? 1 // patch for AC-182773
							: slAdjustedYearDepr / (Params.DepreciationBasis - slAccumulatedDepreciation);
						slDepreciation = SLRate * (Params.DepreciationBasis - prevAccumulatedDepreciation) / (toPeriod - fromPeriod + 1);
						if (SLRate > DBRate)
						{
							depreciation = Math.Abs(slDepreciation) > Math.Abs((Params.DepreciationBasis - Params.AccumulatedDepreciation))
		...
		private bool PerformCalculation(FABookBalance bookBalance, string maxPeriodID = null, PXGraph uiGraph = null)
		{
			...
			FixedAsset asset = SelectFrom<FixedAsset>.Where<FixedAsset.assetID.IsEqual<@P.AsInt>>.View.Select(this, bookBalance.AssetID);
		...
		private void CloseHistoryInClosedPeriods(DepreciationMethodBase depreciationMethod)
		{
			...
			if (bookBalance.UpdateGL == true)
			{
				...
				if (string.CompareOrdinal(maxClosedPeriod, bookBalance.DeprFromPeriod) >= 0)
				{
