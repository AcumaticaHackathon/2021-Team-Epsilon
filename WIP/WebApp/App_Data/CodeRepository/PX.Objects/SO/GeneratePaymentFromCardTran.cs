...
namespace PX.Objects.SO
{
	public class GeneratePaymentFromCardTran : PXGraph<GeneratePaymentFromCardTran>
	{
		...
		{
			var query = new PXSelectJoin<ExternalTransactionExt,
				LeftJoin<SOOrder, On<ExternalTransactionExt.origDocType, Equal<SOOrder.orderType>, And<ExternalTransactionExt.origRefNbr, Equal<SOOrder.orderNbr>>>,
					LeftJoin<SOInvoice, On<ExternalTransactionExt.docType, Equal<SOInvoice.docType>, And<ExternalTransaction.refNbr, Equal<SOInvoice.refNbr>>>>>,
				Where<ExternalTransactionExt.active, Equal<True>,
					...
					And2<Where<Not<SOOrder.hasLegacyCCTran, NotEqual<True>,And<SOInvoice.hasLegacyCCTran, NotEqual<True>>>>,
					And<Where<Not<ExternalTransactionExt.procStatus, NotEqual<ExtTransactionProcStatusCode.captureSuccess>,
			...
			var res = query.View.Select(currents, prms, PXView.Searches, PXView.SortColumns, PXView.Descendings, PXView.Filters, ref startRow, PXView.MaximumRows, ref totalRows)
				.Select(i=>((PXResult)i)[0]);

			foreach (ExternalTransactionExt tran in res)
			{
		...
		public class PaymentCreatorFactory : PXGraph<PaymentCreatorFactory>
		{
			...
			public void ClearGraphs()
			{
				...
				paymentGraph.ccProcTran.Cache.ClearQueryCache();
				paymentGraph.Clear();
				orderGraph.Clear();
				invoiceGraph.Clear();
			}
