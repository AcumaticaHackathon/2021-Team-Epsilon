...
namespace PX.Objects.IN
{
    ...
    public class INReleaseProcess : PXGraph<INReleaseProcess>
    {
        ...
        public static void UpdateSiteHistD(PXGraph graph, INTranSplit tran)
		{
			...
			UpdateHistoryField<ItemSiteHistD.qtyDebit, ItemSiteHistD.qtyDebit>(graph, hist, bucket.SignYtd > 0 ? tran.BaseQty : 0m, true);
			UpdateHistoryField<ItemSiteHistD.qtyCredit, ItemSiteHistD.qtyCredit>(graph, hist, bucket.SignYtd < 0 ? tran.BaseQty : 0m, true);

			UpdateHistoryField<ItemSiteHistD.endQty, ItemSiteHistD.endQty>(graph, hist, tran.BaseQty * bucket.SignYtd, true);
		}

		protected static void FillDateFields(ItemSiteHistD hist, DateTime date)
		{
			hist.SYear = date.Year;
			hist.SMonth = date.Month;
			hist.SDay = date.Day;
        ...
        public static void UpdateSiteHistDCost(PXGraph graph, INTranCost costtran, INTran tran)
        {
            INHistBucket bucket = new INHistBucket(tran);
            ...
            hist.InventoryID = tran.InventoryID;
            hist.SiteID = tran.SiteID;
            hist.SubItemID = costtran.CostSubItemID;
            hist.SDate = tran.TranDate;
            ...
            UpdateHistoryField<ItemSiteHistD.costDebit, ItemSiteHistD.costDebit>(graph, hist, bucket.SignYtd > 0 ? costtran.TranCost : 0m, true);
            UpdateHistoryField<ItemSiteHistD.costCredit, ItemSiteHistD.costCredit>(graph, hist, bucket.SignYtd < 0 ? costtran.TranCost : 0m, true);
        }

        protected virtual void UpdateSiteHistDCost(INTranCost costtran, INTran tran)
        {
            UpdateSiteHistDCost(this, costtran, tran);
        }
		        ...
		        ValidateFinPeriod(doc, tranDataRows.Cast<PXResult<INTran, InventoryItem, INSite>>());
                    ...
                    foreach (INTranCost costtran in intrancost.Cache.Inserted)
                    {
                        ...
                        if (tran != null)
                        {
                            ...
                            UpdateSiteHistDCost(costtran, tran);
...
namespace PX.Objects.IN.Overrides.INDocumentRelease
{
	...
	public class ItemSiteHistDAccumAttribute : PXAccumulatorAttribute
    {
        public ItemSiteHistDAccumAttribute()
            : base(
                  new[] { typeof(ItemSiteHistD.endQty), typeof(ItemSiteHistD.endQty) },
                  new[] { typeof(ItemSiteHistD.begQty), typeof(ItemSiteHistD.endQty) })
        {
        }
    ...
    public class ItemLotSerialAccumulatorAttribute : StatusAccumulatorAttribute
    {
        ...
        public override bool PersistInserted(PXCache sender, object row)
        {
            ...
            catch (PXLockViolationException)
            {
				...
				bool whenRcvdSerialNumber = (bal.LotSerTrack == INLotSerTrack.SerialNumbered && bal.LotSerAssign == INLotSerAssign.WhenReceived);
				if (bal.IsIntercompany == true && whenRcvdSerialNumber && bal.QtyOnHand > 0m && item.QtyOnHand > 1m)
				{
					PXResultset<INItemPlan> plans = GetItemPlans(sender, inventoryID, lotSerialNbr);
					if (plans.RowCast<INItemPlan>().Any(p => p.PlanType == INPlanConstants.Plan62))
						throw new PXIntercompanyReceivedNotIssuedException(sender, item);
				}
		...
		protected virtual PXResultset<INItemPlan> GetItemPlans(PXCache sender, int? inventoryID, string lotSerialNbr)
		{
