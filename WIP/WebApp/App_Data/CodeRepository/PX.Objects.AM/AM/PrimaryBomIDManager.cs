...
namespace PX.Objects.AM
{
    ...
    public class PrimaryBomIDManager : IDManagerBase
    {
        ...
        public string GetPrimaryAllLevels(InventoryItem inventoryItem, INItemSite itemSite, int? subItemID)
        {
            var warehouseBomID = GetItemSitePrimary(itemSite, subItemID);

            return string.IsNullOrWhiteSpace(warehouseBomID) ? GetItemPrimary(inventoryItem, subItemID) : warehouseBomID;
        }
            ...
            bool overrideItemDefault)
        {
            ...
            if (isInsert)
            {
                using (new DisableSelectorValidationScope(_Graph.Caches<AMSubItemDefault>()))
                {
            ...
            return PXSelectReadonly<
                    ...
                    OrderBy<
                        Desc<AMBomItem.status,
                            Desc<AMBomItem.effStartDate,
                                Desc<AMBomItem.revisionID>>>>>
                .SelectWindowed(graph, 0, 1, bomId, new string[]{ AMBomStatus.Hold, AMBomStatus.Active });
        }
