...
namespace PX.Objects.GL
{
	public class OrganizationFinPeriodMaint : PXGraph<OrganizationFinPeriodMaint>, IFinPeriodMaintenanceGraph
	{
		...
		public OrganizationFinPeriodMaint()
		{
			...
			Delete.SetVisible(PXAccess.FeatureInstalled<FeaturesSet.multipleCalendarsSupport>() || PXAccess.FeatureInstalled<FeaturesSet.centralizedPeriodsManagement>());
		}
		...
		{
			Dictionary<string, WebDialogResult> answers = new Dictionary<string, WebDialogResult>();
			List<string> viewNames = MasterCalendarGraph.Views.Keys.ToList();
			foreach (string viewName in viewNames)
			{
				WebDialogResult answer = MasterCalendarGraph.Views[viewName].Answer;
				if (answer != WebDialogResult.None)
				{
					answers.Add(viewName, answer);
				}
			}
			MasterCalendarGraph.Clear();

			foreach(KeyValuePair<string, WebDialogResult> viewAnswer in answers)
			{
				if (MasterCalendarGraph.Views.TryGetValue(viewAnswer.Key, out PXView view))
				{
					view.Answer = viewAnswer.Value;
				}
			}
		}
		...
		public virtual void GenerateCalendar(int? organizationID, int fromYear, int toYear)
		{
			...
			if (OrgFinPeriods.View.Answer.IsNegative()) return;

			MasterCalendarGraph.Periods.View.Answer = OrgFinPeriods.View.Answer;
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				catch (PXDialogRequiredException exception)
				{
					if (exception.Graph is MasterFinPeriodMaint
						&& !string.IsNullOrEmpty(exception.Message)
						&& exception.ViewName == nameof(MasterFinPeriodMaint.Periods))
					{
						OrgFinPeriods.Ask(exception.Header, exception.Message, exception.Buttons, exception.Icon);
					}
					throw;
				}
				ts.Complete();
