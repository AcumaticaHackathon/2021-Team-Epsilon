...
namespace PX.Objects.TX
{
	public class TaxYearMaint: PXGraph<TaxYearMaint>
	{
		...
		public PXAction<TaxYearFilter> SyncTaxPeriods;
		...
		public TaxYearMaint()
		{
			...
			ActionsMenu.AddMenuAction(SyncTaxPeriods);
		...
		public override void Persist()
		{
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (lastPersistablePeriod != null)
				{
					...
					{
						string confirmMessage = string.Format(PXMessages.LocalizeNoPrefix(Messages.SubsequentTaxYearsWillBeDeleted), yearAfterLastPersistable.Year, lastPersistablePeriod.TaxYear, PXAccess.GetOrganizationCD(lastPersistablePeriod.OrganizationID));
						TaxYearFilterSelectView.Ask(confirmMessage, MessageButtons.OK);
		...
		protected virtual void TaxYearFilter_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			SyncTaxPeriods.SetEnabled(taxYearFilter.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod);
			string existingError = PXUIFieldAttribute.GetErrorOnly<TaxYearFilter.year>(cache, taxYearFilter);
			if (taxYearFilter.Year == null)
			{
				cache.RaiseExceptionHandling<TaxYearFilter.year>(taxYearFilter, null, null);
			}
			else if (string.IsNullOrEmpty(existingError))
			{
				cache.RaiseExceptionHandling<TaxYearFilter.year>(taxYearFilter, null, null);
				if (taxYearFilter.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod)
				{
					if (!IsPeriodSynchronized())
					{
						cache.RaiseExceptionHandling<TaxYearFilter.year>(taxYearFilter, null, new PXSetPropertyException(Messages.TaxPeriodsNotSync, PXErrorLevel.Warning, PXAccess.GetOrganizationCD(taxYearFilter.OrganizationID), Messages.SynchronizeWithGL));
					}
					else
					{
						string maxFinYear = FinPeriodRepository.FindLastYear(taxYearFilter.OrganizationID ?? 0, false)?.Year;
						if (taxYearFilter.Year != null && maxFinYear != null && (int.Parse(maxFinYear) < int.Parse(taxYearFilter.Year)))
						{
							cache.RaiseExceptionHandling<TaxYearFilter.year>(e.Row, taxYearFilter.Year, new PXSetPropertyException(
								Messages.FinPeriodsNotGenerated,
								PXErrorLevel.Warning,
								taxYearFilter.Year,
								PXAccess.GetOrganizationCD(taxYearFilter.OrganizationID)));
		...
		protected virtual void TaxYearFilter_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
				...
				&& taxYearFilter.Year != null)
			{
					...
					taxYearFilter, oldTaxYearFilter))
				{
					...
					if (maxFinYear != null)
					{
						if (int.Parse(maxFinYear) < int.Parse(taxYearFilter.Year))
						{
							taxYearFilter.StartDate = null;
							taxYearFilter.IsStartDateEditable = false;
							cache.RaiseExceptionHandling<TaxYearFilter.year>(
								e.Row, 
								taxYearFilter.Year, 
								new PXSetPropertyException(Messages.FinPeriodsNotGenerated, PXErrorLevel.Warning, taxYearFilter.Year, PXAccess.GetOrganizationCD(taxYearFilter.OrganizationID )));
							return;
						}
						else
						{
		...
		[PXButton]
		[PXUIField(DisplayName = Messages.SynchronizeWithGL, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		protected virtual IEnumerable syncTaxPeriods(PXAdapter adapter)
		{
			if (TaxYearExSelectView.Current?.OrganizationID != null
				&& TaxYearExSelectView.Current.VendorID != null && 
				TaxYearExSelectView.Current.TaxPeriodType == VendorTaxPeriodType.FiscalPeriod)
			{
				if (!IsPeriodSynchronized())
				{
					SyncPeriodsWithGL(TaxYearExSelectView.Current);
					Actions.PressSave();
				}
			}
			return adapter.Get();
		...
		protected delegate bool PeriodPredicate (TaxPeriodEx period);

		protected virtual void RegeneratePeriodsAndPutIntoCache(TaxYearEx currentTaxYear)
		{
			GeneratePeriods(currentTaxYear, period => true);
		}

		protected virtual void GeneratePeriods(TaxYearEx currentTaxYear, PeriodPredicate predicate)
		{
			TaxCalendar.CreationParams creationParams = TX.TaxCalendar.CreationParams.FromTaxYear(currentTaxYear);
		...
		{
			IEnumerable<TaxPeriodEx> taxPeriods = GetActualStoredTaxPeriods();
			if (taxPeriods != null)
			{
				foreach (TaxPeriod taxPeriod in taxPeriods)
				{
					DateTime finStartDate = FinPeriodRepository.PeriodStartDate(taxPeriod.TaxPeriodID, taxPeriod.OrganizationID);
					DateTime finEndDate = FinPeriodRepository.PeriodEndDate(taxPeriod.TaxPeriodID, taxPeriod.OrganizationID);
					if (((finStartDate == null && finEndDate == null) || (taxPeriod.StartDate != finStartDate) || (taxPeriod.EndDate != finEndDate.AddDays(1))) 
						&& (taxPeriod.StartDate != taxPeriod.EndDate))
					{
						return false;
					}
				}
		...
		protected virtual void SyncPeriodsWithGL(TaxYearEx currentTaxYear)
		{
			IEnumerable<TaxPeriodEx> taxPeriods = GetActualStoredTaxPeriods();
			foreach (TaxPeriodEx taxPeriod in taxPeriods)
			{
				if (taxPeriod.Status == TaxPeriodStatus.Open && taxPeriod.TaxYear == currentTaxYear.Year)
				{
					TaxPeriodExSelectView.Delete(taxPeriod);
				}
			}
			GeneratePeriods(currentTaxYear, period => period.Status == TaxPeriodStatus.Open);
		}
