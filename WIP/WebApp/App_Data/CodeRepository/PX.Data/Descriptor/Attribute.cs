...
namespace PX.Data
{
	...
	public class PXParentAttribute : PXForeignReferenceAttribute, IPXRowInsertedSubscriber, IPXRowPersistedSubscriber
	{
		...
		private BqlCommand _selectParentWithoutAdditionalCondition; // in case of using PXParent(Type, Type) constructor, contains the select without the additional condition passed as the second argument
		...
		public virtual PXView GetChildrenSelect(PXCache sender)
			=> GetChildrenSelect(sender, false);
		...
		public virtual PXView GetChildrenSelect(PXCache sender, bool isReadOnly)
		{
			...
			return sender.Graph.TypedViews.GetView(_SelectChildren, isReadOnly);
		}
			...
			: base(fieldsRelation, ReferenceOrigin.ParentAttribute, ReferenceBehavior.Cascade, true)
		{
			...
			_selectParentWithoutAdditionalCondition = BqlCommand.CreateInstance(selectParent);
			_SelectParent = _selectParentWithoutAdditionalCondition.WhereAnd(additionalCondition);
			...
			:base(selectParent, ReferenceOrigin.ParentAttribute, ReferenceBehavior.Cascade, true)
		{
			...
			_SelectParent = _selectParentWithoutAdditionalCondition = BqlCommand.CreateInstance(selectParent);
			_ParentType = _SelectParent.GetTables()[0];
		}
		...
		{
			if (!_SelectParent.GetFieldPairs().SequenceEqual(_SelectParent.GetParameterPairs()))
				throw new PXException(ErrorMessages.PXParentUnparametrizedBQLSelect, $"{_BqlTable.FullName}.{_FieldName}");
		}
		...
		{
			var currentTable = BqlTable;
			var invalidFields = _SelectParent.GetParameterPairs()
				.Select(kv => kv.Value.DeclaringType)
				.Select(t => PXExtensionManager.GetExtendedType(t) ?? t) // unwrap PXCacheExtension<>
				.Distinct()
				.Where(t => t != currentTable && t.IsAssignableFrom(currentTable))
				.ToArray();

			if (invalidFields.Any())
			{
				throw new PXException(ErrorMessages.PXParentBQLSelectUsesBqlFieldsFromTheBaseType, 
					$"{_BqlTable.FullName}.{_FieldName}",
					(object) String.Join(", ", invalidFields.Select(t => $"{t.DeclaringType.FullName}.{t.Name}")));
			}
		}
		...
		{
			var currentTable = BqlTable;
			var invalidTables = _selectParentWithoutAdditionalCondition.GetParameterPairs()
				.Select(kv => kv.Value.DeclaringType)
				.Select(t => PXExtensionManager.GetExtendedType(t) ?? t) // unwrap PXCacheExtension<>
				.Distinct()
				.Where(t => !_ChildType.IsAssignableFrom(t) && !_ParentType.IsAssignableFrom(t) && !currentTable.IsAssignableFrom(t))
				.ToArray();

			if (invalidTables.Any())
			{
				throw new PXException(ErrorMessages.PXParentBQLSelectContainsUnrelatedTables, 
					$"{_BqlTable.FullName}.{_FieldName}",
					(object) String.Join(", ", invalidTables.Select(t => t.FullName)));
			}
		}
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			CheckThatConditionsAreParametrized();
			CheckThatThereAreNoBqlFieldsFromTheBaseType();
			CheckThatThereAreNoUnrelatedTablesInTheParentSelect();
	...
	public class PXUIFieldAttribute : PXEventSubscriberAttribute, IPXInterfaceField, IPXExceptionHandlingSubscriber, IPXCommandPreparingSubscriber, IPXFieldSelectingSubscriber, IPXFieldVerifyingSubscriber
	{
		...
		public static Dictionary<string, string> GetErrors(PXCache cache, object data, params PXErrorLevel[] errorLevels)
		{
			...
			foreach (IPXInterfaceField attr in cache.GetAttributes(data, null).OfType<IPXInterfaceField>()
				.Union(cache._KeyValueAttributeUIFields ?? new IPXInterfaceField[0]))
			{
        ...
        {
            if (eventArgs.Exception is PXException pxException)
            {
                _ErrorText = pxException.MessageNoPrefix;

                if (eventArgs.Exception is PXSetPropertyException setPropertyException)
                {
                    _ErrorLevel = setPropertyException.ErrorLevel;
                    _ErrorValue = setPropertyException.ErrorValue ?? eventArgs.NewValue;
                }
                else
                {
                    _ErrorValue = eventArgs.NewValue;
            ...
            {
                _ErrorValue = eventArgs.NewValue;
                _ErrorText = eventArgs.Exception.Message;
                _ErrorLevel = PXErrorLevel.Error;
            }

            var patternValue = _DisplayName ?? _FieldName;
            var originalErrorText = _ErrorText;

            _ErrorText = FormatFieldName(_ErrorText, _FieldName, patternValue);
            
			if (!object.ReferenceEquals(originalErrorText, _ErrorText) 
                && eventArgs.Exception is PXOverridableException exception)
            {
                exception.SetMessage(_ErrorText); 
            }
        }
		...
		internal static string FormatFieldName(string sourceText, string fieldName, string value)
        {
            return sourceText
                .Replace("{0}", value)
                .Replace("[" + fieldName + "]", value, StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            foreach (Match match in _FieldPlaceholderPattern.Matches(text))
            {
		...
		public virtual void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
				...
				_ErrorLevel != PXErrorLevel.RowInfo)
			{
				...
				throw new PXOuterException(
                    GetErrors(sender, e.Row), sender.Graph.GetType(), e.Row,
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			if (!_EnableRights && e.ExternalCall)
			{
					...
					&& (valexisting != null || !sender.Keys.Contains(_FieldName)))
				{
					if (_ViewRights)
					{
						throw new PXSetPropertyException(ErrorMessages.NotEnoughRights, $"[{_FieldName}]");
					}
					else
					{
	...
	public class PXStringListAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXLocalizableList
	{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			// first, check workflow engine, and if there are now combo-box values, then check old automation
            if (WorkflowService?.ApplyComboBoxValues(sender.Graph, BqlTable.FullName, _DatabaseFieldName,
	            ref _AllowedValues, ref _AllowedLabels) != true)
	            _ExplicitCnt = PXAutomation.AppendCombos(BqlTable, _DatabaseFieldName, ref _AllowedValues,
		            ref _AllowedLabels);
            else
	            _ExplicitCnt = 0;
		...
		private void AppendNeutral()
		{
            if (_AllowedLabels != null && (_NeutralAllowedLabels == null || _NeutralAllowedLabels.Length != _AllowedLabels.Length ||
                                           _NeutralAllowedLabels.GetHashCodeOfSequence() != _AllowedLabels.GetHashCodeOfSequence()))
			{
			...
			if (!e.Cancel && e.NewValue != null)
			{
				...
				if (item == null)
				{
					...
					else
					{
						...
						if (!found) throwNoItem(hasRestrictedAccess(sender, _NaturalSelect, e.Row), !sender.Graph.IsContractBasedAPI, e.NewValue);
					}
				}
				else
				{
	...
	public class PXRestrictorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber
	{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (_ShowWarning && sender.Graph.GetType().IsIn(typeof(PXGraph), typeof(PXGenericInqGrph), typeof(Maintenance.GI.GenericInquiryDesigner)).Implies(sender.Graph.UnattendedMode == false))
			{
			...
			if (item != null)
			{
				...
				if (!_AlteredCmd.Meet(view.Cache, item, pars))
				{
					...
					return new PXSetPropertyException(
						_Message,
						GetMessageParameters(sender, itemres, e.Row));
				}
			}
	...
	public class PXFormulaAttribute : PXEventSubscriberAttribute, IPXRowUpdatedSubscriber, IPXRowInsertedSubscriber, IPXRowDeletedSubscriber, IPXDependsOnFields
	{
		...
		/// </summary>
		public virtual bool ForceAggregateRecalculation
		{
			get;
			set;
		}
			...
			where Field : IBqlField
		{
			...
			foreach (PXView view in sender.GetAttributesReadonly(null)
				...
				.Select(parentAttribute => parentAttribute.GetChildrenSelect(sender, IsReadOnly)))
				{
				List<object> records;
					records = view.SelectMultiBound(new object[] { parent });
		...
		protected virtual void UpdateParent(PXCache sender, object row, object oldrow, object newparent)
		{
			...
			if (foreignrecord != null)
			{
				...
				if (state != null && !ForceAggregateRecalculation)
				{
				...
				{
					PXUIFieldAttribute.SetError(parentcache, foreignrecord, _ParentFieldType.Name, null);
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
				...
				!_recursiveCall)
			{
				...
				else if ((e.Operation & PXDBOperation.Option) == PXDBOperation.GroupBy)
				{
					if (Forced)
					{
						e.Expr = new Column(_FieldName, e.Table);
	...
	public class PXAttributeFamilyAttribute : Attribute
	{
		...
		/// 
		internal static void CheckAttributes(PXCache.DACFieldDescriptor prop, PXEventSubscriberAttribute[] attributes)
		{
