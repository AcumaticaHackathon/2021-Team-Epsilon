...
namespace PX.Objects.CR
{
	public class BusinessAccountMaint : PXGraph<BusinessAccountMaint, BAccount>
	{
		...
		public PXAction<BAccount> Activate;
		...
		protected virtual IEnumerable activate(PXAdapter adapter) => adapter.Get<BAccount>();

		public PXAction<BAccount> PutOnHold;
		...
		protected virtual IEnumerable putOnHold(PXAdapter adapter) => adapter.Get<BAccount>();

		public PXAction<BAccount> PutOnCreditHold;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Credit Hold", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable putOnCreditHold(PXAdapter adapter) => adapter.Get<BAccount>();

		public PXAction<BAccount> RemoveCreditHold;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Remove Credit Hold", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable removeCreditHold(PXAdapter adapter) => adapter.Get<BAccount>();

		public PXAction<BAccount> Deactivate;
		...
		protected virtual IEnumerable deactivate(PXAdapter adapter) => adapter.Get<BAccount>();

		public PXAction<BAccount> SetAsOneTime;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Set as One-Time Customer", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable setAsOneTime(PXAdapter adapter) => adapter.Get<BAccount>();

		public PXAction<BAccount> SetAsRegular;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Set as Regular Customer", MapViewRights = PXCacheRights.Select, MapEnableRights = PXCacheRights.Update)]
		protected virtual IEnumerable setAsRegular(PXAdapter adapter) => adapter.Get<BAccount>();
		...
		public class ExtendToCustomer : ExtendToCustomerGraph<BusinessAccountMaint, BAccount>
		{
			...
			public override void Initialize()
			{
				...
				Base.Actions.Move(nameof(Base.Action), nameof(this.extendToCustomer));
			}
		...
		public class ExtendToVendor : ExtendToVendorGraph<BusinessAccountMaint, BAccount>
		{
			...
			protected override void _(Events.RowSelected<BAccount> e)
			{
				...
				Base.Action.SetVisible(nameof(ViewVendor), enabled);
				viewVendor.SetVisible(enabled);

				enabled = extendToVendor.GetEnabled();
				Base.Action.SetVisible(nameof(ExtendToVendor), enabled);
		...
		public class CRDuplicateEntitiesForBAccountGraphExt : CRDuplicateEntities<BusinessAccountMaint, BAccount>
		{
							...
							return screen
								.UpdateDefaultFlow(flow =>
								{
									return flow
										...
										{
											states.Update(CustomerStatus.Prospect, state =>
												state.WithActions(actions => { actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate); }));

											states.Update(CustomerStatus.Active, state =>
												state.WithActions(actions => { actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate); }));

											states.Update(CustomerStatus.Hold, state =>
												state.WithActions(actions => { actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate); }));

											states.Update(CustomerStatus.CreditHold, state =>
												state.WithActions(actions => { actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate); }));

											states.Update(CustomerStatus.OneTime, state =>
												state.WithActions(actions => { actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate); }));
										})
										.WithTransitions(transitions =>
										{
											transitions.UpdateGroupFrom(CustomerStatus.Prospect, ts =>
											{
												ts.Add(t => t
													.To(CustomerStatus.Inactive)
													.IsTriggeredOn<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate));
											});

											transitions.UpdateGroupFrom(CustomerStatus.Active, ts =>
											{
												ts.Add(t => t
													.To(CustomerStatus.Inactive)
													.IsTriggeredOn<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate));
											...
											transitions.UpdateGroupFrom(CustomerStatus.Hold, ts =>
											{
												ts.Add(t => t
													.To(CustomerStatus.Inactive)
													.IsTriggeredOn<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate));
											...
											transitions.UpdateGroupFrom(CustomerStatus.CreditHold, ts =>
											{
												ts.Add(t => t
													.To(CustomerStatus.Inactive)
													.IsTriggeredOn<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate));
											...
											transitions.UpdateGroupFrom(CustomerStatus.OneTime, ts =>
											{
												ts.Add(t => t
													.To(CustomerStatus.Inactive)
													.IsTriggeredOn<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate));
								...
								{
									// "Actions" folder
									actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CheckForDuplicates, a => a.InFolder(FolderType.ActionsFolder));
									actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.MarkAsValidated, a => a.InFolder(FolderType.ActionsFolder));
									actions.Add<CRDuplicateEntitiesForBAccountGraphExt>(e => e.CloseAsDuplicate, a => a.InFolder(FolderType.ActionsFolder));
								});
						});
				}
			}
			...
			protected virtual void _(Events.FieldDefaulting<MergeParams, MergeParams.targetEntityID> e)
			{
				...
				e.NewValue = (current.Type == BAccountType.CustomerType || current.Type == BAccountType.VendorType || current.Type == BAccountType.CombinedType) 
							&& current.Status != CustomerStatus.Inactive
							|| duplicates.Count == 0
					? current.BAccountID
					: duplicates
			...
			protected virtual void _(Events.FieldVerifying<MergeParams, MergeParams.targetEntityID> e)
			{
			...
			protected virtual void _(Events.FieldSelecting<MergeParams, MergeParams.targetEntityID> e)
			{
				if (e.Row == null)
					return;
			...
			protected override void _(Events.RowSelected<Extensions.CRDuplicateEntities.Document> e)
			{
				base._(e);
			...
			protected virtual void _(Events.FieldSelecting<CRDuplicateRecord, CRDuplicateRecord.selected> e)
			{
				if (e.Row == null)
					return;
			...
			public override void MergeEntities(PXGraph targetGraph, BAccount targetEntity, BAccount duplicateAccount)
			{
			...
			public override bool IsValidationOnEntryActive()
			{
				return Setup.Current?.ValidateAccountDuplicatesOnEntry ?? false;
			}
			...
			public override void GetAllProperties(List<FieldValue> values, HashSet<string> fieldNames)
			{
				...
				values.AddRange(GetMarkedPropertiesOf<BAccount>(Base, ref order)
					.Where(fld => fld.Name.IsNotIn(nameof(CR.BAccount.Status), nameof(CR.BAccount.VStatus)))
