	...
	public class ChangeOrderEntry : PXGraph<ChangeOrderEntry, PMChangeOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public virtual IEnumerable ViewChangeOrder(PXAdapter adapter)
		{
			if (Document.Current != null && Document.Current.OrigRefNbr != null)
			{
				...
				target.Document.Current = PMChangeOrder.PK.Find(this, Document.Current.OrigRefNbr);
		...
		protected virtual void _(Events.RowSelected<PMChangeOrder> e)
		{
			...
			if (e.Row != null)
			{
				...
				PXUIFieldAttribute.SetEnabled<PMChangeOrderLine.pOOrderType>(Details.Cache, null, IsImport);
				PXUIFieldAttribute.SetVisible<PMChangeOrderLine.pOOrderType>(Details.Cache, null, IsImport);
			}
		}
		...
		public virtual void ValidateOrdersTotal(PMChangeOrder doc)
		{
			...
			foreach (PXResult<PMChangeOrderLine, POLinePM> res in Details.Select())
			{
				...
				if (!ordersTotal.ContainsKey(orderKey))
				{
					...
					{
						using (new PXFieldScope(orderSelect.View, typeof(POOrder.orderType), typeof(POOrder.orderNbr), typeof(POOrder.curyLineTotal)))
						{
							POOrder order = orderSelect.SelectSingle(line.POOrderType, line.POOrderNbr);
							ordersTotal[orderKey] = order.CuryLineTotal ?? 0;
						}
			...
			foreach (var order in ordersTotal)
			{
				if (order.Value < 0)
				{
					...
					{
						string commitnentType = order.Key.Item2 == POOrderType.RegularSubcontract ? Messages.Subcontract : Messages.PurchaseOrder;
						throw new PXException(Messages.CommitmentTotalIsNegative, commitnentType, order.Key.Item3);
					}
				}
			}
		}
		...
		public virtual POOrder CreateNewOrder(POOrder order, List<PMChangeOrderLine> added)
		{
			...
			if (target.Document.Current.Hold == true)
