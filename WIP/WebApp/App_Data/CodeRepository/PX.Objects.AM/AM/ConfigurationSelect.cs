...
namespace PX.Objects.AM
{
    ...
    public class ConfigurationSelect : PXSelect<AMConfigurationResults>, IRuleProcessor, IRuleConditionValidator
    {
        ...
        /// </summary>
        private bool _IsConfigCreation;
...
#pragma warning restore PX1044
        ...
        public bool RunRuleFormula(out string message)
        {
            ...
            return RunRuleFormula(AMRuleFormulaEngine.Run(_Graph, _IsConfigCreation), false, out message);
        }
            ...
            using (new AMRuleFormulaScope(this))
            {
                ...
                try
                {
                    foreach (var option in ret.OptionsToUpdate)
                    {
                        ...
                        //  This will cause a sub query to AMConfigResultsFeature due to actualqty unbound formula. 
                        //  Set cancelcalculation or cancelparentupdate do not work and moving this after feature update also makes no difference. The only way to avoid is removing the formula from the field but we need it.
                ...
                foreach (AMConfiguration config in Configuration.Select())
                {
                    ...
                    using (var scope = new AMRuleFormulaScope(this))
                    {
                        foreach (AMConfigurationAttribute configAttribute in ConfigAttributes.Select())
                        {
                            ...
                            ConfigAttributes.Cache.SetStatus(configAttribute, PXEntryStatus.Held);
                        ...
                        foreach (AMConfigurationFeature configFeature in ConfigFeatures.Select())
                        {
                            ...
                            ConfigFeatures.Cache.SetStatus(configFeature, PXEntryStatus.Held);
                            ...
                            foreach (AMConfigurationOption configOption in ConfigOptions.Select())
                            {
                                ...
                                ConfigOptions.Cache.SetStatus(configOption, PXEntryStatus.Held);
                        ...
                        {
                            ConfigRules.Cache.SetStatus(configRule, PXEntryStatus.Held);
                        ...
                        if (!RunRuleFormula(out var formulaError))
                        {
