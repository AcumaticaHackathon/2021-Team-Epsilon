...
namespace PX.Objects.AM
{
    public class ProductionTransactionHelper
    {
        ...
        public bool HasWIPAdjustmentChanges => WIPAdjustmentGraph != null && WIPAdjustmentGraph.transactions.Cache.IsDirty;
        ...
        public void DeleteLinkedWIPAdjustmentLines(AMMTran tran)
        {
            ...
            foreach(AMMTran scrapTran in WIPAdjustmentGraph.transactions.Cache.Cached.RowCast<AMMTran>()
                .Where(r => r.OrigDocType == tran.DocType && r.OrigBatNbr == tran.BatNbr && r.OrigLineNbr == tran.LineNbr))
            {
                var scrapRowStatus = WIPAdjustmentGraph.transactions.Cache.GetStatus(scrapTran);
                if(scrapRowStatus == PXEntryStatus.Deleted || scrapRowStatus == PXEntryStatus.InsertedDeleted)
                {
                    continue;
                }

                WIPAdjustmentGraph.transactions.Delete(scrapTran);
            }
        }
