...
namespace PX.Objects.GL
{
	...
	public class JournalWithSubEntry : PXGraph<JournalWithSubEntry, GLDocBatch>, PXImportAttribute.IPXPrepareItems
	{
		...
		#region Buttons
		public PXAutoAction<GLDocBatch> initializeState;
		...
		protected virtual void GLDocBatch_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			else
			{
				...
				bool hasBaccountIDorAccounts = false;
				bool hasRows = false;
				bool isPartiallyReleased = false;
				foreach (GLTranDoc iDet in this.GLTranModuleBatNbr.Select())
				{
					hasRows = true;
					if (!hasBaccountIDorAccounts && (iDet.BAccountID.HasValue || iDet.DebitAccountID.HasValue || iDet.CreditAccountID.HasValue))
					{
						hasBaccountIDorAccounts = true;
					}
					if (iDet.DocCreated == true)
					{
						isPartiallyReleased = true;
					}
					if (hasBaccountIDorAccounts && isPartiallyReleased) break;
				}
				PXUIFieldAttribute.SetEnabled<GLDocBatch.curyID>(cache, batch, !hasBaccountIDorAccounts);
				PXUIFieldAttribute.SetEnabled<GLDocBatch.branchID>(cache, batch, !hasRows);
		...
		protected virtual int? FindDefaultARAccount(GLTranDoc aRow, bool isCredit, Location location)
		{
			switch (aRow.TranType)
			{
				...
				case AR.ARInvoiceType.CashSale:
					if (isCredit == false)
					{
						CashAccount cashAccount = FindDefaultARCashAccount(aRow.PaymentMethodID, aRow.PMInstanceID, aRow.CuryID);
						if (cashAccount != null)
						{
							this.SetCashAccount(cashAccount, isCredit);
							return cashAccount.AccountID;
		...
		{
			return FindDefaultARCashAccount(aPaymentMethodID, aPMInstanceID, BatchModule.Current.CuryID);
		}

		protected virtual CashAccount FindDefaultARCashAccount(string aPaymentMethodID, int? aPMInstanceID, string curyID)
		{
			...
			if (cashAcct == null)
			{
				cashAcct = PXSelectJoin<CashAccount,
											...
											And<CashAccount.curyID, Equal<Required<Currency.curyID>>,
											And<PaymentMethodAccount.useForAR, Equal<True>,
												And<PaymentMethod.useForAR, Equal<True>>>>>, OrderBy<Desc<PaymentMethodAccount.aRIsDefault>>>.Select(this, aPaymentMethodID, curyID);
			}
			return cashAcct;
		}
		#endregion
	...
	public class GLBatchDocRelease : PXGraph<GLBatchDocRelease>
	{
		...
		protected virtual PXGraph CreateAP(GLTranDoc doc, List<GLTranDoc> aList, List<Batch> toPost)
		{
			...
			if (result == null && JournalWithSubEntry.IsAPPayment(doc))
			{
				...
				//Update current hold state for approval process.
				qcGraph.Document.Current.Hold = true;
		...
		private static void ClearEmployeeIDField(APRegister apRegister, PXGraph qcGraph)
		{
			...
			{
				var ePEmployeeEPCompanyTreeMemberResult = PXSelectJoin<CREmployee,
					LeftJoin<TM.EPCompanyTreeMember,
						On<TM.EPCompanyTreeMember.contactID, Equal<CREmployee.defContactID>>>,
					Where<TM.EPCompanyTreeMember.contactID, Equal<Required<APInvoice.employeeID>>>>
				...
				foreach (PXResult<CREmployee, TM.EPCompanyTreeMember> item in ePEmployeeEPCompanyTreeMemberResult)
				{
		...
		protected virtual PXGraph CreateAR(GLTranDoc doc, List<GLTranDoc> aList, List<Batch> toPost)
		{
			...
			if (result == null && JournalWithSubEntry.IsARPayment(doc))
			{
				...
				var updInvoices = qcGraph.ARInvoice_DocType_RefNbr.Cache.Updated.Cast<ARInvoice>().ToList();

				qcGraph.Save.Press();

				foreach (ARInvoice updInvoice in updInvoices)
					PXTimeStampScope.DuplicatePersisted(this.Caches<ARRegister>(), updInvoice, typeof(ARInvoice));
		...
		protected virtual PXGraph CreateCA(GLTranDoc doc, List<GLTranDoc> aList, List<Batch> toPost)
		{
			...
			if (doc.TranModule == GL.BatchModule.CA && doc.TranType == CA.CATranType.CAAdjustment)
			{
				...
				foreach (CAAdj it in caGraph.releaseFromHold.Press(new PXAdapter(caGraph.CurrentDocument, row))) { }
				caGraph.Save.Press();
				doc.RefNbr = row.RefNbr;
				doc.DocCreated = true;
				result = caGraph;
			...
			if (releaseList.Count > 0)
			{
				...
				foreach (CAAdj iRes in releaseList)
				{
					CATrxRelease.ReleaseDoc<CAAdj>(iRes, count, toPost,
						CAAdj.Events.Select(ev=>ev.ReleaseDocument));
					count++;
				}
				doc.Released = true;
			}
