...
namespace PX.Objects.CS
{
	public class RMReportReaderGL : PXGraphExtension<RMReportReader>
    {
		...
		private void LoadDrillDownHistory(int ledgerID, int year, string accountClassID)
        {
			...
			{
				glHistorySelect.WhereAnd<Where<GLHistoryByPeriod.accountClassID, Equal<Required<GLHistoryByPeriod.accountClassID>>>>();
				glHistorySelectParameters.Add(accountClassID);
			}

			// 1. Get all GLHistory records for the year
			BqlCommand yearHistorySelect = glHistorySelect.View.BqlSelect.WhereAnd<
				Where<GLHistoryByPeriod.lastActivityPeriod, Like<Required<GLHistory.finPeriodID>>,
					And<GLHistoryByPeriod.finPeriodID, GreaterEqual<Required<GLHistory.finPeriodID>>,
        ...
        private List<GLHistory> GetPeriodsToCalculate(
        ...
        private object CalculateAndExpandValue(bool drilldown, RMDataSource ds, RMDataSourceGL dsGL, ARmDataSet dataSet, List<Account> accounts, List<Sub> subs, List<Branch> branchList, List<object[]> splitret)
        {
	        ...
	        Parallel.For(0, accounts.Count, options, account =>
            {
				...
				if (!_glhistoryPeriodsNested.TryGetValue(currentAccount.AccountID.Value, out var accountDict)) return;
				...
				Parallel.For(0, subs.Count, options, sub =>
                {
					...
					if (!accountDict.TryGetValue(currentSub.SubID.Value, out var subDict)) return;
                    ...
                    {
						List<GLHistory> periods = null;
                        bool takeLast = false;
						if(subDict.TryGetValue((currentBranch.BranchID.Value, dsGL.LedgerID.Value), out var periodsForKey))
							periods = GetPeriodsToCalculate(periodsForKey, currentAccount, ds, dsGL, out takeLast);
