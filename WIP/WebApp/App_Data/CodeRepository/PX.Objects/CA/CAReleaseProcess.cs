	...
	public class CAReleaseProcess : PXGraph<CAReleaseProcess>
	{
		...
		public PXSelectJoin<CATran,
				...
				Where<CATran.origModule, Equal<BatchModule.moduleCA>,
					And<CATran.origTranType, In3<CATranType.cATransfer, CATranType.cATransferOut, CATranType.cATransferIn>,
					And<CATran.origRefNbr, Equal<Required<CATran.origRefNbr>>,
					And<CATran.released, Equal<boolFalse>>>>>,
				OrderBy<Asc<CATran.tranID>>> CATranFundsTransfers_Ordered;
		...
		public PXSelectJoin<CATran,
					...
					InnerJoin<CurrencyInfo, On<CurrencyInfo.curyInfoID, Equal<CATran.curyInfoID>>>>>,
				Where<CATran.origModule, Equal<BatchModule.moduleCA>,
					And<CATran.origTranType, Equal<CATranType.cAAdjustment>,
					And<CATran.origRefNbr, Equal<Required<CATran.origRefNbr>>,
					And<CATran.released, Equal<boolFalse>>>>>,
				OrderBy<Asc<CATran.tranID>>> CATranCashTrans_Ordered;
		...
		public PXSelectJoin<CATaxTran,
				InnerJoin<Tax, On<Tax.taxID, Equal<CATaxTran.taxID>>>,
				Where<CATaxTran.module, Equal<BatchModule.moduleCA>,
					And<CATaxTran.tranType, Equal<Required<CATaxTran.tranType>>,
					And<CATaxTran.refNbr, Equal<Required<CATaxTran.refNbr>>>>>,
				OrderBy<Asc<Tax.taxCalcLevel>>> CATaxTran_TranType_RefNbr;

		public PXSelectJoin<CAExpenseTaxTran, 
				InnerJoin<Tax, On<Tax.taxID, Equal<CAExpenseTaxTran.taxID>>>,
				Where<CAExpenseTaxTran.module, Equal<BatchModule.moduleCA>,
					And<CAExpenseTaxTran.tranType, Equal<CATranType.cATransferExp>,
					And<CAExpenseTaxTran.refNbr, Equal<Required<CAExpenseTaxTran.refNbr>>,
					And<CAExpenseTaxTran.lineNbr, Equal<Required<CAExpenseTaxTran.lineNbr>>>>>>,
				OrderBy<Asc<Tax.taxCalcLevel>>> CAExpenseTaxTran_TranType_RefNbr;
		...
		public PXSelect<Sub, Where<Sub.subID, Equal<Required<CashAccount.subID>>>> subAccounts;
		public PXSelect<Account, Where<Account.accountID, Equal<Required<CashAccount.accountID>>>> glAccounts;
		...
		protected virtual PXResultset<CAExpenseTax> GetDeductibleLines(Tax salestax, CAExpenseTaxTran x)
		{
			return PXSelect<CAExpenseTax, 
				Where<CAExpenseTax.taxID, Equal<Required<CAExpenseTaxTran.taxID>>, 
					And<CAExpenseTax.tranType, Equal<Required<CAExpenseTaxTran.tranType>>,
					And<CAExpenseTax.refNbr, Equal<Required<CAExpenseTaxTran.refNbr>>,
					And<CAExpenseTax.lineNbr, Equal<Required<CAExpenseTaxTran.lineNbr>>>>>>,
				OrderBy<Desc<CAExpenseTaxTran.curyTaxAmt>>>.Select(this, salestax.TaxID, x.TranType, x.RefNbr, x.LineNbr);
		}

		public virtual IEnumerable<Batch> ReleaseDocProc<TCADocument>(JournalEntry je, 
			ref List<Batch> batchlist,
			TCADocument doc,
			...
			where TCADocument : class, ICADocument, IBqlTable, new()
		{
			...
			VerifyActivityOfCashInTransitAccountAndSubaccount();
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				PXResultset<CATran> resultset = null;
				...
				switch (doc.DocType)
				{
					...
					case (CATranType.CATransferOut):
						resultset = CATranFundsTransfers_Ordered.Select(doc.RefNbr); break;
					case (CATranType.CAAdjustment):
						resultset = CATranCashTrans_Ordered.Select(doc.RefNbr); break;
					default:
				...
				foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo> res in resultset)
				{
					...
					if (doc.DocType == CATranType.CAAdjustment)
					{
						caadj = CAAdj.PK.Find(this, catran.OrigTranType,catran.OrigRefNbr);
					}
					...
					VerifyActivityOfAccountAndSubaccount(cashacct);
					...
					if (shouldSegregate)
					{
						string periodID = FinPeriodRepository.FindFinPeriodIDByMasterPeriodID(PXAccess.GetParentOrganizationID(branchID), catran.TranPeriodID);
						SegregateBatch(je, batchlist, branchID, catran.CuryID, catran.TranDate, periodID, catran.TranDesc, info);
					}
					...
					if (caadj.AdjTranType == CATranType.CAAdjustment && Math.Abs(Math.Round((decimal)(batch.CuryDebitTotal - batch.CuryCreditTotal), 4)) >= 0.00005m)
					{
						...
						if (roundDiff > CurrencyCollection.GetCurrency(je.currencyInfo.BaseCuryID).RoundingLimit)
						{
							throw new PXException(AP.Messages.RoundingAmountTooBig, je.currencyinfo.Current.BaseCuryID, roundDiff,
								PXDBQuantityAttribute.Round(CurrencyCollection.GetCurrency(je.currencyInfo.BaseCuryID).RoundingLimit));
						}
						GLTran tran = new GLTran();
						tran.SummPost = true;
						tran.BranchID = caadj.BranchID;
		...
		{
			Account account = glAccounts.Select(cashacct.AccountID);
			if (account?.Active == false)
			{
				throw new PXException(Messages.CashAccountUsesInactiveGLAccount, cashacct.CashAccountCD, account.AccountCD);
			}

			Sub sub = subAccounts.Select(cashacct.SubID);
			if (sub?.Active == false)
			{
				string subCD = subAccounts.Cache.GetFormatedMaskField<Sub.subCD>(sub);
				throw new PXException(Messages.CashAccountUsesInactiveSub, cashacct.CashAccountCD, subCD);
			}
		}

		private void VerifyActivityOfCashInTransitAccountAndSubaccount()
		{
			Account cashInTransitAccount = glAccounts.Select(casetup.Current.TransitAcctId);
			if (cashInTransitAccount?.Active == false)
			{
				throw new PXException(Messages.CashInTransitGLAccountIsInactive, cashInTransitAccount.AccountCD);
			}

			Sub cashInTransitSub = subAccounts.Select(casetup.Current.TransitSubID);
			if (cashInTransitSub?.Active == false)
			{
				string subCD = subAccounts.Cache.GetFormatedMaskField<Sub.subCD>(cashInTransitSub);
				throw new PXException(Messages.CashInTransitSubIsInactive, subCD);
			}
		}
			...
			where TCADocument : class, ICADocument, new()
		{
			...
			foreach (PXResult<CAExpense> result in CAExpense_RefNbr.Select(doc.RefNbr))
			{
				CAExpense charge = result;
				...
				GenerateGLTranForExpenseTaxes(je, charge);
		...
		{
			foreach (PXResult<CAExpenseTaxTran, Tax> r in CAExpenseTaxTran_TranType_RefNbr.Select(expense.RefNbr, expense.LineNbr))
			{
				CAExpenseTaxTran x = r;
				Tax salestax = r;
				...
				if (expense.DrCr == DrCr.Credit && (salestax.TaxType == CSTaxType.Use || salestax.TaxType == CSTaxType.Sales))
				{
					PostGeneralTax(je, expense, x, salestax, salestax.ExpenseAccountID, salestax.ExpenseSubID);

					if (salestax.TaxType == CSTaxType.Use)
					{
						PostReverseTax(je, expense, x, salestax);
					}
				}
				else
				{
					if (salestax.ReverseTax != true)
					{
						PostGeneralTax(je, expense, x, salestax);
					}
					else
					{
						PostReverseTax(je, expense, x, salestax);
					...
					if (salestax.DeductibleVAT == true)
					{
						if (salestax.ReportExpenseToSingleAccount == true)
						{
							GLTran tran = new GLTran();
							tran.SummPost = false;
							tran.BranchID = expense.BranchID;
							tran.CuryInfoID = expense.CuryInfoID;
							tran.TranType = CATranType.CATransferExp;
							tran.TranClass = GLTran.tranClass.Tax;
							tran.RefNbr = expense.RefNbr;
							tran.TranDate = expense.TranDate;
							tran.AccountID = salestax.ExpenseAccountID;
							tran.SubID = salestax.ExpenseSubID;
							tran.TranDesc = salestax.TaxID;
							bool CR = (expense.DrCr == DrCr.Credit);
							tran.CuryDebitAmt = CR ? x.CuryExpenseAmt : 0m;
							tran.DebitAmt = CR ? x.ExpenseAmt : 0m;
							tran.CuryCreditAmt = CR ? 0m : x.CuryExpenseAmt;
							tran.CreditAmt = CR ? 0m : x.ExpenseAmt;
						...
						{
							PXResultset<CAExpenseTax> deductibleLines = GetDeductibleLines(salestax, x);

							APTaxAttribute apTaxAttr = new APTaxAttribute(typeof(CARegister), typeof(CAExpenseTax), typeof(CAExpenseTaxTran));
							apTaxAttr.DistributeTaxDiscrepancy<CAExpenseTax, CAExpenseTax.curyExpenseAmt, CAExpenseTax.expenseAmt>(this, deductibleLines.FirstTableItems, x.CuryExpenseAmt.Value);

							foreach (PXResult<CAExpenseTax, CASplit> item in deductibleLines)
							{
								CAExpenseTax taxLine = (CAExpenseTax)item;
								CASplit split = (CASplit)item;
		...
		protected virtual void PostGeneralTax(JournalEntry je, CAExpense expense, CAExpenseTaxTran x, Tax salestax, int? accountID = null, int? subID = null)
		{
			bool isCredit = expense.DrCr == DrCr.Credit;
			GLTran taxTran = GenerateGLTran(expense, x, salestax, accountID, subID);
			SetAmounts(x, taxTran, isCredit, true);

			je.GLTranModuleBatNbr.Insert(taxTran);
		}

		protected virtual void PostReverseTax(JournalEntry je, CAExpense expense, CAExpenseTaxTran x, Tax salestax)
		{
			bool isCredit = expense.DrCr == DrCr.Credit;
			GLTran taxTran = GenerateGLTran(expense, x, salestax, null, null);
			SetAmounts(x, taxTran, isCredit, false);

			je.GLTranModuleBatNbr.Insert(taxTran);
		}

		private static GLTran GenerateGLTran(CAExpense expense, CAExpenseTaxTran x, Tax salestax, int? accountID = null, int? subID = null)
		{
			GLTran taxTran = new GLTran();
			taxTran.SummPost = false;
			taxTran.CuryInfoID = expense.CuryInfoID;
			taxTran.TranType = CATranType.CATransferExp;
			taxTran.TranClass = GLTran.tranClass.Tax;
			taxTran.RefNbr = expense.RefNbr;
			taxTran.TranDate = expense.TranDate;
			taxTran.AccountID = accountID ?? x.AccountID;
			taxTran.SubID = subID ?? x.SubID;
			taxTran.TranDesc = salestax.TaxID;
			...
			taxTran.Released = true;
			taxTran.ReferenceID = null;
			taxTran.BranchID = expense.BranchID;
			taxTran.ProjectID = PM.ProjectDefaultAttribute.NonProject();
			return taxTran;
		}

		private static void SetAmounts(CAExpenseTaxTran x, GLTran taxTran, bool isCredit, bool isDirectPosting)
		{
			if (isDirectPosting)
			{
				taxTran.CuryDebitAmt = isCredit ? x.CuryTaxAmt : 0m;
				taxTran.DebitAmt = isCredit ? x.TaxAmt : 0m;
				taxTran.CuryCreditAmt = isCredit ? 0m : x.CuryTaxAmt;
				taxTran.CreditAmt = isCredit ? 0m : x.TaxAmt;
			}
			else
			{
				taxTran.CuryDebitAmt = isCredit ? 0m : x.CuryTaxAmt;
				taxTran.DebitAmt = isCredit ? 0m : x.TaxAmt;
				taxTran.CuryCreditAmt = isCredit ? x.CuryTaxAmt : 0m;
				taxTran.CreditAmt = isCredit ? x.TaxAmt : 0m;
			}
		}
		...
		protected virtual void GenerateGLTranPairForExpense(JournalEntry je, CAExpense charge)
		{
			...
			VerifyActivityOfExpenseAccountAndSub(charge, expCashcacount);
...

			{
				...
				var taxTran = new CAExpenseTaxTran { CuryTaxAmt = 0m, TaxAmt = 0m };
				foreach(PXResult<CAExpenseTaxTran, Tax> result in CAExpenseTaxTran_TranType_RefNbr.Select(charge.RefNbr, charge.LineNbr))
				{
					CAExpenseTaxTran iTaxTran = result;
					Tax iTax = result;

					taxTran.CuryTaxAmt += iTaxTran.CuryTaxAmt + (iTax.DeductibleVAT == true ? iTaxTran.CuryExpenseAmt : 0m);
					taxTran.TaxAmt += iTaxTran.TaxAmt + (iTax.DeductibleVAT == true ? iTaxTran.ExpenseAmt : 0m);
					taxTran.CuryTaxableAmt = iTaxTran.CuryTaxableAmt;
				}

				if (charge.TaxCategoryID != null && charge.TaxZoneID != null && taxTran?.CuryTaxableAmt != null && Math.Abs(taxTran.CuryTaxableAmt.Value) < Math.Abs(charge.CuryTranAmt.Value))
				{
					var curyTaxableAmount = charge.CuryTranAmt - taxTran.CuryTaxAmt;
					var taxableAmount = charge.TranAmt - taxTran.TaxAmt;

					tran.CuryDebitAmt = isCADebit ? 0m : curyTaxableAmount;
					tran.DebitAmt = isCADebit ? 0m : taxableAmount;
					tran.CuryCreditAmt = isCADebit ? curyTaxableAmount : 0m;
					tran.CreditAmt = isCADebit ? taxableAmount : 0m;
				}
				else
				{
		...
		protected virtual void VerifyActivityOfExpenseAccountAndSub(CAExpense charge, CashAccount expCashcacount)
		{
			Account accountFromCashAccount = glAccounts.Select(expCashcacount.AccountID);
			if (accountFromCashAccount?.Active == false)
			{
				throw new PXException(Messages.CashAccountUsesInactiveGLAccount, expCashcacount.CashAccountCD, accountFromCashAccount.AccountCD);
			}
			Account offsetAccount = glAccounts.Select(charge.AccountID);
			if (offsetAccount?.Active == false)
			{
				throw new PXException(Messages.OffsetInactiveAccount, offsetAccount.AccountCD);
			}

			Sub cashInTransitSub = subAccounts.Select(expCashcacount.SubID);
			if (cashInTransitSub?.Active == false)
			{
				string cashInTransitSubCD = subAccounts.Cache.GetFormatedMaskField<Sub.subCD>(cashInTransitSub);
				throw new PXException(Messages.CashAccountUsesInactiveSub, expCashcacount.CashAccountCD, cashInTransitSubCD);
			}
			Sub offsetSub = subAccounts.Select(charge.SubID);
			if (offsetSub?.Active == false)
			{
				string offsetSubCD = subAccounts.Cache.GetFormatedMaskField<Sub.subCD>(offsetSub);
				throw new PXException(Messages.OffsetInactiveSub, offsetSubCD);
			}
		}

		protected virtual void AddRoundingTran(JournalEntry je, ICADocument doc, Batch batch, Currency cury)
		{
		...
		public virtual void ReleaseDeposit(JournalEntry je, ref List<Batch> batchlist, CADeposit doc)
		{
			...
			PXSelectBase<CATran> select = new PXSelectJoin<CATran,
									...
									LeftJoin<CADepositEntry.CAAdjUpdate, On<CADepositEntry.CAAdjUpdate.adjTranType, Equal<CADepositDetail.origDocType>,
													And<CADepositEntry.CAAdjUpdate.adjRefNbr, Equal<CADepositDetail.origRefNbr>,
													And<CADepositDetail.origModule, Equal<GL.BatchModule.moduleCA>>>>,
									LeftJoin<CCBatch, On<CCBatch.depositType, Equal<CADepositDetail.tranType>,
													And<CCBatch.depositNbr, Equal<CADepositDetail.refNbr>>>>>>>>>>>,
									Where<CATran.origModule, Equal<BatchModule.moduleCA>,
									And<CATran.origTranType, Equal<Required<CATran.origTranType>>,
									And<CATran.origRefNbr, Equal<Required<CATran.origRefNbr>>>>>,
			...
			foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo, CADepositDetail, CADepositEntry.ARPaymentUpdate, CADepositEntry.APPaymentUpdate, CADepositEntry.CAAdjUpdate, CCBatch> res in resultset)
			{
				CATran catran = (CATran)res;
				CashAccount cashacct = (CashAccount)res;
				cury = (Currency)res;
				...
				CADepositEntry.CAAdjUpdate caDoc = (CADepositEntry.CAAdjUpdate)res;
				CCBatch ccBatchDoc = res;
				...
				if (object.Equals(prev_tran, catran) == false)
				{
					...
					if (!String.IsNullOrEmpty(caDoc.AdjRefNbr))
					{
						if (doc.TranType == CATranType.CADeposit)
						{
							caDoc.Deposited = true;
						}
						else
						{
							caDoc.Deposited = false;
							caDoc.DepositType = null;
							caDoc.DepositNbr = null;
							caDoc.DepositDate = null;
						}
						this.Caches[typeof(CADepositEntry.CAAdjUpdate)].Update(caDoc);
					}
					...
					{
						ccBatchDoc.Status = CCBatchStatusCode.Deposited;
						this.Caches[typeof(CCBatch)].Update(ccBatchDoc);
					}
				}
				prev_tran = catran;
				...
				if (batch != null)
				{
					...
					if (doc.TranType == CATranType.CAVoidDeposit)
					{
						...
						CCBatch ccBatchDoc = PXSelect<CCBatch, Where<CCBatch.depositType, Equal<Required<CCBatch.depositType>>,
							And<CCBatch.depositNbr, Equal<Required<CCBatch.depositNbr>>>>>.Select(this, orig.TranType, orig.RefNbr);
						if (ccBatchDoc?.BatchID != null)
						{
							ccBatchDoc.Status = CCBatchStatusCode.Processed;
							Caches[typeof(CCBatch)].Update(ccBatchDoc);
						}
					}
				}
				Actions.PressSave();
