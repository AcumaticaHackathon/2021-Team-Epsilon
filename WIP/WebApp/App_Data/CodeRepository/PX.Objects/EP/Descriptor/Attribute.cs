...
namespace PX.TM
{
	...
	public class OwnerAttribute : AcctSub2Attribute, IPXFieldUpdatingSubscriber
	{
		#region State
		...
		[PXProjection(typeof(
			...
			LeftJoin<PX.Objects.CR.Standalone.EPEmployee,
				On<PX.Objects.CR.Standalone.EPEmployee.defContactID, Equal<Contact.contactID>>>>), Persistent = false)]
		...
		public OwnerAttribute(Type workgroupType, Type search, bool validateValue = true, bool inquiryMode = false, Type[] fieldList = null, string[] headerList = null)
		{
		...
		public virtual void FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			...
			if (PXMacroManager.Default.IsVariable(val) && val == "@me")
			{
...
namespace PX.Objects.EP
{
        ...
        where Rejected : class, IBqlField
    {
        ...
        public virtual bool ValidateAccess(int? workgroup, int? ownerID)
        {
			...
			if (PXAccess.GetContactID() == ownerID) return true;
    ...
    public class PXWeekSelector2Attribute : PXWeekSelectorAttribute
    {
        public class FullWeekList
        {
            ...
            private class Definition : IPrefetchable
            {
                ...
                public void Prefetch()
                {
                    ...
                    _weeks = _weeks.Distinct(x => x.WeekID).OrderBy(x => x.WeekID).ToList();
                }
        ...
        protected override IEnumerable GetAllRecords()
        {
            if (IsCustomWeek(_Graph))
            {
				...
				List<EPWeekRaw> allWeeks = FullWeekList.Weeks().Where(x => x.WeekID < setup.FirstCustomWeekID).ToList();
