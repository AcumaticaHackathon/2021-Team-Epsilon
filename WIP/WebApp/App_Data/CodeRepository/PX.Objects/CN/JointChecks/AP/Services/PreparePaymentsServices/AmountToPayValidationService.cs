...
namespace PX.Objects.CN.JointChecks.AP.Services.PreparePaymentsServices
{
	public class AmountToPayValidationService
	{
		...
		private APAdjust Adjustment => ApPayBillsExt.GetSelectedAdjustmentLine();
		...
		{
			var adjustmentLine = ApPayBillsExt.GetSelectedAdjustmentLine();
			...
			var totalAmountToPay = amountToPay + amountToPayCalculationService.GetJointAmountToPay(
									   adjustmentLine.AdjdLineNbr);
			var invoiceBalance = invoiceBalanceCalculationService.GetInvoiceBalance(adjustmentLine);
