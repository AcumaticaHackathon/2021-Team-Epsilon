...
namespace PX.Objects.AM
{
    ...
    public class ECOMaint : PXGraph<ECOMaint, AMECOItem>
    {
        ...
        [PXHidden]
        public PXSelect<INItemSite, Where<INItemSite.inventoryID, Equal<Current<AMECOItem.inventoryID>>,
            And<INItemSite.siteID, Equal<Current<AMECOItem.siteID>>>>> ItemSiteRecord;
        ...
        public PXAutoAction<AMECOItem> initializeState;
        ...
        public PXAction<AMECOItem> ActionDropMenu;
        ...
        public PXAction<AMECOItem> BOMCompare;
        ...
        public virtual IEnumerable bOMCompare(PXAdapter adapter)
        {
            ...
            if (item != null)
            {
                ...
                graph.Filter.Current.ECOID1 = item.ECOID;
                graph.Filter.Current.BOMID1 = item.ECOID;
                graph.Filter.Current.RevisionID1 = AMECOItem.ECORev;
        ...
        protected virtual void AMECOItem_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            Approval.AllowSelect = ambsetup?.Current?.ECORequestApproval == true;

            var item = (AMECOItem)e.Row;
            if (item == null)
            {
                return;
            }

            EnableECOItemFields(sender, item);
            ...
            commitChanges.SetEnabled(item.Approved.GetValueOrDefault());
        }

        protected virtual void AMECOItem_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
        {
            var row = (AMECOItem)e.Row;
            if (row != null)
            {
                UpdateECRStatus(row.ECOID, AMECRStatus.Approved);
            }
        }
        ...
        protected virtual void EnableECOItemFields(PXCache cache, AMECOItem item)
        {
            ...
            var isCompleted = item.Status == AMECRStatus.Completed;
        ...
        /// </summary>
        /// <param name="amECOItem">BOM containing item/site ids</param>
        protected virtual void DefaultItemSiteByBOM(AMECOItem amECOItem)
        {
            if (amECOItem?.InventoryID == null
                || amECOItem.SiteID == null)
            {
                return;
            }

            DefaultItemSite(amECOItem.InventoryID, amECOItem.SiteID);
        }
        ...
        {
            foreach (AMECOItem amECOItem in this.Documents.Cache.Inserted)
            {
                DefaultItemSiteByBOM(amECOItem);
            }
