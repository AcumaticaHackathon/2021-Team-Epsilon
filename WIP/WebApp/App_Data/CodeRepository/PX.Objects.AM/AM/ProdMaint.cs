...
namespace PX.Objects.AM
{
    ...
    public class ProdMaint : PXGraph<ProdMaint, AMProdItem>
    {
        ...
        #region Buttons
        public PXAutoAction<AMProdItem> initializeState;
        ...
        {
            if (string.IsNullOrWhiteSpace(ProdItemSelected.Current.OrdNbr))
            {
                if (ProdItemSelected.Current != null
                    && LinkSOLineRecords.AskExt() == WebDialogResult.OK)
                {
                    CreateInstance<SOOrderEntry>()?.GetExtension<SOOrderEntryAMExtension>()?.LinkProductionOrders(this, GetFirstSelectedSOLine(), ProdItemSelected.Current);
            ...
            else
            {
                SOLine soLine = PXSelect<SOLine, Where<SOLine.orderType, Equal<Required<SOLine.orderType>>,
                                            ...
                                            >>>.Select(this, ProdItemSelected.Current.OrdTypeRef, ProdItemSelected.Current.OrdNbr, ProdItemSelected.Current.OrdLineRef);
                ...
                var userMessage = Messages.GetLocal(Messages.ConfirmRemovalOfSalesOrderLink,
                    ProdItemSelected.Current.OrdTypeRef, ProdItemSelected.Current.OrdNbr, ProdItemSelected.Current.OrdLineRef);

                WebDialogResult response = ProdItemSelected.Ask("Confirm", userMessage, MessageButtons.YesNo);

                if (response == WebDialogResult.Yes)
                {
                    RemoveLinkProductionOrders(ProdItemSelected.Current);
                    this.Actions.PressSave();
                }
            }
        ...
        protected virtual void RemoveLinkProductionOrders(AMProdItem prodItem)
        {
            ...
            PXResult<AMProdItemSplit, INItemPlan> linkResult =
                (PXResult<AMProdItemSplit, INItemPlan>)
                PXSelectJoin<AMProdItemSplit,
                    InnerJoin<INItemPlan,
                        On<AMProdItemSplit.planID, Equal<INItemPlan.planID>>>,
                    Where<AMProdItemSplit.orderType, Equal<Required<AMProdItemSplit.orderType>>,
                        And<AMProdItemSplit.prodOrdID, Equal<Required<AMProdItemSplit.prodOrdID>>>>>.Select(this, prodItem.OrderType, prodItem.ProdOrdID);

            var prodItemSplit = (AMProdItemSplit)linkResult;
            var prodItemPlan = (INItemPlan)linkResult;

            if (prodItemPlan?.PlanID == null || prodItemSplit?.PlanID == null)
            {
                return;
            }

            prodItemPlan.PlanType = prodItem.Hold == true
                || prodItem.StatusID == ProductionOrderStatus.Planned ? INPlanConstants.PlanM1 : INPlanConstants.PlanM2;
            prodItemPlan.BAccountID = null;
            this.Caches[typeof(INItemPlan)].Update(prodItemPlan);

            PXResult<INItemPlan, SOLineSplit, SOLine> linkResult2 =
                (PXResult<INItemPlan, SOLineSplit, SOLine>)
                PXSelectJoin<INItemPlan,
                    ...
                    Where<INItemPlan.supplyPlanID, Equal<Required<INItemPlan.supplyPlanID>>>>.Select(this, prodItemPlan.PlanID);

            var soSplitPlan = (INItemPlan)linkResult2;
            var soLineSplit = (SOLineSplit)linkResult2;
            var soLine = (SOLine)linkResult2;

            if (soSplitPlan?.PlanType == null || soLineSplit?.PlanID == null || soLine == null)
            {
                return;
            }
            ...
            if (prodEvent != null)
            {
                prodEvent.RefNoteID = soSplitPlan.RefNoteID;
