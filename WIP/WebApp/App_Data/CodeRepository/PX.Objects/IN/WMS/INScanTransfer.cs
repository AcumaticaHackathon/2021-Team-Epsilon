...
namespace PX.Objects.IN
{
	...
	public partial class INScanTransfer : WMSBase
	{
		...
		/// <summary>
		/// Overrides <see cref="INTransferEntry.INTran_LocationID_FieldDefaulting(PXCache, PXFieldDefaultingEventArgs)"/>
		/// </summary>
		[PXOverride]
		public virtual void INTran_LocationID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs args, PXFieldDefaulting baseHandler) { /*suppress base*/ }

		/// <summary>
		/// Overrides <see cref="INTransferEntry.INTran_ToLocationID_FieldDefaulting(PXCache, PXFieldDefaultingEventArgs)"/>
		/// </summary>
		[PXOverride]
		public virtual void INTran_ToLocationID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs args, PXFieldDefaulting baseHandler) { /*suppress base*/ }
		#endregion

		protected override bool DocumentLoaded => Transfer != null;
		protected override bool DocumentIsEditable => base.DocumentIsEditable && INRegister.PK.Find(Base, Transfer)?.Released != true;
		...
		protected virtual void ProcessConfirm()
		{
			...
			if (!ValidateConfirmation())
			{
				...
				{
					ClearHeaderInfo();
					SetScanState(header.PrevScanState);
				}
				return;
			}
			...
			if (Base.transfer.Cache.GetStatus(Base.transfer.Current) != PXEntryStatus.Inserted)
				SaveChanges();
		...
		protected virtual void VerifyConfirmedLine(WMSLineVerifyingArguments<Header, INTran> args)
		{
			...
			var uiErrors = GetUIErrors(args.Line);
			if (uiErrors.Any())
			{
				args.ErrorInfo = new PXExceptionInfo(uiErrors.First().Value);
				args.Cancel = true;
				return;
			}
