...
{
	public class ContactMaint : PXGraph<ContactMaint, Contact, Contact.displayName>, ICaptionable<object>
	{
		#region Inner Types
		[Serializable]
        [PXHidden]
		...
		{
			Contact currentItem = item as Contact;
		...
		public PXAction<Contact> Activate;
		...
		protected virtual IEnumerable activate(PXAdapter adapter) => adapter.Get<Contact>();

		public PXAction<Contact> Deactivate;
		...
		protected virtual IEnumerable deactivate(PXAdapter adapter) => adapter.Get<Contact>();
		...
		[PXRestrictor(typeof(Where<Current<Contact.contactType>, NotEqual<ContactTypesAttribute.employee>, 
			...
			Or<BAccount.type, Equal<BAccountType.organizationType>>>>), Messages.BAccountIsType, typeof(BAccountCRM.type))]
		[PXRestrictor(typeof(Where<Current<Contact.contactType>, NotEqual<ContactTypesAttribute.person>, 
			...
			Or<BAccount.type, Equal<BAccountType.combinedType>>>>>>), Messages.BAccountIsType, typeof(BAccountCRM.type))]
		[PopupMessage]
		...
		public class CRDuplicateEntitiesForContactGraphExt : CRDuplicateContactEntities<ContactMaint, Contact>
		{
							...
							return screen
								.UpdateDefaultFlow(flow =>
								{
									return flow
										...
										{
											states.Update(ContactStatus.Active, state =>
												state.WithActions(actions => { actions.Add<CRDuplicateEntitiesForContactGraphExt>(e => e.CloseAsDuplicate); }));

											states.Update(ContactStatus.Inactive, state =>
												state.WithActions(actions => { actions.Add<CRDuplicateEntitiesForContactGraphExt>(e => e.CloseAsDuplicate); }));
										})
										.WithTransitions(transitions =>
										{
											transitions.Add(t => t
												...
												.IsTriggeredOn<CRDuplicateEntitiesForContactGraphExt>(e => e.CloseAsDuplicate)
								...
								{
									// "Actions" folder
									actions.Add<CRDuplicateEntitiesForContactGraphExt>(e => e.CheckForDuplicates, a => a.InFolder(FolderType.ActionsFolder));
									actions.Add<CRDuplicateEntitiesForContactGraphExt>(e => e.MarkAsValidated, a => a.InFolder(FolderType.ActionsFolder));
									actions.Add<CRDuplicateEntitiesForContactGraphExt>(e => e.CloseAsDuplicate, a => a.InFolder(FolderType.ActionsFolder));
								});
						});
				}
			}
			...
			public override void MergeEntities(PXGraph targetGraph, Contact targetEntity, Contact duplicateContact)
			{
		...
		public class UpdateRelatedContactInfoFromContactGraphExt : CRUpdateRelatedContactInfoGraphExt<ContactMaint>
		{
			...
			protected bool CheckIsDefAddressIDChanged(PXCache cache, Contact contact)
			{
				...
				IsDefAddressIDChanged = 
					original != null
					&& original.DefAddressID != contact.DefAddressID;
