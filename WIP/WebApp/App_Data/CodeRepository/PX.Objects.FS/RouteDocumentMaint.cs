    ...
    public class RouteDocumentMaint : PXGraph<RouteDocumentMaint, FSRouteDocument>, IGraphWithInitialization
    {
        ...
        public PXSelect<FSAppointmentInRoute,
               Where<
                   ...
                   And<FSAppointment.canceled, NotEqual<False>,
                   And<FSAppointment.closed, NotEqual<False>>>>>,
               OrderBy<
        ...
        {
            return new RouteAppointmentAssignmentHelper().RouteRecordsDelegate(RouteAppAssignmentFilter,
                    ...
                    var fsAppointmentSet = PXSelectJoin<FSAppointment,
                                           ...
                                           Where2<
                                                Where<
                                                    ...
                                                    Or<FSServiceOrder.closed, Equal<True>>>>>,
                                                And<
        ...
        public PXSelect<FSSrvOrdType,
                    Where<
                        FSSrvOrdType.behavior, Equal<FSSrvOrdType.behavior.Values.routeAppointment>>> ServiceOrderTypeRecords;
             ...
             delegate
             {
                 ...
                 using (PXTransactionScope ts = new PXTransactionScope())
                 {
                     ...
                     if (fsAppointmentRows.Count > 0)
                     {
                         ...
                         graphAppointmentEntry.CalculateRouteStats(fsAppointmentRow, SetupRecord.Current.MapApiKey);
                     }
        ...
        public virtual bool CompleteAppointmentsInRoute(ref string errorMessage)
        {
            ...
            if (bqlResultSet.Count > 0)
            {
                ...
                Dictionary<FSAppointment, string> appWithErrors = CompleteAppointments(soGraph, bqlResultSet);
        ...
        private bool CancelAppointmentsInRoute(ref string errorMessage)
        {
            ...
            var appointments = AppointmentsInRoute.Select()
                                                  .Where(x => ((FSAppointment)x).Completed == false
                                                                && ((FSAppointment)x).Closed == false
                                                                && ((FSAppointment)x).Canceled == false)
                                                  .ToList();

            if (appointments.Count() > 0)
            {
                Dictionary<FSAppointment, string> appWithErrors = CancelAppointments(this, appointments);
        ...
        {
            FSRoute fsRouteRow = FSRoute.PK.Find(this, fsRouteDocumentRow.RouteID);
        ...
        public virtual void UpdateAppointmentsTimeBegin(FSRouteDocument fsRouteDocumentRow)
        {
            ...
            foreach (FSAppointment fsAppointmentRow in AppointmentsInRoute.Select())
            {
                if (fsAppointmentRow.Closed == false
                        && fsAppointmentRow.Canceled == false)
                {
            ...
            delegate
            {
                using (PXTransactionScope ts = new PXTransactionScope())
                {
                    ...
                    if (fsAppointmentRows.Count > 0)
                    {
                        ...
                        graphAppointment.SetRouteMapStats(globalAppointmentGraph, fsAppointmentRow, fsRouteDocumentRow.RouteDocumentID, ref fsRouteDocumentRow, SetupRecord.Current.MapApiKey);
                    }
        ...
        /// </summary>
        public virtual void MoveAppointmentInRoute(PXGraph graph, FSRouteDocument fsRouteDocumentRow, int? positionFrom, int? positionTo)
        {
        ...
        public virtual Dictionary<FSAppointment, string> CancelAppointments(PXGraph graph, List<PXResult<FSAppointmentInRoute>> appointments)
        {
            AppointmentEntry appointmentEntryGraph = PXGraph.CreateInstance<AppointmentEntry>();
            ...
            foreach (FSAppointment fsAppointmentRow in appointments)
            {
                ...
                try
                {
                    if (appointmentEntryGraph.AppointmentRecords.Current.InProcess == true)
                    {
        ...
        protected virtual void FSRouteDocument_ActualStartTime_FieldUpdating(PXCache cache, PXFieldUpdatingEventArgs e)
        {
            ...
            if (newTime != null)
            {
                newTime = PXDBDateAndTimeAttribute.CombineDateTime(fsRouteDocumentRow.ActualStartTime, newTime);
            ...
            {
                RouteRecords.Cache.SetValueExt<FSRouteDocument.actualEndTime>(fsRouteDocumentRow, ((DateTime)fsRouteDocumentRow.ActualStartTime).AddHours(1));
            }
            else
            {
