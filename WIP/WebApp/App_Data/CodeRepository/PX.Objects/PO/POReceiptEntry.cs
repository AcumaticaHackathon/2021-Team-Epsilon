...
{   
	...
	public class POReceiptEntry : PXGraph<POReceiptEntry, POReceipt>, IGraphWithInitialization
	{
		...
		public PXSetup<Branch, Where<Branch.bAccountID, Equal<Optional<POReceipt.vendorID>>>> branch;
		...
		public IEnumerable ApDocs()
		{
			...
			else
			{
				statusMsg = PXMessages.LocalizeFormatNoPrefix(Messages.StatusTotalBilledTotalAccrued,
					FormatQty(links.Sum(t => t.TotalQty)), FormatQty(links.Sum(t => t.TotalAmt)), FormatQty(links.Sum(t => t.AccruedQty)),
					FormatQty(links.Sum(t => t.AccruedAmt)));
			}
		...
		public PXAutoAction<POReceipt> initializeState;
		...
		public virtual IEnumerable CreateReturn(PXAdapter adapter)
		{
			...
			POReceipt poReturn = new POReceipt
			{
			...
			poReturn = receiptEntry.Document.Insert(poReturn);

			if (poReturn.CuryID != doc.CuryID)
			{
		...
		{
			return PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>.Select(this, curyInfoID);
		}
		...
		public virtual void CopyReceiptCurrencyInfoToReturn(CurrencyInfo receipt_CuryInfo, CurrencyInfo return_CuryInfo, bool copyCuryRate = true)
		{
			if (receipt_CuryInfo != null && return_CuryInfo != null)
			{
				...
				currencyinfo.Update(return_CuryInfo);
			}
		}
			...
			PXLongOperation.StartOperation(this, () =>
			{
				...
				foreach (POReceipt receipt in adapter.Get<POReceipt>())
				{
					...
					catch (Exception exception)
					{
						...
						//This code is needed to address the situation with incorrect update of POReceipt in case one or more attempts to email receipt fail during mass processing. 
						Document.Cache.SetStatus(receipt, PXEntryStatus.Notchanged);
						Document.Cache.Remove(receipt);
		...
		protected virtual void POReceipt_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
            ...
            {
				if (PXCurrencyAttribute.IsNullOrEmpty(row.OrderQty) == false)
				{
				...
				!sender.ObjectsEqual<POReceipt.returnInventoryCostMode>(row, oldRow))
			{
				...
				else if (oldRow.ReturnInventoryCostMode == ReturnCostMode.OriginalCost)
				{
					...
					CurrencyInfoAttribute.SetEffectiveDate<POReceipt.receiptDate>(sender, new PXFieldUpdatedEventArgs(row, null, false));
				}
			}
		}
		...
		public virtual void SetOriginalAmountsAllReturnLines(PXCache sender, POReceipt row, POReceipt oldRow, bool updateCurencyInfo)
		{
			...
			var returnAndReceiptLines = SelectFrom<POReceiptLine>.
				LeftJoin<POReceiptLine2>.
					On<POReceiptLine2.receiptNbr.IsEqual<POReceiptLine.origReceiptNbr>.
						And<POReceiptLine2.lineNbr.IsEqual<POReceiptLine.origReceiptLineNbr>>>.
				LeftJoin<POReceipt>.On<POReceipt.receiptNbr.IsEqual<POReceiptLine2.receiptNbr>>.
			...
			if (row.ReturnInventoryCostMode == ReturnCostMode.OriginalCost)
			{
				...
				foreach (PXResult<POReceiptLine, POReceiptLine2, POReceipt, CurrencyInfo> line in returnAndReceiptLines)
				{
					...
					if (returnCuryInfo.CuryID != receiptCurrencyInfo.CuryID ||
						(prevCuryInfo != null && (prevCuryInfo.CuryEffDate != receiptCurrencyInfo.CuryEffDate ||
						(prevCuryInfo.CuryMultDiv == receiptCurrencyInfo.CuryMultDiv && prevCuryInfo.CuryRate != receiptCurrencyInfo.CuryRate) ||
						(prevCuryInfo.CuryMultDiv != receiptCurrencyInfo.CuryMultDiv && prevCuryInfo.CuryRate != receiptCurrencyInfo.RecipRate) ||
						prevCuryInfo.CuryRate != receiptCurrencyInfo.CuryRate ||
						prevCuryInfo.CuryRateTypeID != receiptCurrencyInfo.CuryRateTypeID)))
					{
						Document.Cache.SetValue<POReceipt.returnInventoryCostMode>(row, oldRow.ReturnInventoryCostMode);
		...
		public virtual void SetOriginalAmountsReturnLine(POReceiptLine row)
		{
			...
			Where<POReceiptLine2.receiptType.IsEqual<POReceiptType.poreceipt>.
				And<POReceiptLine2.receiptNbr.IsEqual<@P.AsString>>.
				And<POReceiptLine2.lineNbr.IsEqual<@P.AsInt>>>.View.Select(this, row.OrigReceiptNbr, row.OrigReceiptLineNbr);
		...
		public virtual void SetOriginalReceiptAmounts(POReceiptLine returnLine, POReceiptLine receiptLine)
		{
			...
			updatedReturnLine.CuryDiscAmt = (receiptLine.ReceiptQty == returnLine.ReceiptQty) ? srcCuryDiscAmt
				: PXCurrencyAttribute.RoundCury(transactions.Cache, updatedReturnLine, ((srcCuryDiscAmt * returnLine.ReceiptQty / receiptLine.ReceiptQty) ?? 0m));
			...
			{
				decimal curyTranUnitCost;
				PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, updatedReturnLine, updatedReturnLine.TranUnitCost ?? 0m, out curyTranUnitCost, skipRounding: true);
				updatedReturnLine.CuryTranUnitCost = curyTranUnitCost;
			}
			...
			updatedReturnLine.CuryExtCost = (receiptLine.ReceiptQty == returnLine.ReceiptQty) ? srcCuryExtCost
				: PXCurrencyAttribute.RoundCury(transactions.Cache, updatedReturnLine, ((srcCuryExtCost * returnLine.ReceiptQty / receiptLine.ReceiptQty) ?? 0m));
			...
			updatedReturnLine.TranCost = (receiptLine.ReceiptQty == returnLine.ReceiptQty) ? srcTranCost
				: PXCurrencyAttribute.BaseRound(this, srcTranCost * returnLine.ReceiptQty / receiptLine.ReceiptQty);
			if (updatedReturnLine.TranCost != null)
			{
				decimal curyTranCost;
				PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, updatedReturnLine, updatedReturnLine.TranCost ?? 0m, out curyTranCost);
				updatedReturnLine.CuryTranCost = curyTranCost;
			}
		...
		protected virtual void POReceipt_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            else
            {
                if (this._skipUIUpdate == false)
                {
					...
					bool curyenabled = AllowNonBaseCurrency() && vendor.Current != null && vendor.Current.AllowOverrideCury == true;
					PXUIFieldAttribute.SetEnabled<POReceipt.curyID>(sender, doc, !hasTransactions && curyenabled);
		...
		protected virtual void POReceiptLine_UOM_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (!this.inventoryIDChanging)
			{
				...
				if (row.CuryTranUnitCost != null)
				{
					...
					decimal tranUnitCost;
					PXDBCurrencyAttribute.CuryConvBase(sender, row, row.CuryTranUnitCost ?? 0m, out tranUnitCost, true);
		...
		protected virtual void POReceiptLine_CuryUnitCost_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			else
			{
				...
				if (AllowNonBaseCurrency() && poOriginLine != null && poOriginLine.UOM == row.UOM)
				{
					...
					else
					{
						...
						PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, row, poOriginLine.UnitCost ?? 0m, out curyUnitCost, CommonSetupDecPl.PrcCst);
						e.NewValue = curyUnitCost;
					}
					e.Cancel = true;
					return;
				}
		...
		protected virtual void POReceiptLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row.Released != true)
			{
				...
				bool allowChangingCost = row.AllowEditUnitCost == true &&
					...
					(isReturn && (!hasOrigReceipt || !returnWithOrigCost)));
				PXUIFieldAttribute.SetEnabled<POReceiptLine.manualPrice>(sender, e.Row, allowChangingCost);
				PXUIFieldAttribute.SetEnabled<POReceiptLine.curyUnitCost>(sender, e.Row, allowChangingCost);
		...
		{
			return PXAccess.FeatureInstalled<FeaturesSet.multicurrency>() && (posetup.Current?.ChangeCuryRateOnReceipt == true) && Document.Current?.Released != true &&
				(Document.Current?.ReceiptType == POReceiptType.POReceipt || (Document.Current?.ReceiptType == POReceiptType.POReturn && Document.Current?.ReturnInventoryCostMode != ReturnCostMode.OriginalCost));
		}
		...
		public virtual POReceipt CreateReceiptFrom(POOrder order, bool redirect = false)
		{
			...
			receipt.CuryID = AllowNonBaseCurrency() ? order.CuryID : receipt.CuryID;
		...
		protected virtual void CurrencyInfo_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            if (info != null)
            {
                bool curyenabled = info.AllowUpdate(this.transactions.Cache) && AllowChangingCuryRate();
		...
		public virtual POReceiptLine AddPOLine(POLine aLine, bool isLSEntryBlocked)
		{
			POReceipt doc = this.Document.Current;
		...
		public virtual POReceiptLine AddReturnLine(IPOReturnLineSource origLine)
		{
			...
			if (Document.Current?.CuryID != Accessinfo.BaseCuryID)
			{
				...
				if (Document.Current?.ReturnInventoryCostMode == ReturnCostMode.OriginalCost)
				{
					...
					{
						if (returnCuryInfo.CuryID != receiptCuryInfo.CuryID ||
							returnCuryInfo.CuryEffDate != receiptCuryInfo.CuryEffDate ||
							(returnCuryInfo.CuryMultDiv == receiptCuryInfo.CuryMultDiv && returnCuryInfo.CuryRate != receiptCuryInfo.CuryRate) ||
							(returnCuryInfo.CuryMultDiv != receiptCuryInfo.CuryMultDiv && returnCuryInfo.CuryRate != receiptCuryInfo.RecipRate) ||
							returnCuryInfo.CuryRateTypeID != receiptCuryInfo.CuryRateTypeID)
						{
			...
			CopyFromOrigReceiptLine(newLine, origLine, preserveLineType: false, returnOrigCost: Document.Current?.ReturnInventoryCostMode == ReturnCostMode.OriginalCost);
		...
		public virtual void CopyFromOrigReceiptLine(POReceiptLine destLine, IPOReturnLineSource srcLine)
			=> CopyFromOrigReceiptLine(destLine, srcLine, false, false);

		public virtual void CopyFromOrigReceiptLine(POReceiptLine destLine, IPOReturnLineSource srcLine, bool preserveLineType)
			=> CopyFromOrigReceiptLine(destLine, srcLine, preserveLineType, false);
		...
		public virtual void CopyFromOrigReceiptLine(POReceiptLine destLine, IPOReturnLineSource srcLine, bool preserveLineType, bool? returnOrigCost)
		{
			...
			destLine.CuryDiscAmt = (srcLine.ReturnedQty == 0m) ? srcCuryDiscAmt
				: PXCurrencyAttribute.RoundCury(transactions.Cache, destLine, ((srcCuryDiscAmt * (srcLine.ReceiptQty - srcLine.ReturnedQty) / srcLine.ReceiptQty) ?? 0m));
			...
			{
				decimal curyTranUnitCost;
				PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, destLine, destLine.TranUnitCost ?? 0m, out curyTranUnitCost, skipRounding: true);
				destLine.CuryTranUnitCost = curyTranUnitCost;
			}

			decimal? srcCuryExtCost = srcLine.CuryExtCost;
			destLine.CuryExtCost = (srcLine.ReturnedQty == 0m) ? srcCuryExtCost
				: PXCurrencyAttribute.RoundCury(transactions.Cache, destLine, ((srcCuryExtCost * (srcLine.ReceiptQty - srcLine.ReturnedQty) / srcLine.ReceiptQty) ?? 0m));
			...
			{
				decimal curyTranCost;
				PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, destLine, destLine.TranCost ?? 0m, out curyTranCost);
				destLine.CuryTranCost = curyTranCost;
			}
		}
		...
		public virtual void CalculateAmountFromPOLine(POReceiptLine aDest, POLine aSrc, POOrder order)
		{
			...
			if (AllowNonBaseCurrency() && order?.CuryID == Document.Current.CuryID)
			{
				aDest.CuryUnitCost = aSrc.CuryUnitCost ?? 0m;
				aDest.CuryExtCost = PXCurrencyAttribute.Round(transactions.Cache, aDest, ((aSrc.OrderQty != 0m) ? (aSrc.CuryLineAmt ?? 0m) * aDest.Qty / aSrc.OrderQty : (aSrc.CuryLineAmt ?? 0m)) ?? 0m, CMPrecision.TRANCURY);
				aDest.CuryTranCost = PXCurrencyAttribute.Round(transactions.Cache, aDest, ((aSrc.OrderQty != 0m) ? curyAccrualAmt * aDest.Qty / aSrc.OrderQty : curyAccrualAmt) ?? 0m, CMPrecision.TRANCURY);
			...
			else
			{
				if (Document.Current.CuryID == company.Current.BaseCuryID)
				{
					...
					aDest.CuryExtCost = PXCurrencyAttribute.Round(transactions.Cache, aDest, ((aSrc.OrderQty != 0m) ? aSrc.LineAmt * aDest.Qty / aSrc.OrderQty : aSrc.LineAmt) ?? 0m, CMPrecision.TRANCURY);
					aDest.CuryTranCost = PXCurrencyAttribute.Round(transactions.Cache, aDest, ((aSrc.OrderQty != 0m) ? baseAccrualAmt * aDest.Qty / aSrc.OrderQty : baseAccrualAmt) ?? 0m, CMPrecision.TRANCURY);
					aDest.CuryTranUnitCost = (aSrc.OrderQty != 0m) ? baseAccrualAmt / aSrc.OrderQty : baseAccrualAmt;
				}
				else
				{
					aDest.UnitCost = baseUnitCost;
					aDest.ExtCost = PXCurrencyAttribute.Round(transactions.Cache, aDest, ((aSrc.OrderQty != 0m) ? aSrc.LineAmt * aDest.Qty / aSrc.OrderQty : aSrc.LineAmt) ?? 0m, CMPrecision.TRANCURY);
					aDest.TranCost = PXCurrencyAttribute.Round(transactions.Cache, aDest, ((aSrc.OrderQty != 0m) ? baseAccrualAmt * aDest.Qty / aSrc.OrderQty : baseAccrualAmt) ?? 0m, CMPrecision.TRANCURY);
					aDest.TranUnitCost = (aSrc.OrderQty != 0m) ? baseAccrualAmt / aSrc.OrderQty : baseAccrualAmt;

					decimal curyUnitCost, curyExtCost, curyTranCost, curyTranUnitCost;
					PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, aDest, aDest.UnitCost ?? 0m, out curyUnitCost, CommonSetupDecPl.PrcCst);
					aDest.CuryUnitCost = curyUnitCost;
					PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, aDest, aDest.ExtCost ?? 0m, out curyExtCost);
					aDest.CuryExtCost = curyExtCost;
					PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, aDest, aDest.TranCost ?? 0m, out curyTranCost);
					aDest.CuryTranCost = curyTranCost;
					PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, aDest, aDest.TranUnitCost ?? 0m, out curyTranUnitCost, skipRounding: true);
					aDest.CuryTranUnitCost = curyTranUnitCost;
				}
			}
		}
        ...
        public virtual void ReleaseReceipt(INReceiptEntry docgraph, AP.APInvoiceEntry invoiceGraph, POReceipt aDoc, DocumentList<INRegister> aINCreated, DocumentList<AP.APInvoice> aAPCreated, bool aIsMassProcess)
		{
			...
			foreach (PXResult<POReceiptLine> res in GetLinesToReleaseQuery().Select(aDoc.ReceiptNbr))
			{
				...
				if (rctLineChange || isNonStockKit)
				{
					if (line.IsStockItem() || (line.IsNonStockItem() && insetup.Current.UpdateGL == true && line.POAccrualAcctID != null))
					{
						...
						newline.InventoryID = isNonStockKit && line.IsStockItem() ? split.InventoryID : line.InventoryID;
						newline.SiteID = line.SiteID;

						if (isNonStockKit && !line.IsNonStockItem())
						{
					...
					UpdateReceiptLineOnRelease(res);
				...
				foreach (PXResult<SOLine4, SOLineSplit> sores in result)
				{
					...
					if (poLine.Completed == true)
					{
						...
						if ((order = (SOOrder)PXParentAttribute.SelectParent(solineselect.Cache, solineCopy)) != null)
						{
							...
							decimal? lineAmt = PXCurrencyAttribute.BaseRound(this, splitReceivedQty * actualDiscUnitPrice);
		...
		{
			if (aDoc.ReturnInventoryCostMode == ReturnCostMode.OriginalCost || aDoc.ReturnInventoryCostMode == ReturnCostMode.ManualCost)
		...
		protected virtual void ReleaseReturnWithNoExactCost(INIssueEntry docgraph, AP.APInvoiceEntry invoiceGraph, POReceipt aDoc, DocumentList<INRegister> aCreated, DocumentList<AP.APInvoice> aInvoiceCreated, bool aIsMassProcess)
		{
					...
					OrderBy<Asc<POReceiptLine.receiptNbr, Asc<POReceiptLine.lineNbr>>>>.Select(this))
			{
				...
				{
					poLine = poLinesOpenUPD.Locate(poLine) ?? poLine;
				}
				...
				if (rctLineChange || isKit)
				{
					...
					UpdateReturnOrdersMarkedForDropship(line, poLine);
				}
				prev_line = line;
				if (newline != null && !string.IsNullOrEmpty(split.ReceiptNbr))
				{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				UpdateReceiptOnReturnRelease(isINDocValid ? docgraph.issue.Current : null);
			...
			for (int i = 0; i < receiptLineSplits.Count; i++)
			{
				...
				if (rctLineChange || splitBasedTran)
				{
					if (postToIN)
					{
						...
						SetIssueWithExactCostReturnCostFinal(newline, line, split, item, splitBasedTran, lastLineSplit, aDoc.ReturnInventoryCostMode == ReturnCostMode.ManualCost);
						newline = docGraph.lsselect.Insert(newline);
						if (isStock)
						{
		...
		protected virtual POReceipt UpdateReceiptOnReturnRelease(INRegister inRegister)
		{
			POReceipt receiptCopy = (POReceipt)this.Document.Cache.CreateCopy(this.Document.Current);
		...
		{
			SetIssueWithExactCostReturnCostFinal(newline, rctLine, split, item, splitBasedTran, lastLineSplit, false);
		}

		public virtual void SetIssueWithExactCostReturnCostFinal(INTran newline, POReceiptLine rctLine, POReceiptLineSplit split, InventoryItem item, bool splitBasedTran, bool lastLineSplit, bool isManualCostMode)
		{
			...
			bool confirmed, decimal? qty, decimal? amt)
		{
			...
			else
			{
				...
				ordershipmentselect.Cache.MarkUpdated(existing);
				return existing;
			}
		}
