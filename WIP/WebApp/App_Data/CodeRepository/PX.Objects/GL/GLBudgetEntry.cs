...
namespace PX.Objects.GL
{
	public class GLBudgetEntry : PXGraph<GLBudgetEntry>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		public PXSelect<GLBudget,
			Where<GLBudget.branchID, Equal<Current<BudgetFilter.branchID>>,
				And<GLBudget.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
				And<GLBudget.finYear, Equal<Current<BudgetFilter.finYear>>>>>> Budget;
		...
		public GLBudgetEntry()
		{
			...
			if (IsImport)
			{
				...
				// Acuminator disable once PX1085 DatabaseQueriesInPXGraphInitialization [Performance optimization which is used only for Import scenarios]
				PrepareIndexes();
		...
		)
		{
			...
			BudgetArticles.Cache.AllowInsert = IsFilterSet() && !IsCompareToSetOnFilter();
			BudgetArticles.Cache.AllowDelete = (IsFilterSet() || IsImport) && !IsCompareToSetOnFilter();

			var result = (Filter.Current.ShowTree == true) ? GetTreeArticle(groupID) : GetListArticle();
			if (!useIdx) return result;
		...
		protected IEnumerable<GLBudgetLine> GetTreeArticle(Guid? groupID)
			...
			foreach (PXResult<GLBudgetLine, Account, Sub> result in PXSelectJoin<GLBudgetLine,
					LeftJoin<Account, On<Account.accountID, Equal<GLBudgetLine.accountID>,And<Account.active, Equal<True>>>,
					LeftJoin<Sub, On<Sub.subID, Equal<GLBudgetLine.subID>,
						And<Sub.active, Equal<True>,
						And<Sub.subCD, Like<Current<BudgetFilter.subCDWildcard>>>>>>>,
						Where<GLBudgetLine.branchID, Equal<Current<BudgetFilter.branchID>>,
							And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
							And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>,
					And<GLBudgetLine.parentGroupID, Equal<Required<GLBudgetLine.parentGroupID>>,
					And2<Where<GLBudgetLine.accountID, IsNull,
						Or<Where<Account.accountID, IsNotNull, 
								And<Sub.subID, IsNotNull>>>>,
					And<Match<Current<AccessInfo.userName>>>>>>>>,
				OrderBy<
					Asc<Account.accountCD,
					Asc<GLBudgetLine.subID, 
					Asc<GLBudgetLine.treeSortOrder>>>>>.Select(this, groupID))
					{
				GLBudgetLine article = result;
				if (article.Comparison != null && article.Comparison == true) yield break;

				article.SortOrder = SortOrder++;

				yield return article;
				if (Filter.Current.CompareToBranchID != null &&
				    Filter.Current.CompareToLedgerId != null &&
				    !string.IsNullOrEmpty(Filter.Current.CompareToFinYear))
						{
					yield return ComparisonRow(article, SortOrder++);
					}
				}
			}
		...
		protected IEnumerable<GLBudgetLine> GetListArticle()
		{
			...
			foreach (PXResult<GLBudgetLine, Account, Sub> result in PXSelectJoin<GLBudgetLine,
				...
				OrderBy<
					Asc<Account.accountCD,
					Asc<GLBudgetLine.subID,
					Asc<GLBudgetLine.treeSortOrder>>>>>.Select(this))
			{
				GLBudgetLine article = result;
				if (article.Comparison != null && article.Comparison == true) yield break;
		...
		{
			return Filter.Current != null && Filter.Current.BranchID != null && Filter.Current.LedgerID != null && Filter.Current.FinYear != null;
		}

		private bool IsCompareToSetOnFilter()
		{
			return Filter.Current.CompareToBranchID != null && Filter.Current.CompareToFinYear != null && Filter.Current.CompareToLedgerId != null;
		}
		...
		{
			PXResultset<GLBudgetLine> childGroups = 
				PXSelect<GLBudgetLine, 
					Where<GLBudgetLine.parentGroupID, 
						Equal<Required<GLBudgetLine.parentGroupID>>, 
						And<GLBudgetLine.isGroup, 
							Equal<True>>>>.Select(this, GroupID);
			
			if (PXSelect<GLBudgetLine, 
				Where<GLBudgetLine.groupID, 
					Equal<Required<GLBudgetLine.groupID>>, 
					And<Match<Current<AccessInfo.userName>>>>>.SelectSingleBound(this, null, GroupID).Count == 0)
			{
				if (PXSelect<GLBudgetLine, 
					Where<GLBudgetLine.parentGroupID, 
						Equal<Required<GLBudgetLine.parentGroupID>>, 
		...
		private void CheckBudgetLinesForDuplicates()
		{
			...
			var list = BudgetArticles.Select();
		...
		protected GLBudgetEntry PrepareGraphForLongOperation()
		{
			...
			HoldNotchanged(this.Caches<GLBudget>());
		...
		public override void Persist()
		{
			...
			if (Budget.Current != null && this.IsDirty) 
			{
				Budget.Cache.MarkUpdated(Budget.Current);
			}
		...
		protected virtual void BudgetFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisible(sender, typeof(BudgetFilter.showTree).Name, IsBudgetTree());
			if (row != null)
			{
				if (!IsBudgetTree())
				{
			...
			if (Filter.Current.LedgerID != null)
			{
				Filter.Current.BaseCuryID = ((Ledger)PXSelect<Ledger,
				  Where<Ledger.ledgerID, Equal<Current<BudgetFilter.ledgerID>>>>
				...
				.Select(this, row.CompareToLedgerId)).BaseCuryID)
			{
				...
				{
					Filter.Current.CompareToBranchID = null;
					Filter.Current.CompareToLedgerId = null;
				}
				else
				{ 
		...
		protected virtual void BudgetFilter_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (Filter.Current.BranchID != null && Filter.Current.LedgerID != null && Filter.Current.FinYear != null && 
				...
				Where<GLBudgetLine.branchID, Equal<Current<BudgetFilter.branchID>>,
					And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
					And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>>>>>
				.SelectSingleBound(this, null, null).Any())
				{
					...
					if (result == WebDialogResult.Yes)
					{
						...
						GLBudgetEntry process = PrepareGraphForLongOperation();
					// Acuminator disable once PX1046 LongOperationInEventHandlers [Legacy]
						PXLongOperation.StartOperation(this, (PXToggleAsyncDelegate)(() => {
							process.PreloadBudgetTree();
							process._CurrentAction = GLBudgetEntryActionType.PreloadBudgetTree; // it's need here for Persist()
							process.Persist();
							PXLongOperation.SetCustomInfo(process);
							if (!process.IsContractBasedAPI)
								PXRedirectHelper.TryRedirect(process, PXRedirectHelper.WindowMode.Same);
						}));
					}
					else
					{
		...
		{
			return PXSelect<GLBudgetTree>.SelectSingleBound(this, null, null).Any();
		}
		#endregion
		...
		{
			BudgetFilter filter = Filter.Current;
			if (filter?.BranchID == null || filter?.LedgerID == null || filter?.FinYear == null) return null;

			Budget.Current = Budget.SelectSingle();

			bool lineExists = (new PXSelect<GLBudgetLine,
				Where<GLBudgetLine.branchID, Equal<Current<BudgetFilter.branchID>>,
					And<GLBudgetLine.ledgerID, Equal<Current<BudgetFilter.ledgerID>>,
					And<GLBudgetLine.finYear, Equal<Current<BudgetFilter.finYear>>>>>>(this)).SelectSingle() != null;

			if (Budget.Current == null)
			{
				if (lineExists) 
				{
					Budget.Current = Budget.Insert(new GLBudget() { BranchID = Filter.Current.BranchID, LedgerID = Filter.Current.LedgerID, FinYear = Filter.Current.FinYear });
				}
			}
			else 
			{
		...
		protected virtual void GLBudgetLine_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			...
			if (_BudgetArticlesIndex != null)
				_BudgetArticlesIndex.Add(article);
		...
		protected virtual void GLBudgetLine_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			if (_BudgetArticlesIndex != null)
				_BudgetArticlesIndex.Delete(article);
		}
		...
		private void PrepareIndexes(bool preload = true)
		{
			...
			BudgetFilter filter = Filter.Current;
			if (preload && (filter.BranchID != null && filter.FinYear != null && filter.LedgerID != null))
			{
		...
		private class AccountDefinition : IPrefetchable
		{
			...
			public void Prefetch()
			{
				foreach (PXDataRecord record in PXDatabase.SelectMulti<Account>(new PXDataField<Account.accountID>(),	new PXDataField<Account.accountCD>()))
				{
					...
					_CDID.Add(accountCD.Trim(), accountID??0);
				}
			}
			...
			public static int? getID(string cd)
			{
				int id = -1;
				var instance = PXDatabase.GetSlot<AccountDefinition>(typeof(AccountDefinition).FullName, typeof(Account));
				instance._CDID.TryGetValue(cd.Trim(), out id);
	...
	public partial class BudgetPreloadFilter : IBqlTable
	{
		...
		[PXSelector(typeof(
					...
					Where<Branch.branchID, Equal<Current2<BudgetPreloadFilter.branchID>>,
	...
	public partial class BudgetFilter : IBqlTable
	{
		...
		[PXSelector(typeof(
				Search5<Ledger.ledgerID,
					LeftJoin<OrganizationLedgerLink,
						...
						LeftJoin<Branch,
							On<Branch.organizationID, Equal<OrganizationLedgerLink.organizationID>>>>,
					Where<Ledger.balanceType, Equal<LedgerBalanceType.budget>,
		...
		[PXSelector(typeof(
					...
					LeftJoin<Branch,
						On<Branch.organizationID, Equal<OrganizationLedgerLink.organizationID>>>>,
					Where<Branch.branchID, Equal<Current2<BudgetFilter.compareToBranchID>>,
						Or<Current2<BudgetFilter.compareToBranchID>, IsNull>>,
					Aggregate<GroupBy<Ledger.ledgerID>>>),
			SubstituteKey = typeof(Ledger.ledgerCD),
			DescriptionField = typeof(Ledger.descr))]
		...
		[PXRestrictor(typeof(Where<BudgetFilter.baseCuryID.FromCurrent.IsEqual<Ledger.baseCuryID>
			.Or<Not<FeatureInstalled<PX.Objects.CS.FeaturesSet.multipleBaseCurrencies>>>>), Messages.BudgetDifferentCurrency)]
