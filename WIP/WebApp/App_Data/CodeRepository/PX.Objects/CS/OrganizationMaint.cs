...
{
	public class OrganizationMaint : OrganizationUnitMaintBase<OrganizationBAccount, Where<True, Equal<True>>>
	{
			...
			if (!string.IsNullOrEmpty((string)e.NewValue))
			{
				...
				if (currency == null)
				{
					...
					if (bc != null)
					{
						...
						this.FieldDefaulting.AddHandler<CurrencyInfo.curyID>((cache, args) => { args.NewValue = bc.CuryID; });
						Currency finRow = (Currency)FinancinalCurrency.Cache.CreateInstance();
						finRow.CuryID = bc.CuryID;
						// Acuminator disable once PX1044 ChangesInPXCacheInEventHandlers [Justification]
						finRow = FinancinalCurrency.Insert(finRow);

						CurrencyInfo info =(CurrencyInfo)this.currencyinfo.View.SelectSingleBound(new object[] {finRow});
						info.BaseCalc = true;
						info.IsReadOnly = true;
						info.BaseCuryID = finRow.CuryID;

						info = (CurrencyInfo)this.currencyinfo.View.SelectSingle(finRow.CuryInfoBaseID);
						info.BaseCalc = false;
						info.IsReadOnly = true;
						info.BaseCuryID = finRow.CuryID;
						e.Cancel = true;
		...
		protected virtual void Organization_BaseCuryID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			BAccount.Current.BaseCuryID = org.BaseCuryID;
			BAccount.Cache.Update(BAccount.Current);
		...
		protected virtual void ProcessPublicableToBranchesFieldsChanging(Organization origOrganization, Organization organization, PXEntryStatus status)
		{
			...
			if (forceCopy || shouldAdjustRoleName || shouldAdjustActive || shouldAdjustLedgerID || shouldAdjustCountryID || shouldLogoNameReport || shouldInvokeSyncEx || shouldAdjustBaseCuryID)
			{
				...
				foreach (BranchMaint.BranchBAccount childBranchBAccount in childBranchBAccounts)
				{
					...
					{
						branchBaccountCopy.BaseCuryID = organization.BaseCuryID;
						branchBaccountCopy.BAccountBaseCuryID = organization.BaseCuryID;
					}
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class LocationDetailsExt : LocationDetailsExt<OrganizationMaint, DefContactAddressExt, OrganizationBAccount, OrganizationBAccount.bAccountID>
		{
			#region Events
