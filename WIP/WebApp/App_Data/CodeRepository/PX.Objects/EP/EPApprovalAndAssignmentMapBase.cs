...
namespace PX.Objects.EP
{
	public class EPApprovalAndAssignmentMapBase<TGraph> : PXGraph<TGraph> where TGraph : PXGraph
	{
		...
		public PXSelect<CR.Standalone.EPEmployee, Where<CR.Standalone.EPEmployee.defContactID, Equal<Required<Contact.contactID>>, 
			And<CR.Standalone.EPEmployee.vStatus, Equal<VendorStatus.inactive>>>> EmployeeInactive;
		#endregion
		...
		protected virtual void EPAssignmentMap_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				if (row.EntityType != null && row.GraphType == null)
				{
					...
					var primary = (entityType != null) ? EntityHelper.GetPrimaryGraphType(this, entityType) : null;
					if (primary != null)
					{
