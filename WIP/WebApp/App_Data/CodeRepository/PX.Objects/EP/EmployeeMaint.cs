...
namespace PX.Objects.EP
{
	public class EmployeeMaint : PXGraph<EmployeeMaint>
	{
		...
		public PXSelect<EPCompanyTreeMember,
			Where<EPCompanyTreeMember.contactID, Equal<Current<EPEmployee.defContactID>>>> CompanyTree;
        ...
        public PXSelectJoin<EPRule,
            InnerJoin<EPAssignmentMap, On<EPAssignmentMap.assignmentMapID, Equal<EPRule.assignmentMapID>>>,
             Where<EPRule.ownerID, Equal<Current<EPEmployee.defContactID>>>> AssigmentAndApprovalMaps;
		...
		protected virtual void _(Events.FieldUpdated<EPEmployee, EPEmployee.vStatus> e)
		{
			EPEmployee row = (EPEmployee)e.Row;

			Users user = PXSelect<Users, Where<Users.pKID, Equal<Current<Contact.userID>>>>.SelectSingleBound(this, new object[] { Contact.Current });

			if (user != null)
			{
				user.IsApproved = row.VStatus != VendorStatus.Hold && row.VStatus != VendorStatus.Inactive;
				User.Update(user);
			}
		}
		...
		protected virtual void EPEmployee_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (string.IsNullOrEmpty(doc.TermsID) == false && e.Operation != PXDBOperation.Delete)
			{
				...
				{
					if (sender.RaiseExceptionHandling<EPEmployee.termsID>(e.Row, doc.TermsID, new PXSetPropertyException(Messages.EmployeeTermsCannotHaveCashDiscounts, $"[{nameof(EPEmployee.termsID)}]")))
					{
						throw new PXRowPersistingException(typeof(EPEmployee.termsID).Name, doc.TermsID, Messages.EmployeeTermsCannotHaveCashDiscounts, nameof(EPEmployee.termsID));
					}
				}

				if (terms != null && terms.InstallmentType == CS.TermsInstallmentType.Multiple)
				{
					if (sender.RaiseExceptionHandling<EPEmployee.termsID>(e.Row, doc.TermsID, new PXSetPropertyException(Messages.EmployeeTermsCannotHaveMultiplyInstallments, $"[{nameof(EPEmployee.termsID)}]")))
					{
						throw new PXRowPersistingException(typeof(EPEmployee.termsID).Name, doc.TermsID, Messages.EmployeeTermsCannotHaveMultiplyInstallments, nameof(EPEmployee.termsID));
					}
				}
			}
		}
		...
		protected virtual void EPEmployee_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(employee?.AcctCD)
				&& employee.VStatus != VendorStatus.Inactive
		...
		[PXDBInt(IsKey = true)]
		[PXParent(typeof(Select<EPEmployee, Where<EPEmployee.defContactID, Equal<Current<EPCompanyTreeMember.contactID>>>>))]
		...
		protected virtual void _(Events.CacheAttached<EPCompanyTreeMember.workGroupID> e) { }

		[PXDBInt(IsKey = true)]
		[PXDefault(typeof(EPEmployee.defContactID))]
		[PXUIField(DisplayName = "Member", Visible = false)]
		protected virtual void _(Events.CacheAttached<EPCompanyTreeMember.contactID> e) { }
		...
		protected virtual void EPCompanyTreeMember_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			{
				foreach (EPCompanyTreeMember member in
				PXSelect<EPCompanyTreeMember,
					Where<EPCompanyTreeMember.workGroupID, Equal<Required<EPCompanyTreeMember.workGroupID>>,
						And<EPCompanyTreeMember.isOwner, Equal<boolTrue>>>>.Select(this, row.WorkGroupID))
				{
					if (member.ContactID != row.ContactID)
					{
						EPCompanyTreeMember upd = PXCache<EPCompanyTreeMember>.CreateCopy(member);
						upd.IsOwner = false;
		...
		public static int? GetCurrentOwnerID(PXGraph graph)
		{
			Contact contact = PXSelect<
					Contact,
				Where<
					Contact.contactID, Equal<Current<AccessInfo.contactID>>,
					And<Contact.contactType, Equal<ContactTypesAttribute.employee>>>>
				.SelectSingleBound(graph, null);
			return (contact != null) ? contact.ContactID : null;
		}
        ...
        /// <exclude/>
        public class EmployeeMaintAddressLookupExtension : CR.Extensions.AddressLookupExtension<EmployeeMaint, EPEmployee, Address>
        {
