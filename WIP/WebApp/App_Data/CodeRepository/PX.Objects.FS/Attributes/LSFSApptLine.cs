            ...
            And<FSApptLineSplit.apptNbr, Equal<Current<FSAppointment.refNbr>>>>>
    {
		...
		protected override void _Master_RowUpdated(PXCache sender, PXRowUpdatedEventArgs<FSAppointmentDet> e)
		{
            ...
            else
            {
                ...
                InsertLotSerialsFromServiceOrder(sender, e.Row, lotSerClass);
        ...
        protected virtual void UpdateLotSerialSplitsBasedOnLineLotSerial(PXCache apptLineCache, FSAppointmentDet apptLine, string lotSerTrack, bool? lotSerTrackExpiration)
        {
            ...
            if (qtyDefault > 0m)
            {
                ...
                lotSerialSplit.BaseQty = INUnitAttribute.ConvertToBase(DetailCache, lotSerialSplit.InventoryID, lotSerialSplit.UOM, lotSerialSplit.Qty ?? 0m, INPrecision.QUANTITY);
                    ...
                    AppointmentEntry apptGraph = (AppointmentEntry)apptLineCache.Graph;
                    try
                    {
                        if (apptGraph != null)
                        {
                            apptGraph.SkipLotSerialFieldVerifying = true;
                        }
                    ...
                    finally
                    {
                        if (apptGraph != null)
                        {
                            apptGraph.SkipLotSerialFieldVerifying = false;
                        }
                    }
                }
            else
            {
                    DetailCache.Delete(lotSerialSplit);
                }
            }
        }
        ...
        protected virtual void InsertLotSerialsFromServiceOrder(PXCache sender, FSAppointmentDet apptLine)
        {
            ...
            InsertLotSerialsFromServiceOrder(sender, apptLine, lotSerClass);
        }

        protected virtual void InsertLotSerialsFromServiceOrder(PXCache sender, FSAppointmentDet apptLine, INLotSerClass lotSerClass)
        {
            ...
            if (apptLine.EnablePO == true
                ...
                && lotSerClass.LotSerTrack != INLotSerTrack.LotNumbered)
            {
            ...
            decimal baseExistingSplitTotalQty;
            decimal existingSplitTotalQty;
            List<FSApptLineSplit> existingSplits = GetExistingSplits(apptLine, out baseExistingSplitTotalQty, out existingSplitTotalQty);
            decimal pendingQty = (decimal)apptLine.EffTranQty - existingSplitTotalQty;
            ...
            if (pendingQty > 0m)
                ...
                FSSODet soDetRow = PXSelect<FSSODet, Where<FSSODet.sODetID, Equal<Required<FSSODet.sODetID>>>>.Select(_Graph, apptLine.SODetID);
			            ...
			            {
                    if (string.IsNullOrEmpty(soSplit.LotSerialNbr) == true
                        && soSplit.POReceiptNbr == null)
                    {
                        continue;
                    }
                    ...
                    if (soSplitBalance > 0m)
                    {
                        apptSplit = existingSplits.Find(x =>
                            ...
                            if (apptSplit != null)
                            {
                            soSplitBalance -= (decimal)apptSplit.Qty;
                            }
                    ...
                    if (soSplitBalance > 0m)
                        ...
                        apptSplit.BaseQty = INUnitAttribute.ConvertToBase(DetailCache, apptSplit.InventoryID, apptSplit.UOM, (decimal)apptSplit.Qty, apptSplit.BaseQty, INPrecision.QUANTITY);
		...
		public virtual void _UpdateParent(PXCache sender, FSAppointmentDet row)
		{
            ...
            List<FSApptLineSplit> existingSplits = GetExistingSplits(row, out baseExistingSplitTotalQty, out existingSplitTotalQty);
            ...
            FSAppointment appt = (FSAppointment)_Graph.Caches[typeof(FSAppointment)].Current;
            UpdateLineStatusBasedOnReceivedPurchaseItems(appt, sender, row, MustHaveRequestPOStatus(row), existingSplits, baseExistingSplitTotalQty, existingSplitTotalQty,  true);
        }
        ...
        [Obsolete("Remove in Major Version 2021R2")]
        public virtual void UpdateLineStatusBasedOnReceivedPurchaseItems(PXCache sender, FSAppointmentDet row, bool rowMustHaveRequestPOStatus, List<FSApptLineSplit> existingSplits, decimal? baseExistingSplitTotalQty, decimal? existingSplitTotalQty, bool runSetValueExt)
        {
        }
        ...
        public virtual bool VerifyLotSerialTotalQty(PXCache sender, FSApptLineSplit split, decimal newIncrease)
        {
            ...
            return VerifyLotSerialTotalQty(sender.Graph.Caches[typeof(FSAppointmentDet)], apptLine, newIncrease, true);
        }

        public virtual bool VerifyLotSerialTotalQty(PXCache sender, FSAppointmentDet apptLine, decimal newIncrease, bool runningFieldVerifying)
        {
            decimal baseExistingSplitTotalQty;
            decimal existingSplitTotalQty;
            GetExistingSplits(apptLine, out baseExistingSplitTotalQty, out existingSplitTotalQty);
            ...
            {
                PXSetPropertyException exception = new PXSetPropertyException(TX.Error.TotalLotSerialQtyXExceedsTheQtyRequiredX,
                        PXErrorLevel.Error,
                        newSplitTotalQty.ToString("0"), ((decimal)apptLine.EffTranQty).ToString("0"));
                ...
                {
                    sender.RaiseExceptionHandling<FSAppointmentDet.effTranQty>(apptLine, apptLine.EffTranQty, exception);
                }
		...
		protected override void Detail_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
            ...
            bool original_InternallCall = _InternallCall;
            try
            {
            ...
            {
                _InternallCall = original_InternallCall;
            }
        }
		...
		public virtual void FSAppointmentDet_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
            ...
            VerifyLotSerialTotalQty(sender, (FSAppointmentDet)e.Row, 0m, false);
		}
