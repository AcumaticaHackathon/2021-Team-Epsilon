...
using static PX.Objects.CM.TemporaryHelpers.MCExtensionAdapter;
...
namespace PX.Objects.Common.Discount
{
		...
		where TDiscountDetail : class, IBqlTable, IDiscountDetail, new()
	{
			...
			bool needDiscountID)
		{
			...
			if (!IsDiscountCalculationNeeded(cache, tLine, DiscountType.Line))
				return;

			if (GetUnitPrice(cache, tLine, locationID, curyID, date.Value).skipLineDiscount)
			{
		...
		protected virtual void ApplyDiscountToLine(PXCache sender, TLine line, decimal? qty, decimal? curyUnitPrice, decimal? curyExtPrice, DiscountLineFields dLine, DiscountResult discountResult, int multInv)
		{
			...
			if (!discountResult.IsEmpty)
			{
				...
				if (discountResult.IsAmount)
				{
					...
					decimal curyDiscAmt;
					CuryConvCury(cache, row, discAmt, out curyDiscAmt, CommonSetupDecPl.PrcCst);
					decimal? oldCuryDiscAmt = dLine.CuryDiscAmt;
					dLine.CuryDiscAmt = multInv * RoundCury(cache, row, curyDiscAmt);
				...
				else
				{
					...
					if (lineDiscountTarget == AR.LineDiscountTargetType.SalesPrice)
					{
						...
						curyDiscAmt = priceSign * qtyVal * (curyUnitPrice ?? 0) - RoundCury(cache, row, extPriceAfterDiscount);
		...
		public virtual void CalculateGroupDiscountRate(
			...
			bool forceFormulaCalculation = false)
		{
			var qtyDiscountByBaseUOM = ApplyQuantityDiscountByBaseUOM(cache.Graph);

			foreach (TLine line in documentDetails)
			{
				TLine oldCurrentLine = (TLine)cache.CreateCopy(currentLine);

				AmountLineFields lineAmt = AmountLineFields.GetMapFor(line, cache, qtyDiscountByBaseUOM);
				TLine oldLine = (TLine)cache.CreateCopy(line);
				...
				if (AmountLineFields.GetMapFor(oldLine, cache, qtyDiscountByBaseUOM).GroupDiscountRate != lineAmt.GroupDiscountRate ||
					AmountLineFields.GetMapFor(oldLine, cache, qtyDiscountByBaseUOM).OrigGroupDiscountRate != lineAmt.OrigGroupDiscountRate)
				{
		...
		private void ApplyDocumentDiscountRatesToLine(PXCache cache, TwoWayLookup<TDiscountDetail, TLine> discountsLinkedToLine, List<TLine> documentDetails, TLine currentLine, bool forceFormulaCalculation = false)
		{
			...
			foreach (TLine line in documentDetails)
			{
				...
				AmountLineFields lineAmtFields = AmountLineFields.GetMapFor(line, cache, qtyDiscountByBaseUOM);
				DiscountLineFields discountFields = DiscountLineFields.GetMapFor(line, cache);
				...
				foreach (TDiscountDetail discount in discountsLinkedToLine.LeftsFor(line).Where(x => x.Type.IsIn(DiscountType.Document, DiscountType.ExternalDocument) && x.SkipDiscount != true))
				{
					...
					foreach (TLine dline in discountsLinkedToLine.RightsFor(discount))
					{
						...
						{
							AmountLineFields dlineAmtFields = AmountLineFields.GetMapFor(dline, cache, qtyDiscountByBaseUOM);
				...
				if (AmountLineFields.GetMapFor(oldLine, cache, qtyDiscountByBaseUOM).DocumentDiscountRate != lineAmtFields.DocumentDiscountRate ||
					AmountLineFields.GetMapFor(oldLine, cache, qtyDiscountByBaseUOM).OrigDocumentDiscountRate != lineAmtFields.OrigDocumentDiscountRate)
				{
					RecalcTaxes(cache, line, oldLine, oldCurrentLine);
					if (forceFormulaCalculation)
					{
			...
			var taxAttrs = TX.TaxBaseAttribute.GetAttributes<AmountLineFields.taxCategoryID, TX.TaxAttribute>(cache, null);
			if (taxAttrs == null || taxAttrs.Count == 0)
			{
				TX.TaxBaseAttribute.InvokeRecalcTaxes(cache);
			}
		}
			...
			bool skipParameters = false)
		{
			...
			Decimal fullDiscount =
				Math.Abs(RoundCury(cache, lines, totalGroupDiscountAmt)
						+ RoundCury(cache, lines, totalDocumentDiscountAmt)
						- RoundCury(cache, lines, discountTotal));
			...
			TLine line)
		{
			...
			var newValue = discountLineNbrs.Where(x => x.HasValue).Select(x => x.Value).ToArray();
			...
			{
				if (!newValue.Contains(discountDetail.LineNbr ?? 0))
					newValue = newValue.Concat(new[] { discountDetail.LineNbr ?? 0 }).ToArray();
			}

			if (discountFields.DiscountsAppliedToLine == null ||
				!discountFields.DiscountsAppliedToLine.SequenceEqual(newValue))
			{
				discountFields.DiscountsAppliedToLine = newValue;
				cache.MarkUpdated(line);
			}
		}
		...
		protected virtual void SetDocumentDiscount(
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			if (!IsDiscountCalculationNeeded(cache, currentLine, DiscountType.Document))
			{
				if (GetDiscountDetailsByType(cache, discountDetails, DiscountType.ExternalDocument).Any())
					discountCalculationOptions |= DiscountCalculationOptions.CalculateExternalDocumentDiscountsOnly;
				else
					return;
			}
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			...
			if (curyTotalLineAmount / 100 * discountLimit > curyTotalGroupDiscountAmount || discountLimit == 0m)
			{
				...
				if (!skipAutomaticDocumentDiscounts && !discountCalculationOptions.HasFlag(DiscountCalculationOptions.CalculateExternalDocumentDiscountsOnly))
				{
		...
		protected virtual void GetDiscountAmountByType(PXCache cache, List<TDiscountDetail> discountDetails, string type, out decimal totalDiscountAmount, out decimal curyTotalDiscountAmt)
		{
			...
			foreach (TDiscountDetail detail in discountDetails)
			{
				if ((detail.Type == type || type == null) && detail.SkipDiscount != true && detail.IsOrigDocDiscount != true)
				{
					...
					CuryConvBaseSkipRounding(cache, detail, detail.CuryDiscountAmt ?? 0m, out decimal baseDiscountAmt);
		...
		protected virtual void SumAmounts(PXCache cache, List<TLine> lines, out decimal totalLineAmt, out decimal curyTotalLineAmt)
		{
			...
			{
				bool excludeFromDiscountableAmount = false;
				AmountLineFields item = AmountLineFields.GetMapFor(line, cache, qtyDiscountByBaseUOM);
				DiscountLineFields discountedLine = DiscountLineFields.GetMapFor(line, cache);
				...
				if (discountedLine.LineType == null || discountedLine.LineType != SOLineType.Discount || discountedLine.LineType != APLineType.LandedCostAP)
				{
					...
					CuryConvBaseSkipRounding(cache, item, curyLineAmt, out decimal baseLineAmt);
