...
namespace PX.Objects.AR
{
	...
	public class ARAcctSubDefault
	{
		...
		public class ClassListAttribute : CustomListAttribute
		{
			public ClassListAttribute()
                : base(new string[] { MaskLocation, MaskItem, MaskEmployee, MaskCompany, MaskSalesPerson }, new string[] { !PXAccess.FeatureInstalled<FeaturesSet.accountLocations>() ? AR.Messages.MaskCustomer : AR.Messages.MaskLocation, Messages.MaskNonStockItem, Messages.MaskEmployee, Messages.MaskCompany, Messages.MaskSalesPerson })
			{
			}
			...
			public override void CacheAttached(PXCache sender)
			{
				...
				_AllowedLabels = new[] { !PXAccess.FeatureInstalled<FeaturesSet.accountLocations>() ? AR.Messages.MaskCustomer : AR.Messages.MaskLocation, Messages.MaskNonStockItem, Messages.MaskEmployee, Messages.MaskCompany, Messages.MaskSalesPerson };
	...
	public sealed class CustomerRawAttribute : AcctSubAttribute
	{
		...
		public CustomerRawAttribute(Type SearchType)
			: base()
		{
    ...
    [PXRestrictor(typeof(Where<Customer.status, IsNull,
                        ...
                        Or<Customer.status, Equal<CustomerStatus.oneTime>>>>), AR.Messages.CustomerIsInStatus, typeof(Customer.status))]
    ...
    /// And<Where<Customer.status, Equal<Customer.status.active>,
    /// Or<Customer.status, Equal<Customer.status.oneTime>>>>>>),
	...
	public class RestrictCustomerByUserBranches : PXRestrictorAttribute
	{
		...
		public RestrictCustomerByUserBranches()
			: base(BqlCommand.Compose(typeof(Where<,>),
				typeof(Customer.cOrgBAccountID),
				typeof(RestrictByUserBranches<>),
				typeof(Current<>),
				typeof(AccessInfo.userName)),
				ErrorMessages.ElementDoesntExistOrNoRights)
		{
			AcctCD = typeof(Customer.cOrgBAccountID);
		}
		...
		public RestrictCustomerByUserBranches(Type source, Type WhereType)
			: base(BqlCommand.Compose(typeof(Where2<,>),
				typeof(Where<,>),
				source,
				typeof(RestrictByUserBranches<>),
				typeof(Current<>),
				typeof(AccessInfo.userName),
				WhereType),
				Messages.BranchRestrictedByCustomer)
		{
			AcctCD = source;
		}
		...
		public override object[] GetMessageParameters(PXCache sender, object itemres, object row)
		{
			...
			var cacheCustomer = sender.Graph.Caches[BqlCommand.GetItemType(AcctCD)];
			var msgCustomer = cacheCustomer.GetStateExt(PXResult.Unwrap(itemres, BqlCommand.GetItemType(AcctCD)), "acctCD");
	...
	public class RestrictCustomerByOrganization : RestrictBAccountBySource
	{		
		public RestrictCustomerByOrganization(Type orgBAccountID = null)
			:base(BqlCommand.Compose(typeof(Where<,>),
				typeof(Customer.cOrgBAccountID),
				typeof(RestrictByOrganization<>),
				typeof(Current<>),
				orgBAccountID), orgBAccountID, typeof(Customer.acctCD), message: Messages.BranchRestrictedByCustomer)
		{
			ResetBAccount = true;
		}
	}
	public class RestrictCustomerByBranch : RestrictBAccountBySource
	{
		...
		{
			set { ResetBAccount = value; }
			get { return ResetBAccount; }
		}

		public RestrictCustomerByBranch(Type branchID = null)
			: base(BqlCommand.Compose(typeof(Where<,>),
				typeof(Customer.cOrgBAccountID),
				typeof(RestrictByBranch<>),
				typeof(Current<>),
				branchID), source: branchID, acctCD: typeof(Customer.acctCD), message: Messages.BranchRestrictedByCustomer)
		{
		}

		public RestrictCustomerByBranch(Type source, Type branchID = null)
			: base(BqlCommand.Compose(typeof(Where<,>),
				source,
				typeof(RestrictByBranch<>),
				typeof(Current<>),
				branchID), source: branchID, acctCD: source, message: Messages.BranchRestrictedByCustomer)
		{
		}

		public RestrictCustomerByBranch(Type source, Type WhereType, Type branchID = null)
			: base(BqlCommand.Compose(typeof(Where2<,>),
				typeof(Where<,>),
				source,
				typeof(RestrictByBranch<>),
				typeof(Current<>),
				branchID,
				WhereType), source: branchID, acctCD: source, message: Messages.BranchRestrictedByCustomer)
		{
		}
	}

	public class RestrictBranchByCustomer : RestrictBranchBySource
	{
		public override void CacheAttached(PXCache sender)
		{
		...
		public RestrictBranchByCustomer(Type source, Type cOrgBAccountID = null)
			: base(BqlCommand.Compose(
				...
				typeof(Or<,>),
					...
					typeof(Equal<>),
					typeof(Zero)
				), source: source, branchCD: typeof(Branch.branchCD), message: Messages.BranchRestrictedByCustomer)
		{
		}
	}
	...
	public class RestrictCustomerClassByUserBranches : PXRestrictorAttribute
	{
		public RestrictCustomerClassByUserBranches()
			: base(BqlCommand.Compose(typeof(Where<,>),
				...
				typeof(Current<>),
				typeof(AccessInfo.userName)),
				Messages.CustomerClassRestrictedByOrganization, typeof(CustomerClass.customerClassID))
		{
		}
	}
	...
	[PXRestrictor(typeof(Where<PM.PMProject.visibleInAR, Equal<True>>), PM.Messages.ProjectInvisibleInModule, typeof(PM.PMProject.contractCD))]
	public class ARActiveProjectAttibute : PM.ProjectBaseAttribute
	{
		public ARActiveProjectAttibute() : base(null)
		{
		}
	}
}
