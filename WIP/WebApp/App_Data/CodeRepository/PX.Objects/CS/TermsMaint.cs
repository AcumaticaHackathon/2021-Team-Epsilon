...
namespace PX.Objects.CS
{
	public class TermsMaint : PXGraph<TermsMaint, Terms>
	{
		...
		protected virtual void Terms_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			cache.Adjust<PXUIFieldAttribute>(term)
				.For<Terms.prepaymentPct>(a => a.Visible = term.VisibleTo.IsIn(TermsVisibleTo.Customer, TermsVisibleTo.All))
				.SameFor<Terms.prepaymentRequired>();
			PXUIFieldAttribute.SetEnabled<Terms.prepaymentPct>(cache, term, term.PrepaymentRequired == true);
			PXUIFieldAttribute.SetEnabled<Terms.prepaymentRequired>(cache, term, term.InstallmentType == TermsInstallmentType.Single);
		}
		...
		protected virtual void Terms_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if ((e.Operation & PXDBOperation.Command) != PXDBOperation.Delete)
			{
				...
				if (term.DueType == TermsDueType.Custom)
				{
					...
					if (term.DayDue01 <= 0 || term.DayDue01 > 31)
					{
						cache.RaiseExceptionHandling<Terms.dayDue01>(e.Row, term.DayDue01.Value, new PXSetPropertyException(Messages.ValueNotInRange));
						return;
					}
				...
				if (term.VisibleTo.IsNotIn(TermsVisibleTo.Customer, TermsVisibleTo.All))
				{
					cache.SetValueExt<Terms.prepaymentRequired>(e.Row, false);
				}

				if ((term.PrepaymentPct ?? 0m) == 0m && term.PrepaymentRequired == true)
					cache.RaiseExceptionHandling<Terms.prepaymentPct>(term, null, new PXSetPropertyException(Messages.PrepaymentPercentMustBeDefined));
			}
		}
		...
		protected virtual void Terms_InstallmentType_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (term.InstallmentType == TermsInstallmentType.Multiple)
			{
				...
				cache.SetDefaultExt<Terms.prepaymentRequired>(term);
			}
		...
		protected virtual void _(Events.FieldUpdated<Terms, Terms.prepaymentRequired> eventArgs)
		{
			if (eventArgs.Row == null)
				return;

			if (eventArgs.Row.PrepaymentRequired != true && eventArgs.Row.PrepaymentPct != 0m)
				eventArgs.Row.PrepaymentPct = 0m;
		}

		protected virtual void _(Events.FieldVerifying<Terms, Terms.prepaymentPct> eventArgs)
		{
			if (eventArgs.Row == null)
				return;

			decimal? newValue = (decimal?)eventArgs.NewValue;
			if ((newValue ?? 0m) == 0m && eventArgs.Row.PrepaymentRequired == true)
				throw new PXSetPropertyException(Messages.PrepaymentPercentMustBeDefined);
		}
