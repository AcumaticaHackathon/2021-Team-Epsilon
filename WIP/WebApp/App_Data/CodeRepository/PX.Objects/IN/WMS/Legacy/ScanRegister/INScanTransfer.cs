...
namespace PX.Objects.IN
{
	...
	public partial class INScanTransfer : WMSBase
	{
		...
		public class Header : WMSHeader, ILSMaster
		{
			...
			#endregion
			#region ToSiteID
			[Site(DisplayName = "To Warehouse", Enabled = false)]
		...
		{
			Base.transfer.Current = e.NewValue == null ? null : Base.transfer.Search<INRegister.refNbr>(e.NewValue);
		}
		...
		protected virtual void ProcessConfirm()
		{
			...
			if (Transfer == null)
			{
				...
				Base.CurrentDocument.Current.Hold = false;
				Base.CurrentDocument.Current.Status = INDocStatus.Balanced;
				Base.CurrentDocument.Current.NoteID = header.NoteID;
			}
		...
		protected virtual void VerifyConfirmedLine(WMSLineVerifyingArguments<Header, INTran> args)
		{
			if (!EnsureSplitsLotSerial(args.Header.LotSerialNbr, args.Header, args.Rollback)
				...
				|| !EnsureNoUIErrors(args.Line, args.Rollback))
			{
				args.Cancel = true;
				args.Processed = true;
				return;
