...
namespace PX.Objects.RQ
{
	...
	public class RQBiddingProcess : PXGraph<RQBiddingProcess>
	{
		...
		public virtual IEnumerable process(PXAdapter adapter)
		{
			...
			var reqEntry = PXGraph.CreateLazyInstance<RQRequisitionEntry>();
			...
			foreach (RQRequisition rec in adapter.Get<RQRequisition>())
			{
				...
				if (item.Splittable == true)
				{
					if (item.VendorID == null || item.VendorLocationID == null)
					{
						...
						foreach (RQRequisitionLine line in Lines.Select(item.ReqNbr))
						{
							...
							{
								Lines.Cache.RaiseExceptionHandling<RQRequisitionLine.biddingQty>(line, line.BiddingQty, new PXSetPropertyException(Messages.LineBiddingNotComplete));
								hasError = true;
								vendorLocationID = -1;
								break;
							}
				...
				{
					Document.Cache.RaiseExceptionHandling<RQRequisition.vendorLocationID>(item, null, new PXSetPropertyException(Messages.VendorLocationIsNull));
					hasError = true;
				}
				//this.Save.Press();
				...
				if(!hasError)
				{
					RQRequisition source = PXCache<RQRequisition>.CreateCopy(reqEntry.Value.Document.Search<RQRequisition.reqNbr>(item.ReqNbr));
					RQRequisition upd = PXCache<RQRequisition>.CreateCopy(source);

					upd.CompleteBidding(reqEntry.Value);

					item = PXCache<RQRequisition>.CreateCopy(item);

					foreach (var diffField in reqEntry.Value.Document.Cache.GetDifference(source, upd))
						this.Document.Cache.SetValue(item, diffField.Key, diffField.Value.RightValue);
        ...
        protected virtual void RQBidding_CuryQuoteUnitCost_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
        {
            ...
            {
	            var cury = POItemCostManager.FetchCuryInfo<RQBidding.curyInfoID>(this, bidding);
	            
				POItemCostManager.ItemCost cost =
				  POItemCostManager.Fetch(this, bidding.VendorID, bidding.VendorLocationID, docDate: null, curyID: vendor.CuryID, cury.BaseCuryID,
