...
namespace PX.Objects.CS
{
	...
	public class BranchMaint : OrganizationUnitMaintBase<BranchMaint.BranchBAccount, Where<True, Equal<True>>>
	{
		...
		public partial class BranchBAccount : BAccount
		{
			...
			/// </summary>
			[PXRestrictor(typeof(Where<Organization.organizationType, NotEqual<OrganizationTypes.withoutBranches>, 
				And<Organization.organizationType.IsNotEqual<OrganizationTypes.group>>>), GL.Messages.OnlyACompanyWithBranchesCanBeSpecified)]
			[Organization(true,
			...
			[PXDBString(5, IsUnicode = true, BqlField = typeof(Branch.baseCuryID))]
			[PXUIField(DisplayName = "Base Currency ID")]
			[PXDBDefault(typeof(Organization.baseCuryID))]
			[PXSelector(typeof(Search<CurrencyList.curyID>))]
			public override string BaseCuryID { get; set; }
			#endregion
			#region BAccountBaseCuryID
			public abstract class bAccountBaseCuryID : PX.Data.BQL.BqlString.Field<bAccountBaseCuryID> { }

			[PXDBString(5, IsUnicode = true, BqlField = typeof(BAccount.baseCuryID))]
			[PXDBDefault(typeof(Organization.baseCuryID))]
			public virtual string BAccountBaseCuryID { get; set; }
			#endregion
				...
				if (branch != null)
		{
			...
			CR.BAccount2 bAccount = PXSelectReadonly2<
				...
				InnerJoin<CRLocation, 
					On<CR.BAccount2.bAccountID, Equal<CRLocation.bAccountID>>,
				InnerJoin<Branch,
					On<CRLocation.cBranchID, Equal<Branch.branchID>>>>,
				Where<Branch.branchCD, Equal<Required<Branch.branchCD>>>>
			...
			bAccount = PXSelectReadonly2<
				...
				InnerJoin<CRLocation,
					On<CR.BAccount2.bAccountID, Equal<CRLocation.bAccountID>>,
				InnerJoin<Branch,
					On<CRLocation.vBranchID, Equal<Branch.branchID>>>>,
				Where<Branch.branchCD, Equal<Required<Branch.branchCD>>>>
				.SelectSingleBound(this, null, branchBAccount.BranchBranchCD);
		...
		protected virtual void Address_CountryID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			if (e.Row == null) return;

			Organization organization = CurrentOrganizationView.Select();
			if (organization != null)
			{
				Address curOrganizationAddress = PXSelect<Address, Where<Address.bAccountID, Equal<Required<Address.bAccountID>>>>.Select(this, organization.BAccountID);
				if (curOrganizationAddress != null)
				{
					e.NewValue = curOrganizationAddress.CountryID;
					e.Cancel = true;
				}
			}
		}
		...
		public class DefContactAddressExt : DefContactAddressExt<BranchMaint, BranchBAccount, BranchBAccount.acctName>
			.WithPersistentAddressValidation
		{
			...
			protected virtual void BranchBAccount_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
			{
				...
				Base.CurrentOrganizationView.Current = Base.CurrentOrganizationView.Select();

				Organization curOrganization = Base.CurrentOrganizationView.Current;
				...
				bool isPersistedBranch = !Base.IsPrimaryObjectInserted();
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class DefLocationExt : DefLocationExt<BranchMaint, DefContactAddressExt, LocationDetailsExt, BranchBAccount, BranchBAccount.bAccountID, BranchBAccount.defLocationID>
			.WithUIExtension
		{
			#region Actions
			...
			{
				BranchBAccount acct = Base.BAccount.Current;
				if (Base1.Locations.Current != null && acct != null && Base1.Locations.Current.LocationID != acct.DefLocationID)
				{
					acct.DefLocationID = Base1.Locations.Current.LocationID;
			...
			protected override void _(Events.CacheAttached<BranchBAccount.defLocationID> e) { }

			protected virtual void _(Events.RowSelected<BranchBAccount> e)
			{
				Organization curOrganization = Base.CurrentOrganizationView.Current;
				...
				DefLocationAddress.AllowUpdate = canEditBranch;
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class LocationDetailsExt : LocationDetailsExt<BranchMaint, DefContactAddressExt, BranchBAccount, BranchBAccount.bAccountID>
		{
			#region Events

			protected virtual void _(Events.RowPersisted<BranchBAccount> e)
			{
				if (e.TranStatus == PXTranStatus.Open && (e.Operation & PXDBOperation.Delete) == PXDBOperation.Delete)
				{
					int? id = PXAccess.GetBranchID(e.Row.BranchBranchCD?.Trim());
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class BranchMaintAddressLookupExtension : CR.Extensions.AddressLookupExtension<BranchMaint, BranchBAccount, Address>
		{
			protected override bool GetIsEnabled()
			{
				return base.GetIsEnabled() && Base.GetExtension<DefContactAddressExt>().DefAddress.AllowUpdate;
			}
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class BranchMaintDefLocationAddressLookupExtension : CR.Extensions.AddressLookupExtension<BranchMaint, BranchBAccount, Address>
		{
			protected override bool GetIsEnabled()
			{
				return base.GetIsEnabled() && Base.GetExtension<DefLocationExt>().DefLocation.AllowUpdate;
			}
		...
		/// <exclude/>
		public class ExtendToCustomer : OrganizationUnitExtendToCustomer<BranchMaint, BranchBAccount>
		{
			...
			{
				return new SourceAccountMapping(typeof(BranchBAccount));
			}
		}

		/// <exclude/>
		public class ExtendToVendor : OrganizationUnitExtendToVendor<BranchMaint, BranchBAccount>
		{
			public static bool IsActive() => PXAccess.FeatureInstalled<FeaturesSet.interBranch>();
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class BranchSharedContactOverrideGraphExt : SharedChildOverrideGraphExt<BranchMaint, BranchSharedContactOverrideGraphExt>
		{
			...
			{
				return new RelatedMapping(typeof(BranchBAccount))
				{
					RelatedID = typeof(BranchBAccount.bAccountID),
					ChildID = typeof(BranchBAccount.defContactID)
				};
			}
		...
		/// <exclude/>
		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class BranchSharedAddressOverrideGraphExt : SharedChildOverrideGraphExt<BranchMaint, BranchSharedAddressOverrideGraphExt>
		{
			#region Initialization 
			...
			protected override RelatedMapping GetRelatedMapping()
			{
				...
				{
					RelatedID = typeof(BranchBAccount.bAccountID),
					ChildID = typeof(BranchBAccount.defAddressID)
				};
			}
