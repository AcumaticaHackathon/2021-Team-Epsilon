...
namespace PX.Objects.SO
{
	public partial class SOInvoiceEntry : ARInvoiceEntry
	{
		...
		public void ReleaseInvoiceProcImpl(List<ARRegister> list, bool isMassProcess)
		{
			...
			try
			{
				...
				ARDocumentRelease.ReleaseDoc(list, isMassProcess, null, null, delegate (ARRegister ardoc)
				{
					...
					SOInvoice soInvoice = this.SODocument.Select(ardoc.DocType, ardoc.RefNbr);
					if (soInvoice?.HasLegacyCCTran == true)
					{
						throw new PXException(Messages.CantProcessSOInvoiceBecauseItHasLegacyCCTran, soInvoice.RefNbr);
					}
					...
					if (soInvoice?.CreateINDoc == true)
					{
						...
						this.PostInvoice(invoicePostingContext, ardoc as ARInvoice, issues);
			...
			finally
			{
				...
				PXAutomation.StorePersisted(this, typeof(ARInvoice), new List<object>(processedInvoices));
			}
		}
			...
			: base()
		{
			...
			Transactions.View = new PXView(this, false, cmd);
		...
		public override void Persist()
		{
			...
			if (IsDeletingInvoiceWithExternalTransactions(out var invoice))
			{
				VoidRelatedPayment(invoice);
				return;
			}
		...
		protected virtual bool IsDeletingInvoiceWithExternalTransactions(out (ARInvoice ar, SOInvoice so) invoice)
		{
			foreach (ARInvoice ar in Document.Cache.Deleted)
			{
				foreach (SOInvoice so in SODocument.Cache.Deleted)
				{
					if (string.Equals(so.DocType, ar.DocType) && string.Equals(so.RefNbr, ar.RefNbr) &&
						(ar.IsCCPayment == true || so.IsCCPayment == true) && ExternalTran.View.SelectMultiBound(new object[] { ar, so }).Count > 0)
					{
						invoice = (ar, so);
						return true;
					}
		...
		protected virtual void VoidRelatedPayment((ARInvoice ar, SOInvoice so) invoice)
		{
			...
			ARPayment payment = new ARPayment()
			{
				...
				AdjDate = invoice.so.AdjDate,
				AdjFinPeriodID = invoice.so.AdjFinPeriodID
			};

			payment = PXCache<ARPayment>.CreateCopy(docgraph.Document.Insert(payment));
			payment.CustomerID = invoice.ar.CustomerID;
			payment.CustomerLocationID = invoice.ar.CustomerLocationID;
			payment.ARAccountID = invoice.ar.ARAccountID;
			payment.ARSubID = invoice.ar.ARSubID;

			payment.PaymentMethodID = invoice.so.PaymentMethodID;
			payment.PMInstanceID = invoice.so.PMInstanceID;
			payment.CashAccountID = invoice.so.CashAccountID;
			payment.ExtRefNbr = invoice.so.ExtRefNbr;
			payment.CuryOrigDocAmt = invoice.so.CuryPaymentAmt;
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				ARReleaseProcess.SetPaymentReferenceOnCCTran(docgraph, invoice.so, docgraph.Document.Current);
				ExternalTran.View.Clear();
				ccProcTran.View.Clear();
				docgraph.Document.Cache.RaiseRowSelected(docgraph.Document.Current);

				PXFieldState voidState = (PXFieldState)docgraph.voidCheck.GetState(Document.Current);
				if (voidState == null || voidState.Enabled == false)
					throw new PXException(AR.Messages.ERR_CCTransactionMustBeVoided);

				var tovoid = new List<object> { docgraph.Document.Current };
				...
				docgraph.voidCheck
					.Press(
						new PXAdapter(
							new PXView.Dummy(
								docgraph,
								docgraph.Document.View.BqlSelect,
								tovoid)))
					.Consume();
		...
		protected virtual void VerifyDocumentBalanceAgainstAdjustmentBalance()
		{
					...
					&& ardoc.ApplyPaymentWhenTaxAvailable != true))
			{
				...
				if (arDoc.CuryDocBal - arDoc.CuryBalanceWOTotal - arDoc.CuryPaymentTotal - soDoc.CuryCCCapturedAmt < 0m)
				{
		...
		protected override void ARTran_CuryUnitPrice_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
		{
			...
			if (row?.InventoryID != null && row.UOM != null && row.IsFree != true && row.ManualPrice != true && !cancelUnitPriceCalculation)
			{
				...
				decimal? price = ARSalesPriceMaint.CalculateSalesPrice(
					...
					currencyinfo.Select(), row.UOM, row.Qty, date, row.CuryUnitPrice, taxCalcMode);
		...
		protected override void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			cache.Adjust<PXUIFieldAttribute>()
				.For<ARInvoice.curyPaymentTotal>(a => a.Visible = 
				...
				.SameFor<ARInvoice.curyCCAuthorizedAmt>()
				...
				&& String.IsNullOrEmpty(row.DocType) == false && String.IsNullOrEmpty(row.RefNbr) == false)
			{
					...
					|| row.CuryUnreleasedPaymentAmt == null || row.CuryBalanceWOTotal == null)
				{
					using (new PXConnectionScope())
					{
						...
						PXDBCurrencyAttribute.CalcBaseValues<ARInvoice.curyUnreleasedPaymentAmt>(sender, e.Row);
						sender.RaiseFieldUpdated<ARInvoice.curyCCAuthorizedAmt>(e.Row, null);
						PXDBCurrencyAttribute.CalcBaseValues<ARInvoice.curyCCAuthorizedAmt>(sender, e.Row);
						sender.RaiseFieldUpdated<ARInvoice.curyPaidAmt>(e.Row, null);
						PXDBCurrencyAttribute.CalcBaseValues<ARInvoice.curyPaidAmt>(sender, e.Row);
		...
		protected override void ARTran_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			if (siblings.Count == 0)
			{
				if (ordershipment != null)
				{
					...
					if (ordershipment.CreateINDoc == true && ordershipment.InvtRefNbr == null)
			...
			bool DiscOnDiscDate)
		{
			...
			InvoiceEntry.CalcBalancesFromInvoiceSide(adj, invoice, payment, CurrencyInfo_CuryInfoID, isCalcRGOL, DiscOnDiscDate, others);
		}
		...
		protected virtual ARAdjust2 GetOtherAppsOfPaymentGrouped(ARAdjust2 adj)
		{
			ARAdjust2 other = PXSelectGroupBy<ARAdjust2,
				...
				Aggregate<GroupBy<ARAdjust2.adjgDocType, GroupBy<ARAdjust2.adjgRefNbr,
					Sum<ARAdjust2.curyAdjgAmt, Sum<ARAdjust2.adjAmt>>>>>>
		...
		public virtual PXResultset<ARAdjust2, ARPayment, ExternalTransaction> LoadDocumentsProc(int applcount)
		{
				...
				&& Document.Current.Released == false)
			{
				...
				using (new ReadOnlyScope(Adjustments.Cache, Document.Cache, arbalances.Cache, SODocument.Cache, PaymentTotalsUpd.Cache))
				{
						...
						foreach (ARTran tran in Transactions.Select())
						{
							if (!string.IsNullOrEmpty(tran.SOOrderType) && !string.IsNullOrEmpty(tran.SOOrderNbr))
							{
								...
								if (!orderProcessed.Contains(key))
								{
									...
									foreach (PXResult<ARPayment, CurrencyInfo, ExternalTransaction> res
					...
					foreach (PXResult<AR.ARPayment, CurrencyInfo, ExternalTransaction> res in list)
					{
						ARPayment payment = PXCache<ARPayment>.CreateCopy(res);
						...
						var adjFromCache = (ARAdjust2)Adjustments.Cache.Locate(adj);
						if (adjFromCache == null || Adjustments.Cache.GetStatus(adjFromCache).IsIn(PXEntryStatus.InsertedDeleted, PXEntryStatus.Deleted))
						{
							...
							adjs.Add(new PXResult<ARAdjust2, ARPayment, ExternalTransaction>(Adjustments.Insert(adj), payment, res));
						}
					}
				}
			}
		...
		public virtual void InvoiceOrder(DateTime invoiceDate, PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact> order, PXResultset<SOShipLine, SOLine> details, Customer customer, InvoiceList list, PXQuickProcess.ActionFlow quickProcessFlow, bool groupByDefaultOperation)
		{
			...
			foreach (PXResult<SOShipLine, SOLine, SOSalesPerTran, ARTran, ARTranAccrueCost> res in
				SelectLinesToInvoice(orderShipment))
			{
			...
			ResortTransactions(set, this.UnattendedMode);
			...
			orderShipment = shipmentlist.Update(orderShipment);
			orderShipment = orderShipment.LinkInvoice(SODocument.Current, this);
				...
				foreach (SOOrderDiscountDetail docGroupDisc in discountCodesWithApplicableSOLines.Value.LeftValues)
                {
					...
					foreach (SOLine soLine in discountCodesWithApplicableSOLines.Value.RightsFor(docGroupDisc))
					{
						foreach (ARTran tran in Transactions.Select())
						{
							...
							if (linkedToSOLine || !PXAccess.FeatureInstalled<FeaturesSet.customerDiscounts>())
							{
		...
		protected virtual void ResortTransactions(HashSet<ARTran> set, bool fullResort)
		{
			...
			for (int i = 0; i < linesToResort.Count; i++)
			{
				...
				if (linesToResort[i].Item2.SortOrder != lastSortOrderNbr)
				{
					...
					Transactions.Cache.MarkUpdated(linesToResort[i].Item2);
				}
			}
		...
		protected virtual void InsertApplications(SOOrderShipment orderShipment)
		{
		...
		{
			GetFreightAccountAndSubAccount(order, ShipVia, ownerGuid != null ? PXAccess.GetContactID(ownerGuid) : (int?)null, out accountID, out subID);
		}
		...
		public void UpdateRelatedSOOrderPreAuthAmount(CCTranType lastOperation, ExternalTransaction tran, ExternalTransactionState state)
		{
		...
		public void UpdateRelatedSOOrderCapturedAmount(CCTranType lastOperation, IExternalTransaction tran, ExternalTransactionState state)
		{
			var hasOrigDocRef = tran.OrigDocType != null && tran.OrigRefNbr != null;
