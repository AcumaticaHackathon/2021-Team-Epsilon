			...
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Update)
			{
				...
				if (foundCAAdj != null)
				{
					...
					if (foundCAEntryType != null)
					{
						...
						{
							if (sender.RaiseExceptionHandling<CAEntryType.drCr>(e.Row, row.DrCr, new PXSetPropertyException(Messages.CantEditDisbReceipt, PXErrorLevel.RowError, $"[{nameof(CAEntryType.drCr)}]")))
							{
								throw new PXRowPersistingException(typeof(CAEntryType.drCr).Name, row.DrCr, Messages.CantEditDisbReceipt, nameof(CAEntryType.drCr));
							}
						}
						if (row.Module != foundCAEntryType.Module)
						{
							if (sender.RaiseExceptionHandling<CAEntryType.module>(e.Row, row.Module, new PXSetPropertyException(Messages.CantEditModule, PXErrorLevel.RowError, $"[{nameof(CAEntryType.module)}]")))
							{
								throw new PXRowPersistingException(typeof(CAEntryType.module).Name, row.Module, Messages.CantEditModule, nameof(CAEntryType.module));
							}
						}
					}
				}
