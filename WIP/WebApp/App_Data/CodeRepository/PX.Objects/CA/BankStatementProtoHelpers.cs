...
namespace PX.Objects.CA.BankStatementProtoHelpers
{
    ...
    public class PXInvoiceSelectorAttribute : PXCustomSelectorAttribute
	{
		...
		public static IEnumerable<APAdjust.APInvoice> GetRecordsAP(string AdjdDocType, int? TranID, int? AdjNbr, CABankTran currentBankTran, PXCache adjustments, PXGraph graph)
		{
			foreach (APAdjust.APInvoice result in PXSelectJoin<APAdjust.APInvoice,
								 ...
								 .Select(graph, TranID, AdjNbr, AdjNbr, currentBankTran.PayeeBAccountID, AdjdDocType, currentBankTran.DocType, currentBankTran.DocType, currentBankTran.DocType, currentBankTran.DocType, currentBankTran.MatchingPaymentDate, currentBankTran.MatchingFinPeriodID, currentBankTran.DocType))
			{
				if (ShouldSkipRecord(result.DocType, result.RefNbr, TranID, AdjNbr, currentBankTran, adjustments, graph)) continue;
				yield return result;
			}
	...
	public static class StatementsMatchingProto
	{
		...
		public static decimal CompareDate(CABankTran aDetail, CABankTranInvoiceMatch aTran, Double meanValue, Double sigma)
		{
			DateTime? comparedDate = (aTran?.DiscDate >= aDetail.TranDate.Value) ? aTran?.DiscDate : aTran?.DueDate;
			return CompareDate(aDetail, comparedDate, meanValue, sigma);
		}
