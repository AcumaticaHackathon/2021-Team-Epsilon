			...
			context.AddScreenConfigurationFor(screen =>
			{
				return screen
					...
					.WithActions(actions =>
					{
						...
						actions.Add(g => g.submitQuote, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
						actions.Add(g => g.editQuote, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
						actions.Add(g => g.createSalesOrder, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
						actions.Add(g => g.createInvoice, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
						actions.Add(g => g.validateAddresses, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
						actions.Add<CreateContactExt>(e => e.CreateContact, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
						actions.Add<CreateAccountExt>(e => e.CreateBothContactAndAccount, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
						actions.Add<OpportunityMaint.Discount>(e => e.recalculatePrices, c => c.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
                    })
                    .WithForms(forms =>
					{
						forms.Add(formOpen);
