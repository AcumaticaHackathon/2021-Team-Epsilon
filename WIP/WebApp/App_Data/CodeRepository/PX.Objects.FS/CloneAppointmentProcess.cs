...
namespace PX.Objects.FS
{
    public class CloneAppointmentProcess : PXGraph<CloneAppointmentProcess>
    {
        ...
        public virtual void CloneAppointment(AppointmentEntry gOriginalAppt, AppointmentEntry gNewAppt, DateTime newApptBeginDate, int newApptDuration)
        {
            ...
            fsAppointmentRow.MinLogTimeBegin = null;
            fsAppointmentRow.MaxLogTimeEnd = null;

            fsAppointmentRow.NotStarted = false;
            fsAppointmentRow.Awaiting = false;
            fsAppointmentRow.InProcess = false;
            fsAppointmentRow.Paused = false;
            fsAppointmentRow.Hold = false;
            fsAppointmentRow.Completed = false;
            fsAppointmentRow.Closed = false;
            fsAppointmentRow.Billed = false;
            ...
            foreach (FSAppointmentDet originalItemRow in gOriginalAppt.AppointmentDetails.Select()
                                                            ...
                                                            .Where(row => string.IsNullOrEmpty(row.LinkedEntityType) == true
                                                                          && string.IsNullOrEmpty(row.LinkedDocRefNbr) == true))
            {
        ...
        public virtual void CloneParts(AppointmentEntry gOriginalAppt, AppointmentEntry gNewAppt, FSAppointmentDet sourceRow)
        {
            ...
            FSSODet fsSODetRow = GetSODetFromAppointmentDet(gOriginalAppt, sourceRow);
            if (fsSODetRow == null)
            {
                return;
            }
