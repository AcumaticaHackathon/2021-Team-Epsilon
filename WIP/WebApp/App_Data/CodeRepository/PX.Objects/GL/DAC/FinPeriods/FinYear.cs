...
namespace PX.Objects.GL.FinPeriods.TableDefinition
{
	...
	{
		#region Keys
		public class PK : PrimaryKeyOf<FinYear>.By<organizationID, year>
		{
			public static FinYear Find(PXGraph graph, int? organizationID, string year) => FindBy(graph, organizationID, year);
			public static FinYear FindByBranch(PXGraph graph, int? branchID, string year)
				=> (FinYear)PXSelectJoin<FinYear, InnerJoin<Branch, On<Branch.organizationID, Equal<FinYear.organizationID>>>,
		...
		{
			public class Organization : PX.Objects.GL.DAC.Organization.PK.ForeignKeyOf<FinYear>.By<organizationID> { }
		}
		#endregion
