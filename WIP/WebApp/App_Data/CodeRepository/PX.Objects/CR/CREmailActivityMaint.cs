...
namespace PX.Objects.CR
{
	public class CREmailActivityMaint : CRBaseActivityMaint<CREmailActivityMaint, CRSMEmail>
	{
		...
		public CREmailActivityMaint()
		{
			...
			FieldSelecting.AddHandler(typeof(CRSMEmail), RefNoteType_CbApi_FieldName, RefNoteTypeFieldSelecting);
		}
		...
		public virtual void loadEmailSource()
		{
			...
			if (res == WebDialogResult.OK)
			{
				if (NotificationInfo.Current.Type == TemplateSourceType.Notification)
				{
					...
					object row = new EntityHelper(this).GetEntityRow(Message.Current.RefNoteID, true);
					if (row == null) return;
					Type graphType;
					var type = row.GetType();
					PXPrimaryGraphAttribute.FindPrimaryGraph(this.Caches[type], true, ref row, out graphType);
					type = row.GetType(); // it could be changed
		...
		[PXRestrictor(typeof(Where<
			...
			Or<Where<
				...
				And<EMailAccount.defaultOwnerID, Equal<Current<AccessInfo.contactID>>>>>>), ErrorMessages.RestrictedEmailAccount, typeof(EMailAccount.description))]
		[PXDefault]
		[PXMergeAttributes(Method = MergeMethod.Merge)]
		...
		protected virtual void _(Events.FieldSelecting<CRSMEmail, CRSMEmail.source> e)
		{
			if (IsContractBasedAPI && e.Row is CRSMEmail && e.Row.RefNoteID is Guid refNoteID)
			{
		...
		{
			EMailAccount acc = PXSelect<
					EMailAccount,
				Where<
					EMailAccount.emailAccountID, Equal<Required<EMailAccount.emailAccountID>>,
					And<Match<Current<AccessInfo.userName>>>>>
				.SelectSingleBound(this, null, accountID);

			return acc != null
					&& (acc.EmailAccountType != EmailAccountTypesAttribute.Exchange || acc.DefaultOwnerID == this.Accessinfo.ContactID);
		}
