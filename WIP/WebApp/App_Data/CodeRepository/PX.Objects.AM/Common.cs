...
namespace PX.Objects.AM
{
    public static class Common
    {
        ...
        public static class Cache
        {
            ...
            /// </summary>
            public static void AddRowToCache<dac>(PXGraph graph, dac row) 
                where dac : class, IBqlTable, new()
            {
                if(row == null || graph == null)
                {
                    return;
                }

                // Held is like NotChanged but will be retained during the roundtrip
                graph.Caches[typeof(dac)].SetStatus(row, PXEntryStatus.Held);
            }
        }
