...
namespace PX.Objects.AM
{
    public class ProdDetail : PXGraph<ProdDetail, AMProdItem>
    {
        ...
        // Allow the operation fields to display the CD value - useful for API calls
        [PXMergeAttributes(Method = MergeMethod.Append)]
        ...
        protected virtual void _(Events.CacheAttached<AMProdMatl.operationID> e) { }
        ...
        [PXSelector(typeof(Search<AMProdOper.operationID,
	        ...
	        SubstituteKey = typeof(AMProdOper.operationCD),
            ValidateValue = false, DirtyRead = true)]
        protected virtual void _(Events.CacheAttached<AMProdStep.operationID> e) { }
        ...
        [PXSelector(typeof(Search<AMProdOper.operationID,
	        ...
	        SubstituteKey = typeof(AMProdOper.operationCD),
            ValidateValue = false, DirtyRead = true)]
        protected virtual void _(Events.CacheAttached<AMProdTool.operationID> e) { }
        ...
        [PXSelector(typeof(Search<AMProdOper.operationID,
	        ...
	        SubstituteKey = typeof(AMProdOper.operationCD),
            ValidateValue = false, DirtyRead = true)]
        protected virtual void _(Events.CacheAttached<AMProdOvhd.operationID> e) { }
        ...
        public override void Persist()
        {
            ...
            if (!reschedule || IsImport || IsContractBasedAPI)
            {
                return;
            }
            ...
            PXLongOperation.StartOperation(this, Reschedule);
        }
        ...
        protected virtual bool OperationChangeRequiresReschedule(AMProdOper row1, AMProdOper row2)
        {
            return row1 != null && row2 != null
                                && (row1.WcID != row2.WcID
                                    ...
                                    || row1.FinishTime != row2.FinishTime
        ...
        protected virtual void _(Events.FieldDefaulting<AMProdMatl, AMProdMatl.tranDate> e)
        {
            if (e.Row?.OperationID == null || e.Row.InventoryID == null || ProdOperSelected.Current == null)
            {
                return;
            }
            e.NewValue = ProdOperSelected.Current.StartDate;
        }

        protected virtual void _(Events.RowInserting<AMProdMatl> e)
        {
            if((e.Row?.QtyRemaining ?? 0m) != 0m)
            {
                return;
            }

            // Resolves late evaluation to QtyRemaining which impacts allocations records during import/API
            e.Row.QtyRemaining = PXFormulaAttribute.Evaluate<AMProdMatl.qtyRemaining>(e.Cache, e.Row) as decimal?;
        }
            ...
            foreach (PXResult<INItemPlan, INTranSplit, INTran, INPlanType, INItemPlanDemand> res in list)
            {
                ...
                if (origSchedule != null && (origSchedule.Completed == false || prodMatlSplit.Cache.GetStatus(origSchedule) == PXEntryStatus.Updated))
                {
                    ...
                    else
                    {
                        ...
                        schedule.POLineNbr = origSchedule.POLineNbr;
        ...
        {
            TriggerItemPlanUpdates(this, ProdItemRecords.Current);
        }

        internal static void TriggerItemPlanUpdates(PXGraph graph, AMProdItem prodItem)
        {
            ...
            var prodItemRowStatus = graph.Caches[typeof(AMProdItem)].GetStatus(prodItem);
            var oldHold = (bool?)graph.Caches[typeof(AMProdItem)].GetValueOriginal<AMProdItem.hold>(prodItem);
            if (prodItemRowStatus != PXEntryStatus.Updated || oldHold == null || prodItem.Hold.GetValueOrDefault() == oldHold.GetValueOrDefault())
            {
                return;
            }

            foreach (AMProdItemSplit row in PXSelect<AMProdItemSplit, 
            ...
            {
                graph.Caches[typeof(AMProdItemSplit)].Update(row); //raise events to trigger hold change in AMProdItemSplitPlanIDAttribute
            }
            ...
            foreach (AMProdMatlSplit row in PXSelect<AMProdMatlSplit,
                Where<AMProdMatlSplit.orderType, Equal<Required<AMProdMatl.orderType>>,
                    And<AMProdMatlSplit.prodOrdID, Equal<Required<AMProdMatl.prodOrdID>>>>>.Select(graph, prodItem.OrderType, prodItem.ProdOrdID))
            {
                var prodMatl = graph.Caches[typeof(AMProdMatlSplit)].LocateElse(row);
                if (prodMatl?.InventoryID == null)
                {
                    continue;
                }
                graph.Caches[typeof(AMProdMatlSplit)].Update(row); //raise events to trigger hold change in AMProdMatlSplitPlanIDAttribute
            }
        }
    }
