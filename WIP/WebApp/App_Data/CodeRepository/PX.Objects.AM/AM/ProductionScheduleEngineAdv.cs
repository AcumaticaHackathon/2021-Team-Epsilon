...
namespace PX.Objects.AM
{
    ...
    public class ProductionScheduleEngineAdv : ProductionScheduleEngine
    {
        ...
        {
            //if the time is the very last block of the day, do not round it
            if (date.TimeOfDay.TotalMinutes > 1425)
                return date;
        ...
        public virtual void Process(AMSchdItem schdItem)
        {
            ...
            PXSelectBase<AMSchdOper> cmd = new PXSelectJoin<AMSchdOper,
                ...
                Where<AMSchdOper.orderType, Equal<Required<AMSchdOper.orderType>>,
                    ...
                    And<AMSchdOper.schdID, Equal<Required<AMSchdOper.schdID>>>>>>(this);
                ...
                try
                {
                    ...
                    if (Common.Dates.Compare(planDate, finiteDateWindow) <= 0)
                    {
                        ...
                        if (scheduleBack)
                            ...
                            cmd.OrderByNew<OrderBy<Asc<AMSchdOper.lineNbr, Asc<AMSchdOper.sortOrder>>>>();

                        resultset = cmd.Select(schdItem.OrderType, schdItem.ProdOrdID, schdItem.SchdID);
#if DEBUG
                    ...
                    else
                    {
                        ...
                        prodItem.ConstDate = planDate;
                        prodItem.SchedulingMethod = ScheduleMethod.FinishOn;
                        if (Common.Dates.Compare(prodItem.StartDate, Common.Current.BusinessDate(this)) <= 0)
                        {
                            prodItem.ConstDate = Common.Current.BusinessDate(this);
                            prodItem.SchedulingMethod = ScheduleMethod.StartOn;
                        }
                        ProdItems.Update(prodItem);
                        ProcessSchedule(prodItem);
                    }
        ...
        private AMSchdItem ProcessSchdItem(AMSchdItem schdItem, PXResultset<AMSchdOper> orderSchdResults, bool scheduleBack, ref DateTime currentSchedDate)
        {
            ...
            foreach (PXResult<AMSchdOper, AMWC, AMProdOper> result in orderSchdResults)
            {
                ...
                var schdOper = ProcessOperation(schdItem, schdOperResult, prodOper, workCenter, scheduleBack ? ReadDirection.Backward : ReadDirection.Forward, 
                    ref currentSchedDate);
                if (schdOper == null)
                {
                    continue;
                }
                ...
                if (checkMaterial)
                {
                    ProcessSchdOperMaterial(schdOper, prodOper, schdOper.StartDate.GetValueOrDefault(), 
                        schdItem.StartDate ?? schdOper.StartDate.GetValueOrDefault());
                }
        ...
        private void ProcessSchdOperMaterial(AMSchdOper schdOper, AMProdOper prodOper, DateTime scheduleDate, DateTime planDate)
        {
            var materialAvailabilityList = MaterialAvailability(this, scheduleDate, schdOper.OrderType, schdOper.ProdOrdID, schdOper.OperationID, planDate);
...
#endif
            var totalShiftSchdTime = GetSchdOperDetailCacheInserted(prodOper)?.Sum(c => c.RunTime);
            if(totalShiftSchdTime != null)
            {
                // Sync total plan time as previous number is without any shift specific crew size/efficiency 
                schdOper.TotalPlanTime = totalShiftSchdTime.GetValueOrDefault();
            }
        ...
        private void ProcessWorkCenterSchedule(WorkCenterCalendarHelper workCenterCalendar, ReadDirection scheduleDirection, ref AMSchdOper schdOper, ref DateTime currentSchedDateTime, ref int remainingSchdTime)
        {
            ...
            var useCrewEff = UseShiftCrewSizeEfficiency(workCenterCalendar.WorkCenter);
#if DEBUG
            ...
            {
                // We need to use the shift specific time for crew and efficiency
                var shiftRemainingSchdTime = useCrewEff
                    ? CalcTotalEfficiencyTime(remainingSchdTime, amShift.CrewSize, amShift.ShftEff)
                    : remainingSchdTime;

                ProcessOperationShift(schdOper, amShift, wcDateInfo.ShiftDateInfos[amShift.ShiftID],
                    workCenterCalendar, scheduleDirection, ref currentSchedDateTime, ref shiftRemainingSchdTime);

                remainingSchdTime = useCrewEff
                    ? CalcTotalEfficiencyTimeReverse(shiftRemainingSchdTime, amShift.CrewSize, amShift.ShftEff)
                    : shiftRemainingSchdTime;
        ...
        private DateTime AdjusteDateToEarliestDateTime(AMDateInfo dateInfo, DateTime currentSchedDateTime, ReadDirection scheduleDirection)
        {
            ...
            //if scheduling backwards and the time is before the earliest shift time, use the last time of the day
            if (scheduleDirection == ReadDirection.Backward &&
                currentSchedDateTime.TimeOfDay.CompareTo(dateInfo.StartTime) < 0)
            {
        ...
        private void ProcessOperationShift(AMSchdOper schdOper, AMShift wcShift, AMDateInfo shiftDateInfo, WorkCenterCalendarHelper workCenterCalendar, 
            ReadDirection scheduleDirection, ref DateTime currentSchedDateTime, ref int remainingSchdTime)
        {
            if (schdOper == null)
            {
                throw new ArgumentNullException(nameof(schdOper));
            ...
            ref int remainingSchdTime, ref DateTime currentSchedDateTime)
        {
            ...
            {
                var inserted = InsertAMWCSchdDetail(schdDetail, schdOper, workCenter, wcShift);
                if (inserted == null)
                {
                    continue;
                }
            ...
            ReadDirection scheduleDirection, int remainingSchdTime, DateTime currentSchedDateTime, out List<AMWCSchdDetail> wcSchdDetailList)
        {
            ...
            var useCrewEfficiency = UseShiftCrewSizeEfficiency(workCenter);
            ...
            foreach (var amwcSchdDetail in schdDateDetail.Results)
            {
                ...
                var maxedOut = usedSchdTime + amwcSchdDetail.RunTime.GetValueOrDefault() > remainingSchdTime;
                if (maxedOut)
                {
                    ...
                    amwcSchdDetail.RunTimeBase = partialTime;
                    amwcSchdDetail.RunTime = partialTime;
                ...
                if(useCrewEfficiency && amwcSchdDetail.RunTimeBase > 0)
                {
                    amwcSchdDetail.RunTimeBase = CalcTotalEfficiencyTimeReverse(amwcSchdDetail.RunTimeBase, wcShift?.CrewSize, wcShift?.ShftEff);
                }

                amwcSchdDetail.SchdTime = amwcSchdDetail.RunTime;
                wcSchdDetailList.Add(amwcSchdDetail);
                usedSchdTime += amwcSchdDetail.RunTime.GetValueOrDefault();
        ...
        private void CalculateUsedTime(List<AMWCSchdDetail> wcSchdDetailList, ReadDirection scheduleDirection, DateTime currentSchedDateTime, out int usedTime, out DateTime schedDateTime)
        {
            ...
            {
                usedTime += schdDetail.RunTime.GetValueOrDefault();
        ...
        protected override AMWCSchd GetWorkCenterSchd(string workCenterID, string shiftID, int? siteID, AMDateInfo dateInfo, int workingMinutes)
        {
            ...
            if (wcSchd != null)
            {
                DeleteExistingBreaks(wcSchd, dateInfo);
        ...
        private void DeleteExistingBreaks(AMWCSchd wcSchd, AMDateInfo dateInfo)
        {
            var breaks = SelectFrom<AMWCSchdDetail>.Where<AMWCSchdDetail.wcID.IsEqual<@P.AsString>
                .And<AMWCSchdDetail.shiftID.IsEqual<@P.AsString>>
                .And<AMWCSchdDetail.schdDate.IsEqual<@P.AsDateTime>>
                .And<AMWCSchdDetail.isBreak.IsEqual<True>>>.View.Select(this, wcSchd.WcID, wcSchd.ShiftID, wcSchd.SchdDate);
            foreach(var result in breaks)
            {
                WorkCenterSchdDetailRecs.Delete(result);
            }
        }
        ...
        internal IEnumerable<AMSchdOperDetail> GetSchdOperDetailCacheInserted(IProdOper operation)
        {
            return this.SchdOperDetails.Cache.Inserted.RowCast<AMSchdOperDetail>()
                .Where(r => r.OrderType == operation.OrderType &&
                        r.ProdOrdID == operation.ProdOrdID &&
                        r.OperationID == operation.OperationID);
        }

        internal static List<MaterialAvailabiltyDates> MaterialAvailability(PXGraph graph, DateTime planDate, string orderType, string prodOrdId, 
            int? operationId, DateTime itemPlanDate)
        {
                ...
                .Select(graph, orderType, prodOrdId, operationId))
            {
                ...
                var result = MaterialAvailability(graph, planDate, material, (InventoryItem)pxResult, (AMProdOper)pxResult, itemPlanDate);
                if (result == null)
                {
                    continue;
                }
        ...
        internal static MaterialAvailabiltyDates MaterialAvailability(PXGraph graph, DateTime planDate,
            AMProdMatl prodMatl, InventoryItem inventoryItem, AMProdOper prodOper, DateTime itemPlanDate)
        {
            ...
            if (remainingQty <= 0)
            {
                if (dateAvailable == null || dateAvailable.GreaterThan(planDate))
                {
                    return new MaterialAvailabiltyDates(prodMatl, prodOper?.OperationCD, inventoryItem, planDate, allProdMatlSplitsList)
                    {
                        ...
                        DaysLate = dateAvailable.GetValueOrDefault().Subtract(planDate).Days
                    };
                }
            ...
            if (itemPlansByDate != null)
            {
                ...
                if (linkedSupply != null)
                {
                    ...
                    //if the date available is in the past, don't use the date
                    dateAvailable = (dateAvailable == null || Common.Dates.Today >= dateAvailable.GetValueOrDefault()) ? null : dateAvailable;
            ...
            if (remainingQty > 0)
            {
                POVendorInventory prefVendor = null;
                if(GetAMOrderType(graph, prodMatl.OrderType)?.Function == OrderTypeFunction.Planning)
                {
                    //join initemsite and povendorinventory to get lead time
                    prefVendor = SelectFrom<POVendorInventory>.InnerJoin<INItemSite>
                        .On<POVendorInventory.vendorID.IsEqual<INItemSite.preferredVendorID>
                        .And<POVendorInventory.inventoryID.IsEqual<INItemSite.inventoryID>>>
                        .Where<INItemSite.inventoryID.IsEqual<@P.AsInt>
                        .And<INItemSite.siteID.IsEqual<@P.AsInt>>>.View.Select(graph, prodMatl.InventoryID, prodMatl.SiteID);
                    if(prefVendor != null)
                    {
                        var earliestDate = DateTime.Today.AddDays(prefVendor.VLeadTime.GetValueOrDefault() + 
                            prefVendor.AddLeadTimeDays.GetValueOrDefault());
                        if (earliestDate <= planDate)
                            return new MaterialAvailabiltyDates(prodMatl, prodOper?.OperationCD, inventoryItem, planDate, allProdMatlSplitsList)
                            {
                                AvailableBaseQtyShort = 0m,
                        ...
                        {
                            var operationDaysLate = earliestDate.Subtract(planDate).Days;
                            return new MaterialAvailabiltyDates(prodMatl, prodOper?.OperationCD, inventoryItem, planDate, allProdMatlSplitsList)
                            {
                                AvailableBaseQtyShort = availableBaseQty,
                                FirstLateAvailableDate = itemPlanDate.AddDays(operationDaysLate),
                                FirstLateAvailableBaseQtyShort = 0m,
                            };
                        }
                    }                    
        ...
        public static AMOrderType GetAMOrderType(PXGraph graph, string orderType)
        {
            return PXSelect<AMOrderType, Where<AMOrderType.orderType, Equal<Required<AMOrderType.orderType>>>>.Select(graph, orderType).FirstOrDefault();          
