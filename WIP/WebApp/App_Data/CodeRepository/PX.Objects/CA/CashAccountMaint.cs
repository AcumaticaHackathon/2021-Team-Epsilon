...
namespace PX.Objects.CA
{
    ...
    public class CashAccountMaint : PXGraph<CashAccountMaint, CashAccount>
	{
        ...
        public PXSelect<CADailySummary, Where<CADailySummary.cashAccountID, Equal<Required<CashAccount.cashAccountID>>>> CADailySummaryDetails;
        ...
        protected virtual void CashAccount_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
            ...
            DeleteCashAccountInUse(cashAccount);
        }
        ...
        protected virtual void CashAccount_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
			...
			cashacct.BaseCuryID = PXAccess.GetBranch(cashacct.BranchID)?.BaseCuryID;
        ...
        protected virtual void _(Events.FieldUpdated<CashAccount, CashAccount.branchID> e)
        {
            PXAccess.MasterCollection.Branch branch = PXAccess.GetBranch(e.Row.BranchID);
            e.Row.BaseCuryID = branch.BaseCuryID;

            BranchAttribute.VerifyFieldInPXCache<CashAccountDeposit, CashAccountDeposit.depositAcctID>(this, Deposits.Select());
        }
        ...
        protected virtual void CashAccountETDetail_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            SetupDefaultEntryType(sender, e);
        }

		private void SetupDefaultEntryType(PXCache sender, PXRowUpdatedEventArgs e)
		{
            CashAccountETDetail detail = (CashAccountETDetail)e.Row;
            CashAccountETDetail oldDetail = (CashAccountETDetail)e.OldRow;

            if (detail?.IsDefault != true && oldDetail?.IsDefault != false)
			{
                return;
			}

            CashAccountETDetail defaultDetail = PXSelect<CashAccountETDetail, Where<CashAccountETDetail.isDefault, Equal<True>,
                            And<CashAccountETDetail.accountID, Equal<Required<CashAccountETDetail.accountID>>,
                            And<CashAccountETDetail.entryTypeID, NotEqual<Required<CashAccountETDetail.entryTypeID>>>>>>.Select(this, detail.AccountID, detail.EntryTypeID);

            if(defaultDetail?.IsDefault == true)
			{
                defaultDetail.IsDefault = false;
                ETDetails.Update(defaultDetail);
                ETDetails.View.RequestRefresh();
            }
        }
        ...
        private void DeleteCashAccountInUse(CashAccount cashacct)
        {
            foreach (CADailySummary item in CADailySummaryDetails.Select(cashacct.CashAccountID))
            {
                CADailySummaryDetails.Delete(item);
            }
            
            foreach (CR.Location item in Locations.Select(cashacct.CashAccountID))
            {
