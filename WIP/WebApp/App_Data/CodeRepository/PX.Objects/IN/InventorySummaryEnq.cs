		...
		protected virtual IEnumerable iSERecordsFetch()
		{
			...
			if (filter.ExpandByLotSerialNbr == true)
			{
				...
				foreach (PXResult<INItemPlan, InventoryItem, INSite, INSubItem> res in lstPlans)
				{
					...
					INItemPlanIDAttribute.UpdateAllocatedQuantitiesBase<SiteStatus>(this, aggregate, plan_rec, shippingPlanTypes.First(x => ((INPlanType)x).PlanType == plan_rec.PlanType), aggregate.InclQtyAvail.GetValueOrDefault(), plan_rec.Hold);
