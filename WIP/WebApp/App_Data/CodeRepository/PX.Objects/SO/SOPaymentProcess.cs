...
namespace PX.Objects.SO
{
	...
	public class SOPaymentProcess : PXGraph<SOPaymentProcess>
	{
		...
		protected virtual void AddFilters(PXSelectBase<ARPayment> view, SOPaymentProcessFilter filter)
		{
			...
			switch (filter.Action)
			{
				...
				case SOPaymentProcessFilter.action.ValidateCCPayment:
					view.WhereAnd<Where<ARPayment.syncLock, Equal<True>,
						Or<ExternalTransaction.transactionID, IsNull,
						Or<ExternalTransaction.procStatus,
							In3<ExtTransactionProcStatusCode.authorizeHeldForReview,
						...
						Or<Where<ARPayment.isCCAuthorized, Equal<True>,
							And<ExternalTransaction.fundHoldExpDate, Less<Required<ExternalTransaction.fundHoldExpDate>>>>>>>>>();
					break;
		...
		protected virtual SOPaymentProcessResult CreateSOPaymentProcessResult(PXResult<ARPayment> row)
		{
			...
			var multiStatusAttribute = Payments.Cache
				.GetAttributesReadonly<SOPaymentProcessResult.relatedDocumentStatus>()
				.OfType<MultiDocumentStatusListAttribute>().FirstOrDefault();

			payment.RelatedDocumentStatus = isInvoice ?
				multiStatusAttribute?.GetDocumentStatusValue(Caches[typeof(SOInvoice)], invoice.Status) :
				multiStatusAttribute?.GetDocumentStatusValue(Caches[typeof(SOOrder)], order.Status);
