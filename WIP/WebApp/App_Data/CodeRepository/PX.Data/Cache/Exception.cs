...
namespace PX.Data
{
	...
	public class PXOverridableException : PXException
	{
		...
		public virtual void SetMessage(string message)
		{
			if (MessagePrefix != null)
			{
				...
				{
					message = message
                        .Substring(message.IndexOf(strtemp) + strtemp.Length)
                        .TrimStart(' ');
				}
			}
	...
	{
		private static string _baseLoginUrl;
		[UsedImplicitly]
		private class ServiceRegistration : Module
		{
			protected override void Load(ContainerBuilder builder)
			{
				builder.RegisterBuildCallback(ls =>
				{
					var options = ls.Resolve<IOptions<Export.Authentication.FormsAuthenticationOptions>>().Value;
					_baseLoginUrl = String.Concat(options.LoginUrl,
						"?returnUrl=",
						System.Web.VirtualPathUtility.ToAbsolute(options.DefaultUrl));
				});
			}
		}
