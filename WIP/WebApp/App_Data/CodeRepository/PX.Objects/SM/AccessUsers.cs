...
namespace PX.SM
{
	...
	public class AccessUsers : Access
	{
		...
		public override void Initialize()
		{
			...
			bool shouldCache = ActiveDirectoryProvider.GetUsers().Count() > Options.Value.ADGroupCacheLimit;
               ...
               // bool nullable = true;
                if (loginType?.AllowedLoginType == EPLoginType.allowedLoginType.UI)
                {
                    ...
                    maxValue = Math.Min(WebConfig.MaximumAllowedSessionsCount ?? int.MaxValue,
                        _licensing.License.UsersAllowed);                   
                    //nullable = false;
                }
                else if (loginType?.AllowedLoginType == EPLoginType.allowedLoginType.API)
                {
                    minValue = 1;
                    maxValue = Math.Min(WebConfig.MaximumAllowedSessionsCount ?? int.MaxValue,
                        _licensing.License.MaxApiUsersAllowed);
                    //nullable = false;
                }
                else 
                {
