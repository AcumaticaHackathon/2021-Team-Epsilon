	...
	public class MasterFinPeriodMaint : PXGraph<MasterFinPeriodMaint, MasterFinYear>, IFinPeriodMaintenanceGraph
	{
		...
		protected virtual void MasterFinPeriod_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if (isIntialized)
			{
				...
				if (IsMasterFinPeriodReferenced(period.FinPeriodID))
				{
					throw new PXException(Messages.FinancialPeriodCanNotBeDeleted, PeriodIDAttribute.FormatForError(period.FinPeriodID));
				}
			}
		}
		...
		protected virtual void MasterFinPeriod_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if (e.Operation.Command() == PXDBOperation.Delete &&
				...
				{
					throw new PXException(Messages.FinancialPeriodCanNotBeDeleted, PeriodIDAttribute.FormatForError(fp.FinPeriodID));
				}
			}
		...
		public void SynchronizeMasterAndOrganizationPeriods()
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.multipleCalendarsSupport>())
			{
				...
				{
					if (IsMasterFinPeriodReferenced(deletedMasterPeriod.FinPeriodID))
					{
						throw new PXException(Messages.FinancialPeriodCanNotBeDeleted, PeriodIDAttribute.FormatForError(deletedMasterPeriod.FinPeriodID));
					}
					else
					{
						Dictionary<(int OrganizationID, string Year), short> numberOfDeleted = new Dictionary<(int, string), short>();
						this.EnsureCachePersistence<OrganizationFinPeriod>();
						foreach (OrganizationFinPeriod organizationFinPeriod in SelectFrom<OrganizationFinPeriod>
							.Where<OrganizationFinPeriod.masterFinPeriodID.IsEqual<@P.AsString>>
							.View
							.Select(this, deletedMasterPeriod.FinPeriodID))
						{
							(int, string) organizationYearKey = ((int)organizationFinPeriod.OrganizationID, organizationFinPeriod.FinYear);
							if (numberOfDeleted.ContainsKey(organizationYearKey))
							{
								numberOfDeleted[organizationYearKey]++;
							}
							else
							{
								numberOfDeleted[organizationYearKey] = 1;
							}

							this.Caches<OrganizationFinPeriod>().Delete(organizationFinPeriod);
						}

						this.EnsureCachePersistence<OrganizationFinYear>();
						foreach (KeyValuePair<(int OrganizationID, string Year), short> item in numberOfDeleted)
						{
							OrganizationFinYear organizationFinYear = OrganizationFinYear.PK.Find(this, item.Key.OrganizationID, item.Key.Year);
							organizationFinYear.FinPeriods -= item.Value;
							this.Caches<OrganizationFinYear>().Update(organizationFinYear);
						}
					}
				}
			}
