...
namespace PX.Data
{
	...
	public abstract class PXEventSubscriberAttribute : Attribute
	{
		...
		protected Type prepCacheExtensionType;

		private Type cacheExtensionType;
		public virtual Type CacheExtensionType
		{
			get => cacheExtensionType;

			set 
			{
				cacheExtensionType = value;
				prepCacheExtensionType = GetBaseCacheWithTableAttr(value);
			}
		}

		/// <summary>
		/// Returns the first cache extension in <see cref="PXCacheExtension"/> extension chain, that 
		/// has <see cref="PXTableAttribute"/> or <see cref="PXTableNameAttribute"/> applied.
		/// </summary>
		/// <param name="cache">Cache to start the attribute search from.</param>
		internal static Type GetBaseCacheWithTableAttr(Type cache)
		{
			if (cache == null)
				return null;

			var ext = cache.BaseType;
			while (ext != null)
			{
				if ((ext.IsDefined(typeof(PXTableAttribute), false)
					|| ext.IsDefined(typeof(PXTableNameAttribute), false) && ((PXTableNameAttribute)ext.GetCustomAttributes(typeof(PXTableNameAttribute), false)[0]).IsActive)
					// && !ext.IsAbstract // necessary?
					&& typeof(PXCacheExtension).IsAssignableFrom(ext.BaseType)
					&& typeof(PXCacheExtension) != ext.BaseType)
				{
					break;
				}

				if (ext.FullName.StartsWith(typeof(PXCacheExtension).FullName) && ext.GenericTypeArguments.Length > 0)
					ext = ext.GenericTypeArguments[0];
				else
					ext = ext.BaseType;
			}
	...
	/// </summary>
	public interface IPXUIControlFieldAttribute { }
	...
	public class PXAggregateAttribute : PXEventSubscriberAttribute
	{
		...
		public override Type CacheExtensionType
		{
			get
			{
				return base.CacheExtensionType;
			}
			set
			{
				...
				foreach (PXEventSubscriberAttribute subscr in _Attributes)
				{
					subscr.CacheExtensionType = value;
				}
			}
		}
		/// <exclude/>
	...
	public class PXTableAttribute : PXDBInterceptorAttribute
	{
        ...
        private sealed class BqlRowSelection : BqlCommand, IPXExtensibleTableAttribute
		{
			...
			private void RowSelecting(PXCache sender, PXRowSelectingEventArgs e, Type table)
			{
				...
				if (e.Record != null && !(e.Record is PXDataRecordMap) && e.Record.IsDBNull(e.Position) && this.visited[table] < _recursionSize)
					{
			...
			public BqlRowSelection(PXCache cache, bool single, List<string> links, PXDBInterceptorAttribute parent)
			{
				...
				this.disableStraightJoin = cache.Graph.SqlDialect is MsSqlDialect || Common.WebConfig.DisableStraightJoin;
				...
				if (single && optional.Count > 0 && this.links.Count > 0)
				{
					...
					this.visited = new Dictionary<Type, int>();
			...
			private Dictionary<Type, int> visited;
			...
			public override SQLTree.Query GetQueryInternal(PXGraph graph, BqlCommandInfo info, Selection selection) 
			{
				...
				List<Type> ordered;
				int lastchild = 0;
				if (!disableStraightJoin)
					for (int i = 1; i < this.tables.Count; i++) {
						if (!this.tables[i].IsAssignableFrom(this.tables[i - 1]) || optional.Contains(this.tables[i]))
							break;
						lastchild = i;
					}
				if (lastchild == 0) ordered = new List<Type>(this.tables);
				else {
					ordered = new List<Type>();
					for (int j = lastchild; j >= 0; j--)
						ordered.Add(this.tables[j]);
					for (int j = lastchild + 1; j < this.tables.Count; j++)
						ordered.Add(this.tables[j]);
				}
				...
				for (int i = 0; i < ordered.Count; i++) {
                    ...
                    var hint = Joiner.JoinHint.NONE;
					if(i==0) jt = Joiner.JoinType.MAIN_TABLE;
					else if (i == 1 && i <= lastchild)
                        hint = Joiner.JoinHint.STRAIGHT;
					else if(optional.Contains(ordered[i])) jt=Joiner.JoinType.LEFT_JOIN;

					TableChangingScope.AddUnchangedRealName(ordered[i].Name);
                    Joiner j = new Joiner(jt, TableChangingScope.GetSQLTable(() => new SQLTree.SimpleTable(ordered[i]), ordered[i].Name), query);					
                    j.SetHint(hint);
					joinedTables.Add(ordered[i]);
					if (jt != Joiner.JoinType.MAIN_TABLE) {
						if (this.links.Count > 0) j.On(MakeWhere(joinedTables, cache));
						else j.On(new SQLConst(1).EQ(1));
						joinedTables.Remove(ordered[i]);
					}
					query.AddJoin(j);
				}
				if (extensions != null) {
    ...
    public sealed class PXAccumulatorCollection : Dictionary<string, PXAccumulatorItem>
    {
		...
		/// </code>
		/// </example>
		/// <seealso cref="RestrictFuture{Field}(PXComp, object)"/>
		...
		/// <param name="comparison">A <tt>PXComp</tt> constant that indicates the restriction condition.</param>
		/// <param name="value">The value to be compared to.</param>
		/// <see cref="RestrictPast{Field}(PXComp, object)"/>
		...
		/// <param name="comparison">A <tt>PXComp</tt> constant that indicates the restriction condition.</param>
		/// <param name="value">The value to be compared to.</param>
		/// <seealso cref="RestrictPast{Field}(PXComp, object)"/>
		...
		/// <param name="comparison">A <tt>PXComp</tt> constant that indicates the restriction condition.</param>
		/// <param name="value">The value to be compared to.</param>
		/// <see cref="RestrictFuture{Field}(PXComp, object)"/>
