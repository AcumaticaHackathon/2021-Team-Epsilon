...
namespace PX.Objects.AP
{
	...
	public class APReleaseProcess : PXGraph<APReleaseProcess>
	{
			...
			out List<INRegister> inDocs)
		{
			...
			if (doc.Released != true && (!isPrebooking || doc.Prebooked != true))
			{
				...
				else
				{
					...
					if (PXAccess.FeatureInstalled<FeaturesSet.projectModule>() && !_IsIntegrityCheck)
					{
			...
			List<AllocationServiceBase.POReceiptLineAdjustment> ppvTransactions, List<APTran> ppvLines)
		{
			...
			if (tran.TranType == APDocType.DebitAdj)
			{
				if (tran.POAccrualType == POAccrualType.Order && accrualBilledQty < 0m)
				{
					...
					{
						throw new PXException(Messages.DoubleSubcontractBillReverse, tran.LineNbr, tran.PONbr);
					}
				}
		...
		protected virtual void VerifyRoundingAllowed(APInvoice document, Batch batch, string baseCuryID)
		{
			...
			if (roundDiff > CurrencyCollection.GetCurrency(baseCuryID).RoundingLimit && PXAccess.FeatureInstalled<FeaturesSet.invoiceRounding>())
			{
				throw new PXException(Messages.RoundingAmountTooBig, baseCuryID, roundDiff,
					PXDBQuantityAttribute.Round(CurrencyCollection.GetCurrency(baseCuryID).RoundingLimit));
			}
		}
			...
			else if (prepaymentAdj.AdjgDocType.IsIn(APDocType.Check, APDocType.Prepayment))
			{
				if (_IsIntegrityCheck)
				{
					...
					if (prepaymentRequest.CuryDocBal == 0m)
					{
						...
						RaiseInvoiceEvent(prepaymentRequest, APInvoice.Events.Select(ev => ev.CloseDocument));
					}
					else
					{
						prepaymentRequest.OpenDoc = true;
		...
		protected void DeactivateOneTimeVendorIfAllDocsIsClosed(Vendor vendor)
		{
			...
			VendorMaint_Workflow.SetStatusTo(vendor, VendorStatus.Inactive);
		}
	    ...
	    protected virtual void RaiseInvoiceEvent(APRegister doc, PX.Data.WorkflowAPI.SelectedEntityEvent<APInvoice> invEvent)
	    {
		    if (doc is APInvoice)
		    {
			    invEvent.FireOn<APRegister>(this, (APInvoice) doc);
	    ...
	    protected virtual void RaisePaymentEvent(APRegister doc, PX.Data.WorkflowAPI.SelectedEntityEvent<APPayment> pntEvent)
	    {
		    if (doc is APPayment)
		    {
			    pntEvent.FireOn<APRegister>(this, (APPayment)doc);
			    APDocument.Cache.Update(doc);
			    APDocument.Cache.RestoreCopy(doc, APDocument.Cache.Locate(doc));
		    }
	    }
		...
		protected virtual void RaiseReleaseEvent(APRegister doc)
	    {
		    if (APDocument.Cache.ObjectsEqual(doc, APInvoice_DocType_RefNbr.Current))
		    {
			    ...
			    APInvoice.Events
				    ...
				    .FireOn<APRegister>(this, invoice);
		    ...
		    else if (APDocument.Cache.ObjectsEqual(doc, APPayment_DocType_RefNbr.Current))
		    {
			    ...
			    APPayment.Events
				    .Select(e => e.ReleaseDocument)
				    .FireOn<APRegister>(this, payment);
		...
		protected virtual PXResultset<APRegister> GetDocumentsForIntegrityCheckProc(string minPeriod)
		{
			...
			PXResultset<APRegister> venddocs = new PXResultset<APRegister>();
				...
				.Select(this, minPeriod, minPeriod) )
			{
				...
				if(doc != null)
					venddocs.Add( new PXResult<APRegister>(doc));
			}
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.retainage>())
			{
				...
				{
					if (!origdocs.Contains((APRegister)retdoc))
					{
						venddocs.Add(retdoc);
					}
				}
		...
		{
			var allDocsKeys = documents.RowCast<APRegister>().ToHashSet(APDocument.Cache.GetComparer());
