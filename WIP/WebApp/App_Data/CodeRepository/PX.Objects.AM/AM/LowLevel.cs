...
namespace PX.Objects.AM
{
    public class LowLevel : PXGraph<LowLevel>
    {
        ...
        [Obsolete(InternalMessages.MethodIsObsoleteAndWillBeRemoved2021R2 + "Use UpdateInventoryItem(LowLevelInventoryItem, bool)")]
        ...
        {
            // Prevents select on table when updating cache
            LowLevelInventoryItemRecs.Cache.DisableReadItem = true;

            foreach (LowLevelInventoryItem inventoryItem in LowLevelInventoryItemRecs.Select())
            {
                ...
                if (newLowLevel >= MaxLowLevel)
                {
                    ...
                    PXTrace.WriteInformation(Messages.GetLocal(Messages.LowLevelMaxLevelReachedForItem, inventoryItem.InventoryCD.TrimIfNotNullEmpty(), inventoryItem.AMLowLevel.GetValueOrDefault()));
                }

                //if the value did not change we do not need to update/persist
                if (newLowLevel.GetValueOrDefault() != inventoryItem.AMLowLevel.GetValueOrDefault(-1))
                {
                    inventoryItem.AMLowLevel = newLowLevel.GetValueOrDefault();
