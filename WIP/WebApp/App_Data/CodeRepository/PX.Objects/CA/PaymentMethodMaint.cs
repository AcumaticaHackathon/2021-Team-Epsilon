...
namespace PX.Objects.CA
{
	public class PaymentMethodMaint : PXGraph<PaymentMethodMaint, PaymentMethod>
	{
		...
		protected virtual void PaymentMethod_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool isAccountNumberRequired = row.UseForAR == true && row?.PaymentType == PaymentMethodType.CreditCard && row?.ARIsProcessingRequired == true && row.IsAccountNumberRequired == false;
			bool isARProcessingRequired = row.UseForAR == true && row?.PaymentType == PaymentMethodType.CreditCard && row?.ARIsProcessingRequired == false && row.IsAccountNumberRequired == true;
			...
			else if (isARProcessingRequired)
			{
				exception = new PXSetPropertyException(Messages.IntegratedProcessingMustBeSelected);
			}
		...
		protected virtual void PaymentMethod_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			if (oldRow.PaymentType != row.PaymentType)
			{
				...
				row.ARIsProcessingRequired = row.PaymentType == PaymentMethodType.CreditCard;
			}
