...
namespace PX.Objects.PR
{
	...
	public class PRAufGenerationEngine : PXGraph<PRAufGenerationEngine>
	{
		...
		public SelectFrom<PREmployeeAttribute>
			.RightJoin<PRCompanyTaxAttribute>.On<PRCompanyTaxAttribute.settingName.IsEqual<PREmployeeAttribute.settingName>
				.And<PREmployeeAttribute.bAccountID.IsEqual<EPEmployee.bAccountID.AsOptional>
					.Or<PREmployeeAttribute.bAccountID.IsNull>>>.View EmployeeAttributes;
		...
		protected virtual IEnumerable<EjwRecord> CreateJobWeekRecords(GenRecord general, IEnumerable<PXResult<PRPayment, PRPaymentTaxSplit, PRTaxCode>> taxSplits)
		{
			...
			foreach (EjwRecord record in records.Values)
			{
				record.FederalWithholding = general.FederalWithheld;
					...
					{
					if (_TaxIDMapping.Where(kvp => kvp.Value.Field == AatrixTax.Sit).Select(y => y.Key).Contains(((PRTaxCode)taxSplit).TaxUniqueCode)
						&& (((PRTaxCode)taxSplit).TaxState == Report.Current.State || Report.Current.State == LocationConstants.FederalStateCode))
						{
						record.StateWithholding += ((PRPaymentTaxSplit)taxSplit).TaxAmount;
						}
					if (_TaxIDMapping.Where(kvp => kvp.Value.Field == AatrixTax.Sui).Select(y => y.Key).Contains(((PRTaxCode)taxSplit).TaxUniqueCode)
						&& (((PRTaxCode)taxSplit).TaxState == Report.Current.State || Report.Current.State == LocationConstants.FederalStateCode))
						{
						record.Sui += ((PRPaymentTaxSplit)taxSplit).TaxAmount;
						}
					if (_TaxIDMapping.Where(kvp => kvp.Value.Field == AatrixTax.Sdi).Select(y => y.Key).Contains(((PRTaxCode)taxSplit).TaxUniqueCode)
						&& (((PRTaxCode)taxSplit).TaxState == Report.Current.State || Report.Current.State == LocationConstants.FederalStateCode))
						{
		...
		{
			return CreateEarningPimRecords(reportPeriod)
				.Union(CreateBenefitPimRecords(reportPeriod), new PimRecordComparer())
				.Union(CreateTaxPimRecords(reportPeriod), new PimRecordComparer());
		}

		protected virtual IEnumerable<PimRecord> CreateEarningPimRecords(DatRecord reportPeriod)
		{
			Dictionary<int, PRTypeMeta> wageReportingTypes = PRReportingTypeSelectorAttribute.GetAll<PRWage>().ToDictionary(k => k.ID, v => v);
			HashSet<int> usedEarningReportingTypes = new HashSet<int>();
			foreach (PRPayment payment in PeriodPayments
				.Select(reportPeriod.FirstDate, reportPeriod.LastDate, reportPeriod.FirstDate, reportPeriod.LastDate))
			{
				foreach (PRPaymentEarning paymentEarning in PaymentEarnings.View.SelectMultiBound(new object[] { payment }).Select(x => (PXResult<PRPaymentEarning>)x))
				{
		...
		protected virtual IEnumerable<PimRecord> CreateBenefitPimRecords(DatRecord reportPeriod)
		{
			Dictionary<int, PRTypeMeta> deductionReportingTypes = PRReportingTypeSelectorAttribute.GetAll<PRBenefit>().ToDictionary(k => k.ID, v => v);
			HashSet<int> usedDeductionReportingTypes = new HashSet<int>();
			HashSet<string> statesWithWC = new HashSet<string>();
			foreach (PRPayment payment in PeriodPayments
				.Select(reportPeriod.FirstDate, reportPeriod.LastDate, reportPeriod.FirstDate, reportPeriod.LastDate))
			{
				foreach (PRPaymentDeduct paymentDeduction in PaymentDeductions.View.SelectMultiBound(new object[] { payment }).Select(x => (PXResult<PRPaymentDeduct>)x))
				{
			...
			foreach (KeyValuePair<string, SymmetryToAatrixTaxMapping> kvp in _TaxIDMapping.Where(kvp => kvp.Value.IsWorkersCompensation && statesWithWC.Contains(kvp.Value.State?.Abbr)))
			{
		...
		protected virtual IEnumerable<PimRecord> CreateTaxPimRecords(DatRecord reportPeriod)
		{
			foreach (IGrouping<int?, PXResult<PRPayment, PRPaymentTaxSplit, PRTaxCode>> taxGroup in PeriodPayments
				.Select(reportPeriod.FirstDate, reportPeriod.LastDate, reportPeriod.FirstDate, reportPeriod.LastDate)
				.Select(x => (PXResult<PRPayment, PRPaymentTaxSplit, PRTaxCode>)x)
		...
		protected virtual (EcvRecord, EciRecord) CreateEmployeeAcaRecords(EmpRecord emp)
		{
			...
			if (employeeMonthlyRecords.Any())
			{
				...
				EcvRecord ecv = new EcvRecord()
				{
					...
					PolicyOriginCode = GetAttributeValue(employeeAttributes, AatrixField.ECV.AcaPolicyOrigin)?[0],
