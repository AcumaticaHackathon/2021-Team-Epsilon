...
namespace PX.Objects.AR.CCPaymentProcessing
{
	public class CCCustomerInformationManager
	{
			...
			)
		{
			...
			while ((attempt <= (procCenter.SyncRetryAttemptsNo ?? 0) + 1) && newCard == null)
			{
				...
				if (newCard != null)
				{
					...
					{
						string maskedCardNum = CCProcessingHelper.FormatMaskedCardNumber(newCard.CardNumber, customerPaymentMethod.PaymentMethodID);
				...
				, ICCPaymentProfileDetailAdapter paymentDetail)
		{
			...
			if ((isIDFilled || isOtherDetsFilled) && !isHF || tryGetProfile)
			{
				...
				if (cardData != null && !string.IsNullOrEmpty(cardData.PaymentProfileID))
				{
					...
					{
						string maskedCardNum = CCProcessingHelper.FormatMaskedCardNumber(cardData.CardNumber, paymentProfile.PaymentMethodID);
		...
		public static TranProfile GetOrCreatePaymentProfileByTran(PXGraph graph, ICCPaymentProfileAdapter adapter, string tranId)
		{
			...
			{
				ret = cim.GetOrCreateCustomerProfileFromTransaction(tranId, new CreateTranPaymentProfileParams() { LocalCustomerId = customer.AcctCD.Trim() });
				return ret;
			}
			var cardLimit = processingCenter.CreditCardLimit;
				int customerProfileCount = CCProcessingHelper.CustomerProfileCountPerCustomer(graph,
			...
			{
				int allPaymentProfileCount = 0;
				bool needNewCustomerProfile = false;
				try
				{
					allPaymentProfileCount = cim.GetAllPaymentProfiles().Count();
				...
				{
					ret = cim.GetOrCreateCustomerProfileFromTransaction(tranId, new CreateTranPaymentProfileParams() { LocalCustomerId = prefix + customer.AcctCD.Trim() });
				}
				else
				{
