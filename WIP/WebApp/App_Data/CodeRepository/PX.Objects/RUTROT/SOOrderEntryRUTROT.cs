...
namespace PX.Objects.RUTROT
{
	...
	public class SOOrderEntryRUTROT : PXGraphExtension<MobileCreatePaymentExt, CreatePaymentExt, SOOrderEntry>
	{
		...
		public delegate void MobileInsertSOAdjustmentsDelegate(SOOrder order, ARPaymentEntry docgraph, ARPayment payment);
		[PXOverride]
		public virtual void MobileInsertSOAdjustments(SOOrder order, ARPaymentEntry docgraph, ARPayment payment, MobileInsertSOAdjustmentsDelegate baseMethod)
        {
            baseMethod(order, docgraph, payment);
            SubtractRUTROT(order, docgraph, false);
        }

        protected virtual void SubtractRUTROT(SOOrder order, ARPaymentEntry docgraph, bool updatePayment)
        {
            SOOrderRUTROT orderRR = RUTROTHelper.GetExtensionNullable<SOOrder, SOOrderRUTROT>(order);
            if (orderRR.IsRUTROTDeductible == true)
            {
                ...
                foreach (SOAdjust adj in docgraph.SOAdjustments.Select())
                {
                    ...
                    if (other == null || other.AdjdOrderNbr == null)
                    {
						...
						if (updatePayment)
						{
							var payment = docgraph.Document.Current;
							docgraph.Document.Cache.SetValueExt<ARPayment.curyOrigDocAmt>(payment, payment.CuryOrigDocAmt - rutrot.CuryTotalAmt);
        ...
        [PXOverride]
		public virtual ARPaymentEntry CreatePayment(SOQuickPayment quickPayment, SOOrder order, string paymentType,
			Func<SOQuickPayment, SOOrder, string, ARPaymentEntry> baseMethod)
		{
			var paymentEntry = baseMethod.Invoke(quickPayment, order, paymentType);
			SubtractRUTROT(order, paymentEntry, true);
