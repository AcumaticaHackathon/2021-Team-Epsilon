...
{
    public class ApInvoiceEntryExt : JointPayeeGraphExtBase<APInvoiceEntry>
    {
        ...
        public PXSelect<JointPayee,
	        Where<JointPayee.billId, Equal<Required<JointPayee.billId>>>> JointPayeesImpl;
        ...
        {
			return JointPayeesImpl.SelectWithViewContext(OriginalBillNoteId);
        }

		public override PXSelectBase<JointPayee> JointPayeeViewAccessor => JointPayees;

		public PXAction<APInvoice> DeleteJointPayee;
		...
		protected virtual IEnumerable deleteJointPayee(PXAdapter adapter)
        {
			JointPayee payee = JointPayees.Current;

			if (payee != null)
			{
				JointPayees.Delete(payee);
			}
        ...
        protected virtual void _(Events.CacheAttached<JointPayee.jointPayeeInternalId> e)
        {
        }
        ...
        protected virtual void _(Events.RowSelected<APInvoice> args)
        {
			...
			UpdateDocumentFormLayout(invoice, isBill);
		}
        ...
        {
	        JointPayee jointPayee = e.Row;
	        APInvoice invoice = Base.Document.Current;

	        if (jointPayee == null || invoice == null)
		        return;

	        bool editableByPayment = jointPayee.LinkedToPayment != true;

	        jointPayee.CanDelete = IsJointPayeeDataEditable(invoice) && editableByPayment;

	        e.Cache
		        .Adjust<PXUIFieldAttribute>(jointPayee)
		        .For<JointPayee.jointPayeeInternalId>(attr => attr.Enabled = editableByPayment)
		        .SameFor<JointPayee.jointPayeeExternalName>()
		        .SameFor<JointPayee.billLineNumber>();
        }
        ...
        protected virtual void _(Events.FieldVerifying<APInvoice, APInvoiceJCExt.isJointPayees> e)
        {
	        bool? newValue = (bool?)e.NewValue;

	        if (e.Row.IsRetainageDocument == false
	            && e.Row.Released == true
	            && newValue == true)
        {
		        int reducedCountOfUnreleasedPayments =
			        SelectFrom<APAdjust>
				        .InnerJoin<APInvoice>
				        .On<APAdjust.adjdDocType.IsEqual<APInvoice.docType>
					        .And<APAdjust.adjdRefNbr.IsEqual<APInvoice.refNbr>>>
				        .Where<APAdjust.released.IsEqual<False>
					        .And<Brackets<
						        Brackets<APInvoice.docType.IsEqual<P.AsString>
							        .And<APInvoice.refNbr.IsEqual<P.AsString>>>
						        .Or<Brackets<APInvoice.origDocType.IsEqual<P.AsString>
							        .And<APInvoice.origRefNbr.IsEqual<P.AsString>
								        .And<APInvoice.isRetainageDocument.IsEqual<True>>>>>>>>
				        .AggregateTo<GroupBy<APAdjust.adjgRefNbr>,
					        GroupBy<APAdjust.adjgDocType>>
				        .View
				        .SelectWindowed(Base, 0, 2, e.Row.DocType, e.Row.RefNbr, e.Row.DocType, e.Row.RefNbr)
				        .Count;

		        if (reducedCountOfUnreleasedPayments > 1)
            {
			        throw new PXSetPropertyException(JointCheckMessages.ThereIsMoreThanOneUnreleasedPayment);
		        }
            }
        }
        ...
        protected virtual void _(Events.RowPersisting<APInvoice> args)
        {
            ...
            {
	            args.Row.IsPaymentCycleWorkflow = false;
            }
            if (!IsRetainageBill)
            {
		...
		public virtual void _(Events.RowPersisted<APInvoice> e)
		{
			if (e.TranStatus == PXTranStatus.Open
				&& e.Operation == PXDBOperation.Update)
			{
				SetJointPayeeForRetainageBillsInSeparateGraph(e.Row);
			}
		}
        ...
        private void UpdateDocumentFormLayout(APInvoice invoice, bool isBill)
        {
			PXUIFieldAttribute.SetVisible<APInvoiceJCExt.isJointPayees>(Base.Document.Cache, null, isBill);

			bool isJointPayeesEnabled = IsJointPayeeDataEditable(invoice)
			                            && !JointPayees.Search<JointPayee.linkedToPayment>(true).Any();

			PXUIFieldAttribute.SetEnabled<APInvoiceJCExt.isJointPayees>(Base.Document.Cache, null, isJointPayeesEnabled);

            UpdatePaymentsByLinesAllowedCheckboxAvailability();
            UpdateJointPayeesTabAvailability(invoice);
        ...
        private void UpdateJointPayeesTabAvailability(APInvoice invoice)
        {
			APInvoiceJCExt invoiceJCExt = Base.Document.Cache.GetExtension<APInvoiceJCExt>(invoice);

			bool rowEditable = invoiceJCExt.IsJointPayees == true && IsJointPayeeDataEditable(invoice);

	        JointPayees.AllowUpdate = rowEditable;
	        JointPayees.AllowInsert = rowEditable;
        }
        ...
        public bool IsJointPayeeDataEditable(APInvoice invoice)
        {
	        return invoice.Status != APDocStatus.Closed
	               && invoice.IsRetainageDocument != true;
        }

		protected void SetJointPayeeForRetainageBillsInSeparateGraph(APInvoice invoice)
		{
			APInvoiceJCExt invoiceJCExt = Base.Document.Cache.GetExtension<APInvoiceJCExt>(invoice);

			if (invoice.Released == true && invoice.RetainageApply == true)
			{
				bool? oldIsJointPayee = (bool?)Base.Document.Cache.GetValueOriginal<APInvoiceJCExt.isJointPayees>(invoice);

				if (oldIsJointPayee != invoiceJCExt.IsJointPayees)
				{
					IEnumerable<APInvoice> retainageBills =
						PXSelect<APInvoice,
							Where<APInvoice.isRetainageDocument, Equal<True>,
								And<APInvoice.origDocType, Equal<Required<APInvoice.origDocType>>,
					...
					foreach (APInvoice retainageBill in retainageBills)
					{
						invoiceEntry.Clear();

						invoiceEntry.Document.Current = invoiceEntry.Document.Search<APInvoice.docType, APInvoice.refNbr>(retainageBill.DocType, retainageBill.RefNbr);
						APInvoice currentRetainageBill = invoiceEntry.Document.Current;

						invoiceEntry.Document.Cache.SetValue<APInvoiceJCExt.isJointPayees>(currentRetainageBill, invoiceJCExt.IsJointPayees);
						invoiceEntry.Document.Cache.MarkUpdated(currentRetainageBill);
