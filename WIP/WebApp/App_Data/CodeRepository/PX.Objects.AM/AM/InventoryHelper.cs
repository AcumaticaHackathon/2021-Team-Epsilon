...
namespace PX.Objects.AM
{
    ...
    public static class InventoryHelper
    {
        ...
        {
            var result = (PXResult<INItemSite, InventoryItem, INItemClassRep>)PXSelectJoin<INItemSite,
                InnerJoin<InventoryItem,
                    On<INItemSite.inventoryID, Equal<InventoryItem.inventoryID>>,
                InnerJoin<INItemClassRep,
                    On<INItemClassRep.itemClassID, Equal<InventoryItem.itemClassID>,
                        And<INItemClassRep.replenishmentClassID, Equal<INItemSite.replenishmentClassID>>>>>,
                 Where<INItemSite.replenishmentSource, Equal<INReplenishmentSource.transfer>,
                    And<INItemSite.inventoryID, Equal<Required<INItemSite.inventoryID>>,
                    And<INItemSite.siteID, Equal<Required<INItemSite.siteID>>,
                    And<INItemClassRep.replenishmentSourceSiteID, Equal<Required<INItemClassRep.replenishmentSourceSiteID>>>>
                >>>.Select(graph, inventoryID, toSiteID, fromSiteID);

            var itemClassRep = (INItemClassRep)result;

            return itemClassRep?.TransferLeadTime ?? 0;
        }
