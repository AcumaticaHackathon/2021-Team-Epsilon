...
namespace PX.Objects.CS.Email
{
	...
	public static class PXEmailSyncHelper
	{
		...
		{
			var res = new List<(string field, string value, Exception exception)>();

			Validate(nameof(CRSMEmail.MailFrom), email.MailFrom);
			Validate(nameof(CRSMEmail.MailReply), email.MailReply);
			Validate(nameof(CRSMEmail.MailTo), email.MailTo);
			Validate(nameof(CRSMEmail.MailCc), email.MailCc);
			Validate(nameof(CRSMEmail.MailBcc), email.MailBcc);

			if (res.Count > 0)
			{
				PXTrace.Logger.Warning(new AggregateException(res.Select(r => r.exception)),
					"Email validation failed for CRSMEmail: {NoteID}. Following fields couldn't be validated: {@FieldsAndValues}",
					email.NoteID, res.Select(r => new { FieldName = r.field, Value = r.value }));

				errorMessage = PXLocalizer.LocalizeFormat(MessagesNoPrefix.EmailValidationFailed, string.Join(", ", res.Select(r => r.field)));
				return false;
			}
