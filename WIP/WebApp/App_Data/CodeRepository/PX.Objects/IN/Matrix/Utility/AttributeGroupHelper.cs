...
namespace PX.Objects.IN.Matrix.Utility
{
	public class AttributeGroupHelper
	{
		...
		{
			_graph = graph;
			_templateInventoryItemMaint = graph as TemplateInventoryItemMaint;
		}
		...
		public virtual void Recalculate(InventoryItem template)
		{
			...
			var select = new PXSelectReadonly2<CSAnswers,
				...
				LeftJoin<CSAttributeGroup, On<CSAnswers.attributeID, Equal<CSAttributeGroup.attributeID>,
				...
				Where<InventoryItem.templateItemID, Equal<Required<InventoryItem.inventoryID>>,
						And<Where<CSAnswers.attributeID, Equal<MatrixAttributeSelectorAttribute.dummyAttributeName>,
							Or<CSAttributeGroup.isActive, Equal<True>>>>>,
			...
			{
				foreach (PXResult<CSAnswers, InventoryItem> result in select.Select(_template.InventoryID))
				{
		...
		protected virtual void SetInventoryCombinationNumber(int? inventoryID, int combinationNumber)
		{
			...
			var item = InventoryItem.PK.FindDirty(_graph, inventoryID);
			if (item == null)
			...
			if (_templateInventoryItemMaint != null)
			{
				item = _templateInventoryItemMaint.UpdateChild(item);
			}
			else
			{
		...
		public virtual void OnNewItem(InventoryItem templateItem, InventoryItem newItem, string[] newItemAttributeIDs, string[] newItemAttributeValues)
		{
			...
			const string DummyAttributeValue = MatrixAttributeSelectorAttribute.DummyAttributeValue;
			_columnAttributeValue = _template.DefaultColumnMatrixAttributeID == DummyAttribute ? DummyAttributeValue : null;
			_rowAttributeValue = _template.DefaultRowMatrixAttributeID == DummyAttribute ? DummyAttributeValue : null;
