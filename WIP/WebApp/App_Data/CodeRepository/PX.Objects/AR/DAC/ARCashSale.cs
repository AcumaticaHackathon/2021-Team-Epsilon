...
namespace PX.Objects.AR.Standalone
{
	...
	[PXEMailSource]
	[PXGroupMask(typeof(InnerJoinSingleTable<Customer, On<Customer.bAccountID, Equal<ARCashSale.customerID>, And<Match<Customer, Current<AccessInfo.userName>>>>>))]
	public class ARCashSale : ARRegister, ICCPayment, IApprovable, IApprovalDescription
	{
		...
		{
			public class Branch : GL.Branch.PK.ForeignKeyOf<ARCashSale>.By<branchID> { }
			public class Customer : AR.Customer.PK.ForeignKeyOf<ARCashSale>.By<customerID> { }
			public class CustomerLocation : CR.Location.PK.ForeignKeyOf<ARCashSale>.By<customerID, customerLocationID> { }
			public class CurrencyInfo : CM.CurrencyInfo.PK.ForeignKeyOf<ARCashSale>.By<curyInfoID> { }
			public class Currency : CM.Currency.PK.ForeignKeyOf<ARCashSale>.By<curyID> { }
			public class ARAccount : GL.Account.PK.ForeignKeyOf<ARCashSale>.By<aRAccountID> { }
			public class ARSubaccount : GL.Sub.PK.ForeignKeyOf<ARCashSale>.By<aRSubID> { }
			public class Schedule : GL.Schedule.PK.ForeignKeyOf<ARCashSale>.By<scheduleID> { }
			public class OriginalDocument : AR.ARRegister.PK.ForeignKeyOf<ARCashSale>.By<origDocType, origRefNbr> { }
			public class Terms : CS.Terms.PK.ForeignKeyOf<ARCashSale>.By<termsID> { }
		}
		#endregion
		#region Events
		public new class Events : PXEntityEvent<ARCashSale>.Container<Events>
		{
			public PXEntityEvent<ARCashSale> UpdateStatus;
		}
		#endregion
		...
		[CustomerActive(Visibility = PXUIVisibility.SelectorVisible, DescriptionField = typeof(Customer.acctName), Filterable = true, BqlField = typeof(ARRegister.customerID), TabOrder = 1)]
		[PXDefault()]
		...
		[PXDefault(typeof(Search<Location.cAvalaraExemptionNumber,
			Where<Location.bAccountID, Equal<Current<ARCashSale.customerID>>,
				And<Location.locationID, Equal<Current<ARCashSale.customerLocationID>>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[Branch(typeof(AccessInfo.branchID), IsDetail = false, BqlField = typeof(ARRegister.branchID), TabOrder = 0)]
		[PXFormula(typeof(Switch<Case<Where<ARCashSale.customerLocationID, IsNotNull,
										And<Selector<ARCashSale.customerLocationID, Location.cBranchID>, IsNotNull>>,
									Selector<ARCashSale.customerLocationID, Location.cBranchID>,
								Case<Where<Current2<ARCashSale.branchID>, IsNotNull>,
									Current2<ARCashSale.branchID>>>,
		...
		[PXDefault(typeof(Search<CustomerPaymentMethod.cCProcessingCenterID,
			Where<CustomerPaymentMethod.pMInstanceID, Equal<Current2<ARCashSale.pMInstanceID>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[PXSelector(typeof(Search<ExternalTransaction.tranNumber,
					Where<ExternalTransaction.pMInstanceID, Equal<Current<ARCashSale.pMInstanceID>>,
						And<ExternalTransaction.procStatus, Equal<ExtTransactionProcStatusCode.captureSuccess>>>,
		...
		[PXFormula(typeof(Switch<Case<Where<isMigratedRecord, Equal<False>,
				And<Selector<paymentMethodID, PaymentMethod.paymentType>, Equal<PaymentMethodType.creditCard>,
					And<Selector<paymentMethodID, PaymentMethod.isAccountNumberRequired>, Equal<True>>>>, True>,
			False>))]
