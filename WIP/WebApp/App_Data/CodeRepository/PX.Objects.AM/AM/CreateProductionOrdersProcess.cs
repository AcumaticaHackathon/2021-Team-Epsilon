            ...
            for (var i = 0; i < list.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (isSO2Production)
                    {
                        ...
                        soLineExt.AMSchdNoteID = prodMaintGraph.SchdItemRecords?.Current?.NoteID;

                        //Make sure new created Production Nbr set during persist (when auto number value not yet known)
                        PXDBDefaultAttribute.SetDefaultForUpdate<SOLineExt.aMProdOrdID>(prodMaintGraph.LinkSOLineRecords.Cache, soLine, true);

                        soLine = prodMaintGraph.LinkSOLineRecords.Update(soLine);
                        ...
                        if (soLineSplit != null && splitExt != null)
                        {
                        ...
                        splitExt.AMProdOrdID = prodMaintGraph.ProdMaintRecords?.Current?.ProdOrdID;
                            ...
                            PXDBDefaultAttribute.SetDefaultForUpdate<SOLineSplitExt.aMProdOrdID>(prodMaintGraph.LinkSOLineSplitRecords.Cache, soLineSplit, true);
                            soLineSplit = prodMaintGraph.LinkSOLineSplitRecords.Update(soLineSplit); 
                        }
                    }

                    if (isProd2Prod && prodMaintGraph.ProdMaintRecords?.Current?.NoteID != null)
                    {
                        var parentMatlSplit = (AMProdMatlSplit2)PXSelect<AMProdMatlSplit2,
                                Where<AMProdMatlSplit2.planID, Equal<Required<AMProdMatlSplit2.planID>>>>
                                .Select(prodMaintGraph, demand.PlanID);
                        ...
                        if (parentMatlSplit?.ProdOrdID != null)
                        {
                            ...
                            PXDBDefaultAttribute.SetDefaultForUpdate<AMProdMatlSplit2.aMProdOrdID>(prodMaintGraph.LinkProdMatlSplit.Cache, parentMatlSplit, true);
                            prodMaintGraph.LinkProdMatlSplit.Update(parentMatlSplit);
                        }
                    }
