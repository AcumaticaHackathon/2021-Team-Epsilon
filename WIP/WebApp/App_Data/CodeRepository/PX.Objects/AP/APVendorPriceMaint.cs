...
namespace PX.Objects.AP
{
	public class APVendorPriceMaint : PXGraph<APVendorPriceMaint>, IPXAuditSource //, PXImportAttribute.IPXPrepareItems
	{
		...
		protected virtual void APVendorPrice_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			{
				sender.RaiseExceptionHandling<APVendorPrice.expirationDate>(row, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(APVendorPrice.expirationDate)}]"));
			}
			if (row.IsPromotionalPrice == true && row.EffectiveDate == null)
			{
				sender.RaiseExceptionHandling<APVendorPrice.effectiveDate>(row, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(APVendorPrice.effectiveDate)}]"));
			}
