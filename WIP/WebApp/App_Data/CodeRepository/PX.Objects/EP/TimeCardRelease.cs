...
namespace PX.Objects.EP
{
	...
	public class TimeCardRelease : PXGraph<TimeCardRelease>
	{
		...
		public PXProcessingJoin<EPTimeCardRow,
			...
			LeftJoin<EPEmployeeEx, On<EPEmployeeEx.defContactID, Equal<EPApprovalEx.approvedByID>>>>>>,
		...
		public TimeCardRelease()
		{
			...
			FilteredItems.SetProcessDelegate<TimeCardMaint>(TimeCardRelease.Release);
		...
		{
			timeCardMaint.Clear();
			timeCardMaint.Document.Current = timeCardMaint.Document.Search<EPTimeCard.timeCardCD>(timeCard.TimeCardCD);

			if (timeCardMaint.Document.Current == null)
			{
				throw new PXException(PX.Objects.EP.Messages.TimecardCannotBeReleased_NoRights);
			}
			else
			{
				try
				...
				{
					string message = $"{Messages.TimeCard} = {timeCard.TimeCardCD}; {(e is PXOuterException ? e.Message + "\r\n" + String.Join("\r\n", ((PXOuterException)e).InnerMessages) : e.Message)}";
		...
		public partial class EPApprovalEx : EPApproval
		{
			...
			#endregion
			#region ApprovedByID
			public new abstract class approvedByID : PX.Data.BQL.BqlInt.Field<approvedByID> { }
			[PX.TM.Owner(DisplayName = "Approved by", Visibility = PXUIVisibility.Visible, Enabled = false)]
