...
namespace PX.Objects.PR
{
	public class PRSetupMaint : PXGraph<PRSetupMaint>
	{
		...
		protected virtual void _(Events.FieldVerifying<PRSetup.projectCostAssignment> e)
		{
			...
			VerifyProjectCostAssignment(e.Cache, row, e.NewValue);
		}

		protected virtual void _(Events.FieldVerifying<PRSetup.timePostingOption> e)
		{
			PRSetup row = e.Row as PRSetup;
			if (row == null)
			{
			...
			VerifyTimePostingOption(e.Cache, row, e.NewValue);
		}
		...
		public virtual void _(Events.RowPersisting<PRSetup> e)
		{
			...
			if (e.Row.TimePostingOption == EPPostOptions.PostToOffBalance && e.Row.OffBalanceAccountGroupID == null)
			{
		...
		protected virtual void VerifyProjectCostAssignment(PXCache cache, PRSetup row, object value)
		{
			...
			{
				throw new PXSetPropertyException(Messages.EPPostingOptionNotDoNotPost);
			}
		}

		protected virtual void VerifyTimePostingOption(PXCache cache, PRSetup row, object value)
		{
			if (!EPPostOptions.DoNotPost.Equals(value)
				&& row.ProjectCostAssignment != ProjectCostAssignmentType.NoCostAssigned)
			{
				throw new PXSetPropertyException(Messages.ProjectCostAssignmentNotNoCostAssigned);
			}
		}
		#endregion Helpers
	}
