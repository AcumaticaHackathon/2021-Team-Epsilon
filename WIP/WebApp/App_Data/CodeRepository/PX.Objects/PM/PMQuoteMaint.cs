...
namespace PX.Objects.PM
{
	public class PMQuoteMaint : PXGraph<PMQuoteMaint>, PXImportAttribute.IPXPrepareItems
	{
		...
		protected virtual void _(Events.FieldVerifying<PMQuote, PMQuote.locationID> e)
		{
			var quote = e.Row as PMQuote;
			if (quote != null)
			{
				CR.Location location = CR.Location.PK.Find(this, quote.BAccountID, (int?)e.NewValue);
				if (location?.CTaxCalcMode == TaxCalculationMode.Gross)
				{
					PXSetPropertyException ex = new PXSetPropertyException(Messages.TheGrossTaxCalcModeForBAccountIsNotSupportedInPQuotes);
		...
		{
			PMProject template = PMProject.PK.Find(this, row.TemplateID);
			if (template != null)
			{
				PX.Objects.CR.Standalone.EPEmployee owner = PXSelect<PX.Objects.CR.Standalone.EPEmployee, Where<PX.Objects.CR.Standalone.EPEmployee.defContactID, Equal<Required<PX.Objects.CR.Standalone.EPEmployee.defContactID>>>>.Select(this, template.OwnerID);
				if (owner != null)
