    ...
    public class SchedulePlanBomCopy : ProductionBomCopyBase
    {
        ...
        protected virtual AMSchdOper ScheduleOperation(OperationDetail operationDetail)
        {
            ...
            var amSchdOper = new AMSchdOper
            {
                ...
                FinishTime = amBomOper.FinishTime,
                SetupTime = amBomOper.SetupTime,
                WcID = amBomOper.WcID,
            ...
            while (totalTime > 0)
            {
                ...
                if (schdMinutes > 0)
                {
                    ...
                    if (schdOperDetail != null && wcSchdDetail != null)
                    {
                        ...
                        wcSchdDetail.RunTimeBase = schdMinutes;
                        wcSchdDetail.RunTime = schdMinutes;
                        wcSchdDetail.SchdTime = wcSchdDetail.RunTime;
                        wcSchdDetail.PlanBlocks =
                            ProductionScheduleEngine.MinutesToBlocks(wcSchdDetail.RunTimeBase.GetValueOrDefault(), BlockSize, true);
                        wcSchdDetail.SchdBlocks = 0; // only finite sets schd blocks

                        schdOperDetail.SchdKey = wcSchdDetail.SchdKey;
                        schdOperDetail.RunTimeBase = wcSchdDetail.RunTimeBase.GetValueOrDefault();
