...
namespace PX.Objects.EP
{
	...
	public class TimeCardMaint : PXGraph<TimeCardMaint, EPTimeCard>, PXImportAttribute.IPXPrepareItems
	{
		...
		public PXSelectJoin<EPTimeCard,
						...
						Where<EPTimeCard.createdByID, Equal<Current<AccessInfo.userID>>,
										...
										Or<EPTimeCard.noteID, Approver<Current<AccessInfo.contactID>>,
		...
		private void SelectExistingSummaryRecords(string timecardCD)
		{
			...
			foreach (EPTimeCardSummaryWithInfo item in select.Select(timecardCD))
			{
				...
				if(!existingSummaryRowsByLineNbr.ContainsKey(item.LineNbr.Value))
		...
		public PXSelect<EPTimecardDetail,
			Where<EPTimecardDetail.ownerID, Equal<Current<EPEmployee.defContactID>>,
		...
		public virtual void OnSubmitClicked()
		{
			...
			foreach (PXResult<EPTimecardDetail> res in activities())
			{
				...
				CRCase refCase = (CRCase)PXSelectorAttribute.Select<EPTimecardDetail.caseCD>(Activities.Cache, activity, activity.CaseCD);
				...
				if (project == null)
				{
					string errorMessage = string.Format(ErrorMessages.FieldIsEmpty, PXUIFieldAttribute.GetDisplayName<EPTimecardDetail.projectID>(Activities.Cache));
					PXUIFieldAttribute.SetError<EPTimecardDetail.projectID>(Activities.Cache, activity, errorMessage);
					throw new PXException(errorMessage);
				}

				int? laborClassID = CostEngine.GetLaborClass(activity, employee, refCase);
				if (activity.Released != true)
				{
					if (laborClassID == null)
					{
						if (activity.IsOvertimeCalc == true)
						{
					...
					List<ActivityValidationError> activityErrors = ValidateActivityOnSubmit(activity, project);
					valiationErrors.AddRange(activityErrors);

					DisplayErrors(activity, project, activityErrors);
				}
			...
			for (int i = 0; i < details.Count; i++)
			{
				...
				{
					EPTimeCardSummaryWithInfo summary = GetSummaryRecord(details[i]);
					Summary.Cache.SetValue<EPTimeCardSummaryWithInfo.labourItemID>(summary, details_laborItemID[i]);
					Summary.Cache.MarkUpdated(summary);
		...
		public virtual void PreloadFromPreviousTimecard()
		{
			...
			foreach (EPTimeCardSummaryWithInfo item in summarySelect.Select(previous.TimeCardCD))
			{
					...
					item.EarningType != EpSetup.Current.VacationsType)
				{
					...
					foreach (EPTimeCardSummaryWithInfo existing in Summary.Select())
					{
						...
						PMProject project = PMProject.PK.Find(this, item.ProjectID);
						if (project.IsCompleted == true || project.IsCancelled == true || project.VisibleInTA != true)
						{
							skip = true;
						}

						PMTask task = PMTask.PK.Find(this, item.ProjectTaskID);
						{
							if (task != null && (task.IsCompleted == true || task.IsCancelled == true || project.VisibleInTA != true))
							{
								skip = true;
							}
						}
			...
			if (Document.Cache.GetStatus(Document.Current) != PXEntryStatus.Inserted)
            {
                ...
                {
                    PXGraph.ThrowWithoutRollback(e);
                }
            }
		}
		...
		protected virtual void EPTimecardDetail_EarningTypeID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			{
				e.Cancel = true;
				throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(EPTimecardDetail.earningTypeID)}]");
			}
		}
		...
		protected virtual void EPTimeCardSummaryWithInfo_EarningType_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			{
				e.Cancel = true;
				throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(EPTimeCardSummary.earningType)}]");
			}
		}
		...
		protected virtual void EPTimeCard_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			...
			try
			{
				...
				PXSelectBase<EPTimecardDetail> selectDetail = new PXSelectJoin<EPTimecardDetail,
					InnerJoin<CREmployee, On<CREmployee.defContactID, Equal<EPTimecardDetail.ownerID>>>,
		...
		protected virtual void EPTimeCard_IsApproved_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if ((bool?)e.NewValue == true)
			{
				...
				foreach (PXResult res in activities())
				{
					...
					// Sanitize activity status, see AC-183801
					if (act.ApprovalStatus == ActivityStatusListAttribute.PendingApproval && act.ApproverID == null)
					{
						object result = PXFormulaAttribute.Evaluate<PMTimeActivity.approvalStatus>(Caches[typeof(PMTimeActivity)], act);
						if(!string.IsNullOrEmpty(result?.ToString()))
						{
							act.ApprovalStatus = result.ToString();
							// Acuminator disable once PX1044 ChangesInPXCacheInEventHandlers Without update, status will be left as Pending Approval
		...
		protected virtual void EPTimeCardSummaryWithInfo_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			if (earningType != null && !skipProjectDefaultingFromEarningType)
			{
				...
				{
					PMTask defTask = PMTask.PK.Find(this, earningType.TaskID);
		...
		protected virtual void EPTimeCardSummaryWithInfo_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (oldRow.ProjectID != row.ProjectID || oldRow.ProjectTaskID != row.ProjectTaskID 
				...
				|| oldRow.EarningType != row.EarningType || oldRow.JobID != row.JobID 
				|| oldRow.ShiftID != row.ShiftID || oldRow.CostCodeID != row.CostCodeID
				|| oldRow.UnionID != row.UnionID || oldRow.LabourItemID != row.LabourItemID 
				|| oldRow.WorkCodeID != row.WorkCodeID)
			{
			...
			if (oldRow.ProjectID != row.ProjectID || oldRow.EarningType != row.EarningType)
			{
				sender.SetDefaultExt<EPTimeCardSummaryWithInfo.labourItemID>(e.Row);
			}
		}
		...
		protected virtual void EPTimeCardSummaryWithInfo_ProjectID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			PMTask currentTask = PMTask.PK.Find(this, row.ProjectTaskID);
			if (currentTask != null)
			{
		...
		protected virtual void EPTimecardDetail_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			row.OwnerID = employee.With(_ => _.DefContactID);
		...
		protected virtual void EPTimecardDetail_RowUpdating(PXCache sender, PXRowUpdatingEventArgs e)
		{
			...
			if (row.ProjectID == null && row.TimeSpent.GetValueOrDefault() != 0)
				sender.RaiseExceptionHandling<EPTimecardDetail.projectID>(row, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(EPTimecardDetail.projectID)}]"));
		...
		protected virtual void EPTimecardDetail_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			{
				sender.SetDefaultExt<EPTimecardDetail.labourItemID>(e.Row);
			}
		...
		public virtual void ProcessCorrectingTimecard(PM.RegisterEntry releaseGraph, EPTimeCard timecard)
		{
			...
			InnerJoin<CREmployee,
				On<CREmployee.defContactID, Equal<EPTimecardDetailOrig.ownerID>>,
			LeftJoin<EPTimecardDetailEx,
				...
				And<EPTimecardDetailOrig.trackTime, Equal<True>,
		...
		public virtual void UpdateAdjustingActivities(EPTimeCardSummary summary, Dictionary<DayOfWeek, DayActivities> dict, DayOfWeek dayOfWeek, DateTime startDate, bool skipDescriptionUpdate)
		{
			...
			if (summaryTimeTotal != dayTotal)
			{
				...
				else if (adjust != null)
				{
					...
					object result = PXFormulaAttribute.Evaluate<PMTimeActivity.approvalStatus>(Caches[typeof(PMTimeActivity)], adjust);
					if (!string.IsNullOrEmpty(result?.ToString()))
					{
						adjust.ApprovalStatus = result.ToString();
					}
		...
		public virtual void ValidateProjectAndProjectTask(EPTimecardDetail timeCardDetail)
		{
			if (timeCardDetail != null && Document.Current != null && Document.Current.IsReleased != true)
			{
				...
				{
					PMTask projectTask = PMTask.PK.Find(this, timeCardDetail.ProjectTaskID);
		...
		public virtual PM.PMTran CreateItemTransaction(EPTimeCardItem record, int? employeeID, DateTime? date, decimal? qty, PXCache tranCache)
		{
			...
			if (contract.BaseType == CT.CTPRType.Project)//contract do not record money only usage.
			{
				...
				{
					PMTask task = PMTask.PK.Find(this, record.TaskID);

					#region Combine Account and Subaccount

					if (ExpenseAccountSource == PMAccountSource.Project)
					{
						if (contract.DefaultExpenseAccountID != null)
						{
					...
					if (!string.IsNullOrEmpty(ExpenseSubMask))
					{
						...
						if (ExpenseSubMask.Contains(PMAccountSource.Project) && contract.DefaultExpenseSubID == null)
						{
						...
						subCD = PM.SubAccountMaskAttribute.MakeSub<PMSetup.expenseSubMask>(this, ExpenseSubMask,
							new object[] { nsItem.COGSSubID, contract.DefaultExpenseSubID, task.DefaultExpenseSubID, emp.ExpenseSubID },
			...
			{
				accountGroupID = EPSetupMaint.GetOffBalancePostingAccount(this, epsetup, employeeID);
				accountID = null;
		...
		public virtual void ValidateDisplayVSReportedTimeZones(EPTimeCard timeCard)
		{
			...
			else
			{
				queryResult = PXSelectJoinGroupBy<EPTimecardDetail,
									InnerJoin<CREmployee,
										On<CREmployee.defContactID, Equal<EPTimecardDetail.ownerID>>>,
		...
		public class EPTimeCardSummaryWithInfo : EPTimeCardSummary
		{
			...
			[PXFormula(typeof(
				Switch<
					...
					Case<Where<projectID, IsNotNull>, Selector<projectID, PMProject.approverID>>>
				))]
	...
	public class TimecardApprovalAction : EPApprovalAutomation<EPTimeCard, EPTimeCard.isApproved, EPTimeCard.isRejected, EPTimeCard.isHold, EPSetupTimeCardApproval>
    {
		public TimecardApprovalAction(PXGraph graph): base(graph)
        {
		}

		public TimecardApprovalAction(PXGraph graph, Delegate @delegate) : base(graph, @delegate)
		{
		}

		public override bool Approve(EPTimeCard source)
		{
			OnBeforeApproval(source);
		...
		protected virtual void OnBeforeApproval(EPTimeCard row)
        {
			...
			bool fail = false;
			foreach (PXResult res in ((TimeCardMaint) this._Graph).activities())
			{
				TimeCardMaint.EPTimecardDetail act = (TimeCardMaint.EPTimecardDetail)res[typeof(TimeCardMaint.EPTimecardDetail)];
				...
				if ((act.TimeSpent.GetValueOrDefault() != 0 || act.TimeBillable.GetValueOrDefault() != 0) &&
					...
					act.ApprovalStatus != ActivityStatusListAttribute.Canceled)
				{
					((TimeCardMaint)this._Graph).Activities.Cache.RaiseExceptionHandling<TimeCardMaint.EPTimecardDetail.timeSpent>(act, act.TimeSpent,
																						new PXSetPropertyException(
																							Messages
																								.ActivityIsNotApproved,
																							PXErrorLevel.RowError));
