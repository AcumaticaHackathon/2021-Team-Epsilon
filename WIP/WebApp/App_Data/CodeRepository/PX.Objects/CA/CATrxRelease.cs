...
namespace PX.Objects.CA
{
	...
	public class CATrxRelease : PXGraph<CATrxRelease>
	{
									...
									if (docTransfer.Hold == false)
									{
										if (docTransfer.Released == false)
										{
											ReleaseDoc<CATransfer>(docTransfer, i, externalPostList,
												CATransfer.Events.Select(ev=>ev.ReleaseDocument));
										}
									}
									else
									{
									...
									case GL.BatchModule.CA:
										switch (caRegisterItem.TranType)
										{
											case CATranType.CAAdjustment:
												...
												if (docAdj != null)
												{
													ReleaseAndRecordCADoc(jeForCA, rgForCA, docAdj, 
														caRegisterItem.TranID.Value, 
														CAAdj.Events.Select(e=>e.ReleaseDocument));
												}
												else
													throw new Exception(Messages.DocNotFound);
												break;
											case CATranType.CATransfer:
												...
												if (docTransfer != null)
												{
													ReleaseAndRecordCADoc(jeForCA, rgForCA, docTransfer, caRegisterItem.TranID.Value,
														CATransfer.Events.Select(e=>e.ReleaseDocument));
												}
												else
													throw new Exception(Messages.DocNotFound);
												break;
		...
		private static void ReleaseAndRecordCADoc<TCADoc>(JournalEntry je, 
			CAReleaseProcess rg,
			TCADoc doc, 
			long tranID, 
			...
			where TCADoc : class, ICADocument, IBqlTable, new()
		{
			...
			rg.Clear();
			var batches = rg.ReleaseDocProc(je, ref ignoredBatchList, doc, releaseEvent);
		...
		public static void ReleaseDoc<TCADocument>(TCADocument _doc, 
			int _item, 
			...
			where TCADocument : class, ICADocument, IBqlTable, new()
		{
			...
			bool failed = false;
			rg.Clear();
			rg.ReleaseDocProc(je, ref batchlist, _doc, releaseEvent);
