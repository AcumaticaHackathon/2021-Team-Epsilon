...
namespace PX.Objects.AP
{
	public class VendorClassMaint : PXGraph<VendorClassMaint>
	{
		...
		public VendorClassMaint()
		{
			...
			ActionsMenu.AddMenuAction(resetGroup);
		}
		...
		public virtual void VendorClass_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if (!String.IsNullOrEmpty(row.CuryID) && (row.AllowOverrideCury??false) != true)
			{
				...
				if (acct != null)
				{
					...
					{
                        if (cache.RaiseExceptionHandling<VendorClass.cashAcctID>(e.Row, acct.CashAccountCD, new PXSetPropertyException(Messages.VendorCuryDifferentDefPayCury, $"[{nameof(VendorClass.cashAcctID)}]")))
						{
							throw new PXRowPersistingException(typeof(VendorClass.cashAcctID).Name, null, Messages.VendorCuryDifferentDefPayCury, nameof(VendorClass.cashAcctID));
						}
					}
				}
			}
