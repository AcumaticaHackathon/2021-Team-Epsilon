...
namespace PX.Objects.AR
{
	...
	public class ARCustomerBalanceEnq : PXGraph<ARCustomerBalanceEnq>
	{
		...
		protected virtual void ARHistoryFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetRequired<ARHistoryFilter.orgBAccountID>(sender, multipleBaseCurrenciesInstalled);
			bool enableReports = multipleBaseCurrenciesInstalled ? row.Period != null && row.OrgBAccountID != null : row.Period != null;
		...
		public virtual string GetLastActivityPeriod(int? aCustomerID, bool IncludeChildAccounts)
		{
			...
			CuryARHistory result = PXSelectJoin<CuryARHistory,
											...
											Where<ChildBAccount.consolidatingBAccountID, Equal<Required<CuryARHistory.customerID>>,
												Or<ChildBAccount.bAccountID, Equal<Required<CuryARHistory.customerID>>>>,
											OrderBy<Desc<CuryARHistory.finPeriodID>>>
											.SelectSingleBound(this, null, aCustomerID, aCustomerID);
			if (result != null)
				return result.FinPeriodID;
			return null;
		}
		...
		protected virtual void Aggregate(ARHistoryResult aDest, ARHistoryResult aSrc)
		{
			...
			aDest.CuryEndRetainedBalance += aSrc.CuryEndRetainedBalance ?? Decimal.Zero;
			...
			aDest.EndRetainedBalance += aSrc.EndRetainedBalance ?? Decimal.Zero;
        ...
        protected virtual void AggregateLatest(ARHistoryResult aDest, ARHistoryResult aSrc)
		{
			...
			else
			{
				if (string.Compare(aSrc.FinPeriodID, aDest.FinPeriodID) < 0)
				{
					...
					aDest.EndRetainedBalance = aDest.BegRetainedBalance + aSrc.EndRetainedBalance ?? Decimal.Zero - aSrc.BegRetainedBalance ?? Decimal.Zero;
					...
					aDest.CuryEndRetainedBalance = aDest.CuryBegRetainedBalance + aSrc.CuryEndRetainedBalance ?? Decimal.Zero - aSrc.CuryBegRetainedBalance ?? Decimal.Zero;
				}
				else
				{
					//Invert 
