...
namespace PX.Objects.DR
{
	...
	public class DRTerms
	{
			...
			IPXRowPersistingSubscriber
		{
			...
			public DatesAttribute(Type startDateField, Type endDateField, Type inventoryField, Type deferralCodeField) 
				: this(startDateField, endDateField, inventoryField, deferralCodeField, null)
			{ 
			}
			...
			public void RowSelected(PXCache sender, PXRowSelectedEventArgs e)
			{
				if(e.Row != null)
				{
					...
					if (_Hold != null)
					{
						Type parentType = BqlCommand.GetItemType(_Hold);
						PXCache parentcache = sender.Graph.Caches[parentType];
						object parent = PXParentAttribute.SelectParent(sender, e.Row, parentType);

						bool hold = (bool)parentcache.GetValue(parent, _Hold.Name);
						var requiresTerms = (bool?)sender.GetValue(e.Row, _FieldOrdinal);
						var startDate = (DateTime?)sender.GetValue(e.Row, _StartDateField.Name);
						var endDate = (DateTime?)sender.GetValue(e.Row, _EndDateField.Name);

						bool isTermRequired = hold == false && requiresTerms == true;
						bool isTermStartDateEmpty = isTermRequired && !startDate.HasValue;
						bool isTermEndDateEmpty = isTermRequired && !endDate.HasValue;

						sender.RaiseExceptionHandling(_StartDateField.Name, e.Row, startDate, isTermStartDateEmpty ? new PXSetPropertyException(Messages.DRTermStartDateShouldNotBeEmpty, PXErrorLevel.Warning) : null);
						sender.RaiseExceptionHandling(_EndDateField.Name, e.Row, endDate, isTermEndDateEmpty ? new PXSetPropertyException(Messages.DRTermEndDateShouldNotBeEmpty, PXErrorLevel.Warning) : null);
						...
						if (requiresTerms == true &&
								...
								endDate <= startDate)
						{
							if (sender.RaiseExceptionHandling(_EndDateField.Name, e.Row, endDate, new PXSetPropertyException(DR.Messages.TermCantBeNegativeWithTermStartDateValue, startDate)))
							{
