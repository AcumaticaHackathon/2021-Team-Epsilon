		...
		public virtual void UpdateRates()
		{
			if (Documents.Current != null)
			{
				...
				foreach (CarrierPlugin plugin in GetApplicableCarrierPlugins())
				{
					...
					{
								string warning = string.Format(Messages.AutoPackagingZeroPackWarning, plugin.CarrierPluginID);
					...
					if (result != null && result.IsSuccess)
					{
						foreach (RateQuote item in result.Result)
						{
							...
							foreach (Carrier shipVia in selectCarrier.Select(info.Plugin.CarrierPluginID, item.Method.Code))
							{
								...
								if (!item.IsSuccess)
								{
									string errorMessages = item.Messages.Select(x => string.IsNullOrEmpty(x.Code) ? x.Description : string.Format("{0}: {1}", x.Code, x.Description)).Aggregate((i, j) => i + Environment.NewLine + j + Environment.NewLine);
									CarrierRates.Cache.RaiseExceptionHandling(typeof(SOCarrierRate.method).Name, r, null, new PXSetPropertyException("{0}", PXErrorLevel.RowError, errorMessages));
								}
							}
						}
					}
