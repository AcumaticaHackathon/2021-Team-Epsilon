...
namespace PX.Objects.FS
{
    public class SM_CustomerMaint : PXGraphExtension<CustomerMaint>
    {
        ...
        {
            config
                .GetScreenConfigurationContext<CustomerMaint, Customer>()
                .UpdateScreenConfigurationFor(screen =>
                {
                    return screen
                        .UpdateDefaultFlow(flow =>
                        {
                            ...
                            {
                                states.Update(CustomerStatus.Active,
                                    state => state.WithActions(actions =>
                                    {
                                        actions.Add<SM_CustomerMaint>(e => e.openMultipleStaffMemberBoard);
                                        actions.Add<SM_CustomerMaint>(e => e.openSingleStaffMemberBoard);
                                    }));

                                states.Update(CustomerStatus.OneTime,
                                    state => state.WithActions(actions =>
                                    {
                                        actions.Add<SM_CustomerMaint>(e => e.openMultipleStaffMemberBoard);
                                        actions.Add<SM_CustomerMaint>(e => e.openSingleStaffMemberBoard);
                        ...
                        {
                            // "Actions" folder
                            actions.Add<SM_CustomerMaint>(g => g.openMultipleStaffMemberBoard, a => a.InFolder(FolderType.ActionsFolder, nameof(CustomerMaint.ChangeID)));
                            actions.Add<SM_CustomerMaint>(g => g.openSingleStaffMemberBoard, a => a.InFolder(FolderType.ActionsFolder, nameof(SM_CustomerMaint.openMultipleStaffMemberBoard)));

                            // "Inquiries" folder
                            actions.Add<SM_CustomerMaint>(g => g.viewServiceOrderHistory, a => a.InFolder(FolderType.InquiriesFolder));
                            actions.Add<SM_CustomerMaint>(g => g.viewAppointmentHistory, a => a.InFolder(FolderType.InquiriesFolder));
                            actions.Add<SM_CustomerMaint>(g => g.viewEquipmentSummary, a => a.InFolder(FolderType.InquiriesFolder));
                            actions.Add<SM_CustomerMaint>(g => g.viewContractScheduleSummary, a => a.InFolder(FolderType.InquiriesFolder));
                        });
                });
        }
        ...
        public virtual void Persist(Action baseMethod)
        {
            ...
            if (PersistedCustomerWithBillingOptionsChanged != null)
            {
                FSxCustomer fsxCustomerRow = Base.BAccount.Cache.GetExtension<FSxCustomer>(customer);

                if (fsxCustomerRow.BillingOptionsChanged == true)
                {
                    UpdateBillingInfoInDocsLO(Base, PersistedCustomerWithBillingOptionsChanged.BAccountID, null);
        ...
        {
            return CheckDuplicatedEntry(cache, fsCustomerBillingSetupRow, true);
        }

        protected virtual bool CheckDuplicatedEntry(PXCache cache, FSCustomerBillingSetup currentRow, bool externalCall)
        {
            foreach (FSCustomerBillingSetup row in CustomerBillingCycles.Select())
            {
                if (row.SrvOrdType == currentRow.SrvOrdType
                        && row.CBID != currentRow.CBID
                        && row.Active == true
                        && currentRow.Active == true)
                {
                    PXException exception = new PXSetPropertyException(ErrorMessages.DuplicateEntryAdded);

                    cache.RaiseExceptionHandling<FSCustomerBillingSetup.srvOrdType>(
                                    currentRow, currentRow.SrvOrdType,
                                    exception);
        ...
        public virtual void PreUpdateBillingInfoDocs(PXGraph callerGraph, int? currentCustomerID, int? currentBillingCycleID)
        {
            SetupMaint.PreUpdateBillingInfoDocsInt(callerGraph, currentCustomerID, currentBillingCycleID);
        }
        ...
        protected virtual void _(Events.RowPersisted<Customer> e)
        {
            ...
            {
                PreUpdateBillingInfoDocs(Base, customerRow.BAccountID, null);
            }
                    ...
                    && e.TranStatus == PXTranStatus.Completed)
            {
                if (fsxCustomerRow.BillingOptionsChanged == true)
                {
                    ...
                    PersistedCustomerWithBillingOptionsChanged = customerRow;
                }

                if (fsxCustomerRow.IsExtendingToCustomer == true)
                {
        ...
        protected virtual void _(Events.RowInserting<FSCustomerBillingSetup> e)
        {
            ...
            if (CheckDuplicatedEntry(e.Cache, e.Row, e.ExternalCall))
            {
                e.Cancel = true;
                return;
            }
        ...
        protected virtual void _(Events.RowUpdating<FSCustomerBillingSetup> e)
        {
            ...
            if (CheckDuplicatedEntry(e.Cache, e.NewRow, e.ExternalCall))
            {
                e.Cancel = true;
                return;
            }
        ...
        protected virtual void _(Events.RowDeleting<FSCustomerBillingSetup> e)
        {
            ...
            bool convertDeletingToDeactivation = IsCBIDRelatedToPostedDocuments(fscustomerBillingSetupRow.CBID);
            if (convertDeletingToDeactivation)
            {
                e.Row.Active = false;
        ...
        protected virtual void _(Events.RowPersisting<FSCustomerBillingSetup> e)
        {
            ...
            CheckDuplicatedEntry(e.Cache, e.Row, false);
            ...
            if (e.Operation == PXDBOperation.Update)
            {
                bool restoreOriginalValues = false;

                if (row.Active == true)
                {
                    bool createDuplicate = IsCBIDRelatedToPostedDocuments(row.CBID);

                    if (createDuplicate)
                    {
                        fsxCustomerRow.BillingOptionsChanged = true;

                        cache.SetValue<FSCustomerBillingSetup.active>(row, false);

                        var duplicate = (FSCustomerBillingSetup)cache.CreateCopy(row);
                        duplicate.CBID = null;
                ...
                if (restoreOriginalValues == true)
                {
                    foreach (var field in cache.Fields)
                    {
                            ...
                            && field.ToLower() != typeof(FSCustomerBillingSetup.active).Name.ToLower())
                        {
                            cache.SetValue(row,
                                           field.ToString(),
                                           cache.GetValueOriginal(row, field.ToString()));
                        }
                    }
                }
            }
