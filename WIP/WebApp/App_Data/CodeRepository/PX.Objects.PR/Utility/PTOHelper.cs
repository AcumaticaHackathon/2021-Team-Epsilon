...
namespace PX.Objects.PR
{
	public class PTOHelper
	{
		...
		private static IPTOBank GetSourceBank(PRPTOBank bank, PREmployeeClassPTOBank classBank, PREmployeePTOBank employeeBank)
		{
			IPTOBank sourceBank;
		...
		public static void GetPTOHistory(PXGraph graph, DateTime targetDate, int employeeID, IPTOBank bank, out decimal accumulated, out decimal used, out decimal available)
		{
			...
			used = 0;
			available = 0;
			PTOHelper.GetPTOBankYear(targetDate, bank.PTOYearStartDate.Value, out DateTime startDate, out DateTime endDate);
		...
		public static IEnumerable<IPTOBank> GetEmployeeBanks(PXGraph graph, PRPayment current)
		{
			var queryResult = SelectFrom<PREmployeePTOBank>
				...
				.View.Select(graph)
				.Select(x => (PXResult<PREmployeePTOBank, PREmployee, PREmployeeClassPTOBank, PRPTOBank>)x).ToList();

			var filteredResults = queryResult.Select(x => GetSourceBank(x, x, x))
				.Where(x => x.StartDate.Value.Date <= current.TransactionDate.Value.Date)
