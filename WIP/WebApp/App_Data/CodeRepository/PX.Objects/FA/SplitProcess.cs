...
namespace PX.Objects.FA
{
	public class SplitProcess : PXGraph<SplitProcess>
	{
		...
		public void _(Events.FieldVerifying<SplitFilter.splitDate> e) 
		{
			SplitFilter filter = (SplitFilter)e.Row;
			if (filter == null) return;

			FADetails details = PXSelect<FADetails, Where<FADetails.assetID, Equal<Required<FADetails.assetID>>>>.Select(this, filter.AssetID);

			if (details?.ReceiptDate != null && details.ReceiptDate.Value.CompareTo((DateTime?)e.NewValue) > 0)
			{
				throw new PXSetPropertyException(Messages.FixedAssetCannotBeSplitInSateLessThanPurchaseDate, details.ReceiptDate.Value.ToShortDateString());
			}
		}
		...
		public void SplitFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			Splits.SetProcessAllEnabled(filter.AssetID != null && filter.SplitPeriodID != null && String.IsNullOrEmpty(PXUIFieldAttribute.GetErrorOnly<SplitFilter.splitDate>(sender, filter)));
