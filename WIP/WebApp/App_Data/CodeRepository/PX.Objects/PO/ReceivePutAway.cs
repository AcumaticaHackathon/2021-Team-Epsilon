...
{
	[PX.Objects.IN.WMS.Legacy.LegacyWMS]
	...
	public class ReceivePutAway : WMSBase
	{
		...
		public class Header : WMSHeader, ILSMaster
		{
			...
			#region NoteID
			[BorrowedNote(typeof(POReceipt), typeof(POReceiptEntry))]
		...
		#region Return
		public SelectFrom<POReceiptLineSplit>.View Returned;
		public virtual IEnumerable returned() => received();

		public SelectFrom<POReceiptLineSplit>.View ReturnedNotZero;
		public virtual IEnumerable returnedNotZero() => receivedNotZero();
		#endregion
		...
		[PXButton, PXUIField(DisplayName = "Transfer Line Details")]
		...
		protected override void ProcessDocumentNumber(string barcode)
		{
			...
			if (newReceipt == false)
			{
				...
				{
					ReportError(Msg.ReceiptInvalid, receipt.ReceiptNbr, Base.Document.Cache.GetStateExt<POReceipt.status>(receipt));
					return;
				}
		...
		protected virtual WMSFlowStatus ProcessReceived()
		{
			WMSFlowStatus Implementation()
			{
				...
				if (!EnsureLocationPrimaryItem(header.InventoryID, header.LocationID))
					WMSFlowStatus.Fail(IN.Messages.NotPrimaryLocation);
		...
		protected virtual WMSFlowStatus ProcessPutAwayNoCart()
		{
			WMSFlowStatus Implementation()
			{
				...
				if (!EnsureLocationPrimaryItem(header.InventoryID, header.ToLocationID))
					WMSFlowStatus.Fail(IN.Messages.NotPrimaryLocation);
		...
		private INTransferEntry CreateTransferEntry()
		{
			...
			ie.FieldDefaulting.AddHandler<INTran.pOReceiptType>((cache, args) => args.NewValue = ((INTransferEntry)cache.Graph).transfer.Current?.POReceiptType);
			ie.FieldDefaulting.AddHandler<INTran.pOReceiptNbr>((cache, args) => args.NewValue = ((INTransferEntry)cache.Graph).transfer.Current?.POReceiptNbr);
		...
		protected virtual WMSFlowStatus EnsureReceiptTransferSplitLink(POReceiptLineSplit poSplit, INTranSplit inSplit, decimal deltaQty)
		{
			var allLinks =
				SelectFrom<POReceiptSplitToTransferSplitLink>
				.Where<POReceiptSplitToTransferSplitLink.FK.TransferLineSplit.SameAsCurrent
			...
			POReceiptSplitToTransferSplitLink currentLink = allLinks.FirstOrDefault(
				link => POReceiptSplitToTransferSplitLink.FK.TransferLineSplit.Match(Base, inSplit, link)
					&& POReceiptSplitToTransferSplitLink.FK.ReceiptLineSplit.Match(Base, poSplit, link));

			decimal transferQty = allLinks.Where(link => POReceiptSplitToTransferSplitLink.FK.TransferLineSplit.Match(Base, inSplit, link)).Sum(link => link.Qty ?? 0);
			decimal receiptQty = allLinks.Where(link => POReceiptSplitToTransferSplitLink.FK.ReceiptLineSplit.Match(Base, poSplit, link)).Sum(link => link.Qty ?? 0);
		...
		protected virtual WMSFlowStatus SyncWithCart(Header header, POReceiptLineSplit receivedSplit, decimal qty)
		{
			INCartSplit[] linkedSplits =
				SelectFrom<POReceiptSplitToCartSplitLink>
				.InnerJoin<INCartSplit>.On<POReceiptSplitToCartSplitLink.FK.CartSplit>
				.Where<POReceiptSplitToCartSplitLink.FK.ReceiptLineSplit.SameAsCurrent
		...
		protected virtual WMSFlowStatus EnsureReceiptCartSplitLink(POReceiptLineSplit poSplit, INCartSplit cartSplit, decimal deltaQty)
		{
			...
			POReceiptSplitToCartSplitLink currentLink = allLinks.FirstOrDefault(
				link => POReceiptSplitToCartSplitLink.FK.CartSplit.Match(Base, cartSplit, link)
					&& POReceiptSplitToCartSplitLink.FK.ReceiptLineSplit.Match(Base, poSplit, link));
		...
		protected virtual void ReleaseReceipt(bool completePOLines)
		{
			...
			(wArgs) =>
			{
				var document = wArgs.Document;
				if (Base.Document.Current.Hold == true)
				{
					var receiptGraph = PXGraph.CreateInstance<POReceiptEntry>();
					receiptGraph.Document.Current = receiptGraph.Document.Search<POReceipt.receiptNbr>(document.ReceiptNbr, document.ReceiptType);
					receiptGraph.releaseFromHold.Press();
					document = receiptGraph.Document.Current;
				}

				POReleaseReceipt.ReleaseDoc(new List<POReceipt>() { document }, false);
		...
		private bool EnsureLocationPrimaryItem(int? inventoryID, int? locationID)
		{
			var location = INLocation.PK.Find(Base, locationID);
			if (location != null)
			{
		...
		protected decimal GetCartQty(POReceiptLineSplit posplit) => CartSplitLinks.SelectMain().Where(link => POReceiptSplitToCartSplitLink.FK.ReceiptLineSplit.Match(Base, posplit, link)).Sum(_ => _.Qty ?? 0);
		protected decimal GetOverallCartQty(POReceiptLineSplit posplit) => AllCartSplitLinks.SelectMain().Where(link => POReceiptSplitToCartSplitLink.FK.ReceiptLineSplit.Match(Base, posplit, link)).Sum(_ => _.Qty ?? 0);
		...
		public new abstract class Msg : WMSBase.Msg
		{
			...
			public const string ReceiptInvalid = "The {0} receipt cannot be processed because it has the {1} status.";
			public const string ReceiptMultiSites = "The {0} receipt should have only one warehouse to be processed.";
			...
			public const string ReceiptIsReleased = "The receipt has been successfully released.";
			...
			public const string TransferIsReleased = "The transfer has been successfully released.";
