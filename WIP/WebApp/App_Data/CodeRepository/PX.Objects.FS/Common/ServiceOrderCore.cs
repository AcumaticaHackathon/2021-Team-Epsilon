...
namespace PX.Objects.FS
{
    public static class ServiceOrderCore
    {
        ...
        {
            OpenEmployeeBoard_Handler(graph, serviceOrderRecords.Current);
        }
                                                     ...
                                                     FSServiceOrder fsServiceOrder)
        {
            ...
            if (fsServiceOrder.Status != ID.Status_ServiceOrder.OPEN)
            {
            ...
            ServiceOrderCore.GetPendingLines(graph, (int)fsServiceOrder.SOID, ref bqlResultSet_SODet);

            if (bqlResultSet_SODet.Count > 0)
            {
                throw new PXRedirectToBoardRequiredException(Paths.ScreenPaths.MULTI_EMPLOYEE_DISPATCH, ServiceOrderCore.GetServiceOrderUrlArguments(fsServiceOrder));
            }
            else
            {
        ...
        private static Exception CheckIfCustomerBelongsToProject(PXGraph graph, PXCache cache, FSServiceOrder fsServiceOrderRow, Contract ContractRow)
        {
            if (fsServiceOrderRow == null)
            {
                return null;
                                                   ...
                                                   bool allowCustomerChange = false)
        {
            ...
            bool internalSOType = fsSrvOrdTypeRow.Behavior == ID.Behavior_SrvOrderType.INTERNAL_APPOINTMENT;
        ...
        public static void EnableDisable_ActionButtons(ServiceOrderEntry graph,
                                                       ...
                                                       PXAction<FSServiceOrder> addInvBySite,
                                                       PXAction<FSServiceOrder> addInvSelBySite,
                                                       ...
                                                       PXAction<FSServiceOrder> addReceipt)
        {
            ...
            else
            {
                ...
                createPurchaseOrder.SetEnabled(fsServiceOrderRow?.CanCreatePurchaseOrder == true && fsSrvOrdTypeRow?.PostToSOSIPM == true);
        ...
        public static DateTime? CalculateCutOffDate(string frequencyType, int? weeklyFrequency, int? monthlyFrequency, DateTime? orderDate)
        {
            ...
            orderDate = new DateTime(orderDate.Value.Year, orderDate.Value.Month, orderDate.Value.Day);
        ...
        private static void UpdatePendingPostFlags(ServiceOrderEntry graphServiceOrderEntry, FSServiceOrder fsServiceOrderRow, PXSelectBase<FSSODet> serviceDetails)
        {
            int? linesToPost = 0;
        ...
        {
            ValidateCustomerBillingCycle<FSServiceOrder.customerID>(cache, fsServiceOrderRow, fsServiceOrderRow.CustomerID, fsSrvOrdTypeRow, setupRecordRow);
        }

        public static void ValidateCustomerBillingCycle<TField>(PXCache cache, Object row, int? customerID, FSSrvOrdType fsSrvOrdTypeRow, FSSetup setupRecordRow)
            where TField : IBqlField
        {
            ...
            if (AccountIsAProspect(cache.Graph, customerID) == false
                    && CustomerHasBillingCycle(cache.Graph, setupRecordRow, customerID, fsSrvOrdTypeRow.SrvOrdType) == false)
            {
                cache.RaiseExceptionHandling<TField>(row, customerID, new PXSetPropertyException(TX.Error.MISSING_CUSTOMER_BILLING_CYCLE, PXErrorLevel.Error));
            }
        }
        ...
        public static string GetSODetStatus(FSSODet fsSODetRow, decimal? apptQty = null, decimal? apptEstimatedDuration = null)
        {
            ...
            else if (fsSODetRow.IsExpenseReceiptItem == true) 
            {
