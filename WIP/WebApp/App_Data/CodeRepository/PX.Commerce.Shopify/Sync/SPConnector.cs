...
namespace PX.Commerce.Shopify
{
	...
	public class SPProcessorsFactory : IProcessorsFactory
	{
		...
		{
			yield return new KeyValuePair<Type, int>(typeof(SPCustomerProcessor), 10);
			yield return new KeyValuePair<Type, int>(typeof(SPLocationProcessor), 20);
			yield return new KeyValuePair<Type, int>(typeof(SPStockItemProcessor), 30);
			yield return new KeyValuePair<Type, int>(typeof(SPNonStockItemProcessor), 40);
			yield return new KeyValuePair<Type, int>(typeof(SPTemplateItemProcessor), 50);
			yield return new KeyValuePair<Type, int>(typeof(SPImageProcessor), 60);
			yield return new KeyValuePair<Type, int>(typeof(SPAvailabilityProcessor), 70);
			yield return new KeyValuePair<Type, int>(typeof(SPSalesOrderProcessor), 80);
			yield return new KeyValuePair<Type, int>(typeof(SPPaymentProcessor), 90);
			yield return new KeyValuePair<Type, int>(typeof(SPShipmentProcessor), 100);
			yield return new KeyValuePair<Type, int>(typeof(SPRefundsProcessor), 110);
			yield break;
		}
	}
	#endregion
			...
			where TInfo : class
		{
			...
			try
			{
				...
				if (infoType == BCObjectsConstants.BCPayment)
				{
					...
					{
						var paymentList = orderList.SelectMany(x => x.PaymentGatewayNames.Where(n =>n != null && !payments.Any(p => string.Equals(p.Name, n, StringComparison.OrdinalIgnoreCase))).Distinct().Select(s => new PaymentMethod() { Name = s }));
	...
	public class SPConnectorDescriptor : IConnectorDescriptor
	{
		...
		protected SPRestClient GetSPRestClient(Int32? binding)
		{
			...
			int? apiLimit = ShopifyConstants.ApiCallLimitDefault;
