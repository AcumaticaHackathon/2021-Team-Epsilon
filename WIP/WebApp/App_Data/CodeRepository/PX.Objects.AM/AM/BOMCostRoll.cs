...
namespace PX.Objects.AM
{
    public class BOMCostRoll : PXGraph<BOMCostRoll>
    {
        ...
        // Remove INUnit attribute to avoid sub query
        [PXDBString(6, IsUnicode = true, InputMask = ">aaaaaa")]
        [PXUIField(DisplayName = "UOM", Visibility = PXUIVisibility.Visible)]
        protected virtual void _(Events.CacheAttached<AMBomMatl.uOM> e) { }

        //avoid run of PXDBCalc when not necessary
        [PXDBDecimal(6)]
        [PXDefault(TypeCode.Decimal, "1.0")]
        [PXUIField(DisplayName = "Qty Required")]
        protected virtual void _(Events.CacheAttached<AMBomMatl.qtyReq> e) { }
        ...
        protected virtual void RollCosts()
        {
            ...
            if (!IsMultiLevel)
            {
                foreach (PXResult<AMBomItem, AMBomItemActiveAggregate, AMBomItemBomDefaults, InventoryItem, INItemSite> result in resultset)
                {
                    // adding to cache so we can retrieve from sub calls and remove related sub queries 
                    Common.Cache.AddRowToCache(this, (InventoryItem)result);
                    Common.Cache.AddRowToCache(this, (INItemSite)result);
            ...
            var cmdBOM = string.IsNullOrWhiteSpace(Settings.Current.BOMID)
                ...
                : (PXSelectBase<AMBomItem>) new PXSelectJoin<AMBomItem,
                    ...
                    InnerJoin<InventoryItem,
                        On<AMBomItem.inventoryID, Equal<InventoryItem.inventoryID>>,
                    LeftJoin<INItemSite, On<AMBomItem.inventoryID, Equal<INItemSite.inventoryID>,
                        And<INItemSite.siteID, Equal<AMBomItem.siteID>>>>>>>>(this);
            ...
            foreach (PXResult<AMBomMatlBomCostDrillDown, AMBomItem, AMBomItemActiveAggregate, AMBomItemBomDefaults> result in PXSelectJoin<
                ...
                OrderBy<
                    ...
                    Asc<AMBomMatlBomCostDrillDown.lineID>>>>
                        >
                .Select(this, bomID, revisionID, new string[] { AMBomStatus.Hold, AMBomStatus.Active }))
            {
                var bomItem = (AMBomItem) result;
        ...
        protected virtual bool ProcessCost(AMBomItem bomItem, int level, bool isDefault)
        {
            ...
            var invItem = InventoryHelper.CacheQueryInventoryItem(InvItemRecs.Cache, bomcostrec.InventoryID);

            bomcostrec.ItemClassID = invItem?.ItemClassID;

            //Set the Current and Pending cost from INItemSite
            var inItemSite = InventoryHelper.CacheQueryINItemSite(ItemSiteRecs.Cache, bomcostrec.InventoryID, bomcostrec.SiteID);
            ...
            else
            {
                bomcostrec.LotSize = invItem == null ? 
                    ...
                    InventoryHelper.GetMfgReorderQty(this, invItem, inItemSite);
            }
        ...
        protected virtual void UpdateMaterialUnitCost(AMBomMatl amBomMatl, decimal? unitCost)
        {
            ...
            PXDatabase.Update<AMBomMatl>(
                new PXDataFieldAssign<AMBomMatl.unitCost>(PXDbType.Decimal, unitCost.GetValueOrDefault()),
                new PXDataFieldAssign<AMBomMatl.lastModifiedDateTime>(PXDbType.DateTime, Common.Dates.UtcNow),
                new PXDataFieldAssign<AMBomMatl.createdByScreenID>(PXDbType.Char, 8, Accessinfo?.ScreenID.Replace(".", "")),
        ...
        protected virtual OperationCosts GetMaterialCost(AMBomCost currentAMBomCost, IEnumerable<PXResult<AMBomMatl, InventoryItem>> material, bool isMultLevel, out List<string> materialMessage)
        {
            ...
            foreach(var result in material)
            {
                ...
                if (isMultLevel)
                {
                    ...
                    var bomItem = PrimaryBomIDManager.GetNotArchivedRevisionBomItem(this,
                        bomManager.GetPrimaryAllLevels(inventoryItem,
                            InventoryHelper.CacheQueryINItemSite(ItemSiteRecs.Cache, matlRec.InventoryID, matlSiteID),
                            matlRec.SubItemID));
        ...
        protected virtual decimal? GetUnitCost(InventoryItem inventoryItem, int? siteid)
        {
            ...
            var itemsite = InventoryHelper.CacheQueryINItemSite(ItemSiteRecs.Cache, inventoryItem.InventoryID, siteid);
            ...
            itemsite = PXSelect<INItemSite,
                Where<INItemSite.inventoryID, Equal<Required<INItemSite.inventoryID>>,
                    And<INItemSite.isDefault, Equal<True>>>>.Select(this, inventoryItem.InventoryID);
