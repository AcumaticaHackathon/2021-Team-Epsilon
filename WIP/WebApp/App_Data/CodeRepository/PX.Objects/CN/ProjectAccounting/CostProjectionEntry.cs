...
namespace PX.Objects.CN.ProjectAccounting
{
	public class CostProjectionEntry : PXGraph<CostProjectionEntry, PMCostProjection>, PXImportAttribute.IPXPrepareItems
	{
		...
		private void CalculateTotals()
		{
			...
			PMCostProjection document = Document.Current;

			document.TotalBudgetedAmount = 0m;
			document.TotalActualAmount = 0m;
			document.TotalUnbilledAmount = 0m;
			document.TotalAmount = 0m;
			document.TotalProjectedAmount = 0m;

			foreach (PMCostProjectionLine line in Details.Select())
			{
				document.TotalBudgetedAmount += line.BudgetedAmount.GetValueOrDefault();
				document.TotalActualAmount += line.ActualAmount.GetValueOrDefault();
				document.TotalUnbilledAmount += line.UnbilledAmount.GetValueOrDefault();
				document.TotalAmount += line.Amount.GetValueOrDefault();
				document.TotalProjectedAmount += line.ProjectedAmount.GetValueOrDefault();
			}
