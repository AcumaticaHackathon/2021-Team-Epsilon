...
namespace PX.Objects.AR
{
	...
	public class ARDocumentEnq : PXGraph<ARDocumentEnq>
	{
				...
				if (header.ShowAllDocs == false)
		{
			...
			BqlCommand selectAdjusted = header.IncludeUnreleased == true
				? BqlCommand.AppendJoin<
						InnerJoin<ARAdjust,
							On<ARAdjust.adjgDocType, Equal<ARDocumentResult.docType>,
							And<ARAdjust.adjgRefNbr, Equal<ARDocumentResult.refNbr>>>>>(selectDocument)
				: BqlCommand.AppendJoin<
					InnerJoin<ARAdjust,
						On<ARAdjust.adjgDocType, Equal<ARDocumentResult.docType>,
							And<ARAdjust.adjgRefNbr, Equal<ARDocumentResult.refNbr>,
							And<ARAdjust.released, Equal<True>>>>>>(selectDocument);
			selectAdjusted =
				BqlCommand.AppendJoin<
					InnerJoin<Branch,
					...
					InnerJoin<FinPeriod,
						...
						And<FinPeriod.finPeriodID, Equal<Current<ARDocumentFilter.period>>>>>>>(selectAdjusted);

			BqlCommand selectAdjusting = header.IncludeUnreleased == true
				? BqlCommand.AppendJoin<
					InnerJoin<ARAdjust,
						On<ARAdjust.adjdDocType, Equal<ARDocumentResult.docType>,
							And<ARAdjust.adjdRefNbr, Equal<ARDocumentResult.refNbr>,
							And<Not<Where<ARAdjust.adjgDocType, Equal<ARDocumentResult.docType>,
									And<ARAdjust.adjgRefNbr, Equal<ARDocumentResult.refNbr>>>>>>>>>(selectDocument)
				: BqlCommand.AppendJoin <
						InnerJoin < ARAdjust,
							On< ARAdjust.adjdDocType, Equal<ARDocumentResult.docType>,
			...
			selectAdjusting =
				BqlCommand.AppendJoin<
					...
					InnerJoin<FinPeriod,
						On<Branch.organizationID, Equal<FinPeriod.organizationID>,
						And<FinPeriod.finPeriodID, Equal<Current<ARDocumentFilter.period>>>>>>>(selectAdjusting);

			BqlCommand selectRetainageInvoices = header.IncludeUnreleased == true
					? BqlCommand.AppendJoin<
					...
					InnerJoin<ARRetainage,
						On<ARRetainage.origDocType, Equal<ARDocumentResult.docType>,
							And<ARRetainage.origRefNbr, Equal<ARDocumentResult.refNbr>,
							And<ARRetainage.isRetainageDocument, Equal<True>,
							And<ARRetainage.released, Equal<True>>>>>>>(selectDocument);
			...
			bool byPeriod) 
		{
			...
			if (byPeriod)
			{
				...
				res.RetainageUnreleasedAmt *= res.SignBalance;
				res.CuryRetainageUnreleasedAmt *= res.SignBalance;
			}
			else
			{
				res.OrigDocAmt = res.SignBalance * res.OrigDocAmt;
			...
			res.RetainageTotal *= res.SignBalance;
			res.CuryRetainageTotal *= res.SignBalance;
			res.OrigDocAmtWithRetainageTotal *= res.SignBalance;
			res.CuryOrigDocAmtWithRetainageTotal *= res.SignBalance;
			res.RetainageUnreleasedAmt *= res.SignBalance;
			res.CuryRetainageUnreleasedAmt *= res.SignBalance;
			//SetValuesSign(res);
		...
		protected virtual void SetValuesSign(ARDocumentResult aRes)
		{
			if (aRes.SignBalance.HasValue)
			{
				...
				aRes.RetainageTotal *= sign;
				aRes.CuryRetainageTotal *= sign;
				aRes.OrigDocAmtWithRetainageTotal *= sign;
				aRes.CuryOrigDocAmtWithRetainageTotal *= sign;
			}
		}
