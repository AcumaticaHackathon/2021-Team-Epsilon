...
namespace PX.Objects.CM
{
	public static class CurrencyCollection
	{
		...
		{
			if (slot.BaseCuryID != null && slot.Currency.TryGetValue(slot.BaseCuryID, out var currency))
				return currency;
			return null;
		}
		...
		private static Definition slot =>
			PXDatabase.GetSlot<Definition>(nameof(CurrencyCollection),
				new Type[] {typeof(Currency), typeof(CurrencyList), typeof(Company)});
	}
		...
		where TNode : class, IBqlTable, new()
	{
		...
		[PXButton(ImageKey = PX.Web.UI.Sprite.Main.Money, Tooltip = Messages.ToggleCurrencyViewTooltip, VisibleOnDataSource = false, CommitChanges = false)]
	...
	public class CurySymbolAttribute : PXEventSubscriberAttribute
	{
		...
		#region Ctor
		public CurySymbolAttribute(Type branchID = null, Type curyID = null, Type curyInfoId = null)
		{
	...
	public static class InvoiceEntry
	{
		...
		/// </summary>
		public static void CalcBalancesFromInvoiceSide(
			ARAdjust2 adj,
			ARInvoice invoice,
			ARPayment payment,
			...
			ARAdjust2 others = null)
		{
			...
			InitBalancesFromInvoiceSide(currencyinfoselect, adj, invoice, payment, others);
			...
			PaymentEntry.AdjustBalance(currencyinfoselect, adj, adj.AdjdCuryInfoID, adj.CuryAdjdAmt, adj.CuryAdjdDiscAmt, adj.CuryAdjdWOAmt, false);

			if (isCalcRGOL && (adj.Voided != true))
			{
				CalcRGOLFromInvoiceSide(currencyinfoselect, payment, adj);
		...
		/// </summary>
		public static void InitBalancesFromInvoiceSide(
			PXSelectBase<CurrencyInfo> currencyinfoselect,
			ARAdjust2 adj,
			ARInvoice invoice,
			ARPayment payment,
			ARAdjust2 others = null)
		{
				...
				adj.AdjdDocType != ARDocType.CreditMemo)
			{
				Customer invoiceCustomer = PXSelect<
					...
					.Select(currencyinfoselect.Cache.Graph, adj.AdjdCustomerID);

				if (invoiceCustomer?.SmallBalanceAllow == true)
		{
					decimal invoice_smallbalancelimit;
					CurrencyInfo invoice_info = CurrencyInfoCache.GetInfo(currencyinfoselect,adj.AdjdCuryInfoID);
		...
		/// </summary>
		public static void CalcRGOLFromInvoiceSide<TInvoice, TAdjustment>(
			PXSelectBase<CurrencyInfo> currencyinfoselect, 
			TInvoice document, 
			TAdjustment adj)
			...
			where TAdjustment : class, IBqlTable, IAdjustment
		{
			...
			CurrencyInfo payment_originfo = CurrencyInfoCache.GetInfo(currencyinfoselect,document.CuryInfoID);
