...
namespace PX.Objects.AR
{
    ...
    public class ARPriceWorksheetMaint : PXGraph<ARPriceWorksheetMaint, ARPriceWorksheet>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
        ...
        #region Actions
        public PXAutoAction<ARPriceWorksheet> initializeState;
		...
		public virtual void ReleaseWorksheetImpl(ARPriceWorksheet priceWorksheet)
		{
            ...
            using (PXTransactionScope ts = new PXTransactionScope())
            {
				using (new GroupedCollectionScope<ARSalesPrice>(pricesGroups))
				{
					...
					{
						var group = CreateSalesPrice(detail, isPromotional, null, null, taxCalcMode);
        ...
        protected virtual void ProcessReleaseForNonPromotionalSalesPrices(PXResultset<ARSalesPrice> salesPrices, ARPriceWorksheetDetail priceLine)
        {
            ...
            {
                ARSalesPrice newSalesPrice = CreateSalesPrice(priceLine, isPromotional, isFairValue, isProrated, worksheetEffectiveDate, worksheetExpirationDate, taxCalcMode);
            ...
            else
            {
                ...
                ARSalesPrice newSalesPrice = CreateSalesPrice(priceLine, isPromotional, isFairValue, isProrated, worksheetEffectiveDate, newSalesPriceExpirationDate, taxCalcMode);
																							...
																							bool? isFairValue, bool? isProrated)
		{
            ...
            {               
                ...
                else if (salesPrice.EffectiveDate <= worksheetEffectiveDate && salesPrice.ExpirationDate == null && worksheetEffectiveDate.Value != null)
                {
                    ...
                    ARSalesPrice newSalesPrice = CreateSalesPrice(priceLine, isPromotional, isFairValue, isProrated, worksheetEffectiveDate, worksheetExpirationDate, Document.Current.TaxCalcMode);
                    ARSalesPrices.Insert(newSalesPrice);
                }
        ...
        protected virtual void ProcessReleaseForPromotionalSalesPrices(PXResultset<ARSalesPrice> salesPrices, ARPriceWorksheetDetail priceLine)
        {
            ...
            ARSalesPrice newSalesPrice = CreateSalesPrice(priceLine, isPromotional, worksheetEffectiveDate, worksheetExpirationDate, taxCalcMode);
            ARSalesPrices.Insert(newSalesPrice);
        }
		...
		protected virtual ARSalesPrice CreateSalesPrice(ARPriceWorksheetDetail priceLine, bool? isPromotional, DateTime? effectiveDate, DateTime? expirationDate)
			=> CreateSalesPrice(priceLine, isPromotional, effectiveDate, expirationDate, PriceTaxCalculationMode.Undefined);

        protected virtual ARSalesPrice CreateSalesPrice(ARPriceWorksheetDetail priceLine, bool? isPromotional, DateTime? effectiveDate, DateTime? expirationDate, string taxCalcMode)
            => CreateSalesPrice(priceLine, isPromotional, false, false, effectiveDate, expirationDate, taxCalcMode);
        ...
        protected virtual ARSalesPrice CreateSalesPrice(ARPriceWorksheetDetail priceLine, bool? isPromotional, bool? isFairValue, bool? isProrated, DateTime? effectiveDate, DateTime? expirationDate)
            => CreateSalesPrice(priceLine, isPromotional, isFairValue, isProrated, effectiveDate, expirationDate, PriceTaxCalculationMode.Undefined);
        
        protected virtual ARSalesPrice CreateSalesPrice(ARPriceWorksheetDetail priceLine, bool? isPromotional, bool? isFairValue, bool? isProrated, DateTime? effectiveDate, DateTime? expirationDate, string taxCalcMode)
        {
        ...
        public virtual IEnumerable CopyPrices(PXAdapter adapter)
        {
			...
			{
                CopyPricesSettings.Current.TaxCalcMode = Document.Current.TaxCalcMode;
            }
            ...
            PXResultset<ARSalesPrice> salesPrices =
                PXSelectJoinGroupBy<ARSalesPrice,
                    ...
                    Where<InventoryItem.itemStatus, NotEqual<INItemStatus.inactive>,
                        ...
                        And2<Where<ARSalesPrice.taxCalcMode, Equal<Required<ARPriceWorksheet.taxCalcMode>>,
        ...
        {
            if (CalculatePricesSettings.AskExtRequired())
            {
		...
		private void CalculatePendingPrices(CalculatePricesFilter settings)
		{
            ...
            {
                var r = (PXResult<InventoryItem, InventoryItemCurySettings, INItemCost>)
                    PXSelectJoin<InventoryItem,
		                LeftJoin<InventoryItemCurySettings,
							...
							And<InventoryItemCurySettings.curyID, Equal<Current<CalculatePricesFilter.baseCuryID>>>>,
                        LeftJoin<INItemCost, 
                ...
                InventoryItemCurySettings curyItem = r;
                ...
                KeyValuePair<bool, decimal> calcRes = CalculateCorrectedAmountForPendingPricesCalculation(settings, item, curyItem, itemCost,
                                                                                                          iis, worksheetDetail);
        ...
        {
	        bool isBaseCury = settings.BaseCuryID == CurrencyCollection.GetBaseCurrency()?.CuryID;
	        decimal? lastCost = itemSite?.LastCost ?? itemCost.LastCost;
	        decimal? avgCost = itemSite?.AvgCost ?? itemCost.AvgCost;
	        decimal? stdCost = itemSite?.StdCost ?? (isBaseCury ? item.StdCost : curyItem.StdCost);
	        decimal? recCost = itemSite?.RecPrice ?? (isBaseCury ? item.RecPrice : curyItem.RecPrice);
        ...
        protected virtual void ARPriceWorksheet_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            PXUIFieldAttribute.SetVisible(Details.Cache, "TaxCategoryID", PXAccess.FeatureInstalled<FeaturesSet.netGrossEntryMode>());
        ...
        protected virtual void ARPriceWorksheet_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
            ...
            if (doc.IsPromotional == true && doc.ExpirationDate == null)
                sender.RaiseExceptionHandling<ARPriceWorksheet.expirationDate>(doc, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(ARPriceWorksheet.expirationDate)}]"));
        }
        ...
        protected virtual void ARPriceWorksheet_EffectiveDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if (e.NewValue == null)
                throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(ARPriceWorksheet.effectiveDate)}]");
        ...
        protected virtual void ARPriceWorksheet_ExpirationDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if (doc.IsPromotional == true && e.NewValue == null)
                throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(ARPriceWorksheet.expirationDate)}]");
        ...
        protected virtual void ARPriceWorksheetDetail_PriceCode_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if (det == null) return;
            if (e.NewValue == null)
                throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(ARPriceWorksheetDetail.priceCode)}]");
        ...
        [PXMergeAttributes]
        [PXFormula(typeof(Selector<ARPriceWorksheetDetail.inventoryID, InventoryItem.taxCategoryID>))]
        public virtual void _(Events.CacheAttached<ARPriceWorksheetDetail.taxCategoryID> e) { }
		...
		protected virtual void ARPriceWorksheetDetail_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
			...
			if ((det.PriceType == PriceTypes.Customer && det.CustomerID == null) || (det.PriceType == PriceTypes.CustomerPriceClass && det.CustPriceClassID == null))
			{
                sender.RaiseExceptionHandling<ARPriceWorksheetDetail.priceCode>(
					det, 
					null, 
					new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(ARPriceWorksheetDetail.priceCode)}]"));
    ...
    public partial class AddItemFilter : INSiteStatusFilter
    {
        ...
        [PXSelector(typeof(Search<TM.EPCompanyTree.workGroupID,
            Where<TM.EPCompanyTree.workGroupID, TM.IsWorkgroupOrSubgroupOfContact<Current<AccessInfo.contactID>>>>),
	...
	public partial class ARAddItemSelected : IBqlTable
    {
        ...
        [TM.Owner(typeof(InventoryItem.priceWorkgroupID), BqlField = typeof(InventoryItem.priceManagerID), DisplayName = "Price Manager")]
        ...
        where StatusFilter : AddItemFilter, new()
    {
		...
		protected override PXView CreateIntView(PXGraph graph)
		{
			...
			view.WhereAnd<Where<Current<AddItemFilter.myWorkGroup>, Equal<boolFalse>,
						 Or<ARAddItemSelected.priceWorkgroupID, TM.IsWorkgroupOfContact<CurrentValue<AddItemFilter.currentOwnerID>>>>>();
