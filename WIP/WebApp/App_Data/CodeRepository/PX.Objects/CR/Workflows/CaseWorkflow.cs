...
namespace PX.Objects.CR.Workflows
{
	...
	public class CaseWorkflow : PX.Data.PXGraphExtension<CRCaseMaint>
	{
		...
		public override void Configure(PXScreenConfiguration config)
		{
			...
			var gactionAssign = context.ActionDefinitions.CreateExisting(g => g.assign, a => a.InFolder(FolderType.ActionsFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));
			var gactionViewInvoice = context.ActionDefinitions.CreateExisting(g => g.viewInvoice, a => a.InFolder(FolderType.InquiriesFolder).WithPersistOptions(ActionPersistOptions.ForcePersist));

            var gactionOpenFromPortal =
                context.ActionDefinitions.CreateExisting<CaseWorkflow>(g => g.openCaseFromPortal,
                    a => a.IsHiddenAlways().DisplayName("Open Case From Portal").WithPersistOptions(ActionPersistOptions.ForcePersist));

            var gactionOpenFromProcessing =
                context.ActionDefinitions.CreateExisting<CaseWorkflow>(g => g.openCaseFromProcessing,
                    a => a.IsHiddenAlways().DisplayName("Open Case From Processing").WithPersistOptions(ActionPersistOptions.ForcePersist));

            var gactionCloseFromPortal =
                context.ActionDefinitions.CreateExisting<CaseWorkflow>(g => g.closeCaseFromPortal,
                    a => a.IsHiddenAlways().DisplayName("Close Case From Portal").WithPersistOptions(ActionPersistOptions.ForcePersist));

			var gactionTakeCase = context.ActionDefinitions.CreateExisting(g => g.takeCase, a => a
				.DoesNotPersist()
				.PlaceAfter(g => g.Inquiry));
