...
namespace PX.Objects.AR
{
	...
	public class ARCreateWriteOff : PXGraph<ARCreateWriteOff>
	{
		...
		protected virtual void ARWriteOffFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetRequired<ARWriteOffFilter.orgBAccountID>(sender, true);
		}
	...
	public class ARSmallCreditWriteOffEntry : PXGraph<ARSmallCreditWriteOffEntry>, IARWriteOffEntry
	{
		...
		protected virtual void ARAdjust2_AdjdCuryInfoID_CacheAttached(PXCache sender) { }
		...
		protected virtual void ARAdjust2_AdjdOrigCuryInfoID_CacheAttached(PXCache sender) { }
		...
		protected virtual void ARAdjust2_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
					...
					And<ARPayment.refNbr, Equal<Required<ARPayment.refNbr>>>>>>.Select(this, adjust.AdjgDocType, adjust.AdjgRefNbr))
			{
				...
				InvoiceEntry.CalcBalancesFromInvoiceSide(adjust, Document.Current, payment, CurrencyInfo_CuryInfoID, false, e.ExternalCall);

				decimal? CuryApplAmt = adjust.CuryDocBal;
				decimal? CuryApplDiscAmt = 0m;

				adjust.CuryAdjdAmt = CuryApplAmt;
				adjust.CuryAdjdDiscAmt = CuryApplDiscAmt;

				InvoiceEntry.CalcBalancesFromInvoiceSide(adjust, Document.Current, payment, CurrencyInfo_CuryInfoID, true, e.ExternalCall);
			}
		}
	}
	...
	public class ARSmallBalanceWriteOffEntry : PXGraph<ARSmallBalanceWriteOffEntry>, IARWriteOffEntry
	{
		...
		public virtual void CreateWriteOff(ReasonCode reasonCode, string WOSubCD, DateTime? WODate, string masterPeriodID, ARRegister ardoc)
		{
			...
			CurrencyInfo info = CurrencyInfoAttribute.SetDefaults<ARPayment.curyInfoID>(this.Document.Cache, payment);
			...
			foreach (ARTran tran in trans)
			{
				...
				adj.WriteOffReasonCode = reasonCode.ReasonCodeID;
