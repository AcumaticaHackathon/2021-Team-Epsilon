...
namespace PX.Objects.SO
{
	...
	public class SOReleaseInvoice : PXGraph<SOReleaseInvoice>
	{
		...
		{
			PXUIFieldAttribute.SetVisible<SOInvoiceFilter.showPrinted>(e.Cache, e.Row, e.Row?.Action == WellKnownActions.SOInvoiceScreen.PrintInvoice);
		...
		protected virtual PXSelectBase<ARInvoice> GetInvoiceListSelectCommand(SOInvoiceFilter filter)
		{
			switch (filter.Action)
			{
				...
				case WellKnownActions.SOInvoiceScreen.PrintInvoice:
				{
					if (filter.ShowPrinted != true)
						return new
							SelectFrom<ARInvoice>.
							Where<ARInvoice.printInvoice.IsEqual<True>>.
							View(this);
					else
						return new
							SelectFrom<ARInvoice>.
							Where<ARInvoice.dontPrint.IsNotEqual<True>>.
							View(this);
				}
				default:
				{
		...
		protected virtual void ApplyAdditionalFilters(PXSelectBase<ARInvoice> command, SOInvoiceFilter filter)
		{
			...
			command.WhereAnd<Where<WorkflowAction.IsEnabled<ARInvoice, SOInvoiceFilter.action>>>();
