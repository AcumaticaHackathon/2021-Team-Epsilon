...
namespace PX.Objects.PO
{
	...
	public partial class POLine : PX.Data.IBqlTable, IItemPlanMaster, ISortOrder, ITaxableDetail, IMatrixItemLine, ICommitmentSource
	{
		#region Keys
		...
		{
			public class Branch : GL.Branch.PK.ForeignKeyOf<POLine>.By<branchID> { }
			...
			public class SiteStatus : IN.INSiteStatus.PK.ForeignKeyOf<POLine>.By<inventoryID, subItemID, siteID> { }
			public class Requisition : RQ.RQRequisition.PK.ForeignKeyOf<POLine>.By<rQReqNbr> { }
			public class RequisitionLine : RQ.RQRequisitionLine.PK.ForeignKeyOf<POLine>.By<rQReqNbr, rQReqLineNbr> { }
			public class ItemPlan : INItemPlan.PK.ForeignKeyOf<POLine>.By<planID> { }
			public class Vendor : AP.Vendor.PK.ForeignKeyOf<POLine>.By<vendorID> { }
			public class CurrencyInfo : CM.CurrencyInfo.PK.ForeignKeyOf<POLine>.By<curyInfoID> { }
			public class TaxCategory : TX.TaxCategory.PK.ForeignKeyOf<POLine>.By<taxCategoryID> { }
			public class Tax : TX.Tax.PK.ForeignKeyOf<POLine>.By<taxID> { }
			public class ExpenseAccount : GL.Account.PK.ForeignKeyOf<POLine>.By<expenseAcctID> { }
			public class ExpenseSubaccount : Sub.PK.ForeignKeyOf<POLine>.By<expenseSubID> { }
			public class Project : PMProject.PK.ForeignKeyOf<POLine>.By<projectID> { }
			public class Task : PMTask.PK.ForeignKeyOf<POLine>.By<taskID> { }
			public class AccrualAccount : GL.Account.PK.ForeignKeyOf<POLine>.By<pOAccrualAcctID> { }
			public class AccrualSubaccount : Sub.PK.ForeignKeyOf<POLine>.By<pOAccrualSubID> { }
			public class CostCode : PMCostCode.PK.ForeignKeyOf<POLine>.By<costCodeID> { }
			public class ReasonCode : CS.ReasonCode.PK.ForeignKeyOf<POLine>.By<reasonCode> { }
			public class Discount : APDiscount.PK.ForeignKeyOf<POLine>.By<discountID, vendorID> { }
			public class DiscountSequence : VendorDiscountSequence.PK.ForeignKeyOf<POLine>.By<vendorID, discountID, discountSequenceID> { }

			//todo public class UnitOfMeasure : INUnit.PK.ForeignKeyOf<POLine>.By<inventoryID, uOM> { }

			[Obsolete("This foreign key is obsolete and is going to be removed in 2021R1. Use Requisition instead.")]
			public class RQRequisition : Requisition { }
			[Obsolete("This foreign key is obsolete and is going to be removed in 2021R1. Use RequisitionLine instead.")]
			public class RQRequisitionLine : RequisitionLine { }
		}
		#endregion
		#region Selected
		...
		#endregion
		#region ProcessNonStockAsServiceViaPR
		public abstract class processNonStockAsServiceViaPR : PX.Data.BQL.BqlBool.Field<processNonStockAsServiceViaPR> { }
		protected Boolean? _ProcessNonStockAsServiceViaPR;
		/// <summary>
		/// When set to <c>true</c>, indicates that non-stock item will be processed as service via Purchase Receipt.
		/// </summary>
		[PXDBBool()]
		[PXDefault(typeof(Switch<Case<Where2<FeatureInstalled<FeaturesSet.pOReceiptsWithoutInventory>, And<Selector<POLine.inventoryID, InventoryItem.nonStockReceiptAsService>, Equal<True>>>, True>, False>))]
		[PXUIField(DisplayName = "Process as Service via Purchase Receipt", Enabled = false, Visible = false)]
		public virtual Boolean? ProcessNonStockAsServiceViaPR
		{
			get
			{
				return this._ProcessNonStockAsServiceViaPR;
			}
			set
			{
				this._ProcessNonStockAsServiceViaPR = value;
			}
		}
		#endregion
		#region AccrueCost
		...
		[PXDefault(typeof(Coalesce<
				...
				Search<LocationBranchSettings.vSiteID,
					Where<LocationBranchSettings.locationID, Equal<Current<POOrder.vendorLocationID>>,
        ...
        [PXDefault(TypeCode.Decimal, "0.0")]
		[PXUIVerify(typeof(Where<discPct, GreaterEqual<decimalMinus100>, And<discPct, LessEqual<decimal100>>>), PXErrorLevel.Error, AR.Messages.DiscountPercentShouldBeBetweenMinus100And100, CheckOnRowSelected = true)]
		...
		[PXDefault(TypeCode.Decimal, "0.0")]
		[PXUIVerify(typeof(Where<curyExtCost, GreaterEqual<decimal0>, Or<Not<POLineType.Goods.Contains<lineType>>>>), PXErrorLevel.Error, CS.Messages.FieldShouldNotBeNegative, typeof(curyExtCost), MessageArgumentsAreFieldNames = true, CheckOnRowSelected = true)]
		...
		[PXDBDate]
		[PXDefault(PersistingCheck = PXPersistingCheck.Nothing)]
		[PXUIField(DisplayName = "Term Start Date", Visibility = PXUIVisibility.SelectorVisible, Visible = false)]
		...
		[PXDBDate]
		[PXDefault(PersistingCheck = PXPersistingCheck.Nothing)]
		[PXUIField(DisplayName = "Term End Date", Visibility = PXUIVisibility.SelectorVisible, Visible = false)]
		...
		/// </summary>
		/// <value>
		/// The value of this field is set by the <see cref="POOrderEntry"/> graph based on the settings of the
		/// <see cref="InventoryID">item</see> selected for the line. In other contexts it is not populated.
		/// See the attribute on the <see cref="POOrderEntry.POLine_ItemRequiresTerms_CacheAttached"/> handler for details.
		/// </value>
		[PXBool]
		...
		#region SOOrderStatus
		public abstract class sOOrderStatus : PX.Data.BQL.BqlString.Field<sOOrderStatus> { }

		[PXUIField(DisplayName = "Sales Order Status", Visibility = PXUIVisibility.Visible, Enabled = false, FieldClass = nameof(FeaturesSet.DropShipments))]
		[SO.SOOrderStatus.List]
		[PXString(1, IsFixed = true)]
		public virtual String SOOrderStatus
		{
			get;
			set;
		}
		...
		[PXUIField(DisplayName = "Sales Order Nbr.", Enabled = false, FieldClass = nameof(FeaturesSet.DropShipments))]
		[PXSelector(typeof(Search<SO.SOOrder.orderNbr, Where<SO.SOOrder.orderType, Equal<Current<POLine.sOOrderType>>>>))]
		...
		[PXUIField(DisplayName = "Sales Order Line Nbr.", Enabled = false, FieldClass = nameof(FeaturesSet.DropShipments))]
		[PXInt]
		...
		[PXBool]
		[PXUIField(DisplayName = "SO Linked", Visibility = PXUIVisibility.Visible, FieldClass = nameof(FeaturesSet.DropShipments))]
		...
		[PXFormula(typeof(Switch<
			...
			Switch<Case<Where2<Where2<Not<FeatureInstalled<FeaturesSet.pOReceiptsWithoutInventory>>, And<Where<Current<POSetup.addServicesFromNormalPOtoPR>, Equal<True>, And<Current<POLine.orderType>, Equal<POOrderType.regularOrder>,
				Or<Current<POSetup.addServicesFromDSPOtoPR>, Equal<True>, And<Current<POLine.orderType>, Equal<POOrderType.dropShip>>>>>>>, Or<Current<POLine.processNonStockAsServiceViaPR>, Equal<True>>>, POAccrualType.receipt>, POAccrualType.order>>))]
		[POAccrualType.List]
		...
		#region HasInclusiveTaxes
		public abstract class hasInclusiveTaxes : PX.Data.BQL.BqlBool.Field<hasInclusiveTaxes> { }
		protected Boolean? _HasInclusiveTaxes;
		[PXDBBool()]
		[PXDefault(false)]
		public virtual Boolean? HasInclusiveTaxes
		{
			get
			{
				return this._HasInclusiveTaxes;
			}
			set
			{
				this._HasInclusiveTaxes = value;
			}
		}
		#endregion

		#region AllowEditUnitCostInPR
		public abstract class allowEditUnitCostInPR : PX.Data.BQL.BqlBool.Field<allowEditUnitCostInPR> { }
		protected Boolean? _AllowEditUnitCostInPR;
		[PXDBBool()]
		[PXUIField(DisplayName = "Editable Unit Cost in Receipt", Visible = false, Enabled = false)]
		[PXDefault(false)]
		public virtual Boolean? AllowEditUnitCostInPR
		{
			get
			{
				return this._AllowEditUnitCostInPR;
			}
			set
			{
				this._AllowEditUnitCostInPR = value;
			}
		}
		#endregion

		#region ViewDemandEnabled
		public abstract class viewDemandEnabled : PX.Data.BQL.BqlBool.Field<viewDemandEnabled> { }
		[PXBool]
		[PXFormula(typeof(IIf<Where<lineType, In3<
			...
			POLineType.goodsForReplenishment>>, True, False>))]
		public virtual Boolean? ViewDemandEnabled
		{
			get;
			set;
		}
