...
namespace PX.Objects.DR
{
	...
	public class DraftScheduleMaint : PXGraph<DraftScheduleMaint, DRSchedule>
	{
		...
		//unnecessary LeftJoin<DRSchedule... was added to avoid row duplication due to
		//a MergeCache bug: MergeCache doesn't merges items for PXSelect
		//but it works for IBqlJoinedSelect
		public PXSelectJoin<
			DRScheduleDetail,
			LeftJoin<DRSchedule,
		...
		protected int? FillBranch(DRScheduleDetail scheduleDetail)
		{
			...
			if (Schedule.Current == null) return scheduleDetail.BranchID;
		...
		protected virtual void DRSchedule_LineNbr_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			foreach (PXResult<DRScheduleDetail, DRSchedule> res in Components.Select())
			{
				DRScheduleDetail component = (DRScheduleDetail)res;
				component.BranchID = FillBranch(component);
		...
		protected virtual void DRSchedule_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			{
				Components.Cache.RaiseExceptionHandling<DRScheduleDetail.documentType>(Components.Current, null, new PXSetPropertyException(Data.ErrorMessages.FieldIsEmpty, $"[{nameof(DRScheduleDetail.documentType)}]"));
			}

			if (string.IsNullOrEmpty(row.FinPeriodID))
			{
				Components.Cache.RaiseExceptionHandling<DRScheduleDetail.finPeriodID>(Components.Current, null, new PXSetPropertyException(Data.ErrorMessages.FieldIsEmpty, $"[{nameof(DRScheduleDetail.finPeriodID)}]"));
			}
		...
		private void SetScheduleStatus(DRSchedule schedule)
		{
			...
			else
			{
				...
				foreach (PXResult<DRScheduleDetail, DRSchedule> res in this.Components.Select())
				{
		...
		protected virtual void DRSchedule_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (!sender.ObjectsEqual<DRSchedule.documentType, DRSchedule.refNbr, DRSchedule.lineNbr, DRSchedule.bAccountID, DRSchedule.finPeriodID, DRSchedule.docDate>(e.Row, e.OldRow))
			{
				...
				{
					DRScheduleDetail detail = (DRScheduleDetail)res;
					DRScheduleDetail copy = (DRScheduleDetail)Caches[typeof(DRScheduleDetail)].CreateCopy(detail);
					SynchronizeDetailPropertiesFromSchedule(row, copy);
		...
		{
			DRScheduleDetail scheduleDetail = e.Row as DRScheduleDetail;
			e.NewValue = FillBranch(scheduleDetail);
			e.Cancel = true;
		}
		...
		private bool AllComponentsHaveTransactions()
		{
			...
			foreach (PXResult<DRScheduleDetail, DRSchedule> res in Components.Select())
			{
				DRScheduleDetail scheduleDetail = (DRScheduleDetail)res;
				if (!Transactions.Any(
			...
			if (isAnyComponentFlexible && !deferralSchedule.TermStartDate.HasValue)
			{
				cache.RaiseExceptionHandling<DRSchedule.termStartDate>(
					...
					new PXSetPropertyException<DRSchedule.termStartDate>(
						...
						PXErrorLevel.Error,
                        $"[{nameof(DRSchedule.termStartDate)}]"));
			}
