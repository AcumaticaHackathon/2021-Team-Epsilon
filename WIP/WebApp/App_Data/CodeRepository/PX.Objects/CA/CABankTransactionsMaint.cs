...
namespace PX.Objects.CA
{
    public partial class CABankTransactionsMaint : PXGraph<CABankTransactionsMaint>
	{
		...
		private void EnableCreateTab(PXCache sender, CABankTran row, bool needsPMInstance)
		{
			...
			bool noAdjustmentsYet = Adjustments.SelectSingle() == null;
		...
		protected virtual List<CABankTran> DoCAMatchInner(PXCache aUpdateCache, IEnumerable<CABankTran> aRows)
		{
			...
			int rowCount = 0;
			decimal paymentMatchTreshold = (CurrentMatchSesstings.MatchThreshold ?? 0m) / 100m;
			decimal paymentRelevanceTreshold = (CurrentMatchSesstings.RelativeMatchThreshold ?? 0m) / 100m;
			...
			foreach (KeyValuePair<string, List<CABankTranDocRef>> iCandidate in cross)
			{
				...
				if (bestMatch != null && bestMatch.TranID != null)
				{
					...
					if (detail != null && TranMatch.Select(detail.TranID).Count == 0)
					{
						...
						TranMatch.Insert(match);
						detail.CreateDocument = false;
						detail = aUpdateCache.Update(detail) as CABankTran;
		...
		{
			decimal relevanceTreshold = skipLowRelevance ? aSettings.RelativeMatchThreshold.Value : Decimal.Zero;
	    ...
	    {
	        decimal relevanceTreshold = skipLowRelevance ? settings.RelativeMatchThreshold.Value : Decimal.Zero;
        ...
        protected virtual IEnumerable FindDetailMatchingInvoices(CABankTran aDetail, IMatchSettings aSettings, decimal aRelevanceTreshold, CABankTranInvoiceMatch[] aBestMatches)
		{
				...
				amount > 0)
			{
				...
				PXSelectBase<Light.ARInvoice> sel = CreateARInvoiceQuery(aDetail, aSettings, amount, out bqlParams);
			...
			if (aDetail.DrCr == CA.CADrCr.CACredit && !clearingAccount)
			{
				...
				PXSelectBase<Light.APInvoice> sel = CreateAPInvoiceQuery(aDetail, aSettings, amount, out bqlParams);
		...
		protected virtual PXSelectBase<Light.ARInvoice> CreateARInvoiceQuery(CABankTran aDetail, IMatchSettings aSettings, decimal? tranAmount, out List<object> bqlParams)
		{
			bqlParams = new List<object>() { aDetail.TranType, aDetail.CuryID };
			...
			{
				CashAccount cashAccount = CashAccount.PK.Find(this, aDetail.CashAccountID);
				sel.WhereAnd<
				...
				bqlParams.Add(cashAccount.BranchID);
			}

			return sel;
		}

		protected virtual PXSelectBase<Light.APInvoice> CreateAPInvoiceQuery(CABankTran aDetail, IMatchSettings aSettings, decimal? tranAmount, out List<object> bqlParams)
		{
			bqlParams = new List<object>() { aDetail.TranType, aDetail.CuryID, aDetail.TranDate, aDetail.TranPeriodID };
			...
			foreach (CABankTran row in aRows)
			{
				...
				if (CheckRuleMatches(row, rule))
				{
					...
					try
					{
						...
						ApplyRule(transaction, rule);
						Details.Cache.SetValueExt<CABankTran.documentMatched>(transaction, transaction.CreateDocument == true && ValidateTranFields(aUpdateCache, transaction));
