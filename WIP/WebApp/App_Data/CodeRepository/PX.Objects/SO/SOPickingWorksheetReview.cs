...
namespace PX.Objects.SO
{
		...
		private void PrintPickListImpl(PXAdapter adapter)
		{
			...
			if (report != null)
			{
				if (PXAccess.FeatureInstalled<CS.FeaturesSet.deviceHub>())
						{
					var printerSettings = PX.SM.SMPrintJobMaint.GetPrintSettings(adapter, new CR.NotificationUtility(this).SearchPrinter, SONotificationSource.Customer, SOReports.PrintPickerPickList, Accessinfo.BranchID);
	...
	public class SOPickingWorksheetPickListConfirmation : PXGraphExtension<SOPickingWorksheetReview>
	{
		...
		public virtual void ConfirmPickList(SOPicker pickList, int? sortingLocationID)
		{
			...
			var pickListEnterableItems = entries
				...
				.Where(item => item.StkItem == true && InventoryItem.FK.LotSerialClass.FindParent(Base, item).IsUnassigned)
	...
	public class SOShipmentEntryUnlinkWorksheetExt : PXGraphExtension<SOShipmentEntry>
	{
		...
		protected virtual void unlinkFromWorksheet()
		{
			...
			var worksheet = SOPickingWorksheet.PK.Find(Base, Base.Document.Current.CurrentWorksheetNbr);
			...
			if (unlinkable)
			{
				...
				unlinkable &=
				  ...
				  Where<
					...
					View.Select(Base, e.Row.ShipmentNbr).AsEnumerable().Any() == false;
			}
