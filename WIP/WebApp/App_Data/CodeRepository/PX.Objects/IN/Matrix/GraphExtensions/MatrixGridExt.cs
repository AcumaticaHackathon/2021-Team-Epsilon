...
namespace PX.Objects.IN.Matrix.GraphExtensions
{
			...
			where MainItemType : class, IBqlTable, new()
	{
		...
		protected virtual List<PXResult<CSAnswers, InventoryItem>> SelectInventoryWithAttributes()
		{
			...
			{
				return select.Select(GetTemplateID())
					.AsEnumerable()
					.Cast<PXResult<CSAnswers, InventoryItem>>()
					.ToList();
			}
		}
		...
		protected virtual InventoryMapValue CreateInventoryMapValue(int? inventoryID, PXResult<CSAnswers, InventoryItem> result)
		...
		protected virtual MatrixAttributeValues GetMatrixAttributeValues()
		{
			...
			var colAttributesSelect = new PXSelect<CSAttributeDetail,
			...
			var rowAttributesSelect = new PXSelect<CSAttributeDetail,
