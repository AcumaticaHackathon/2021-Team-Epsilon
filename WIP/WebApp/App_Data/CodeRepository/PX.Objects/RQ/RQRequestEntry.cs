...
namespace PX.Objects.RQ
{
	public class RQRequestEntry : PXGraph<RQRequestEntry, RQRequest>, IGraphWithInitialization
	{
		...
		*/
		...
		public virtual IEnumerable budget() => GetBudget();
		...
		#region Actions
		public PXAutoAction<RQRequest> initializeState;
		...
		protected virtual IEnumerable ReleaseFromHold(PXAdapter adapter)
		{
			foreach (RQRequest order in adapter.Get<RQRequest>())
			{
				...
				if (order.Hold != true && order.Approved != true)
				{
					...
					{
						var processor = CreateInstance<EPAssignmentProcessor<RQRequest>>();
		...
		[PXUIField(DisplayName = "Request Form", MapEnableRights = PXCacheRights.Select)]
		[PXButton(CommitChanges = true)]
		protected virtual IEnumerable RequestForm(PXAdapter adapter) => Report(adapter, "RQ641000");

		public PXAction<RQRequest> ViewDetails;
		[PXButton(ImageKey = PX.Web.UI.Sprite.Main.DataEntry)]
		[PXUIField(DisplayName = "Requisition Details", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		[PXLookupButton]
		[PXUIField(DisplayName = "View Requisition", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
			...
			foreach (RQRequestLine line in this.Lines.Select())
			{
				...
				if (item == null)
				{
					...
					.SelectWindowed(this, 0, 1,
						...
						line.ExpenseSubID,
						budgetPeriod);
					if (gs != null && (account.Type == AccountType.Asset || account.Type == AccountType.Liability ||
						FinPeriodUtils.FiscalYear(gs.FinPeriodID) == FinPeriodUtils.FiscalYear(budgetPeriod)))
					{
		...
		protected virtual void RQRequest_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (reqClassCur != null)
			{
				...
				// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Justification: virtual fields]
				(row.VendorHidden, row.CustomerRequest, row.BudgetValidation) =
					(reqClassCur.VendorNotRequest == true, reqClassCur.CustomerRequest == true, reqClassCur.BudgetValidation > 0);
		...
		protected virtual void RQRequest_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (!sender.ObjectsEqual<RQRequest.openOrderQty>(e.Row, e.OldRow))
				RQRequest.Events.Select(ev => ev.OpenOrderQtyChanged).FireOn(this, row);
		}
		...
		/// <exclude/>
		public class RQRequestEntryAddressLookupExtension : CR.Extensions.AddressLookupExtension<RQRequestEntry, RQRequest, POShipAddress>
		{
			protected override string AddressView => nameof(Base.Shipping_Address);
		}
		#endregion

		#region AffectedInRequisition
		public class AffectedInRequisition : ProcessAffectedEntitiesInPrimaryGraphBase<AffectedInRequisition, RQRequisitionEntry, RQRequest, RQRequestEntry>
		{
			protected override bool PersistInSameTransaction => true;
			protected override bool EntityIsAffected(RQRequest entity) => WhenAnyFieldIsAffected(entity, e => e.OpenOrderQty);
			protected override void ProcessAffectedEntity(RQRequestEntry primaryGraph, RQRequest entity) => RQRequest.Events.Select(ev => ev.OpenOrderQtyChanged).FireOn(primaryGraph, entity);
		}
		#endregion
	}
