...
namespace PX.Objects.FS
{
        ...
        And<FSSODetSplit.refNbr, Equal<Current<FSServiceOrder.refNbr>>>>>
    {
        ...
        public bool IsQuoteServiceOrder
        {
            get
            {
                FSSrvOrdType srvOrdtype = PXSetup<FSSrvOrdType>.Select(this._Graph);
            ...
            : base(graph)
        {
            ...
            FSSrvOrdType srvOrdTypeRow = ((ServiceOrderEntry)graph).ServiceOrderTypeSelected?.Current;

            if (allocationAction != null)
            {
                allocationAction.SetVisible(PXAccess.FeatureInstalled<FeaturesSet.inventory>() 
                                                && srvOrdTypeRow?.Behavior !=  FSSrvOrdType.behavior.Values.Quote);
            }
        }
        #endregion
        ...
        public override IEnumerable BinLotSerial(PXAdapter adapter)
        {
            ...
            FSSODet detail = (FSSODet)MasterCache.Current;
            ...
            if (IsLSEntryEnabled || IsAllocationEntryEnabled)
            {
                ...
                /*
                if (detail.SOLineType != SOLineType.Inventory)
                {
        ...
        {
            return base.ReadInventoryItem(sender, inventoryID);
        }
        ...
        public virtual IQtyAllocated AvailabilityFetchImpl(PXCache sender, ILSMaster row, AvailabilityFetchMode fetchMode)
        {
            if (row != null)
            {
                ...
                if (copy == null)
                {
                    ...
                    if (fetchMode.HasFlag(AvailabilityFetchMode.TryOptimize) && _detailsRequested++ == 50)
                        {
                            ...
                            InnerJoin<INSiteStatus, On<
                                FSSODet.inventoryID, Equal<INSiteStatus.inventoryID>,
        ...
        public override IQtyAllocated AvailabilityFetch(PXCache sender, ILSDetail row, AvailabilityFetchMode fetchMode)
        {
            int? locationID = row.LocationID;
            IQtyAllocated result = null;

            try
            {
                row.LocationID = null;
                result = base.AvailabilityFetch(sender, row, fetchMode);
            }
            finally
            {
            ...
            if (lineQtyAvail == null || lineQtyHardAvail == null)
            {
                ...
                foreach (FSSODetSplit split in SelectDetail(DetailCache, fsSODetRow))
                {
                    ...
                    if (detail.PlanID != null)
                    {
                        ...
                        if (plan != null)
                        {
                            detail = PXCache<FSSODetSplit>.CreateCopy(detail);
                            detail.PlanType = plan.PlanType;
                        }
                    }

                    PXParentAttribute.SetParent(DetailCache, detail, typeof(FSSODet), fsSODetRow);
        ...
        {
            var fetchMode = ((FSSODet)e.Row)?.Completed == true
                ? AvailabilityFetchMode.None
                : AvailabilityFetchMode.ExcludeCurrent;
            IQtyAllocated availability = AvailabilityFetch(sender, (FSSODet)e.Row, fetchMode | AvailabilityFetchMode.TryOptimize);
        ...
        protected override void _Master_RowInserted(PXCache sender, PXRowInsertedEventArgs<FSSODet> e)
        {
            ...
            if (iLSMasterRow.InventoryID == null || row.IsPrepaid == true || IsQuoteServiceOrder)
            {
                return;
            }
        ...
        protected override void _Master_RowUpdated(PXCache sender, PXRowUpdatedEventArgs<FSSODet> e)
        {
            ...
            if (row == null || (oldRow.InventoryID == null && row.InventoryID == null) || row.IsPrepaid == true || IsQuoteServiceOrder) return;
            ...
            if (INLotSerialNbrAttribute.IsTrackSerial(item, split.TranType, split.InvtMult) && split.LotSerialNbr != null)
            {
                ...
                else
                {
                    ...
                    if (split.Operation != SOOperation.Receipt)
                    {
                        ...
                        bool shouldThrowExceptions = ShouldThrowExceptions(sender.Graph);

                        if (extCall || shouldThrowExceptions == true)
                        {
                            split.IsAllocated = false;
        ...
        public virtual bool ShouldThrowExceptions(PXGraph graph)
        {
            if (graph is ServiceOrderEntry)
            {
                var serviceOrderEntry = (ServiceOrderEntry)graph;

                if (serviceOrderEntry.GraphAppointmentEntryCaller != null)
                {
                    return true;
                }
            }
