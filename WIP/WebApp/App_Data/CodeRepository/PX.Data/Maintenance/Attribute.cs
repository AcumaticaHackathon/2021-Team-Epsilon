...
namespace PX.Data
{
	...
	public abstract class PXPrimaryGraphBaseAttribute : Attribute
	{
		...
		/// </summary>
		public virtual Type[] VerifyRightsBy { get; set; }
	...
	public class PXPrimaryGraphAttribute : PXPrimaryGraphBaseAttribute
	{
		...
		protected internal static Dictionary<Type, List<KeyValuePair<Type, PXPrimaryGraphAttribute>>> GraphDefined
		{
			get
			{
		...
		public override IEnumerable<Type> GetAllGraphTypes()
		{
			if (_TheOnlyGraph != null)
			{
				return new[] {_TheOnlyGraph};
			}
			if (_GraphTypes == null || _GraphTypes.Length == 0 || _Conditions == null)
			{
				return Enumerable.Empty<Type>();
			}
			return _GraphTypes;
	...
	public class PXImportAttribute : PXViewExtensionAttribute
	{
			...
			where TBatchTable : class, IBqlTable, new()
		{
				...
				IDictionary<string, StateInfo> statesCache, out KeyValuePair<IDictionary, IDictionary> result)
			{
				...
				var nullValueStr = importSettingsCurrent == null ? null : importSettingsCurrent.NullValue ?? String.Empty;
				...
				foreach (PXImportColumnsSettings property in _importColumnsSettings.SelectMulti(_viewName))
				{
					...
					string contentValue = contentReader.GetValue((int)property.ColumnIndex).Trim();
					...
					if (contentValue != null)
					{
						...
						if (!cache.Fields.Contains(realPropertyName)
						...
						{
							var state = cache.GetStateExt(null, realPropertyName) as PXFieldState;
	...
	{
		public PXGroupMaskAttribute(Type groupRestriction)
		{
			JoinRestriction = groupRestriction;
		}
		public virtual Type JoinRestriction { get; set; }
		public virtual Type WhereRestriction { get; set; }
	}
	#endregion
}
