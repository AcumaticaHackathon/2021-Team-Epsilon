...
namespace PX.Objects.AM
{
    ...
    public class ProductionBomCopy : ProductionBomCopyBase
    {
        ...
        internal AMProdOper FindCachedOperByCD(string orderType, string prodOrdID, string operationCD)
        {
            return ProcessingGraph.Caches<AMProdOper>()?.Cached.RowCast<AMProdOper>()
                ?.Where(r => r.OrderType == orderType && r.ProdOrdID == r.ProdOrdID && r.OperationCD == operationCD)
                ?.First();
        }
