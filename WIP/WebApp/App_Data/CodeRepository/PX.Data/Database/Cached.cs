...
namespace PX.Data
{
	...
	public class PXView
	{
		...
		internal Guid? GetExternalFilterID()
		{
				...
				else if (sorts[i].Description != null && sorts[i].Description.Expr != null)
				{
					...
					if (_Select is IBqlAggregate && !String.IsNullOrEmpty(fieldName))
					{
						if (_Selection == null)
						{
							...
							SQLTree.Query q=_Select.GetQueryInternal(_Graph, new BqlCommandInfo(false) { Tables = new List<Type>() }, _Selection);
						}
		...
		internal static bool prepareFilters(PXGraph graph, PXCache originalCache, BqlCommand bqlCommand, ref PXFilterRow[] filters, string forceView = null)
		{
			...
			if (filters != null)
			{
				...
				foreach (var f in filters)
				{
					...
					if (state != null)
					{
						...
						if (dateState != null)
						{
							...
							{
								DateTime? dtValue = DateTime.TryParse(f.Value as string, cache.Graph.Culture, DateTimeStyles.None, out var val) ? val : f.Value as DateTime?;
								DateTime? dtValue2 = DateTime.TryParse(f.Value2 as string, cache.Graph.Culture, DateTimeStyles.None, out val) ? val : f.Value2 as DateTime?;
								if (dtValue != null && dtValue.Value.TimeOfDay == TimeSpan.Zero)
								{
									switch (f.Condition)
									{
										case PXCondition.EQ:
											...
											var eqNewFilterGe = (PXFilterRow) f.Clone();
											var eqNewFilterLt = (PXFilterRow) f.Clone();
											eqNewFilterGe.OrOperator = false;
											eqNewFilterGe.Condition = PXCondition.GE;
											eqNewFilterGe.CloseBrackets = 0;
											eqNewFilterLt.Condition = PXCondition.LT;
											eqNewFilterLt.OpenBrackets = 0;
											eqNewFilterLt.Value = dtValue.Value.AddDays(1);
											filtersList.Remove(f);
											filtersList.Insert(ePos, eqNewFilterLt);
											filtersList.Insert(ePos, eqNewFilterGe);
											filtersChanged = true;
											break;
										case PXCondition.BETWEEN:
											if (dtValue2 != null && dtValue2.Value.TimeOfDay == TimeSpan.Zero)
											{
												...
												var bNewFilterGe = (PXFilterRow) f.Clone();
												var bNewFilterLt = (PXFilterRow) f.Clone();
												bNewFilterGe.OrOperator = false;
												bNewFilterGe.Condition = PXCondition.GE;
												bNewFilterGe.CloseBrackets = 0;
												bNewFilterGe.Value2 = null;
												bNewFilterLt.Condition = PXCondition.LT;
												bNewFilterLt.OpenBrackets = 0;
												bNewFilterLt.Value = dtValue2.Value.AddDays(1);
												bNewFilterLt.Value2 = null;
												filtersList.Remove(f);
												filtersList.Insert(bPos, bNewFilterLt);
												filtersList.Insert(bPos, bNewFilterGe);
												filtersChanged = true;
											}
											break;
										case PXCondition.NE:
											...
											var neNewFilterGe = (PXFilterRow) f.Clone();
											neNewFilterLt.OrOperator = false;
											neNewFilterLt.Condition = PXCondition.LT;
											neNewFilterLt.CloseBrackets = 0;
											neNewFilterGe.Condition = PXCondition.GE;
											neNewFilterGe.OpenBrackets = 0;
											neNewFilterGe.Value = dtValue.Value.AddDays(1);
											filtersList.Remove(f);
											filtersList.Insert(nePos, neNewFilterGe);
				...
				foreach (PXFilterRow fr in filters)
				{
					...
					{
						if (state != null && state.DataType == typeof(bool))
						{
							fr.Condition = PXCondition.NE;
							fr.OrigValue = fr.Value = true;
						}
					...
					if (fr.Variable != null)
					{
						...
						state = cache.GetStateExt(null, fieldName) as PXFieldState;
					}
					...
					switch (fr.Condition)
					{
						...
						case PXCondition.NE:
							...
							if (fr.Variable == FilterVariableType.CurrentUser && !corrected)
							{
								...
								if (fdescr == null || fdescr.Expr == null || (fdescr.DataType != PXDbType.UniqueIdentifier && fdescr.DataType != PXDbType.Int))
									continue;
								fr.Value = fdescr.DataType == PXDbType.UniqueIdentifier
									? (object)PXAccess.GetUserID()
									: (object)PXAccess.GetContactID();
								...
								if (state != null && state.DataType != fr.Value.GetType())
								{
									...
									fr.Value = value ?? (fdescr.DataType == PXDbType.UniqueIdentifier
										? (object)PXAccess.GetUserID()
										: (object)PXAccess.GetContactID());
								}
							}
							if (fr.Value != null)
							{
								...
								try
								{
									if (fr.Value is string str)
									{
										object val;
										var strState = state as PXStringState;
										if (strState != null && !String.IsNullOrEmpty(strState.InputMask) && PX.Common.Mask.IsMasked(str, strState.InputMask, true))
										{
											val = PX.Common.Mask.Parse(strState.InputMask, str);
										}
										else
										{
											val = str;
										...
										{
											if (fr.Tag != null && string.Equals(fr.Tag.ToString(), PX.SM.Messages.Export, StringComparison.InvariantCultureIgnoreCase) 
													|| string.Equals(val.ToString(), str, StringComparison.CurrentCultureIgnoreCase) || val is DateTime)
												fr.Value = val;
											else
											{
												operation |= PXDBOperation.External;
						...
						case PXCondition.RLIKE:
							if (fr.Value is string s)
							{
								try
								{
									object val = s;
									var strState = state as PXStringState;
									if (strState != null && !String.IsNullOrEmpty(strState.InputMask))
									{
										if (PX.Common.Mask.IsMasked(s, strState.InputMask, true))
										val = PX.Common.Mask.Parse(strState.InputMask, s);
									else
									{
											// If user filters by masked values, we need to parse them by part of the mask
											// because values in DB are stored without masks applied
											for (int i = 0; i < strState.InputMask.Length - s.Length + 1; i++)
											{
												string mask = strState.InputMask.Substring(i, s.Length);
												if (PX.Common.Mask.IsMasked(s, mask, true))
												{
													val = PX.Common.Mask.Parse(mask, s);
													break;
												}
											}
										}
						...
						case PXCondition.ISNOTNULL:
							try
							{
								...
								{
									if (graph.IsContractBasedAPI) return false;
		...
		private BqlCommand appendSearches(PXSearchColumn[] sorts, List<PXDataValue> pars, BqlCommand cmd, ref int topCount, bool reverseOrder, out PXDataValue firstsearch)
		{
			firstsearch = null;
			...
			)
		{
			...
			cmd = appendSearches(sorts, pars, cmd, ref topCount, reverseOrder, out PXDataValue firstsearch);

			//if we have a select like PXSelect<SOOrder, Where<SOOrder.orderType, Equal<Optional<SOOrder.orderType>>>>
			//and search this query for a particular record passing order type and order number in searches
			//and order type in searches does not meet order type in parameters
			//then we do not want to pass the query to sql for performance reasons
			if (firstsearch != null && Cache.Keys.Count > 1 && sorts.Length > 1 && String.Equals(Cache.Keys[0], sorts[0].Column, StringComparison.OrdinalIgnoreCase)
				&& sorts[0].SearchValue != null
				&& cmdpars.Length > 0 && !cmdpars[0].NullAllowed && String.Equals(Cache.Keys[0], cmdpars[0].GetReferencedType().Name, StringComparison.OrdinalIgnoreCase)
				&& _Select is IHasBqlWhere haswhere)
			{
				Type twhere = haswhere.GetWhere().GetType();
				if (twhere.IsGenericType)
				{
					Type[] awhere = twhere.GetGenericArguments();
					if (awhere.Length > 1 && String.Equals(Cache.Keys[0], awhere[0].Name, StringComparison.OrdinalIgnoreCase)
						&& awhere[1].IsGenericType && awhere[1].GetGenericTypeDefinition() == typeof(Equal<>))
					{
						if (pars[0].Value is string s1)
						{
							if (firstsearch.Value is string s2
								&& PXLocalesProvider.CollationComparer.Compare(s1.Trim(), s2.Trim()) != 0)
							{
								return ret;
							}
						}
						else if (!(firstsearch.Value is string)
							&& !object.Equals(pars[0].Value, firstsearch.Value))
						{
							return ret;
						}
					}
			...
			)
		{
			...
			if (_Select is IBqlJoinedSelect || !(list is VersionedList) || ((VersionedList)list).Version != version)
			{
				...
				if (!(fullKeyReferenced == true && list.Count == 1 && list[0] != null && object.ReferenceEquals(Cache.Locate(list[0]), list[0])))
				{
					...
					{
						bool meet = _Select.Meet(Cache, item, parameters);
						if (meet && emulation)
						{
							if (query == null)
							{
							...
							meet = meet && Cache._InvokeMeetVerifier(item, query, parameters);
						}
						if (meet)
						{
			...
			if (Cache.BqlSelect != null || Cache.HasChangedKeys())
			{
			...
			for (int i = 0; i < filters.Length; i++)
			{
				...
				if (fr.UseExt)
				{
					getValue = list[0] is PXResult
						...
						{
							object value =
								PXFieldState.UnwrapValue(cache.GetValueExt(((PXResult)item)[t], dataField));
				...
				var state = GetStateFromCorrectCache(fr.DataField) as PXFieldState;
				if (state is PXStringState strState && strState.MultiSelect && fr.Value != null)
					fr.Value = fr.Value.ToString().Trim(new[] { ',' });

				Predicate<object> cmp = null;
				Predicate<object> isNullAsEmpty = item => getValue(item) is string s && string.IsNullOrWhiteSpace(s) && !Graph.IsContractBasedAPI;
				switch (fr.Condition)
				{
					case PXCondition.IN:
					case PXCondition.NI:
					...
					case PXCondition.EQ:
						if (fr.Value != null)
						{
							if (fr.Value is string)
							{
								...
								cmp = delegate(object item)
								{
									string val = (getValue(item) as string)?.Trim();
					...
					case PXCondition.LLIKE:
					case PXCondition.RLIKE:
						if (fr.Value is int iv) fr.Value = iv.ToString();
						if (fr.Value is string)
						{
					...
					case PXCondition.ISNULL:
						cmp = item => getValue(item) == null || isNullAsEmpty(item);
						break;
					case PXCondition.ISNOTNULL:
						cmp = item => !isNullAsEmpty(item) && getValue(item) != null;
						break;
					case PXCondition.OVERDUE:
					case PXCondition.TOMMOROW:
					case PXCondition.THIS_WEEK:
					...
					case PXCondition.THIS_MONTH:
					case PXCondition.BETWEEN:
						Predicate<object> leftCondition = null;
						Predicate<object> rightCondition = null;

						if (fr.Value is string startFilterValueString)
							{
							startFilterValueString = startFilterValueString.Trim();
							leftCondition = value =>
								{
								string valueString = (value as string)?.Trim();
								return PXLocalesProvider.CollationComparer.Compare(startFilterValueString, valueString) <= 0;
							};
						}
						else if (fr.Value is IComparable startFilterValue)
							leftCondition = value => startFilterValue.CompareTo(value) <= 0;

						if (fr.Value2 is string endFilterValueString)
						{
							endFilterValueString = endFilterValueString.Trim();
							rightCondition = value =>
							{
								string valueString = (value as string)?.Trim();
								return PXLocalesProvider.CollationComparer.Compare(endFilterValueString, valueString) >= 0;
							};
						}
						else if (fr.Value2 is IComparable endFilterValue)
							rightCondition = value => endFilterValue.CompareTo(value) >= 0;
			...
			else
			{
				...
				if (list == null)
				{
					...
					if (!PXDatabase.ReadDeleted)
					{
						...
						if (isFromCache && perf != null && PXPerformanceMonitor.SqlProfilerEnabled && PXPerformanceMonitor.SqlProfilerIncludeQueryCache)
						{
							var cacheSample = perf.AddSqlSample(queryKey.CommandText ?? this.ToString(), queryKey.ToString(), true);
							if (cacheSample != null)
								cacheSample.RowCount = list.Count;
						}
					}
					if (list == null)
					{
                        topCount = AdjustTopCountByChangedRecords(topCount, searches, sortcolumns);
				...
				else if (list.Count > 0 && !(list[0] is PXResult) && _AlwaysReturnPXResult)
				{
					for (int i = 0; i < list.Count; i++)
					{
						list[i] = CreateResult(new object[] { list[i] });
					}
				}
        ...
        {
            if (searches!=null&&sortcolumns!=null&&Cache.Keys.All(c => sortcolumns.Contains(c, StringComparer.OrdinalIgnoreCase)) && searches.All(c => c != null)) return topCount;
		...
		{
			if ((tables.Length > 1 || _AlwaysReturnPXResult && tables.Length > 0) && _CreateInstance == null)
			{
        ...
        {
            bool resetTopCount = false;
            PXSearchColumn[] sorts = prepareSorts(null, null, null, 0, out _, out _, ref resetTopCount);
            if (_Executing.Count > 0 && _Executing.Peek().View == this)
            {
                PXView.Sorts.Clear();
		...
		/// <param name="buttons">A <see cref="MessageButtons">MessageButtons</see> value indicating
		...
		/// <returns>The <see cref="WebDialogResult">WebDialogResult</see> value that
		...
		/// <returns>The <see cref="WebDialogResult">WebDialogResult</see> value that
		/// indicates the clicked button.</returns>
		public WebDialogResult Ask(object row, string header, string message, MessageButtons buttons,
			IReadOnlyDictionary<WebDialogResult, string> buttonNames, MessageIcon icon)
		{
		...
		public PXNonSqlView(PXGraph graph) : base(graph)
		{
		  ...
		  //  graph.OnBeforePersist += copySourcePassToInterceptor;
			...
			graph.Caches.SubscribeCacheCreated<TTable>(delegate
			{
				...
				if (WebConfig.IsClusterEnabled || WebConfig.SerializeSessionItems)
				{
					Cache.RowUpdated += (sender, args) =>
					{
						if (args.Row != null && args.OldRow != null)
						{
							var row = args.Row; 
							var oldRow = args.OldRow;
							var i = interceptor.GetIndexOf(oldRow, sender);
							if (i >= 0)
							{
								var clone = sender.CreateCopy(row);
								var lst = interceptor.Source(true);
								var oldCachedRecord = lst[i];
								if (!sender.ObjectsEqual(row, oldRow))
								{
									sender.Remove(oldCachedRecord);
								}
								lst[i] = clone;
							}
						}
