	...
	public abstract class BudgetControlGraph<TGraph> : PXGraphExtension<TGraph>	where TGraph : PXGraph
	{
		...
		private string GetWarningMessage(PMBudgetControlLine budgetControlLine)
		{
			var result = budgetControlLine.BudgetExist == true ?
					string.Format(PXLocalizer.Localize(Messages.BudgetControlWarning), budgetControlLine.BudgetedAmount, budgetControlLine.ConsumedAmount, budgetControlLine.AvailableAmount, budgetControlLine.DocumentAmount, budgetControlLine.RemainingAmount) :
					string.Format(PXLocalizer.Localize(Messages.BudgetControlNotFoundWarning), budgetControlLine.BudgetedAmount, budgetControlLine.DocumentAmount, budgetControlLine.RemainingAmount);
			return result;
		}
