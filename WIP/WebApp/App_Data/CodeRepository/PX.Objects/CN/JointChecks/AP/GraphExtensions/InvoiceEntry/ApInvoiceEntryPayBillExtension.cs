...
namespace PX.Objects.CN.JointChecks.AP.GraphExtensions.InvoiceEntry
{
    public class ApInvoiceEntryPayBillExtension : PXGraphExtension<ApInvoiceEntryExt, APInvoiceEntry>
    {
        ...
        {
	        Base.Document.Current.IsPaymentCycleWorkflow = JointPayeePaymentDataProvider
        ...
        private void CheckPaymentLifeCycle(PXGraph graph)
        {
            ...
            if (allRelatedBills.Any(ext => ext.IsPaymentCycleWorkflow == true))
            {
                throw new PXException(JointCheckMessages.BillHasUnreleasedJointPayeePayment);
            }
        }
        ...
        private void LinkComplianceDocumentsToCheck(APPaymentEntry paymentEntry)
        {
			...
			Base.Caches<ComplianceDocumentReference>().ClearQueryCache();
			Base.Caches<ComplianceDocumentReference>().Clear();

			PXDBTimestampAttribute complianceDocumentTimestampAttribute = Base.Caches<ComplianceDocument>()
				.GetAttributesOfType<PXDBTimestampAttribute>(null, nameof(ComplianceDocument.Tstamp))
				.First();
			complianceDocumentTimestampAttribute.RecordComesFirst = true;

			PXDBTimestampAttribute complianceDocumentReferenceTimestampAttribute = Base.Caches<ComplianceDocumentReference>()
				.GetAttributesOfType<PXDBTimestampAttribute>(null, nameof(ComplianceDocumentReference.Tstamp))
				.First();
			complianceDocumentReferenceTimestampAttribute.RecordComesFirst = true;
			...
			{
				var complianceDocumentCache = Base.Caches<ComplianceDocument>();
