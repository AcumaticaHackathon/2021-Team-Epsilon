...
{
	public class CAReconEntry : PXGraph<CAReconEntry>, ICaptionable<object>
	{
		...
		internal class CATranExtRowUpdatingContext : IDisposable
		{
			...
			public void Dispose()
			{
				Graph.SearchInCache = true;
			}
		}

		public string Caption(object item)
		{
			CARecon currentItem = (CARecon) item;
			if (currentItem == null)
			{
				return string.Empty;
			}

			CashAccount cashAcc = PXSelectorAttribute.Select<CashAccount.cashAccountID>(CAReconRecords.Cache, currentItem) as CashAccount;
			if (this.CAReconRecords.Cache.GetStatus(currentItem) == PXEntryStatus.Inserted)
			{
				return (cashAcc == null) ? PXMessages.Localize(PX.Data.InfoMessages.NewRecord) : $"{cashAcc.CashAccountCD} {cashAcc.Descr}";
			}
			else
			{
				return (cashAcc == null) ? currentItem.ReconNbr : $"{currentItem.ReconNbr} - {cashAcc.CashAccountCD} {cashAcc.Descr}: {currentItem.ReconDate.Value.ToShortDateString()}";
			}
		}
		...
		public virtual IEnumerable createAdjustment(PXAdapter adapter)
		{
			...
			if (header != null && IsHeaderUpdateEnabled(header))
			{
				...
				if (result == WebDialogResult.OK || result == WebDialogResult.Yes)
				{
					bool releaseTransaction = result == WebDialogResult.Yes;
					using (new PXTimeStampScope(this.TimeStamp))
					{
						CATran rectran = AddTrxFilter.VerifyAndCreateTransaction(this, AddFilter, currencyinfo, releaseTransaction);
						Save.Press();
					}
				}
		...
		[PXUIField(
			DisplayName = "Reconciliation Statement Report",
			MapEnableRights = PXCacheRights.Select,
			MapViewRights = PXCacheRights.Select,
			Visible = true)]
		[PXButton]
		public virtual IEnumerable PrintReconciliationReport(PXAdapter adapter, string reportID = null)
		{
			...
			foreach (CARecon recon in adapter.Get<CARecon>())
			{
				CashAccount cashAccount = CashAccount.PK.Find(this, recon.CashAccountID);
				parameters["CashAccount"] = cashAccount?.CashAccountCD;
				parameters["ReconNbr"] = recon.ReconNbr;
				parameters["ShowReconciledTransactions"] = (recon.Reconciled != true).ToString();
			}
			throw new PXReportRequiredException(parameters, reportID, "Report " + reportID);
		}
		#endregion

		#region Button ReleaseAdjustment
		public PXAction<CARecon> ReleaseAdjustment;
		[PXUIField(DisplayName = "Release", MapEnableRights = PXCacheRights.Insert, MapViewRights = PXCacheRights.Insert)]
		[PXButton]
		public virtual IEnumerable releaseAdjustment(PXAdapter adapter)
		{
			return adapter.Get();
		}
		#endregion
		...
		protected virtual IEnumerable carecontranrecords()
		{
			...
			if (doc.ShowBatchPayments == true)
			{
				foreach (CATranExt result in GetBatchTransactions(catrans))
				{
					...
					{
						if ((cached.OrigModule == BatchModule.AP || cached.OrigModule == BatchModule.PR) && cached.OrigTranType == CATranType.CABatch && cached.OrigRefNbr == result.OrigRefNbr)
						{
							cache.Hold(cached);
							tran = cached;
							break;
			...
			if (transToReconcile.Any())
			{
				...
				foreach (CATranExt tran in transToReconcile)
				{
					...
					if (skipVoids)
					{
						if (Skip(voidingTran: tran, voidedTran: null))
						{
			...
			foreach (var batch in idsCollection.BreakInto(batchSize))
			{
				...
				if (doc.SkipVoided == true)
				{
					voidingTrans.AddRange(PXSelectReadonly<CATranVoiding,
												  ...
												  And<CATranVoiding.cashAccountID, Equal<Required<CATranExt.cashAccountID>>,
												  And<Where<CATranVoiding.tranDate, LessEqual<Current2<CARecon.loadDocumentsTill>>,
														Or<Current2<CARecon.loadDocumentsTill>, IsNull>>>>>>
												.Select(this, tranIDs, doc.CashAccountID)
		...
		private List<CATranExt> GetBatchTransactions(HashSet<long> catrans)
		{
			...
			foreach (PXResult<CABatch, CABatchDetail, CATran, CABankTranMatch, CABankTran, CABankTranMatch2> result in lines)
			{
				...
				{
					tran = new CATranExt();
					CopyCABatchToCATran(batch, tran);
					tran.Voided = false;
					tran.VoidingNotReleased = false;
					tran.VoidingTranID = null;
		...
		public override void Persist()
		{
			...
			CAReconRecords.Cache.RaiseRowSelected(CAReconRecords.Current);
		}
		...
		protected virtual bool Skip(CATran voidingTran, CATran voidedTran = null)
		{
			...
			CATranExt voidingTranLocated = voidingTran as CATranExt;
			if (SearchInCache)
			{
				...
				voidingTranLocated = CAReconTranRecords.Locate(new CATranExt { CashAccountID = voidingTran.CashAccountID, TranID = voidingTran.TranID });
			}
			...
			return (voidedTran.Reconciled == false && voidingTran.Reconciled == false) ||
				...
				(voidedTran.Reconciled == true && voidingTran.Reconciled == false && voidingTranLocated?.Reconciled == true && voidedTran.ReconNbr == voidingTranLocated?.ReconNbr);
		}
		...
		protected virtual void UpdateBatchTransactions(CATranExt row)
		{
			// Please be advised that UpdateBatchTransactions method is also implemented in PRxCAReconEntry Graph Extension.
		...
		public virtual void UpdateBatch(CATranExt row)
		{
		...
		public virtual CATran CopyCABatchToCATran(CABatch batch, CATran destinationTran)
		{
			...
			if (batch.OrigModule == BatchModule.PR)
			{
				destinationTran.OrigModule = BatchModule.PR;
			}
		...
		protected virtual void CATranExt_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			bool skipVoids = CAReconRecords.Current?.SkipVoided ?? false;
			if ((row.OrigModule == BatchModule.AP || row.OrigModule == BatchModule.PR) && row.OrigTranType == CATranType.CABatch)
			{
				e.Cancel = true;
			}
		...
		protected virtual void CATranExt_RowUpdating(PXCache cache, PXRowUpdatingEventArgs e)
		{
			...
			CATranExt row = (CATranExt)e.Row;
			CARecon recon = CAReconRecords.Current;
			if (newRow == null || row == null || recon == null)
			{
				return;
			}
			...
			using (new CATranExtRowUpdatingContext(this))
			{
				if (skipVoids && newRow.Reconciled == true && Skip(newRow, null) && !Skip(row, null))
				{
		...
		public virtual void AddTrxFilter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			ReleaseAdjustment.SetEnabled(row.Hold == false);
		}
		#endregion
