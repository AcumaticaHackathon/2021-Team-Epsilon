...
using PXCurrencyAttribute = PX.Objects.CM.Extensions.PXCurrencyAttribute;
using PXDBCurrencyAttribute = PX.Objects.CM.Extensions.PXDBCurrencyAttribute;
...
namespace PX.Objects.Extensions.MultiCurrency
{
			...
			where TPrimary : class, IBqlTable, new()
	{
		...
		{
			CurrencyInfo info = GetDefaultCurrencyInfo();
			if (info != null)
			{
		...
		{
			CuryConvBase(GetDefaultCurrencyInfo(), curyval, out baseval);
		}
		...
		{
			CuryConvCury(GetDefaultCurrencyInfo(), baseval, out curyval);
		}
		...
		public PXView GetView(Type table, Type field)
		{
			...
			if (ret == null)
			{
				return new PXView(Base, false, ret.View.BqlSelect, new PXSelectDelegate(() =>
					{
						long curyInfoID = GetDefaultCurrencyInfo()?.CuryInfoID ?? 0;
		...
		[PXButton(ImageKey = PX.Web.UI.Sprite.Main.Money, Tooltip = CM.Messages.ToggleCurrencyViewTooltip, VisibleOnDataSource = false, CommitChanges = false)]
				...
				if (info != null)
				{
					...
					if (info.CuryInfoID > 0 
					    && (CM.CurrencyCollection.IsBaseCuryInfo(old) || 
					        CM.CurrencyCollection.IsBaseCuryInfo(info)))
					{
		...
		{
			var branchID = this.Documents.Current?.BranchID;
