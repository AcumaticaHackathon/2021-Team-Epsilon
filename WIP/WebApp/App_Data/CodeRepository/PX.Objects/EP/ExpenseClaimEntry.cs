...
namespace PX.Objects.EP
{
	public class ExpenseClaimEntry : PXGraph<ExpenseClaimEntry, EPExpenseClaim>, PXImportAttribute.IPXPrepareItems
	{
		...
		public class ExpenceClaimApproval<SourceAssign> : EPApprovalAutomation<SourceAssign, EPExpenseClaim.approved, EPExpenseClaim.rejected, EPExpenseClaim.hold, EPSetupExpenseClaimApproval>
			where SourceAssign : EPExpenseClaim, new()
		{
			...
			protected override void AssignMaps(PXCache cache, SourceAssign doc)
			{
				if (!PXAccess.FeatureInstalled<FeaturesSet.approvalWorkflow>())
				{
					EPSetup setup = PXSetup<EPSetup>.SelectSingleBound(cache.Graph, new object[] { doc });
					try
					{
						Assign(doc, null, setup.ClaimAssignmentNotificationID);
					}
					catch (PXSetPropertyException ex)
					{
						cache.SetValue<EPExpenseClaim.hold>(doc, true);
			...
			protected override IEnumerable<ApproveInfo> GetApproversFromNextStep(SourceAssign source, EPAssignmentMap map, int? currentStepSequence)
			{
				...
				if (employee.SupervisorID != null)
				{
					...
					if (super != null)
					{
						...
						source.OwnerID = super.DefContactID;

						yield return new ApproveInfo
						{
							OwnerID = super.DefContactID,
							WorkgroupID = null
						};
					}
				}
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = Messages.Hold)]
		...
		public virtual IEnumerable Release(PXAdapter adapter)
		{
			...
			{
				TryValidateRequireUniqueVendorRef();
		...
		protected virtual void CreateNew()
		{
			...
			if (expenseClaim != null && expenseClaim.EmployeeID != null)
			{
				...
				graph.CurrentClaim.Update(ExpenseClaim.Current);
		...
		{
			EPExpenseClaim.Events.Select(e => e.Submit).FireOn<EPExpenseClaim>(this, claim);
		}
		...
		public virtual void _(Events.FieldVerifying<EPExpenseClaimDetails.corpCardID> e)
		{
			...
			int? newCorpCardID = (e.NewValue is int?)
				? (int?) e.NewValue
				: CACorpCard.UK.Find(this, (string) e.NewValue)?.CorpCardID;

			ReceiptEntryExt.VerifyClaimAndCorpCardCurrencies(
				newCorpCardID,
				ExpenseClaim.Current,
				() => e.NewValue = (e.NewValue is int?)
					? CACorpCard.PK.Find(this, (int?) e.NewValue)?.CorpCardCD
					: e.NewValue);
		}
		...
		protected virtual void EPExpenseClaim_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			{
				var curyInfo =(CurrencyInfo) currencyinfo.View.SelectSingleBound(new[] {e.Row});
				if (curyInfo != null && CurrencyCollection.GetCurrency(curyInfo.BaseCuryID).RoundingLimit < Math.Abs(row.TaxRoundDiff ?? 0m))
				{
					throw new PXException(AP.Messages.RoundingAmountTooBig, curyInfo.BaseCuryID, row.TaxRoundDiff,
						PXDBQuantityAttribute.Round(CurrencyCollection.GetCurrency(curyInfo.BaseCuryID).RoundingLimit));
				}
			}
		}
		...
		public virtual void ValidateProjectAndProjectTask(EPExpenseClaimDetails info)
		{
			if (info != null)
			{
				...
				if (info.TaskID != null)
				{
					PMTask projectTask = PMTask.PK.Find(this, info.TaskID);
		...
		private void TryValidateRequireUniqueVendorRef()
		{
			bool hasErrorLines = false;
			foreach (EPExpenseClaimDetails item in ExpenseClaimDetails.Select())
			{
				CashAccount cashAccount = PXSelect<
					CashAccount,
					Where<CashAccount.accountID, Equal<Required<EPExpenseClaimDetails.expenseAccountID>>>>
					.Select(this, item.ExpenseAccountID);
				...
				PaymentMethod paymentMethod = PXSelectJoin<
					...
					.Select(this, cashAccount.CashAccountID);

				if (paymentMethod.APRequirePaymentRef.GetValueOrDefault(false))
				{
					CACorpCard card = CACorpCard.PKID.Find(this, item.CorpCardID);

					ExpenseClaimDetails.Cache.RaiseExceptionHandling<EPExpenseClaimDetails.expenseAccountID>(item, item.ExpenseAccountID,
						new PXSetPropertyException(Messages.ExpenseClaimCannotBeReleased, PXErrorLevel.RowError, cashAccount.CashAccountCD, card.Name, paymentMethod.PaymentMethodID));
					hasErrorLines = true;
				}
			}
			if (hasErrorLines)
			{
				throw new PXException(Messages.ExpenseClaimCannotBeReleasedSummary);
			}
		}
		#endregion
	}
