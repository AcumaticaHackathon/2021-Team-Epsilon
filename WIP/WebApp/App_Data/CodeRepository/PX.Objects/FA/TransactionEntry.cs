...
namespace PX.Objects.FA
{
	...
	public class TransactionEntry : PXGraph<TransactionEntry, FARegister>
	{
		...
		public SelectFrom<FATran>
			...
			.And<FATran.refNbr.IsEqual<FARegister.refNbr.FromCurrent>>>.View TransferPurchasingTrans;
		#endregion
		...
		protected virtual void _(Events.RowDeleted<FARegister> e)
		{
			var register = (FARegister)e.Row;

			if (register != null && register.Origin == FARegister.origin.Transfer && register.Released != true)
			{
				FATran tran = TransferPurchasingTrans.SelectSingle();
		...
		private void RestorePreviousLocation(FATran tran)
		{
			...
			{
				throw new PXException(Messages.PrevLocationRevisionNotFound, asset.AssetCD);
			}

			FAClass restoringClass = SelectFrom<FAClass>.Where<FAClass.assetID.IsEqual<@P.AsInt>>.View.Select(this, restoringLocation.ClassID);
			...
			asset.UnderConstruction = restoringClass.UnderConstruction ?? asset.UnderConstruction;
		...
		#region Buttons
		public PXAutoAction<FARegister> initializeState;
