...
namespace PX.Objects.PO
{
	...
	public class POOrderEntry : PXGraph<POOrderEntry, POOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		[PXCopyPasteHiddenFields(typeof(POOrder.sOOrderType), typeof(POOrder.sOOrderNbr))]
		...
		protected virtual void _(Events.FieldSelecting<POOrderAPDoc.statusText> e)
		{
			...
			{
				e.ReturnValue = PXMessages.LocalizeFormatNoPrefix(Messages.StatusTotalBilled,
						FormatQty(apTranTotal?.SignedBaseQty ?? 0),
						FormatQty((apTranTotal?.SignedCuryTranAmt ?? 0) + (apTranTotal?.SignedCuryRetainageAmt ?? 0)));
			}
		}
		...
		public PXAutoAction<POOrder> initializeState;
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Re-Open Order", MapEnableRights = PXCacheRights.Select)]
			...
			PXLongOperation.StartOperation(this, () =>
			{
				...
				foreach (POOrder order in adapter.Get<POOrder>())
				{
					...
					catch (Exception exception)
					{
						...
						//This code is needed to address the situation with incorrect update of POOrder in case one or more attempts to email PO fail during mass processing. 
		...
		[PXButton(CommitChanges = true)]
		[PXUIField(DisplayName = "View Purchase Order Receipt and Billing History", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		public virtual IEnumerable ViewPurchaseOrderReceipt(PXAdapter adapter)
			=> Report(adapter.Apply(a => a.Menu = "View Purchase Order Receipt and Billing History"), null, "PO643000", false, false);
		...
		protected virtual bool GetRequireControlTotal(string aOrderType)
		{
			bool result = false;
			switch (aOrderType)
			{
		...
		private bool IsForInventory(POLine row)
		{
			return row.LineType == POLineType.GoodsForInventory ||
		...
		protected virtual void POOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			this.createPOReceipt.SetEnabled((doc.OrderType == POOrderType.RegularOrder || doc.OrderType == POOrderType.DropShip)//todo move to WF
					&& doc.Status == POOrderStatus.Open);
			this.createAPInvoice.SetEnabled((POOrderType.IsNormalType(doc.OrderType) || doc.OrderType == POOrderType.DropShip)//todo move to WF
					&& doc.Status.IsIn(POOrderStatus.AwaitingLink, POOrderStatus.Open, POOrderStatus.Completed));
		...
		protected virtual void ThrowErrorWhenPurchaseReceiptExists(POReceipt receipt, POOrder order)
		{
			...
			{
				throw new PXSetPropertyException(Messages.POLineHasReleaseReceiptsAndCantBeCancelled, order.OrderNbr);
			}
		}
		...
		protected virtual void POOrder_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			...
			LeftJoin<POOrderPrepayment,
				...
				And<POOrderPrepayment.orderType, Equal<Current<POOrder.orderType>>,
			...
			OrderBy<Desc<POOrderPrepayment.aPRefNbr>>>.SelectSingleBound(this, new object[] { doc });
			if (apRecord != null && apRecord.GetItem<POOrderPrepayment>().APRefNbr != null)
			{
				throw new PXException(Messages.POOrderHasPrepaymentRequestAndCannotBeDeleted);
			}
		...
		public virtual bool UpdateClosedState(POLine blanketLine)
        {
			var doc = Document.Current;
			if (doc?.OrderType != POOrderType.Blanket
				|| doc.OrderType != blanketLine.OrderType
				|| doc.OrderNbr != blanketLine.OrderNbr)
				return false;

			var line = (POLine)Transactions.Cache.Locate(blanketLine)
				?? POLine.PK.Find(this, blanketLine);

			bool invertClosed;
			if (line.Completed != true)
				invertClosed = line.Closed == true;
			else
			{
				var anyOpen = (POLine)SelectFrom<POLine>
					.Where<POLine.pOType.IsEqual<@P.AsString.ASCII>
					.And<POLine.pONbr.IsEqual<@P.AsString>>
					.And<POLine.pOLineNbr.IsEqual<@P.AsInt>>
					.And<POLine.closed.IsNotEqual<True>>>
					.View
					.ReadOnly
					.SelectWindowed(this, 0, 1, line.OrderType, line.OrderNbr, line.LineNbr) != null;

				invertClosed =
					line.Closed == true && anyOpen
					|| line.Closed != true && !anyOpen;
			}
			...
			if (invertClosed)
			{
				...
				Transactions.Cache.Update(line);
			}
			return invertClosed;
		}
				...
				default:
					...
					if (item != null)
					{
						...
						if (POLineType.IsNonStock(row.LineType))
						{
							...
							int? projectTask_SubID = PMTask.PK.Find(this, row.TaskID)?.DefaultExpenseSubID;
		...
		protected virtual void POLine_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			bool isForInventory = IsForInventory(row);
            ...
            {
				string msg = isForInventory ? Messages.POLineQuantityMustBeGreaterThanZero : Messages.POLineQuantityMustBeGreaterThanZeroNonStock;
		...
		public partial class POOrderR : IBqlTable
		{
			...
			{
				public static POOrderR Find(PXGraph graph, string orderType, string orderNbr) => FindBy(graph, orderType, orderNbr);
			}
			#endregion
			#region OrderType
			public abstract class orderType : PX.Data.BQL.BqlString.Field<orderType> { }
		...
		public partial class SOLineSplit3 : IBqlTable, ISortOrder
		{
			...
			[PXDBCalced(typeof(Switch<Case<Where<SOLine.baseOpenQty, Equal<SOLine.baseOrderQty>,
				And<SOLine.baseOrderQty, Equal<SOLineSplit.baseQty>,
		...
		[PXDefault(typeof(Search<CREmployee.defContactID, Where<CREmployee.bAccountID.IsEqual<POOrder.employeeID.FromCurrent>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		[PXMergeAttributes(Method = MergeMethod.Merge)]
