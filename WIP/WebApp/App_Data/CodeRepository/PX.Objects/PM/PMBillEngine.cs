...
namespace PX.Objects.PM
{
	public class PMBillEngine : PXGraph<PMBillEngine>, IRateTable
	{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (KeyValuePair<string, Tuple<int?, List<BillingData>>> kv in invoices)
					{
						...
						if (project.CreateProforma == true)
						{
							...
							if (PXAccess.FeatureInstalled<FeaturesSet.construction>() && ProformaEntry.ProgressiveLines.Cache.Inserted.Any_())
							{
								string proformaNumber = IncLastProformaNumber(project.LastProformaNumber);

								if (proformaNumber.Length > PMProforma.projectNbr.Length)
								{
									throw new PXSetPropertyException(Messages.LastApplicationNbrIncreaseLength, PMProforma.projectNbr.Length, project.ContractCD);
								}

								try
								{
									ProformaEntry.Document.Current.ProjectNbr = proformaNumber;
									ProformaEntry.Document.UpdateCurrent();
								}
								catch (PXSetPropertyException ex)
								{
									throw new PXException(ex, Messages.LastApplicationNbrIsInvalid);
								}

								project.LastProformaNumber = proformaNumber;
								Project.Update(project);
							}
						}
						else
						{
							...
							{
								invoice = (ARInvoice)InvoiceEntry.Document.Cache.CreateCopy(InvoiceEntry.Document.Current);
								invoice.Hold = false;
		...
		private string IncLastProformaNumber(string lastProformaNumber)
		{
			string lastNumber = string.IsNullOrEmpty(lastProformaNumber) ? "0000" : lastProformaNumber;
			...
			return NumberHelper.IncreaseNumber(lastNumber, 1);
		}
		...
		public virtual void AutoReleaseCreatedDocuments(PMProject project, BillingResult result, PMRegister wipReversalDoc)
		{
			if (project.AutomaticReleaseAR == true && result.Invoices.Count > 0)
			{
				...
				{
					throw new PXException(Messages.AutoReleaseAROnBillingFailed, ex);
				}
		...
		public virtual PMProforma InsertNewProformaDocument(string finPeriod, Customer customer, PMProject project, DateTime billingDate, string docDesc, int? locationID)
		{
			...
			int? docLocationID = locationID ?? (project.LocationID ?? customer.DefLocationID);
			Location location = Location.PK.Find(this, customer.BAccountID, docLocationID);
			if (location?.CTaxCalcMode == TX.TaxCalculationMode.Gross)
			{
				throw new PXException(Messages.TheGrossTaxCalcModeForCustomerIsNotSupportedInProFormaInvoices, location.LocationCD, customer?.AcctCD, location.LocationCD);
			}

			doc.LocationID = docLocationID;
			...
			ProformaEntry.Document.Cache.SetValueExt<PMProforma.customerID>(doc, customer.BAccountID); //check customer visibility
			...
			DateTime billingDate, string docDesc, int? locationID)
		{
			...
			if (!string.IsNullOrEmpty(project.TermsID))
				InvoiceEntry.Document.SetValueExt<ARInvoice.termsID>(invoice, project.TermsID);
		...
		public virtual Tuple<string, bool> GetDocTypeAndRetainage(PMProject project, List<BillingData> value)
		{
			...
			if (amount >= 0)
				...
				return new Tuple<string, bool>(ARDocType.CreditMemo, applyRetainage);
		}
			...
			foreach (PMRecurringItem billing in recurrentItems)
			{
				...
				if (bill)
				{
					...
					if (billing.AccountSource != PMAccountSource.None)
					{
						...
						{
							if (project.DefaultSalesAccountID != null)
							{
								line.AccountID = project.DefaultSalesAccountID;
							}
							else if (billing.InventoryID != null)
							{
								InventoryItem item = PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.Select(this, billing.InventoryID);
						...
						{
							if (task.DefaultSalesAccountID != null)
							{
								line.AccountID = task.DefaultSalesAccountID;
							}
							else if (billing.InventoryID != null)
							{
								InventoryItem item = PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.Select(this, billing.InventoryID);
						...
						else if (line.AccountID != null && !string.IsNullOrEmpty(billing.SubMask))
						{
							subCD = PMRecurentBillSubAccountMaskAttribute.MakeSub<PMBillingRule.subMask>(this, billing.SubMask,
								new object[] { billing.SubID, project.DefaultSalesSubID, task.DefaultSalesSubID },
								new Type[] { typeof(PMBillingRule.subID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID) });
						}
					}
			...
			Dictionary<int, decimal> availableQty, Dictionary<int, PMRecurringItem> billingItems, bool groupConditionsIgnore)
		{
			...
			int mult = 1;
			PMAccountGroup ag = PMAccountGroup.PK.Find(this, rule.AccountGroupID);
			if (ag == null)
			{
		...
		public virtual int? CalculateTargetSalesAccountID(PMBillingRule rule, PMProject project, PMTask task, PMTran tran, PMProformaLine line, Customer customer)
		{
			...
			else if (rule.AccountSource == PMAccountSource.Project)
			{
				if (project.DefaultSalesAccountID != null)
				{
					result = project.DefaultSalesAccountID;
				}
				else
				{
			...
			else if (rule.AccountSource == PMAccountSource.Task)
			{
				if (task.DefaultSalesAccountID != null)
				{
					result = task.DefaultSalesAccountID;
				}
				else
				{
		...
		public virtual string CalculateTargetSalesSubaccountCD(PMBillingRule rule, PMProject project, PMTask task, int? subIDTran, int? employeeID, int? branchID, int? inventoryID, Customer customer)
		{
			...
			if (!string.IsNullOrEmpty(rule.SubMask) && rule.SubMask.Contains(AcctSubDefault.Branch) && branchID != null)
			{
				...
				if (branch != null)
				{
					CR.Standalone.Location extAddress = PXSelectJoin<CR.Standalone.Location,
						LeftJoin<CR.BAccount,
							On<CR.BAccount.bAccountID, Equal<Required<CR.BAccount.bAccountID>>>>,
						Where<
							CR.Standalone.Location.bAccountID, Equal<CR.BAccount.bAccountID>,
							And<CR.Standalone.Location.locationID, Equal<CR.BAccount.defLocationID>>>>.Select(this, branch.BAccountID);
					if (extAddress != null)
					{
			...
			new object[] { rule.SubID, project.DefaultSalesSubID, task.DefaultSalesSubID, employeeSubID, tranSubID, inventorySubID, customerSubID, branchSubID },
			new Type[] { typeof(PMBillingRule.subID), typeof(PMProject.defaultSalesSubID), typeof(PMTask.defaultSalesSubID), typeof(EP.EPEmployee.salesSubID), typeof(PMTran.subID), typeof(InventoryItem.salesSubID), typeof(CR.Location.cSalesSubID), typeof(CR.Location.cMPSalesSubID) });
		...
		protected virtual void CalculateFormulas(PMProject project, PMDataNavigator navigator, PMBillingRule rule, PMTran tran, out decimal? qty, out decimal? amt, out string description)
		{
			...
			if (rule.Type == PMBillingType.Transaction && !string.IsNullOrEmpty(rule.QtyFormula) && tran.RemainderOfTranID == null)
			{
				try
				{
			...
			if (rule.Type == PMBillingType.Transaction && !string.IsNullOrEmpty(rule.AmountFormula) && tran.RemainderOfTranID == null)
			{
				try
				{
		...
		public virtual decimal? GetPrice(PMTran tran)
		{
			...
			if (tran.InventoryID != null && tran.InventoryID != PMInventorySelectorAttribute.EmptyInventoryID)
			{
				...
				result = ARSalesPriceMaint.CalculateSalesPrice(Caches[typeof(PMTran)], customerPriceClass, projectTask.CustomerID, tran.InventoryID, dummy, tran.Qty, tran.UOM, tran.Date.Value, alwaysFromBase, taxCalcMode);

				if (alwaysFromBase != true && result == null)
				{
					result = ARSalesPriceMaint.CalculateSalesPrice(Caches[typeof(PMTran)], customerPriceClass, projectTask.CustomerID, tran.InventoryID, dummy, tran.Qty, tran.UOM, tran.Date.Value, true, taxCalcMode);
				}
			}
