...
namespace PX.Objects.AM
{
    public class ProductionGLTranBuilder
    {
        ...
        public List<AMMTran> PreviousCostTrans;
        public bool HasPreviousCostTrans => PreviousCostTrans != null && PreviousCostTrans.Count > 0;
        ...
        protected void SetGraphs(JournalEntry journalEntryGraph, ProductionCostEntry amGlTranGraph)
        {
            ...
            PreviousCostTrans = new List<AMMTran>();
        }
            ...
            string referenceCostId)
        {
            ...
            try
            {
                ...
                var tranAmountAdj = GetAdjustedTranAmount(ammTran, amTranType, referenceCostId.TrimIfNotNull(), costOperationId, tranAmount);
                var laborTimeAdj = GetAdjustedLaborTime(ammTran, amTranType, costOperationId, laborTime);
                if (tranAmountAdj == 0m && laborTimeAdj == 0)
                {
                    return;
                }
                ...
                var wiPisDebit = tranAmountAdj > 0;
                ...
                var gltranDEBIT = new GLTran
                {
                    ...
                    CreditAmt = 0m,
                    CuryCreditAmt = 0m,
                    DebitAmt = Math.Abs(tranAmountAdj),
                    CuryDebitAmt = Math.Abs(tranAmountAdj),
                    TranDate = ammTran.TranDate ?? _journalEntryGraph.Accessinfo.BusinessDate,
                ...
                var gltranCREDIT = new GLTran
                {
                    ...
                    RefNbr = amProdItem.ProdOrdID,
                    CreditAmt = Math.Abs(tranAmountAdj),
                    CuryCreditAmt = Math.Abs(tranAmountAdj),
                    DebitAmt = 0m,
                    CuryDebitAmt = 0m,
                    TranDate = ammTran.TranDate ?? _journalEntryGraph.Accessinfo.BusinessDate,
                    FinPeriodID = ammTran.FinPeriodID,
            ...
            )
        {
            ...
            try
            {
                ...
                var tranAmountAdj = GetAdjustedTranAmount(ammTran, amTranType, tranAmount);
                if (tranAmountAdj == 0m)
                {
                    return;
                }
                ...
                var gltranDEBIT = new GLTran
                {
                    ...
                    CreditAmt = 0m,
                    CuryCreditAmt = 0m,
                    DebitAmt = tranAmountAdj,
                    CuryDebitAmt = tranAmountAdj,
                    TranDate = ammTran.TranDate ?? _journalEntryGraph.Accessinfo.BusinessDate,
                    FinPeriodID = ammTran.FinPeriodID,
                    OrigModule = Common.ModuleAM,
                ...
                var gltranCREDIT = new GLTran
                {
                    ...
                    RefNbr = amLaborCode.LaborCodeID,
                    CreditAmt = tranAmountAdj,
                    CuryCreditAmt = tranAmountAdj,
                    DebitAmt = 0m,
                    CuryDebitAmt = 0m,
                    TranDate = ammTran.TranDate ?? _journalEntryGraph.Accessinfo.BusinessDate,
                    FinPeriodID = ammTran.FinPeriodID,
        ...
        internal decimal GetAdjustedTranAmount(AMMTran tran, string tranType, decimal? tranAmount)
        {
            return GetAdjustedTranAmount(tran, tranType, null, null, tranAmount);
        }

        // Account for re-release adjusted amounts
        protected virtual decimal GetAdjustedTranAmount(AMMTran tran, string tranType, string referenceCostID, int? costOperationId, decimal? tranAmount)
        {
            if(!HasPreviousCostTrans || tran?.DocType == null || string.IsNullOrWhiteSpace(tranType))
            {
                return tranAmount ?? 0m;
            }

            var tranAmountFound = 0m;
            foreach (var previousTran in GetRelatedTrans(tran, tranType, costOperationId, referenceCostID))
            {
                tranAmountFound += previousTran?.TranAmt ?? 0m;
            }

            var diff = tranAmount.GetValueOrDefault() - tranAmountFound;
            return Math.Abs(diff) > 0.01m ? diff : 0m;
        }

        // Account for re-release adjusted amounts
        protected virtual int GetAdjustedLaborTime(AMMTran tran, string tranType, int? costOperationId, int? laborTime)
        {
            if(!HasPreviousCostTrans || tran?.DocType == null)
            {
                return laborTime ?? 0;
            }

            var laborTimeFound = 0;
            foreach (var previousTran in GetRelatedTrans(tran, tranType, costOperationId, null))
            {
                laborTimeFound += previousTran?.LaborTime ?? 0;
            }

            return laborTime.GetValueOrDefault() - laborTimeFound;
        }

        private IEnumerable<AMMTran> GetRelatedTrans(AMMTran tran, string tranType, int? costOperationId, string referenceCostID)
        {
            var operationId = costOperationId ?? tran?.OperationID;

            var results = PreviousCostTrans
                .Where(r => r.OrigDocType == tran.DocType
                && r.OrigBatNbr == tran.BatNbr
                && r.OrigLineNbr == tran.LineNbr
                && r.TranType == tranType
                && r.OrderType == tran.OrderType
                && r.ProdOrdID == tran.ProdOrdID
                && r.OperationID == operationId);

            if(!string.IsNullOrWhiteSpace(referenceCostID))
            {
                results = results.Where(r => r.ReferenceCostID == referenceCostID);
            }
