...
{
	public class RQRequisitionEntry : PXGraph<RQRequisitionEntry, RQRequisition>, IGraphWithInitialization
	{
		#region Views
		...
		public PXWorkflowEventHandler<RQRequisition, RQRequisitionOrder, SOOrder> OnSOOrderUnlinked;
		public PXWorkflowEventHandler<RQRequisition, RQRequisitionOrder, POOrder> OnPOOrderUnlinked;
		#endregion

		#region Actions
		public PXAutoAction<RQRequisition> initializeState;

		public PXAction<RQRequisition> ViewBidding;
		[PXButton(CommitChanges = true, ImageKey = PX.Web.UI.Sprite.Main.DataEntry)]
		[PXUIField(DisplayName = "View Bidding", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		private RQRequisitionLine MergeRequisitionLineCreatedFromRequest(RQRequisitionLine destLine, RQRequisitionLine lineToMerge, decimal? curyEstUnitCost, decimal? curyEstExtCost)
		{
			...
			foreach (PXResult<RQRequisitionContent, RQRequestLine> rec in rqContentAndRequestLines)
			{
				...
				{
					content.ReqQty = INUnitAttribute.ConvertFromBase(Lines.Cache, lineToMerge.InventoryID, lineToMerge.UOM, content.BaseReqQty ?? 0m, INPrecision.QUANTITY);
		...
		private RQRequisitionLine MergeRequisitionLineCreatedFromDraft(RQRequisitionLine destLine, RQRequisitionLine lineToMerge, decimal? curyEstUnitCost, decimal? curyEstExtCostFromLineToMerge)
		{
			destLine = PXCache<RQRequisitionLine>.CreateCopy(destLine);
			decimal? qtyFromLineToMerge = destLine.UOM == lineToMerge.UOM
				? lineToMerge.OrderQty
				: INUnitAttribute.ConvertFromBase(Lines.Cache, lineToMerge.InventoryID, lineToMerge.UOM, lineToMerge.BaseOrderQty ?? 0m, INPrecision.QUANTITY);
		...
		[PXButton(ImageKey = PX.Web.UI.Sprite.Main.DataEntry)]
		[PXUIField(DisplayName = Messages.RequestDetails, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		[PXLookupButton]
		[PXUIField(DisplayName = Messages.ViewRequest, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		[PXLookupButton]
		[PXUIField(DisplayName = Messages.ViewOrder, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
						...
						if (oldOrder == null)
						{
							...
							if (item.VendorID != null && item.VendorLocationID != null)
							{
									...
									.SelectMultiBound(this, new object[] { item }))
								{
									...
									if (item.POType == POOrderType.DropShip)
									{
										lineType = inv?.StkItem == true ? POLineType.GoodsForDropShip
											: inv?.NonStockReceipt == true && inv?.NonStockShip == true ? POLineType.NonStockForDropShip
											: line.LineType;
									}
									else
									{
										lineType = inv?.StkItem == true ? POLineType.GoodsForSalesOrder : line.LineType;
									}
							...
							else
							{
								...
								OrderBy<Asc<RQBidding.vendorID, Asc<RQBidding.vendorLocationID, Asc<RQRequisitionLine.lineNbr>>>>>.Select(this, item.ReqNbr))
								{
									...
									if (custQty > 0)
									{
										...
										po4so.Add(line.LineNbr, InsertPOLine(graph, line, custQty, bidding.CuryQuoteUnitCost, bidding, lineType, biddier?.PromisedDate));
		...
		/// <param name="rqLine">The requisition line.</param>
		/// <returns/>
		protected virtual RQSOSource CreateRQSOSourceFromRequest(RQRequest request, RQRequestLine requestLine, RQRequisitionContent rqContent, RQRequisitionLine rqLine)
		{
			return new RQSOSource
			{
				UOM = requestLine.UOM,
		...
		{
			if (sograph.Document.Current == null || sograph.Document.Current.CustomerID != rqSourceForSO.CustomerID
			...
			POLinkDialog pOLinkDialog = sograph.GetExtension<POLinkDialog>();
			foreach (POLine poLine in po4so[rqSourceForSO.LineNbr].Where(po4soLine => po4soLine.OrderQty > 0))
			{
				...
				pOLinkDialog.LinkPOSupply(newSoLine);
		...
		/// <param name="rqSourceForSO">The requisition DTO for SO documents.</param>
		/// <returns/>
		protected virtual SOLine CreateSOLineFromPO4SOLine(SOOrderEntry soGraph, POLine poLine, decimal qty, RQRequisition requisition, RQSOSource rqSourceForSO)
		{
			SOLine quote =
				PXSelectJoin<SOLine,
				InnerJoin<RQRequisitionLine,
		...
		protected virtual void FillSOLine(SOOrderEntry sograph, SOLine newSoLine, RQSOSource rqSourceForSO, bool areCurrenciesSame, decimal profitMultiplier)
		{
			newSoLine.ManualPrice = true;
		...
		[PXButton(CommitChanges = true)]
		[PXUIField(DisplayName = Messages.CreateQuotation, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		protected virtual void CreateQTOrderAndLines(SOOrderEntry sograph, RQRequisition requisition, RQRequisitionLine rqLine, InventoryItem inventoryItem, List<SOOrder> newSOOrders)
		{
		...
		/// <param name="profitMultiplier">The profit multiplier.</param>
		protected virtual void FillSOLine(SOOrderEntry sograph, SOLine newSoLine, RQRequisition requisition, RQRequisitionLine rqLine, RQBidding bidding, decimal profitMultiplier)
		{
			newSoLine.ManualPrice = true;
		...
		[PXUIField(DisplayName = Messages.ChooseVendor, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		[PXButton(CommitChanges = true, VisibleOnDataSource = false)]
		[PXUIField(DisplayName = Messages.VendorResponse, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		[PXButton]
		[PXUIField(DisplayName = Messages.VendorInfo, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		protected virtual POLine InsertPOLine(POOrderEntry graph, RQRequisitionLine line, decimal? qty, decimal? unitCost, IBqlTable costOriginDac, string lineType)
		...
		private void PersistOrder(POOrderEntry graph)
		{
			...
			if (upd.Hold == true && Setup.Current.POHold != true)
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Cancel Request", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		protected virtual IEnumerable CancelRequest(PXAdapter adapter) => adapter.Get();

		public PXAction<RQRequisition> markQuoted;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Mark as Quoted", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		protected virtual IEnumerable ReleaseFromHold(PXAdapter adapter)
		{
			foreach (RQRequisition order in adapter.Get<RQRequisition>())
			{
				...
				if (order.Hold != true && order.Approved != true)
				{
					if (Setup.Current.RequisitionAssignmentMapID != null)
					{
						var processor = CreateInstance<EPAssignmentProcessor<RQRequisition>>();
		...
		[PXUIField(DisplayName = "Send Request", MapEnableRights = PXCacheRights.Select)]
		[PXButton(CommitChanges = true, VisibleOnDataSource = false)]
		protected virtual IEnumerable SendRequestToCurrentVendor(PXAdapter adapter) => VendorNotifications(adapter, "RQPROPOSAL", currentVendor: true, updateVendorStatus: true);

		public PXAction<RQRequisition> sendRequestToAllVendors;
		[PXUIField(DisplayName = "Send Requests for Proposal", MapEnableRights = PXCacheRights.Select)]
		[PXButton(CommitChanges = true)]
		protected virtual IEnumerable SendRequestToAllVendors(PXAdapter adapter) => VendorNotifications(adapter, "RQPROPOSAL", currentVendor: false, updateVendorStatus: true);
		...
		protected virtual IEnumerable Report(PXAdapter adapter, [PXString(8, InputMask = "CC.CC.CC.CC")] string reportID)
		{
			...
			if (!String.IsNullOrEmpty(reportID))
			{
				...
				throw new PXReportRequiredException(parameters, reportID,
					PXBaseRedirectException.WindowMode.New, "Report " + reportID);
			}
		...
		[PXUIField(DisplayName = "Request for Proposal", MapEnableRights = PXCacheRights.Select)]
		[PXButton(CommitChanges = true)]
		protected virtual IEnumerable RequestForProposal(PXAdapter adapter) => Report(adapter, "RQ611000");
		#endregion
		...
		protected virtual void RQRequisition_VendorID_FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			...
			if (vendor != null && PXAccess.FeatureInstalled<FeaturesSet.multicurrency>())
			{
				...
				else if (vendor.AllowOverrideRate != true && current.CuryRateTypeID != null && vendor.CuryRateTypeID != current.CuryRateTypeID)
		...
		public virtual void InsertRequestLine(RQRequestLine line, decimal selectQty, bool mergeLines)
		{
			...
			else
			{
				...
				if (curyEstUnitCost != 0 && curyEstUnitCost != req.CuryEstExtCost)
				{
					...
					UpdateExistingRQRequisitionLineCosts(req, line, selectQty, total, areCurrenciesSame: Document.Current.CuryID == r.CuryID);
		...
		protected virtual void UpdateExistingRQRequisitionLineCosts(RQRequisitionLine requisitionLine, RQRequestLine requestLine, decimal selectQty, decimal? totalCost, bool areCurrenciesSame)
		{
		...
		/// <exclude/>
		public class RQRequisitionEntryAddressLookupExtension : CR.Extensions.AddressLookupExtension<RQRequisitionEntry, RQRequisition, POShipAddress>
		{
			protected override string AddressView => nameof(Base.Shipping_Address);
		}

		/// <exclude/>
		public class RQRequisitionEntryRemitAddressLookupExtension : CR.Extensions.AddressLookupExtension<RQRequisitionEntry, RQRequisition, PORemitAddress>
		{
