...
namespace PX.TM
{
    ...
    public class CompanyTreeMaint : PXGraph<CompanyTreeMaint>
    {
        ...
        protected virtual IEnumerable members()
        {
            ...
            if (Folders.Current != null)
            {
                     ...
                     Where<EPCompanyTreeMember.workGroupID, Equal<Required<EPCompanyTreeMember.workGroupID>>>>.Select(this, Folders.Current.WorkGroupID))
                {
                    ...
                    if (_companyTreeMember.ContactID != null )
                    {
                        if (_user == null)
                        {
                            this.Caches[typeof(EPCompanyTreeMember)].RaiseExceptionHandling<EPCompanyTreeMember.contactID>(_companyTreeMember, null, new PXSetPropertyException(PX.Objects.EP.Messages.UserDeleted, PXErrorLevel.Warning));
                        }
                        else if (_user.State != Users.state.Active && _user.State != Users.state.Online)
                        {
                            this.Caches[typeof(EPCompanyTreeMember)].RaiseExceptionHandling<EPCompanyTreeMember.contactID>(_companyTreeMember, null, new PXSetPropertyException(PX.Objects.EP.Messages.UserIsInactive, PXErrorLevel.Warning));
                        }
                        if (_employeer == null)
                        {
                        ...
                        else if (_employeer.VStatus == null)
                        {
                        ...
                        else if (_employeer.VStatus != VendorStatus.Active)
                        {
        ...
        {
            if (Members.Current.ContactID != null)
            {
                EmployeeMaint graph = CreateInstance<EmployeeMaint>();
                graph.Employee.Current = graph.Employee.Search<EPEmployee.defContactID>(Members.Current.ContactID);
        ...
        private void UpdateOwner(object row)
        {
            ...
            if (curr.IsOwner == true)
            {
                ...
                {
                    if (item.IsOwner != true || item.ContactID == curr.ContactID) continue;
        ...
        private void VerifyForUsing(EPCompanyTreeMaster row)
        {
            ...
            var assignmentMapType = PXMessages.LocalizeNoPrefix(Objects.EP.Messages.Assignment);
            var approvalMapType = PXMessages.LocalizeNoPrefix(Objects.EP.Messages.Approval);
                            ...
                            Select(this, row.WorkGroupID))
            {
                ...
                var mapType = map.MapType == EPMapType.Assignment
                    ? assignmentMapType
                    : approvalMapType;

                sb.AppendLine(PXMessages.LocalizeFormatNoPrefix(
                    Objects.EP.Messages.WorkgroupIsInUseAtAssignmentMap, row.Description, mapType, map.Name));
            }
            var ex = sb.ToString();
            ...
            sb.Clear();
            foreach (EPApproval item in
                SelectFrom<EPApproval>
                    .Where<EPApproval.workgroupID.IsEqual<@P.AsInt>>
                .View
                .SelectSingleBound(this, null, row.WorkGroupID))
            {
                sb.AppendLine(PXMessages.LocalizeFormatNoPrefix(
                    Objects.EP.Messages.WorkgroupIsInUseInApproval,
                        row.Description, _entityHelper.GetFriendlyEntityName(item.RefNoteID), _entityHelper.GetEntityRowID(item.RefNoteID), item.Descr));
            }
