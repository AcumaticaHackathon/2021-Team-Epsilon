...
namespace PX.Objects.AP
{
	...
	public class APPriceWorksheetMaint : PXGraph<APPriceWorksheetMaint, APPriceWorksheet>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		public PXSelectJoin<
			...
			LeftJoin<BAccount,
				On<BAccount.bAccountID, Equal<APPriceWorksheetDetail.vendorID>>>,
        ...
        #region Actions
        public PXAutoAction<APPriceWorksheet> initializeState;
        ...
        private void CalculatePendingPrices(CalculatePricesFilter settings)
        {
            ...
            foreach (APPriceWorksheetDetail worksheetDetail in Details.Select())
            {
                ...
                var query = (PXResult<InventoryItem, InventoryItemCurySettings, INItemCost>)
                ...
                InventoryItemCurySettings curyItem = query;
                INItemCost itemCost = query;
                KeyValuePair<bool, decimal> calcRes = CalculateCorrectedAmountForPendingPricesCalculation(settings, inventoryItem, curyItem,
        ...
        {
            if (settings.BaseCuryID == CurrencyCollection.GetBaseCurrency()?.CuryID)
            {
                var cache = this.Caches<InventoryItemCurySettings>();
                curyItem = (InventoryItemCurySettings)cache.CreateInstance();
                curyItem.CuryID = CurrencyCollection.GetBaseCurrency()?.CuryID;
                curyItem.StdCost = inventoryItem.StdCost;
                curyItem.RecPrice = inventoryItem.RecPrice;
                curyItem.BasePrice = inventoryItem.BasePrice;
                curyItem.LastStdCost = inventoryItem.LastStdCost;
            }
            ...
            switch (settings.PriceBasis)
            {
                ...
                case AR.PriceBasisTypes.RecommendedPrice:
                    skipUpdate = settings.UpdateOnZero != true && (curyItem.RecPrice == null || curyItem.RecPrice == 0);
                    correctedAmt = curyItem.RecPrice ?? 0m;
                    break;
            }
       ...
       protected virtual decimal AdjustByUOM(InventoryItem inventoryItem, APPriceWorksheetDetail worksheetDetail, decimal correctedAmtInBaseUnit)
        ...
        protected virtual void APPriceWorksheet_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
            ...
            if (doc.IsPromotional == true && doc.ExpirationDate == null)
                sender.RaiseExceptionHandling<APPriceWorksheet.expirationDate>(doc, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(APPriceWorksheet.expirationDate)}]"));
        }
        ...
        protected virtual void APPriceWorksheet_EffectiveDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if (e.NewValue == null)
                throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(APPriceWorksheet.effectiveDate)}]");
        ...
        protected virtual void APPriceWorksheet_ExpirationDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            ...
            if (doc.IsPromotional == true && e.NewValue == null)
                throw new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(APPriceWorksheet.expirationDate)}]");
        ...
        protected virtual void APPriceWorksheetDetail_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
            ...
            {
                sender.RaiseExceptionHandling<APPriceWorksheetDetail.vendorID>(det, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(APPriceWorksheetDetail.vendorID)}]"));
                return;
            }

            if (Document.Current != null && Document.Current.Hold != true && det.PendingPrice == null)
                sender.RaiseExceptionHandling<APPriceWorksheetDetail.pendingPrice>(det, null, new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(APPriceWorksheetDetail.pendingPrice)}]"));
        }
        #endregion
	...
	public partial class ARAddItemSelected : IBqlTable
    {
		...
		/// </summary>
		/// <value>
		/// Corresponds to the <see cref="CACorpCard.PKID"/> field.
		/// </value>
		[TM.Owner(typeof(InventoryItem.productWorkgroupID), BqlField = typeof(InventoryItem.productManagerID), DisplayName = "Product Manager")]
        ...
        where StatusFilter : AddItemFilter, new()
    {
		...
		protected override PXView CreateIntView(PXGraph graph)
		{
			...
			view.WhereAnd<Where<Current<AddItemFilter.myWorkGroup>, Equal<boolFalse>,
						 Or<ARAddItemSelected.productWorkgroupID, TM.IsWorkgroupOfContact<CurrentValue<AddItemFilter.currentOwnerID>>>>>();
