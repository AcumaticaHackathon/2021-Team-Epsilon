...
namespace PX.Objects.AR
{
	...
	public class ARInvoiceEntry : ARDataEntryGraph<ARInvoiceEntry, ARInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		[PXProcessButton(ImageKey = PX.Web.UI.Sprite.Main.Shedule, VisibleOnDataSource = false)]
		[ARMigrationModeDependentActionRestriction(
			restrictInMigrationMode: true,
			restrictForRegularDocumentInMigrationMode: true,
			restrictForUnreleasedMigratedDocumentInNormalMode: true)]
		...
		public virtual PXResultset<ARAdjust2, ARPayment, ExternalTransaction> LoadDocumentsProc()
		{
				...
				&& Document.Current.Voided != true)
			{
				...
				{
					foreach (PXResult<Standalone.ARRegisterAlias, CurrencyInfo, ARAdjust2, SOAdjust, ARPayment, ExternalTransaction> res 
					...
					)
					{
						...
						if (Adjustments.Cache.Locate(adj) == null	|| Adjustments.Cache.GetStatus(adj) == PXEntryStatus.InsertedDeleted)
						{
							...
							else
							{
								try
								{
									InvoiceEntry.InitBalancesFromInvoiceSide(CurrencyInfo_CuryInfoID, adj, Document.Current, payment);
								}
								catch (Exception ex)
								{
		...
		protected virtual IEnumerable SendEmail(PXAdapter adapter) => this.Actions["NewMailActivity"].Press(adapter);
		...
		protected virtual ARInvoice CreateReversalARInvoice(ARInvoice doc, ReverseInvoiceArgs reverseArgs)
		{
			...
			invoice.IsRetainageReversing = doc.IsOriginalRetainageDocument() || doc.IsChildRetainageDocument();
		...
		public virtual IEnumerable adjustments()
		{
			...
			foreach (PXResult<ARAdjust2, ARPayment, Standalone.ARRegisterAlias, CurrencyInfo, ExternalTransaction> res
		...
		public virtual ARInvoiceState GetDocumentState(PXCache cache, ARInvoice doc)
		{
			...
			res.PaymentsByLinesAllowed = doc.PaymentsByLinesAllowed == true;
			...
			res.IsRetainageReversing = (doc.IsOriginalRetainageDocument() || doc.IsChildRetainageDocument()) && doc.IsRetainageReversing == true;
		...
		protected virtual void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (state.ShouldDisableHeader)
			{
				...
				release.SetEnabled(state.IsUnreleasedWO || state.IsUnreleasedPPD || state.IsCancellationDocument);
			...
			else
			{
				...
				PXUIFieldAttribute.SetEnabled<ARInvoice.retainageApply>(cache, doc, !state.IsDocumentDebitMemo);
			...
			bool creditMemoAdjustmentsEnabled = Transactions.Cache.AllowUpdate && !state.IsDocumentScheduled && !state.PaymentsByLinesAllowed;

			Adjustments_1.AllowSelect = state.IsDocumentCreditMemo && !state.IsDocumentReleased;
			Adjustments_1.Cache.AllowInsert = creditMemoAdjustmentsEnabled;
			Adjustments_1.Cache.AllowDelete = creditMemoAdjustmentsEnabled;
			Adjustments_1.Cache.AllowUpdate = creditMemoAdjustmentsEnabled;

			Adjustments_2.AllowSelect = state.IsDocumentCreditMemo && state.IsDocumentReleased;
			Adjustments_2.Cache.AllowInsert = creditMemoAdjustmentsEnabled;
			Adjustments_2.Cache.AllowDelete = creditMemoAdjustmentsEnabled;
			Adjustments_2.Cache.AllowUpdate = creditMemoAdjustmentsEnabled;
			...
			{
				PXUIFieldAttribute.SetVisible<ARInvoice.curyOrigDocAmt>(cache, e.Row, ARSetup.Current.RequireControlTotal == true || e.Row != null && ((ARInvoice)e.Row).Released == true);
			}
		...
		protected virtual void _(Events.FieldVerifying<ARTran.curyRetainageAmt> e)
		{
			...
			if (Math.Sign(row.CuryExtPrice ?? 0m) * Math.Sign((decimal?)e.NewValue ?? 0) < 0m)
			{
				throw new PXSetPropertyException(Messages.IncorrectExtPriceAndCuryRetainageAmt);
			}

			if (Math.Abs(row.CuryExtPrice ?? 0m) < Math.Abs((decimal?)e.NewValue ?? 0))
			{
				throw new PXSetPropertyException( Math.Sign((decimal?)e.NewValue ?? 0) > 0
					? Messages.IncorrectRetainagePositiveAmount
					: Messages.IncorrectRetainageNegativeAmount);
			}
		}
		...
		protected virtual void ARTran_CuryUnitPrice_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (row != null && row.InventoryID != null && row.UOM != null && row.ManualPrice != true && !skipDefaulting)
			{
				...
				e.NewValue = ARSalesPriceMaint.CalculateSalesPrice(sender, customerPriceClass,
					((ARTran)e.Row).CustomerID, 
					((ARTran)e.Row).InventoryID, 
					((ARTran)e.Row).SiteID, 
					currencyinfo.Select(), 
					((ARTran)e.Row).UOM, 
					((ARTran)e.Row).Qty, date,
					row.CuryUnitPrice, taxCalcMode) ?? 0m;
			...
			bool DiscOnDiscDate)
		{
			...
			{
				InvoiceEntry.CalcBalancesFromInvoiceSide(adj, invoice, payment, CurrencyInfo_CuryInfoID, isCalcRGOL, DiscOnDiscDate);
				return;
			}
		}
