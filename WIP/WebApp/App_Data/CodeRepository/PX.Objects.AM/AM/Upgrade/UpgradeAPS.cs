 ...
 */
        ...
        private void CleanupWcSchdRecords()
        {
            ...
            foreach (AMWCSchdNotMatchingDetail result in PXSelect<AMWCSchdNotMatchingDetail>.Select(_upgradeGraph))
            {
                ...
                counter++;
                result.SchdTime = result.DetailRunTime.GetValueOrDefault();
    ...
    [PXProjection(typeof(Select2<
        ...
        Where<AMWCSchd.schdTime, NotEqual<IsNull<AMWCSchdDetailGroupByWcSchiftDate.runTimeBase, int0>>,
    ...
    public class AMWCSchdNotMatchingDetail : IBqlTable
    {
        ...
        #region DetailRunTimeBase (SUM)

        public abstract class detailRunTimeBase : PX.Data.BQL.BqlInt.Field<detailRunTimeBase> { }

        [PXDBTimeSpanLong(Format = TimeSpanFormatType.LongHoursMinutes, BqlField = typeof(AMWCSchdDetailGroupByWcSchiftDate.runTimeBase))]
        [PXUIField(DisplayName = "Detail Schedule Time w / o Efficiency", Enabled = false)]
        public virtual int? RunTimeBase { get; set; }

        #endregion
        #region DetailRunTime (SUM)

        public abstract class detailRunTime : PX.Data.BQL.BqlInt.Field<detailRunTime> { }

        [PXDBTimeSpanLong(Format = TimeSpanFormatType.LongHoursMinutes, BqlField = typeof(AMWCSchdDetailGroupByWcSchiftDate.runTime))]
        [PXUIField(DisplayName = "Detail Schedule Time", Enabled = false)]
        public virtual int? DetailRunTime { get; set; }

        #endregion
        #region DetailPlanBlocks (SUM)
        public abstract class detailPlanBlocks : PX.Data.BQL.BqlInt.Field<detailPlanBlocks> { }

        [PXDBInt(MinValue = 0, BqlField = typeof(AMWCSchdDetailGroupByWcSchiftDate.planBlocks))]
        ...
        [PXDBInt(MinValue = 0, BqlField = typeof(AMWCSchdDetailGroupByWcSchiftDate.schdBlocks))]
