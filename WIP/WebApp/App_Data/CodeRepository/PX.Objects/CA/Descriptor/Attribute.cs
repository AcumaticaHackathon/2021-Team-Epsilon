				...
				if (PXSubstManager.IsSuitableTypeExportAssembly(ass, false))
				{
					...
					if (types != null)
					{
								...
								.Any(interfaceType => interfaceType.IsAssignableFrom(assemblyType) && assemblyType != interfaceType)))
						{
							...
							if (attribute != null)
							{
								Type dependsFromFeature = (attribute as PXDisplayTypeNameAttribute)?.DependsFromFeature;
								if (dependsFromFeature != null &&
									PXAccess.FeatureInstalled(dependsFromFeature.FullName) == false)
								{
									continue;
								}
							}
